...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        # kwarg that ``AIAgent.__init__`` no longer accepts. Only apply it on the
        # global default route: a session /model override selects its own provider,
        # and its runtime must stay native unless its own config says otherwise.
        if self._deepagents_mode and runtime_kwargs is not None and "provider" not in (override or {}):
