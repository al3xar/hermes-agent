...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if hades_home_str:
            takeover_path = Path(hades_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(hades_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded hades-agent but never re-ran ``hades setup`` to regenerate
    ...
    # that's the common case for hades.
    ...
    # constant in hades_cli/gateway.py.
    headroom = 30.0
