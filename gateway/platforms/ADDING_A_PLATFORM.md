...
**zero changes to core Hades code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `hades gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `hades_cli/config.py` so the setup
...
## 12. Status Display (`hades_cli/status.py`)
...
## 13. Gateway Setup Wizard (`hades_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ hades_cli/ toolsets.py \
  --include="*.py" -l | sort -u
