...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # /model work directly without /hades prefix). A single regex
            ...
            # manifest (see `hades slack manifest`). In Socket Mode, Slack
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/hades$")

            @self._app.command(_slash_pattern)
            async def handle_hades_command(ack, command):
            ...
            for _action_id in (
                "hades_approve_once",
                "hades_approve_session",
                "hades_approve_always",
                "hades_deny",
            ):
            ...
            for _action_id in (
                "hades_confirm_once",
                "hades_confirm_always",
                "hades_confirm_cancel",
            ):
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                ...
                # forms like ``!stop@hades`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "hades_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "hades_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "hades_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "hades_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "hades_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "hades_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "hades_confirm_once": "once",
            "hades_confirm_always": "always",
            "hades_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "hades_approve_once": "once",
            "hades_approve_session": "session",
            "hades_approve_always": "always",
            "hades_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/hades <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/hades
        ...
        if slash_name in {"hades", ""}:
            # Legacy /hades <subcommand> [args] routing + free-form questions.
            ...
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/hades   ``).
        ...
        # "Running /cmd…" ack shown by handle_hades_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/hades <question>" must produce public replies so
