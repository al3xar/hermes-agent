...
# Audio file extensions Hades recognizes for native audio delivery.
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Hades-created Telegram private-chat
...
# Default location: {HADES_HOME}/cache/images/ (legacy: image_cache/)
...
_HADES_ROOT = get_default_hades_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "HADES_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "HADES_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "HADES_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "HADES_MEDIA_DELIVERY_STRICT"
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Hades profile and shared Hades root both contain control
    # files and credentials. Only cache subdirectories under them are
    # explicitly allowlisted above.
    for hades_root in (_HADES_HOME, _HADES_ROOT):
        denied.append(hades_root / ".env")
        denied.append(hades_root / "auth.json")
        denied.append(hades_root / "credentials")
        denied.append(hades_root / "config.yaml")
    return denied
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Hades
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``HADES_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Hades-managed cache, under an operator-allowlisted root
    (``HADES_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.hades/.env,
    # ~/.hades/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?hades\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+hades[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # Hades commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_debounce_seconds: float = _float_env(
            "HADES_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "HADES_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.hades/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          HADES_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          HADES_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          HADES_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _hades_run_generation is set on
