...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "HADES_HOME", "HADES_PROFILE", "HADES_CONFIG", "HADES_ENV",
})
...
DEFAULT_CONFIG = {
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://hades-agent.nousresearch.com/docs/api/model-catalog.json",
    ...
    "gateway": {
        ...
        # the legacy Hades agent loop runs as usual.
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            ...
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                ...
                results["config_added"].append("display.tool_progress=off (from HADES_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from HADES_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            ...
            if old_tz and old_tz.strip():
                ...
                results["config_added"].append(f"timezone={old_tz.strip()} (from HADES_TIMEZONE)")
            else:
                config["timezone"] = ""
