"""CLI handlers for the ``hades proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "hades proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"hades auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Hades proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Hades proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: hades proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``hades proxy <subcommand>``."""
    ...
    print(
        "hades proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  hades proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
