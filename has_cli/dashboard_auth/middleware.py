...
from hades_cli.dashboard_auth.cookies import read_session_cookies
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /hades``, the
    ``login_url`` is prefixed (``/hades/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["hades.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``hades_session_rt``. If we bailed on ``not at`` here we'd
