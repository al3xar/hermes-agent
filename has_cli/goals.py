...
goal satisfied by the assistant's last response?". If not, Hades feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.HadesCLI`` or the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    ``hades_home`` path so profile switches still pick up the right DB.
...
def run_kanban_goal_loop(
    *,
    ...
    (``hades -p <profile> chat -q "work kanban task <id>"``). The worker's
