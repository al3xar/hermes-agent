...
The Hades docs site hosts a JSON manifest of curated models for providers
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``hades update`` pulls the latest repo, so the freshly-pulled
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``hades model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
