...
What translates (MCP servers):
  Hades mcp_servers.<n>.command/args/env  → codex stdio transport
  Hades mcp_servers.<n>.url/headers       → codex streamable_http transport
  Hades mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Hades mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Hades-specific keys (sampling, etc.) — codex's MCP client has no
...
# Hades keys that codex's MCP schema doesn't support — dropped during
...
def _translate_one_server(
    name: str, hades_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Hades MCP server config to the codex inline-table dict
    ...
    if not isinstance(hades_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(hades_cfg.get("command"))
    has_url = bool(hades_cfg.get("url"))
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Hades
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="hades-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    codex-routed hades-tools call fails silently once the directory is GC'd.
...
def _build_hades_tools_mcp_entry() -> dict:
    ...
    so a hades installed under /opt/, /usr/local/, or a venv all work.
    ...
    hades_home = os.environ.get("HADES_HOME") or ""
    if hades_home and _looks_like_test_tempdir(hades_home):
        hades_home = ""
    if hades_home:
        env["HADES_HOME"] = hades_home
    # PYTHONPATH passes through so a worktree-launched hades finds the
    ...
    env["HADES_QUIET"] = "1"
    env["HADES_REDACT_SECRETS"] = env.get("HADES_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_hades_tools: bool = True,
) -> MigrationReport:
    """Translate Hades mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hades_config: full ~/.hades/config.yaml dict
        ...
        expose_hades_tools: when True (default), register Hades' own
    ...
    hades_servers = (hades_config or {}).get("mcp_servers") or {}
    if not isinstance(hades_servers, dict):
    ...
    for name, cfg in hades_servers.items():
    ...
    # The server itself is agent/transports/hades_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_hades_tools:
        translated["hades-tools"] = _build_hades_tools_mcp_entry()
        if "hades-tools" not in report.migrated:
            report.migrated.append("hades-tools")
