"""
hades fallback — manage the fallback provider chain.
...
Subcommands:
  hades fallback [list]   Show the current fallback chain (default when no subcommand)
  hades fallback add      Pick provider + model via the same picker as `hades model`,
                           then append the selection to the chain
  hades fallback remove   Pick an entry to delete from the chain
  hades fallback clear    Remove all fallback entries
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `hades model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  hades fallback add")
        print()
        return
    ...
    print("  Docs: https://hades-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `hades model`, then append the selection to the chain."""
    ...
    print("  `hades model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `hades fallback list` to view, or `hades fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``hades fallback [subcommand]``."""
