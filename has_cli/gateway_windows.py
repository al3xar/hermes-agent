...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m hades_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Hades_Gateway"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Hades profile."""
...
def _launch_elevated_install(
    ...
    *,
    ...
    try:
        ...
        for key, old in (
            ("HADES_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("HADES_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("HADES_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\hades\\gateway-service\\<task_name>.cmd``
    (or ``<HADES_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Hades installs stay self-contained).
    """
    _assert_windows()
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    source checkout only if ``HADES_HOME`` cannot be resolved yet.
    """
    from hades_cli.config import get_hades_home
...
def _build_gateway_cmd_script(
    ...
    lines.append(f'set "HADES_HOME={hades_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HADES_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports hades_constants-based logic during startup.
...
def _write_task_script() -> Path:
    ...
    content = _build_gateway_cmd_script(python_path, working_dir, hades_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Hades builds and failed
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("HADES_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("HADES_GATEWAY_INSTALL_START_ON_LOGIN")
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `hades -p <profile> gateway install` must be safe.
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"hades {profile_arg} gateway start" if profile_arg else "hades gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: hades gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Hades gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: hades gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: hades gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from hades_cli.config import get_hades_home
        print(f"    type {Path(get_hades_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_hades_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from hades_cli.config import get_hades_home

    hades_home = Path(get_hades_home()).resolve()
    print()
    print("Next steps:")
    print("  hades gateway status                      # Check status")
    print(f"  type {hades_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Hades gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: hades gateway status")
                    return
...
def _print_deep_probes() -> None:
    ...
    home = Path(get_hades_home()).resolve()
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: hades gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: hades gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from hades_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
    from gateway.status import get_running_pid
