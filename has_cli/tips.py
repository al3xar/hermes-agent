...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hades -c.",
    ...
    "Ctrl+Z suspends Hades to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "hades -c resumes your most recent CLI session. hades -c \"project name\" resumes by title.",
    "hades -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hades -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hades chat -t web,terminal enables only specific toolsets for a focused session.",
    "hades chat -s github-pr-workflow preloads a skill at launch.",
    "hades chat -q \"query\" runs a single non-interactive query and exits.",
    "hades chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "hades chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hades --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hades chat --source telegram tags the session for filtering in hades sessions list.",
    "hades -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "hades doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hades dump outputs a compact setup summary — great for bug reports.",
    "hades config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hades config edit opens config.yaml in your default editor.",
    "hades config check scans for missing or stale configuration options.",
    "hades sessions browse opens an interactive session picker with search.",
    "hades sessions stats shows session counts by platform and database size.",
    "hades sessions prune --older-than 30 cleans up old sessions.",
    "hades skills search react --source skills-sh searches the skills.sh public directory.",
    "hades skills check scans installed hub skills for upstream updates.",
    "hades skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hades skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hades mcp add github --command npx adds MCP servers from the command line.",
    "hades mcp serve runs Hades itself as an MCP server for other agents.",
    "hades auth add lets you add multiple API keys for credential pool rotation.",
    "hades completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hades logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hades backup creates a zip backup of your entire Hades home directory.",
    "hades profile create coder creates an isolated profile that becomes its own command.",
    "hades profile create work --clone copies your current config and keys to a new profile.",
    "hades update syncs new bundled skills to ALL profiles automatically.",
    "hades gateway install sets up Hades as a system service (systemd/launchd).",
    "hades memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hades webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hades tools disables unused tools, hades skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "hades models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hades -w).",
    ...
    "HADES_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Hades tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'hades profile create coder' creates the 'coder' command.",
    "hades profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Hades prints a resume command with session ID and stats.",
    "hades sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hades -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "hades skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .hades/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hades cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Hades runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "hades gateway install sets it up as a system service that starts on boot.",
    ...
    "Context files (.hades.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Hades loads project context from .hades.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "hades mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "hades mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "hades mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — hades tools can toggle them per platform.",
    ...
    "hades chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hades plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hades acp runs Hades as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "HADES_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "hades auth add nous or hades auth add openai-codex sets up OAuth-based providers.",
    ...
    "hades status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Hades your own.",
    ...
    "Before a gateway session reset, Hades auto-flushes important facts to memory in the background.",
    ...
    "Each profile gets its own subprocess HOME at HADES_HOME/home/ — isolated git, ssh, npm, gh configs.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hades profile create ls' would be rejected.",
    "hades profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "hades claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "HADES_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hades tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Hades auto-continues turn after turn until a judge says done.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Hades is working.',
    ...
    "HADES_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",
    ...
    # --- Curator ---
    'hades curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hades curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hades curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'hades auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'HADES_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    ...
    'hades dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hades dashboard embeds the full Hades TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'HADES_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'HADES_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'HADES_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'HADES_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    'hades -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hades chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hades chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hades chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "hades chat --source tool tags programmatic chats so they don't clutter hades sessions list.",
    'hades dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hades sessions rename <ID> "new title" renames any past session; hades sessions delete <ID> removes one.',
    'hades import restores a session export or profile archive produced by sessions export or profile export.',
    'hades fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hades pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hades setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hades status --deep runs the full health sweep across every component; plain hades status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'HADES_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HADES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hades/plugins/ — trust-gated by design.',
    "HADES_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'HADES_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    ...
    'HADES_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'HADES_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'HADES_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'HADES_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'HADES_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'HADES_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'HADES_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Hades block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from hades sessions list by default — set --source explicitly to see them.',
