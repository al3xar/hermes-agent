...
manual BotFather token copy-paste. Hades talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.hades-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "HadesSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``hades_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``hades_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"hades_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Hades Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
