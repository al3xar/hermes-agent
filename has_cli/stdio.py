...
Entry points (``cli.py`` ``main``, ``hades_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Hades is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``HADES_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    # already-running shells don't see the change — which means hades
...
def _default_windows_editor() -> str:
    ...
    ``hades config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Hades (User env var in Windows Settings, or
    export in a PowerShell profile) and Hades picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    ...
    ``%LOCALAPPDATA%\\hades\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``hades`` from right after install) retain their old PATH.

    Any subprocess Hades spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "hades", "git", "cmd"),
        os.path.join(local_appdata, "hades", "git", "bin"),
        os.path.join(local_appdata, "hades", "git", "usr", "bin"),
        # Hades venv Scripts directory — host of the hades.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes hades, but harmless to include.
        os.path.join(local_appdata, "hades", "hades-agent", "venv", "Scripts"),
