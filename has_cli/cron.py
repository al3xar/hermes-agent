...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(hades\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*hades)"
    r"|(systemctl\s+(restart|stop|start)\s+.*hades)"
    r"|(p?kill\s+.*hades.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'hades cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: hades gateway install", Colors.DIM))
        print(color("                    sudo hades gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    hades gateway install    # Install as a user service")
        print("    sudo hades gateway install --system  # Linux servers: boot-time system service")
        print("    hades gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `hades gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: hades cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
