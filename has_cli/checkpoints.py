"""`hades checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hades/checkpoints/``.  Actions:

    hades checkpoints               # same as `status`
    hades checkpoints status        # total size, project count, breakdown
    hades checkpoints list          # per-project checkpoint counts + workdir
    hades checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    hades checkpoints clear [-f]    # nuke the entire base (asks first)
    hades checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    hades checkpoints
    hades checkpoints prune --retention-days 3 --max-size-mb 200
    hades checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: hades checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``hades checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `hades checkpoints` → status
