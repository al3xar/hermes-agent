...
``hades_cli/dump.py`` and ``hades_cli/banner.py``).  That doesn't work inside
...
To make ``hades dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$HADES_GIT_SHA``
arg into ``<project_root>/.hades_build_sha``.  This module is the single
...
- Returns ``None`` when the file is absent.  Source installs and dev images
  built without the ``HADES_GIT_SHA`` build-arg fall through to live-git
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".hades_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.hades_build_sha`` if present.  The file is
    written by the Dockerfile's ``HADES_GIT_SHA`` build-arg and contains
