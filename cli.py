...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def show_banner(self):
        ...
        if use_compact:
            ...
            build_welcome_banner(
                ...
                runtime=self._resolve_deepagents_runtime(self.config),
            )
    ...
    def _show_status(self):
        ...
        # Execution runtime (native vs deepagents/LangGraph). Resolved from
        # config so it shows before the agent is lazily constructed.
        runtime_mode = self._resolve_deepagents_runtime(self.config)
        runtime_info = (
            f" [dim {separator_color}]·[/] [{label_color}]runtime: {runtime_mode}[/]"
        )

        self._console_print(
            f"  {api_indicator} [{accent_color}]{model_short}[/] "
            f"[dim {separator_color}]·[/] [bold {label_color}]{tool_status}[/]"
            f"{toolsets_info}{provider_info}{runtime_info}"
        )
