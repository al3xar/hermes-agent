...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    hades_node_bin = str(hades_home / "node" / "bin")
    hades_node_root = str(hades_home / "node")
    hades_nm_bin = str(hades_home / "node_modules" / ".bin")
    return [hades_node_bin, hades_node_root, hades_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Hades silently
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-hades_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed hades processes — this way every clean hades exit sweeps
    ...
    # Sweep orphans from other crashed hades processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live hades processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current hades PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live hades process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning hades process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced hades PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent hades instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-hades_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live hades process.
            continue
...
def _find_agent_browser() -> str:
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                hades_nm = str(get_hades_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=hades_nm)
            if not recheck:
                hades_node_bin = str(get_hades_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=hades_node_bin)
            if not recheck:
                hades_node_root = str(get_hades_home() / "node")
                recheck = shutil.which("agent-browser", path=hades_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this hades PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = hades_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Hades
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/hades-agent:latest")
                else:
