...
Lets the LLM write a Python script that calls Hades tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `hades_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `hades_tools.py` with file-based RPC stubs
...
# available on every platform Hades itself runs on.
...
# match a safe prefix, the operational HADES_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "HADES_" prefix was deliberately removed (#27303) — it leaked
# HADES_*-named config that lacks a secret substring (e.g. HADES_BASE_URL,
# HADES_KANBAN_DB, HADES_*_WEBHOOK).  The child only needs the few
# location/profile vars in _HADES_CHILD_ALLOWED below; HADES_RPC_SOCKET /
# HADES_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational HADES_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set hades_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_HADES_CHILD_ALLOWED = frozenset({
    "HADES_HOME",
    "HADES_PROFILE",
    "HADES_CONFIG",
    "HADES_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational HADES_* vars (_HADES_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret HADES_* vars dropped by the tightened allowlist (#27303). The
    # broad "HADES_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like HADES_KANBAN_DB / HADES_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _HADES_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("HADES_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped HADES_* var.
            _dropped_hades.append(k)
    if _dropped_hades:
        logger.debug(
            "execute_code: dropped %d non-allowlisted HADES_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Hades-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Hades-profile soft guard."""',
...
def generate_hades_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the hades_tools.py stub module.
...
"""Auto-generated Hades tools RPC stubs."""
...
def _connect():
    ...
    HADES_RPC_SOCKET can be either:
...
"""Auto-generated Hades tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("HADES_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hades_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated hades_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/hades_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_hades_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/hades_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with hades_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="hades_sandbox_")
    ...
    # selector from HADES_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"hades_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated hades_tools module.
        ...
        tools_src = generate_hades_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "hades_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  HADES_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        # Ensure the hades-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from hades_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _hades_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _hades_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # HADES_TIMEZONE is an internal Hades setting and must not leak
        # into child processes.
        _tz_name = os.getenv("HADES_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("HADES_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {HADES_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.hades/.env')).
...
def _load_config() -> dict:
    ...
    ``hades --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (hades-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``hades tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and hades-agent's own python.
    if mode == "strict":
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Hades tools programmatically. "
        ...
        f"Available via `from hades_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into hades_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from hades_tools import {import_str}` "
