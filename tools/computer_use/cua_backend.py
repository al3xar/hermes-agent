...
can break on OS updates. Pin the installed version via `HADES_CUA_DRIVER_
...
PINNED_CUA_DRIVER_VERSION = os.environ.get("HADES_CUA_DRIVER_VERSION", "0.5.0")
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or HADES_CUA_DRIVER_CMD resolves."""
...
def cua_driver_install_hint() -> str:
    return (
        ...
        "Or run `hades tools` and enable the Computer Use toolset to install it automatically."
    )
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps hades `capture(mode, app)` → cua-driver `list_windows` +
