...
# regardless of approval level (e.g. logout kills the session Hades runs in).
_BLOCKED_KEY_COMBOS = {
    ...
    with _backend_lock:
        if _backend is None:
            ...
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown HADES_COMPUTER_USE_BACKEND={backend_name!r}")
            _backend.start()
        return _backend
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "Run `hades tools` and enable Computer Use to install cua-driver.",
        })
