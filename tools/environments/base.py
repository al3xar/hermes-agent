"""Base class for all Hades execution environment backends.
...
# HADES_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
...
def _cwd_marker(session_id: str) -> str:
    return f"__HADES_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Hades backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/hades-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/hades-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../hades-snap-*.sh: No such file or directory``
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"HADES_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via HADES_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __HADES_CWD_{session}__ marker from stdout output.
