...
# Hades-internal env vars that should NOT leak into terminal subprocesses.
_HADES_PROVIDER_ENV_FORCE_PREFIX = "_HADES_FORCE_"

# Hades-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Hades inference secret exactly
...
_HADES_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_hades_home(env: dict) -> None:
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_HADES_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _HADES_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_HADES_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_HADES_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _HADES_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_hades_home(sanitized)
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\hades\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\hades\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hades_portable_git = os.path.join(_local_appdata, "hades", "git") if _local_appdata else ""
    if _hades_portable_git:
        for candidate in (
            os.path.join(_hades_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_hades_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        ...
        "Or set HADES_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_HADES_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_HADES_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _HADES_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        run_env[path_key] = _append_missing_sane_path_entries(run_env.get(path_key, ""))

    _inject_context_hades_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {HADES_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Hades trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        if _IS_WINDOWS:
            ...
            try:
                ...
                cache_dir = Path(tempfile.gettempdir()) / "hades_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._hades_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_hades_pgid", None)
                    if pgid is None:
                        raise
