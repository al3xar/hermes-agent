...
from tools.environments.local import _HADES_PROVIDER_ENV_BLOCKLIST
...
def _load_hades_env_vars() -> dict[str, str]:
...
def _get_active_profile_name() -> str:
    """Return the active Hades profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale hades-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=hades-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Hades process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=hades-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a hades process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Hades
    process leaves its container behind permanently because there's no
    subsequent Hades process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=hades-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=hades-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``HADES_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        ...
        logger.info("Using HADES_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'hades'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``hades-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # s6-overlay images (e.g. hades-agent:latest) already use /init as PID 1
        ...
        container_name = f"hades-{uuid.uuid4().hex[:8]}"
        # Labels make hades-created containers identifiable to:
        #   * the orphan reaper (`hades-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`hades-task-id`, `hades-profile`)
        #   * operators running `docker ps --filter label=hades-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Hades profile is captured at
        ...
        label_args = [
            "--label", "hades-agent=1",
            "--label", f"hades-task-id={task_label}",
            "--label", f"hades-profile={profile_name}",
        ]
        ...
        self._labels = {
            "hades-agent": "1",
            "hades-task-id": task_label,
            "hades-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Hades process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Hades secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _HADES_PROVIDER_ENV_BLOCKLIST)
        hades_env = _load_hades_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = hades_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("hades-task-id", "")
        profile_label = self._labels.get("hades-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"hades-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``hades-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=hades-agent=1",
                    "--filter", f"label=hades-task-id={task_label}",
                    "--filter", f"label=hades-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Hades process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Hades exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Hades process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Hades startup. That covers the
        ...
        # outlives Hades processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"hades-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``hades /quit``
