...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set HADES_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (HADES_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("HADES_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("HADES_SESSION_PLATFORM"))
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``HADES_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``HADES_PLATFORM`` environment variable
    3. ``HADES_SESSION_PLATFORM`` from gateway session context
    """
    try:
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``hades skills`` config UI). Default False
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: hades plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(
    ...
    try:
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `hades skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.hades.* first (agentskills.io convention), fall back to top-level
        hades_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            hades_meta = metadata.get("hades", {}) or {}

        tags = _parse_tags(hades_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            hades_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
