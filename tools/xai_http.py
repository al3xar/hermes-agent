...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``hades tools`` repaint, tool-registration scans,
...
def get_env_value(name: str, default=None):
    ...
    """
    try:
        from hades_cli.config import get_env_value as _hades_get_env_value

        value = _hades_get_env_value(name)
...
def hades_xai_user_agent() -> str:
    """Return a stable Hades-specific User-Agent for xAI HTTP calls."""
    try:
        from hades_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Hades-Agent/{__version__}"
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    stored in ``~/.hades/.env`` (the standard Hades location) are honored —
