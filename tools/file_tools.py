...
_hades_config_resolved: str | None = None
_hades_config_resolved_loaded = False


def _get_hades_config_resolved() -> str | None:
    """Return the resolved absolute path of the Hades config file (cached)."""
    global _hades_config_resolved, _hades_config_resolved_loaded
    if _hades_config_resolved_loaded:
        return _hades_config_resolved
    _hades_config_resolved_loaded = True
    try:
        from hades_cli.config import get_config_path
        _hades_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _hades_config_resolved = str(Path("~/.hades/config.yaml").expanduser().resolve())
        except Exception:
            _hades_config_resolved = None
    return _hades_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    # this file.
    hades_config = _get_hades_config_resolved()
    if hades_config and (resolved == hades_config or normalized == hades_config):
        return (
            f"Refusing to write to Hades config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.hades/config.yaml directly or use 'hades config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Hades mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.hades/…`` mirror created by a
    ...
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.hades/…`` path.
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Hades-profile guard. The
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Hades-profile guard for
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Hades profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Hades profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with hades_tools.write_file() for very "
            "large files."
        )
