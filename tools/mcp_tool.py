...
transport, discovers their tools, and registers them into the hades-agent
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hades_home = os.path.expanduser(
                os.getenv(
                    "HADES_HOME", os.path.join(os.path.expanduser("~"), ".hades")
                )
            )
            candidates = [
                os.path.join(hades_home, "node", "bin", resolved_command),
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Hades MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Hades gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Hades is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `hades mcp login {server_name}` (or delete the tokens "
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``hades tools`` interactive configuration — connects to each
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple hades
