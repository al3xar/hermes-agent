...
One ``CDPSupervisor`` runs per Hades ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "hades-dialog-bridge.invalid"
...
(() => {
  if (window.__hadesDialogBridgeInstalled) return;
  window.__hadesDialogBridgeInstalled = true;
  const ENDPOINT = "http://hades-dialog-bridge.invalid/";
