...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Hades exit
...
# display_hades_home imported lazily at call site (stale-module safety during hades update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of hades directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "hades-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("HADES_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (HADES_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (HADES_INTERACTIVE=1):
    ...
    # interactive Hades password prompt or the sudo -S password-pipe path.
    ...
    # silently without Hades prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and env_var_enabled("HADES_INTERACTIVE"):
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Hades can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``hades-agent=1`` for the current
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Hades processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Hades processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, HadesSweEnv, ...)
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for hades-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Hades processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Hades can track the process, then run "
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # hades-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in hades-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("HADES_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("HADES_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("HADES_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("HADES_SESSION_USER_ID", "")
                        _gw_user_name = _gse("HADES_SESSION_USER_NAME", "")
                        _gw_message_id = _gse("HADES_SESSION_MESSAGE_ID", "")
