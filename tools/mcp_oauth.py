...
This module provides the glue:
    - ``HadesTokenStorage``: persists tokens/client-info to disk so they
...
_USER_SKIPPED_SENTINEL = "__hades_user_skipped__"
...
# ---------------------------------------------------------------------------
# HadesTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class HadesTokenStorage:
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Hades records an absolute wall-clock ``expires_at`` alongside the
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://hades-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `hades mcp login <server>` later to "
