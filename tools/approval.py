...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HADES_SESSION_KEY", default)
...
def _is_gateway_approval_context() -> bool:
    ...
    Legacy gateway integrations set HADES_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind HADES_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds HADES_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("HADES_CRON_SESSION"):
        return False
    if env_var_enabled("HADES_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $HADES_HOME, or by the resolved absolute
# active profile home path such as /home/hades/.hades/config.yaml. The
...
# theater. Mirrors _HADES_ENV_PATH; matches the HADES_HOME override form as
# well as ~/.hades/.
_HADES_CONFIG_PATH = (
    r'(?:~\/\.hades/|'
    r'(?:\$home|\$\{home\})/\.hades/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhades\s+gateway\s+(stop|restart)\b', "stop/restart hades gateway (kills running agents)"),
    (r'\bhades\s+update\b', "hades update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `hades gateway restart`
    ...
    (r'\b(pkill|killall)\b.*\b(hades|gateway|cli\.py)\b', "kill hades/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill hades` but not
    # `kill -9 $(pgrep -f hades)` because the substitution is opaque
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HADES_CONFIG_PATH}|{_HADES_ENV_PATH})', "in-place edit of Hades config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HADES_CONFIG_PATH}|{_HADES_ENV_PATH})', "in-place edit of Hades config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HADES_CONFIG_PATH}|{_HADES_ENV_PATH})', "in-place edit of Hades config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/.hades/ form so the Hades config/env patterns catch it. In Docker and
    # gateway deployments the agent often references the resolved absolute path
    # directly (e.g. `sed -i ... /home/hades/.hades/config.yaml`) rather than
    # ~, $HOME, or $HADES_HOME. Done at detection time (not via an import-time
    # pattern snapshot) so it tracks the live HADES_HOME even when that is set
    # after this module is imported — as the hermetic test conftest does.
    command = _rewrite_resolved_hades_home(command)
    return command


def _rewrite_resolved_hades_home(command: str) -> str:
    ...
    ``~/.hades/`` so the static ``_HADES_CONFIG_PATH`` / ``_HADES_ENV_PATH``
    ...
    for path in candidates:
        if not path or path in seen:
            continue
        seen.add(path)
        # Guard against a degenerate HADES_HOME (e.g. "/" or "") rewriting
        ...
        # /home/hades/.hades or a per-test tempdir, never a bare root.
        normalized = path.rstrip("/")
        if not normalized.startswith("/") or normalized.count("/") < 2:
            continue
        command = command.replace(normalized + "/", "~/.hades/")
    return command
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    try:
        ...
        if "HADES_SPINNER_PAUSE" in os.environ:
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("HADES_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set HADES_INTERACTIVE or HADES_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("HADES_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("HADES_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("HADES_EXEC_ASK")
