...
#      `hades-agent-dev` skill contains a security postmortem mentioning
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("HADES_SESSION_PLATFORM")
    origin_chat_id = get_session_env("HADES_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        ...
        return {
            ...
            "chat_name": get_session_env("HADES_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via hades model.
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "profile": {
                "type": "string",
                "description": "Optional Hades profile name to run the job under. When set, the scheduler resolves that profile, applies a context-local Hades home override, loads that profile's config/.env for the run, and bridges HADES_HOME into subprocesses. Any temporary process-environment changes from profile .env loading are restored after the job exits. Use 'default' for the root Hades profile. Named profiles must already exist. When unset (default), preserves the scheduler's existing profile. On update, pass an empty string to clear. Jobs with profile run sequentially (not parallel) to keep profile-scoped runtime state isolated."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("HADES_INTERACTIVE")
        or env_var_enabled("HADES_GATEWAY_SESSION")
        or env_var_enabled("HADES_EXEC_ASK")
    )
