...
def read_suppressed_names() -> Set[str]:
    ...
    what makes pruning a built-in durable: without it, ``hades update`` would
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip Hades metadata, VCS, virtualenv/dependency, and cache dirs
...
def list_archived_skill_names() -> List[str]:
    ...
    so the directory name is the skill name. Used by ``hades curator
    list-archived`` to help users pass a name to ``hades curator restore``.
    """
