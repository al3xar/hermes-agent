...
Backend is selected during ``hades tools`` setup (web.backend in config.yaml).
When available, Hades can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Hades config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Hades' config/.env layer (``hades config set``, ``hades tools``)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``hades tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `hades tools` repaint.
        try:
