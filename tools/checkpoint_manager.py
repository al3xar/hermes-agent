...
-----------------------------------------------------------------------------
    ...
    ~/.hades/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/hades/<hash16>        — per-project branch tip
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Hades convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("HADES_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Hades infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``hades checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `hades checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "hades@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Hades Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``HADES_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at HADES_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "HADES_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `hades checkpoints` CLI
# ---------------------------------------------------------------------------
