...
def register_credential_file(
    ...
    *relative_path* is relative to ``HADES_HOME`` (e.g. ``google_token.json``).
    ...
    """
    hades_home = _resolve_hades_home()
    ...
    host_path = hades_home / relative_path
    ...
    containment_error = validate_within_dir(host_path, hades_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.hades/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="hades-skills-safe-"))
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.hades``,
    SSH ``<remote_home>/.hades``, etc.) and whether translation is wanted.
