...
orchestrator work. A normal ``hades chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``hades kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``hades kanban complete …``
   inside the container, where ``hades`` isn't installed and the DB
...
Humans continue to use the CLI (``hades kanban …``), the dashboard
(``hades dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``hades chat`` without the kanban toolset see zero
