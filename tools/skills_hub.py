...
Used by hades_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hades_meta = meta_block.get("hades", {})
                if isinstance(hades_meta, dict):
                    tags = hades_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Hades centralized index source
# ---------------------------------------------------------------------------

HADES_INDEX_URL = "https://hades-agent.nousresearch.com/docs/api/skills-index.json"
HADES_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "hades-index.json"
HADES_INDEX_TTL = 6 * 3600  # 6 hours


def _load_hades_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for HADES_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if HADES_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - HADES_INDEX_CACHE_FILE.stat().st_mtime
            if age < HADES_INDEX_TTL:
                return json.loads(HADES_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(HADES_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Hades index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Hades index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        HADES_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        HADES_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class HadesIndexSource(SkillSource):
    """Skill source backed by the centralized Hades Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "hades-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "hades-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        HadesIndexSource(auth=auth), # Centralized index (search + resolved install paths)
