...
def _is_hades_provider_credential(name: str) -> bool:
    """True if ``name`` is a Hades-managed provider credential (API key,
    token, or similar) per ``_HADES_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Hades API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _HADES_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_HADES_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    ...
    Non-Hades third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_hades_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Hades provider "
                "credential %r (blocked by _HADES_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                if _is_hades_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Hades "
                        "provider credential %r from config.yaml (blocked "
                        "by _HADES_PROVIDER_ENV_BLOCKLIST). Operator "
