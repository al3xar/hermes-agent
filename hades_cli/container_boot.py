...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-hades-setup
...
def reconcile_profile_gateways(
    *,
    ...
    Args:
        hades_home: The container's HADES_HOME (typically /opt/data).
            Profiles live under ``<hades_home>/profiles/<name>/``;
            the default profile lives at ``<hades_home>`` itself.
    ...
    # ``hades gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_prior_state(hades_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(hades_home)
    ...
    profiles_root = hades_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `hades profile create` (config.yaml
            # is not — that comes later via `hades setup`). Use it as the
            ...
            # via ``hades -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(hades_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = hades_home / "gateway_state.json"
...
def _is_legacy_gateway_run_request(argv: Sequence[str]) -> bool:
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "hades":
        args = args[1:]
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `hades -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit hades
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the hades user) won't hit EACCES. See
...
def _write_reconcile_log(
    hades_home: Path, actions: list[ReconcileAction],
) -> None:
...
def main() -> int:
    ...
    hades_home = Path(os.environ.get("HADES_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        hades_home=hades_home, scandir=scandir,
    )
    for a in actions:
        print(
