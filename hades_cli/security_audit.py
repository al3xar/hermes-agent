...
Scans three surfaces a Hades user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Hades venv (every PyPI dist via ``importlib.metadata``).
...
def _discover_plugins(hades_home: Path) -> list[Component]:
...
def run_audit(
    *,
    ...
    home = hades_home or Path(get_hades_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, hades_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(hades_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `hades security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, hades_home=home
    )
    if total == 0:
