        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {
                        "key": "HADES_LANGFUSE_PUBLIC_KEY",
                        "prompt": "Langfuse public key (pk-lf-...)",
                        "url": "https://cloud.langfuse.com",
                    },
                    {
                        "key": "HADES_LANGFUSE_SECRET_KEY",
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(
                    f"    npm install failed - run manually: cd {display_hades_home()}/hades-agent && npm install --workspaces=false"
                )
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning(
                "    Node.js not found - browser tools require: npm install (in hades-agent directory)"
            )
            return
        ...
        if _running_in_docker():
            ...
            _print_info("      docker pull ghcr.io/nousresearch/hades-agent:latest")
            return
        ...
        _print_info(
            "    Switch voices by setting tts.piper.voice in ~/.hades/config.yaml"
        )
        ...
        try:
            ...
            _print_info(
                "    Run later: hades auth add xai-oauth   (or set XAI_API_KEY)"
            )
            return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(
        color(
            f"  Tool configuration saved to {display_hades_home()}/config.yaml",
            Colors.DIM,
        )
    )
    print(
        color("  Changes take effect on next 'hades' or gateway restart.", Colors.DIM)
    )
    print()
