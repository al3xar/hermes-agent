...
configured for "cli" in `hades tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HADES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `hades chat`:
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to HADES_INFERENCE_MODEL
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "hades -z: --provider requires --model (or HADES_INFERENCE_MODEL). "
...
def _create_session_db_for_oneshot():
    ...
    Oneshot bypasses ``HadesCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when hades is invoked for
    ...
    agent = AIAgent(
        ...
        #                HADES_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via HADES_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via HADES_YOLO_MODE=1
