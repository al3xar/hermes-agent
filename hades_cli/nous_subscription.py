...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `hades model`, so direct credentials should NOT
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`hades tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``hades tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``hades model``'s Nous
    login, this:
