...
try:
    ...
    # running `hades dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(
    ...
    The scheduler tick loop normally lives in ``hades gateway run`` — but the
    desktop app spawns a ``hades dashboard`` backend, not a gateway, so a cron
    ...
    HADES_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (HADES_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hades
...
# HADES_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
_SESSION_TOKEN = os.environ.get(
    "HADES_DASHBOARD_SESSION_TOKEN"
) or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Hades-Session-Token"
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Hades-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Hades (no terminal prompt), "
...
def _default_hades_root_is_opt_data() -> bool:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    if not _local_dashboard_request(request) or _default_hades_root_is_opt_data():
...
async def get_status():
    ...
    # and which providers are registered so ``hades status`` and the
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the hades home root).
    """
...
async def run_curator():
    ...
    try:
        proc = _spawn_hades_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def run_prompt_size():
    try:
        proc = _spawn_hades_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_hades_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_hades_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
def _spawn_hades_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "HADES_NONINTERACTIVE": "1"},
    }
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``hades gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``hades gateway restart`` children race each other on the
    ...
    return _spawn_hades_action(["gateway", "restart"], "gateway-restart"), False


@app.post("/api/gateway/restart")
async def restart_gateway():
    """Kick off a ``hades gateway restart`` in the background."""
    try:
...
@app.post("/api/hades/update")
async def update_hades():
    """Kick off ``hades update`` in the background."""
    install_method = detect_install_method(PROJECT_ROOT)
    if install_method == "docker":
        message = format_docker_update_message()
        _record_completed_action("hades-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "hades-update",
    ...
    try:
        proc = _spawn_hades_action(["update"], "hades-update")
    except Exception as exc:
        _log.exception("Failed to spawn hades update")
...
@app.get("/api/hades/update/check")
async def check_hades_update(force: bool = False):
    """Report whether a Hades update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hades/update`` actually runs ``hades update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Hades version string
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Hades surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Hades supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in hades_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options():
    ...
    try:
        ...
        # onboarding picker) sees the SAME full provider universe `hades model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `hades model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment):
    ...
    try:
        ...
        if scope == "main":
            ...
            # post-model-selection behaviour (hades_cli/main.py
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Hades from Slack via Socket Mode.",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.hades-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"HadesDashboard/{__version__}"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Hades appears
...
# the canonical ``hades auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Hades resolves Anthropic creds in this order at runtime:
    1. ``~/.hades/.anthropic_oauth.json`` — Hades-managed PKCE flow
    ...
    try:
        ...
        read_hades_oauth_credentials = None  # type: ignore
        _HADES_OAUTH_FILE = None  # type: ignore

    hades_creds = None
    if read_hades_oauth_credentials:
        try:
            hades_creds = read_hades_oauth_credentials()
        except Exception:
            hades_creds = None
    if hades_creds and hades_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hades_pkce",
            "source_label": f"Hades PKCE ({_HADES_OAUTH_FILE})",
            "token_preview": _truncate_token(hades_creds.get("accessToken")),
            "expires_at": hades_creds.get("expiresAt"),
            "has_refresh_token": bool(hades_creds.get("refreshToken")),
        }
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("hades_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Hades-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _HADES_OAUTH_FILE

            if _HADES_OAUTH_FILE.exists():
                _HADES_OAUTH_FILE.unlink()
        except Exception:
            pass
...
# Guarded so hades web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(
    ...
    """Persist Anthropic PKCE creds to both Hades file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``hades auth add anthropic``.
    """
    from agent.anthropic_adapter import _HADES_OAUTH_FILE
    ...
    _HADES_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _HADES_OAUTH_FILE.with_name(
        f"{_HADES_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _HADES_OAUTH_FILE)
        try:
            _HADES_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "hades-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
# the tokens exactly like `hades auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `hades auth add xai-oauth`'s credential-pool insert.
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `hades sessions stats`).
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process HADES_HOME at import time. The dashboard is a single
...
async def add_mcp_server(body: MCPServerCreate):
    from hades_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def test_mcp_server(name: str):
    ...
    from hades_cli.mcp_config import _get_mcp_servers, _probe_single_server
...
async def list_mcp_catalog():
    ...
    `hades mcp catalog` / `hades mcp install` read.
    """
    try:
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall):
    ...
    if entry.install is not None:
        try:
            proc = _spawn_hades_action(["mcp", "install", name], "mcp-install")
        except Exception as exc:
...
# spawn the real `hades gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_hades_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_hades_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
# `hades memory setup`).  The dashboard covers the common admin actions:
...
async def run_doctor():
    try:
        proc = _spawn_hades_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_hades_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_hades_action(args, "backup")
    except Exception as exc:
...
async def run_import(body: ImportRequest):
    ...
    try:
        proc = _spawn_hades_action(["import", archive], "import")
    except Exception as exc:
...
async def list_checkpoints():
    ...
    # Checkpoints live under <hades_home>/checkpoints/.  Surface a count +
...
async def prune_checkpoints():
    try:
        proc = _spawn_hades_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
async def install_skill_hub(body: SkillInstallRequest):
    ...
    try:
        proc = _spawn_hades_action(["skills", "install", identifier], "skills-install")
    except Exception as exc:
...
async def update_skills_hub():
    try:
        proc = _spawn_hades_action(["skills", "update"], "skills-update")
    except Exception as exc:
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`hades -p <name> skills install`)
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_hades_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "hades setup" if name == "default" else f"{name} setup"
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``hades profile use`` —
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``hades profile use``).
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``hades profile describe
    <name> --auto``.
...
async def toggle_toolset(name: str, body: ToolsetToggle):
    ...
    helper the CLI ``hades tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str):
    ...
    Surfaces the same provider rows the CLI ``hades tools`` picker shows
...
async def select_toolset_provider(name: str, body: ToolsetProviderSelect):
    ...
    core extracted from the CLI configurator — so the GUI and ``hades tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    the same store ``hades tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(name: str, body: ToolsetPostSetup):
    ...
    ``hades tools post-setup <key>`` and the frontend tails the log via
...
#
# The endpoint spawns the same ``hades --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from hades_cli.win_pty_bridge import (
...
def _resolve_chat_argv(
    ...
    Default: whatever ``hades --tui`` would run.  Tests monkeypatch this
    ...
    ``HADES_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``HADES_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from hades_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("HADES_TUI_DISABLE_MOUSE", "1")
    env.setdefault("HADES_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["HADES_TUI_RESUME"] = resume

    if sidecar_url:
        env["HADES_TUI_SIDECAR_URL"] = sidecar_url

    if gateway_ws_url := _build_gateway_ws_url():
        env["HADES_TUI_GATEWAY_URL"] = gateway_ws_url
...
# HADES_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/hades/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /hades`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__HADES_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/hades``)
        ...
        ``__HADES_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__HADES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HADES_BASE_PATH__="{prefix}";'
                f"window.__HADES_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__HADES_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__HADES_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HADES_BASE_PATH__="{prefix}";'
                f"window.__HADES_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/hades`` they'd hit ``mission-control.tilos.com/fonts/...``
...
def _discover_user_themes() -> list:
    ...
    """
    themes_dir = get_hades_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    3. Project plugins: ./.hades/plugins/  (only if HADES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    # ``hades_cli/plugins.py`` and the documented user contract.
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"hades auth {name}"
                        break
            except Exception:
                pass
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.hades/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"hades_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing HADES_DASHBOARD_OAUTH_CLIENT_ID).
    ...
    print(f"  Hades Web UI → http://{host}:{port}")
