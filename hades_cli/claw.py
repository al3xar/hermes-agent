...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_hades.py script in known locations."""
...
def _cmd_migrate(args):
    ...
    # migrate-hades posture (two-phase: run once without secrets, rerun with
    ...
    print_info(f"Target:      {hades_home}")
    ...
    # restorable with `hades import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            ...
            backup_archive = create_pre_migration_backup(hades_home=hades_home)
            if backup_archive:
