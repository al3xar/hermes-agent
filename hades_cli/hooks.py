"""hades hooks — inspect and manage shell-script hooks.

Usage::

    hades hooks list
    hades hooks test <event> [--for-tool X] [--payload-file F]
    hades hooks revoke <command>
    hades hooks doctor
...
def hooks_command(args) -> None:
    """Entry point for ``hades hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: hades hooks {list|test|revoke|doctor}")
        print("Run 'hades hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `hades hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `hades hooks test` and `hades hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Hades wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `hades hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `hades hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `hades hooks doctor`.")
