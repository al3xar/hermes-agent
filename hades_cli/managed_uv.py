...
Hades owns its own uv binary at ``$HADES_HOME/bin/uv`` (or ``uv.exe`` on
...
def managed_uv_path() -> Path:
    """Return the path where Hades keeps *its* uv binary.
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``hades update`` so the managed copy stays current.
