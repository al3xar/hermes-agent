...
# display_hades_home imported lazily at call sites (stale-module safety during hades update)
...
def _print_setup_summary(config: dict, hades_home):
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {hades_home}/cron/, sessions/, logs/"
    )
    print()
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into HADES_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("HADES_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HADES_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        ...
        if "/profiles/" in hades_home:
            profile_name = hades_home.rstrip("/").rsplit("/", 1)[-1]
    except Exception:
        pass
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        print_info(
            "   You can generate it manually later with: hades slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info(
            "📬 Home Room: where Hades delivers cron job results and notifications."
        )
        print_info(
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info(
            "No platforms selected. Run 'hades setup gateway' later to configure."
        )
        return
    ...
    if any_messaging:
        ...
        if supports_systemd and has_legacy_hades_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info(
                    "   hades gateway run          # Run as container main process"
                )
                print_info("")
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_hades.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_hades migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_hades", _OPENCLAW_SCRIPT)
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(
            color(
                "  Would overwrite (conflicts with existing Hades config):",
                Colors.YELLOW,
            )
        )
    ...
    if warnings_shown:
        ...
        print(
            color(
                "  Note: OpenClaw config values may have different semantics in Hades.",
                Colors.YELLOW,
            )
        )
        print(
            color(
                "  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Hades's yolo mode.",
                Colors.YELLOW,
            )
        )
        print(
...
def _offer_openclaw_migration(hades_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=hades_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info("Migration cancelled. You can run it later with: hades claw migrate")
        print_info(
    ...
    if conflicts:
        print_info(
            f"Skipped {conflicts} item(s) that already exist in Hades (use hades claw migrate --overwrite to force)."
        )
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(
        color(
            "│     ⚕ Hades Setup — Nous Portal (one-shot)             │", Colors.MAGENTA
        )
    )
    print(
        color(
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Hades Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, hades_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(hades_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, hades_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {hades_home}")
    ...
    _print_setup_summary(config, hades_home)


def _run_first_time_quick_setup(config: dict, hades_home, is_existing: bool):
...
def _run_quick_setup(config: dict, hades_home):
