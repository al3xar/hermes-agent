...
Pure display functions with no HadesCLI state dependency.
"""
...
def check_for_updates() -> Optional[int]:
    """Check whether a Hades update is available.

    Two paths: if ``HADES_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = hades_home / ".update_check"
    embedded_rev = os.environ.get("HADES_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # HADES_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `hades update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/hades/update/check`
    ...
    if embedded_rev:
        ...
        # $HADES_HOME/hades-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hades_home / "hades-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Hades git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$HADES_HOME/hades-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``hades_cli/build_info.py``) and return it as a frozen
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Hades checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/hades-agent repo (forks don't get a link).
    """
    ...
    try:
        ...
        if behind is not None and behind != 0:
            ...
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built hades; we know an update
