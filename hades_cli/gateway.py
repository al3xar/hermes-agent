"""
Gateway subcommand for hades CLI.

Handles: hades gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when hades_constants is cached from a pre-update version during `hades update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "hades CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``hades gateway status`` from falsely counting the ``hades`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``hades gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    patterns = [
        ...
        # ``hades.exe --profile`` / ``hades.exe -p`` would also match
        # ``hades.exe --profile foo dashboard`` and other CLI subcommands,
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"hades_home={current_home_lc}" in command_lc
            )
        ...
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "hades_home=" in command_lc
            and f"hades_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``hades update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Hades profile are returned.
    """
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``hades update`` users
    # with no running gateway until they re-invoke ``hades gateway``
...
def _sync_hades_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``HADES_HOME``.

    Under ``sudo``, ``HADES_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hades_home` falls back to ``/root/.hades`` — the wrong
    ...
    unit_home = env.get("HADES_HOME", "").strip()
    if not unit_home:
        return
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}hades gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}hades gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `hades gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``hades gateway status`` output so users with
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Hades Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``hades gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``HADES_GATEWAY_DETACHED=1``; older wrappers without the env marker are
...
def _profile_arg(hades_home: str | None = None) -> str:
    ...
    Args:
        hades_home: Optional explicit HADES_HOME path. Defaults to the current
...
# Legacy service names from older Hades installs that predate the
...
# "hades" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("hades.service",)
...
def _find_legacy_hades_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Hades gateway units.

    Detects unit files installed by older Hades versions that used a
    different service name (e.g. ``hades.service`` before the rename to
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      ...
      ``hades-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``hades.service`` running an unrelated
...
def has_legacy_hades_units() -> bool:
    """Return True when any legacy Hades gateway unit files exist."""
    return bool(_find_legacy_hades_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Hades gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_hades_units()
    if not legacy:
        return
    print_warning("Legacy Hades gateway unit(s) detected from an older install:")
    ...
    print_info("    hades gateway migrate-legacy")


def remove_legacy_hades_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Hades gateway unit files.

    Iterates over whatever ``_find_legacy_hades_units()`` returns — which is
    ...
    legacy = _find_legacy_hades_units()
    if not legacy:
        print("No legacy Hades gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Hades gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: hades gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo hades gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    hades gateway uninstall")
    print_info("    sudo hades gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Hades can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo hades gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo hades gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo hades gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Hades often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.hades`` → ``ai.hades.gateway.plist`` (backward compatible).
    Profile ``~/.hades/profiles/coder`` → ``ai.hades.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.hades.gateway-{suffix}" if suffix else "ai.hades.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.hades/hades-agent  -> /home/alice/.hades/hades-agent
...
def _hades_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.hades                    → /home/alice/.hades
      /root/.hades/profiles/coder     → /home/alice/.hades/profiles/coder
      /opt/custom-hades               → /opt/custom-hades  (kept as-is)
    """
    current_hades = get_hades_home().resolve()
    current_default = (Path.home() / ".hades").resolve()
    target_default = Path(target_home_dir) / ".hades"

    # Default ~/.hades → remap to target user's default
    if current_hades == current_default:
        return str(target_default)

    # Profile or subdir of ~/.hades → preserve the relative structure
    try:
        relative = current_hades.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.hades) — keep as-is
        return str(current_hades)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    hades_node = hades_home / "node" / "bin"
    if _is_dir(hades_node):
        candidates.append(str(hades_node))
    hades_nm = hades_home / "node_modules" / ".bin"
    if _is_dir(hades_nm):
        candidates.append(str(hades_nm))
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``hades update`` later relocates/removes — the path
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        hades_home = _hades_home_for_target_user(home_dir)
        ...
        working_dir = str(hades_home) if hades_home else _remap_path_for_user(working_dir, home_dir)
...
Environment="HADES_HOME={hades_home}"
Restart=always
RestartSec=5
...
"""
    ...
    hades_home = str(get_hades_home().resolve())
    profile_arg = _profile_arg(hades_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # generated unit bakes that ``/tmp/pytest-of-.../hades_test`` path into
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Hades install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo hades gateway uninstall --system")
    print_info("         hades gateway install")
    print_info("         hades gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (hades.service from pre-rename installs)
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}hades gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}hades gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}hades gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_hades_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `hades gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `hades gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}hades gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}hades gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}hades gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}hades gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.hades.gateway-{suffix}" if suffix else "ai.hades.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``hades gateway start/stop/status`` call).
...
# (the documented `nohup hades gateway run` workaround). See #23387.
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup hades gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: hades gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup hades gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Hades install"
    )
    return True
...
def launchd_stop():
    ...
    # `hades gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Hades install")
    else:
        print("⚠ Service definition is stale relative to the current Hades install")
        print("  Run: hades gateway start")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print(
        "  The image entrypoint normally drops privileges to the 'hades' user. "
    ...
    print(
        "  Set HADES_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `hades gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set HADES_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `hades gateway restart` which already
    ...
    # `hades gateway start/restart` — leaving the gateway vulnerable to
    ...
    # HADES_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Hades will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @hades:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. hades) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Hades delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Hades agent",
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Hades will use (e.g., hades@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Hades connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: hades pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Hades will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``hades setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run hades under
        WSL.
    """
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: hades pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'hades gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Hades will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Hades will store the returned account_id/token in ~/.hades/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `hades gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `hades pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `hades pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "HadesAgent"')
...
def gateway_setup():
    ...
    if any_configured:
        ...
        if service_running:
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: hades gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo hades gateway install --system"
                        )
                    print_info("  Or run in foreground:  hades gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: hades gateway run")
                print_info(
                    "  For persistence:   tmux new -s hades 'hades gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: hades gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup hades gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'hades gateway setup' when ready.")
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``hades gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``hades gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``hades gateway run`` are unaffected.
      2. ``HADES_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``hades gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``HADES_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    print(
        ...
        "  dashboard supervised alongside if HADES_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or HADES_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-hades service's
    # pattern in docker/s6-rc.d/main-hades/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: hades gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: hades gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hades 'hades gateway run'"
                )
                print()
            ...
            print()
            print(
                "  hades gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hades 'hades gateway run'         # persistent via tmux"
            )
            print(
                "  nohup hades gateway run > ~/.hades/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  hades profile create <name>     # creates the s6 service slot")
                print("  hades -p <name> gateway start   # bring it up via s6")
                print("  hades status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: hades gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: hades gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: hades gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  hades profile delete <name>     # tears down the s6 service slot")
                print("  hades -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `hades -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: hades gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HADES_GATEWAY") == "1":
            print_error(
                ...
                "Use `hades gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HADES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `hades gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: hades gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  hades gateway install")
                else:
                    print("To install as a service:")
                    print("  hades gateway install")
                    print("  sudo hades gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  hades gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup hades gateway run > ~/.hades/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hades 'hades gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  hades gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  hades gateway install  # Install as user service")
                    print(
                        "  sudo hades gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Hades gateway unit files from
        # pre-rename installs (e.g. hades.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_hades_units(interactive=not yes, dry_run=dry_run)
