"""Profile distributions — shareable, packaged Hades profiles via git.

A distribution is a Hades profile published as a git repository (or
...
* ``hades profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``hades skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``hades profile``, not a parallel tree):

    hades profile install <source> [--name N] [--alias] [--force] [--yes]
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            hades_requires=str(data.get("hades_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.hades_requires:
            out["hades_requires"] = self.hades_requires
        if self.author:
...
def check_hades_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Hades {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``hades profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Hades profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Hades distribution."
        )

    # Version check up-front so we fail fast
    check_hades_requires(manifest.hades_requires, hades_version)
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="hades_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `hades profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`hades profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="hades_dist_update_") as tmp:
        plan = plan_install(
