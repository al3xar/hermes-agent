...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hades-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.hades/plugins/<name>/``
3. **Project plugins** – ``./.hades/plugins/<name>/`` (opt-in via
...
# ---------------------------------------------------------------------------
#
# Set ``HADES_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When HADES_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Hades loggers.
    """
    ...
    logger.debug(
        "HADES_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManifest:
    ...
    # lookups and by ``hades plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``hades honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``hades <subcommand>``
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``hades model → Configure auxiliary models`` picker
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to hades_cli/. Two layouts are
        ...
        # 3. Project plugins (./.hades/plugins/)
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with hades and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Hades ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `hades plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``hades_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``hades_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="hades-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``hades tools`` TUI so plugin-provided toolsets appear
