"""
Hades Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hades desktop`` / ``hades gui``)
     Built inside the agent checkout under ``$HADES_HOME/hades-agent/``:
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Hades.app`` or ``~/Applications/Hades.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Hades`` (NSIS per-user)
...
the app name ("Hades"), separate from ``$HADES_HOME``:
  - macOS:   ``~/Library/Application Support/Hades``
  - Windows: ``%APPDATA%\\Hades``
  - Linux:   ``$XDG_CONFIG_HOME/Hades`` (default ``~/.config/Hades``)
...
``hades uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(hades_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return hades_home / "hades-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Hades"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Hades"
...
def source_built_gui_artifacts(hades_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``hades desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``hades-agent/`` source + ``venv/`` and never
    ...
    return [
        ...
        hades_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Hades" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Hades.app"),
            home / "Applications" / "Hades.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Hades).
            local_base / "Programs" / "Hades",
            # Older / alternate layout some builds used.
            local_base / "hades-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Hades")
    else:
        ...
        paths += [
            data_base / "applications" / "hades.desktop",
            data_base / "applications" / "Hades.desktop",
        ]
    return paths


def agent_is_installed(hades_home: Path) -> bool:
...
def gui_is_installed(hades_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(hades_home):
        if p.exists():
            return True
...
def gui_install_summary(hades_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = hades_home if hades_home is not None else get_hades_home()
...
def uninstall_gui(hades_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove hades' or "
            "'sudo dnf remove hades'). AppImage builds are a single file you "
