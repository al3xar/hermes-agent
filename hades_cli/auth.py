...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = (
        os.getenv("HADES_QWEN_BASE_URL", "").strip().rstrip("/")
...
def resolve_codex_runtime_credentials(
    *,
    ...
    refresh_timeout_seconds = float(
        os.getenv("HADES_CODEX_REFRESH_TIMEOUT_SECONDS", "20")
    )
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(
        os.getenv("HADES_XAI_REFRESH_TIMEOUT_SECONDS", "20")
    )
    ...
    *,
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print(
                "Hades will create its own session to avoid conflicts with Codex CLI / VS Code."
            )
            try:
                do_import = (
                    input(
