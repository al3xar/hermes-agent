...
def remove_path_from_shell_configs():
    """Remove Hades PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing hades-agent or hades PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Hades Agent" comment and following line
                if '# Hades Agent' in line or '# hades-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('hades' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing hades
                if 'hades' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the hades wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "hades",
        Path("/usr/local/bin/hades"),
    ]
...
def remove_node_symlinks(hades_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``hades`` command:
    ...
    """
    node_dir = (hades_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``hades gateway run`` processes
...
#
#   1. Sets User-scope env vars ``HADES_HOME`` and ``HADES_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\hades\git\cmd``, ``%LOCALAPPDATA%\hades\git\bin``,
#      ``%LOCALAPPDATA%\hades\git\usr\bin``, ``%LOCALAPPDATA%\hades\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\hades\git\`` and Node to
#      ``%LOCALAPPDATA%\hades\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``hades dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\hades\gateway-service\`` and sometimes
...
def _hades_path_markers(hades_home: Path) -> list[str]:
    """Path-entry substrings that identify Hades-owned User-PATH entries."""
    root = str(hades_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare hades-agent install dir.
    markers = [root + "\\hades-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if HADES_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "hades".  We don't want to catch
    # unrelated entries like "chades-foo" or "ephermeral", so we look for
    # backslash-hades as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hades_home: Path) -> list[str]:
    """Strip Hades-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hades_path_markers(hades_home)
            kept: list[str] = []
...
def remove_hades_env_vars_windows() -> list[str]:
    """Delete HADES_HOME and HADES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("HADES_HOME", "HADES_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``hades -p <name> gateway stop|uninstall`` because
    ...
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                hades_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
...
def run_gui_uninstall(args):
    ...
    Mirrors ``hades uninstall --gui``. Removes the desktop app's built
    ...
    summary = gui_install_summary(hades_home)
    ...
    if not summary["gui_installed"]:
        ...
        print(f"  Checked: {hades_home}, and the standard app locations for this OS.")
        return
    ...
    print()
    if agent_is_installed(hades_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Hades agent at {hades_home / 'hades-agent'}")
        print(f"  • Your config, sessions, and secrets under {hades_home}")
        print()
...
def run_uninstall(args):
    ...
    # offer to clean them up too instead of leaving zombie HADES_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_hades_home(hades_home)
    ...
    print(f"  Config:  {hades_home / 'config.yaml'}")
    print(f"  Secrets: {hades_home / '.env'}")
    print(f"  Data:    {hades_home / 'cron/'}, {hades_home / 'sessions/'}, {hades_home / 'logs/'}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``hades`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in hades_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\hades\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(hades_home))))
        ...
        log_info("Removing HADES_HOME / HADES_GIT_BASH_PATH User env vars...")
        removed_env = remove_hades_env_vars_windows()
        if removed_env:
    ...
    log_info("Removing hades command...")
    ...
    removed_node_links = remove_node_symlinks(hades_home)
    ...
    #     step-5 rmtree(hades_home) would sweep the in-tree artifacts anyway,
    ...
    try:
        ...
        gui_removed = uninstall_gui(hades_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.hades/, just remove the hades-agent subdir
            if hades_home in project_root.parents or project_root.parent == hades_home:
    ...
    #     the step-5 rmtree(hades_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(hades_home)
    ...
    if full_uninstall:
        ...
        try:
            if hades_home.exists():
                shutil.rmtree(hades_home)
                log_success(f"Removed {hades_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {hades_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {hades_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://hades-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://hades-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
