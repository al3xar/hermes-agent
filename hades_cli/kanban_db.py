...
``<root>`` is the **shared Hades root** (the parent of any active
...
``hades -p <profile>`` joins the same board as the dispatcher that
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``HADES_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``HADES_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``hades kanban boards
...
* ``HADES_KANBAN_DB`` — pin the database file path directly.
* ``HADES_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``HADES_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``HADES_KANBAN_DB``,
``HADES_KANBAN_WORKSPACES_ROOT``, and ``HADES_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/hades-kanban-v1-spec.pdf`` for the full
...
# ``HADES_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``HADES_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``HADES_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``HADES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "HADES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "hades_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``hades-agent``
...
def kanban_home() -> Path:
    """Return the shared Hades root that anchors the kanban board.

    Resolution order:

    1. ``HADES_KANBAN_HOME`` env var when set and non-empty (explicit
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``hades kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``HADES_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``hades kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``hades kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``HADES_KANBAN_DB`` env var — pins the path directly. Honoured for
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``HADES_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``HADES_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``hades kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``HADES_SESSION_ID``. NULL for
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Hades treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``HADES_KANBAN_DB`` env → ``HADES_KANBAN_BOARD`` env →
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``hades kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``HADES_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def create_task(
    ...
    *,
    ...
    each name to ``hades --skills ...`` alongside the built-in
    ...
    # `hades --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``hades kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `hades kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``hades kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``hades kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``hades kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``hades kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``HADES_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
...
#      via ``hades kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``HADES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Hades
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``hades kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Hades profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Hades profile.
        # `_default_spawn` invokes ``hades -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Hades
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `hades kanban tail` — without
...
def _absolute_hades_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Hades shim."""
...
def _hades_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Hades executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_hades_argv()
    return [_absolute_hades_path(path)]


def _resolve_hades_argv() -> list[str]:
    """Resolve the ``hades`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$HADES_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("hades")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\hades.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m .hades_cli.main`` — fallback for setups where
       Hades is launched from a venv and the ``hades`` shim is not on
    ...
    if env_bin:
        if _looks_like_path(env_bin):
            return _hades_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _hades_path_argv(resolved_env_bin)
        return _module_hades_argv()

    hades_bin = _safe_which_no_cwd("hades") if _IS_WINDOWS else shutil.which("hades")
    if hades_bin:
        return _hades_path_argv(hades_bin)
    return _module_hades_argv()


def _kanban_worker_skill_available(hades_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``hades -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset HADES_HOME means the worker falls back to the default root
    # home (``~/.hades``), which ships the bundled skill.
    base = _Path(hades_home) if hades_home else (_Path.home() / ".hades")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``hades -p <profile> chat -q ...`` subprocess.
    ...
    ``HADES_KANBAN_DB`` / ``HADES_KANBAN_BOARD`` / workspaces_root env
    ...
    # env, and when the child process starts `hades -p <name>` the
    # _apply_profile_override() runs *before* hades_constants is imported.
    ...
    try:
        ...
        # _apply_profile_override() via HADES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # HADES_HOME never had profiles created.
        pass
    if task.tenant:
        env["HADES_TENANT"] = task.tenant
    env["HADES_KANBAN_TASK"] = task.id
    env["HADES_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["HADES_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["HADES_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["HADES_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["HADES_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["HADES_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`hades -p <name>`
    ...
    env["HADES_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["HADES_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["HADES_KANBAN_BOARD"] = resolved_board
    # HADES_PROFILE is the author the kanban_comment tool defaults to.
    # `hades -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["HADES_PROFILE"] = profile_arg
    ...
    if _kanban_worker_skill_available(env.get("HADES_HOME")):
    ...
    # `hades kanban log` on a specific board reads its own file and
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``hades kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HADES_PROFILE values like "hades-system"
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``hades kanban assignees`` for the terminal.
