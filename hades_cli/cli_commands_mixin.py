...
``cli.py``'s ``HadesCLI`` class. ``HadesCLI`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: hades --checkpoints")
    ...
    def _handle_paste_command(self):
        ...
        from hades_cli.clipboard import hades_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(
                f'  {_DIM}Tip: type your next message, or run hades chat -q --image {_termux_example_image_path(image_path.name)} "What do you see?"{_RST}'
            )
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in hades_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `hades gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `hades sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint(
                "  Use /history or `hades sessions list` to see available sessions."
            )
            return
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to .hades_cli.curator so the CLI and the `hades curator`
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``hades bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: hades bundles create "
        ...
        _cprint(
            ...
            f"Manage with `hades bundles`.{_RST}"
        )
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Hades keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from hades_cli.skin_engine import (
                list_skins,
                set_active_skin,
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(
                f"  Custom skins: drop a YAML file in {display_hades_home()}/skins/\n"
            )
            return
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Hades is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Hades is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Hades is busy."
            _cprint(
    ...
    def _handle_update_command(self) -> bool:
        ...
        ``hades update`` so the user sees update output directly and gets
        ...
        raw = self._prompt_text_input_modal(
            ...
            detail="This will exit the current session and run `hades update`.",
            choices=choices,
        )
        if raw is None:
