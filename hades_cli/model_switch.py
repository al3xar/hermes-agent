...
_HADES_MODEL_WARNING = (
    "Nous Research Hades 3 & 4 models are NOT agentic and are not designed "
    "for use with Hades Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hades 3 / Hades 4 chat families.
# The previous substring check (`"hades" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hades-brain:qwen3-14b-ctx16k`` that just
# happen to carry "hades" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hades-3-Llama-3.1-70B, hades-4-405b, openrouter/hades3:70b
# Negative examples it must NOT match:
#   hades-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HADES_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hades[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_hades_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Hades 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_HADES_NON_AGENTIC_RE.search(model_name))


def _check_hades_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hades 3/4 chat model."""
    if is_nous_hades_non_agentic(model_name):
        return _HADES_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``hades config set model.aliases.xxx``)
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'hades model' for available providers, or define it "
    ...
    # Mirrors the same logic in .hades_cli.runtime_provider.resolve_runtime_provider;
    ...
    hades_warn = _check_hades_model_warning(new_model)
    if hades_warn:
        warnings.append(hades_warn)
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Build curated model lists keyed by hades provider ID
    ...
    for hades_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(hades_id)
        if (
            _alias_target
            and _alias_target != hades_id
        ...
        pconfig = PROVIDER_REGISTRY.get(hades_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (HADES_OVERLAYS) with proper auth store checking.
        ...
        # /model picker sees the SAME list `hades model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(hades_id)
        if not model_ids:
            model_ids = curated.get(hades_id, [])
            if hades_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(hades_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Hades-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hades_cli.providers import HADES_OVERLAYS
    from hades_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hades provider ID.
    # HADES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hades IDs ("copilot").
    _mdev_to_hades = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HADES_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Hades slug — e.g. "github-copilot" → "copilot"
        hades_slug = _mdev_to_hades.get(pid, pid)
        if hades_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(hades_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, hades_slug):
        ...
        if not has_creds:
            try:
                ...
                logger.debug("Credential pool check failed for %s: %s", hades_slug, exc)
        ...
        # configured.
        if not has_creds and hades_slug == "anthropic":
            try:
        ...
        if hades_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(hades_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `hades model` picker deliberately shows ONLY the curated agentic
            ...
            if not model_ids:
                model_ids = curated.get(hades_slug, []) or curated.get(pid, [])
                if hades_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(hades_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": hades_slug,
            "name": get_label(hades_slug),
            "is_current": hades_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hades",
        ...
        seen_slugs.add(hades_slug.lower())
        _record_builtin_endpoint(hades_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or HADES_OVERLAYS (keeps /model in sync
    # with `hades model`).
    try:
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Hades writes ``models:`` as a dict keyed by model id
            # (see hades_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, hades_cli/config.py) already consume that dict.
