"""``hades debug`` debug tools for Hades Agent.

Currently supports:
    hades debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[hades debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``hades debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``hades debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Hades version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `hades debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "hades-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``hades debug share`` invocation added ~20 MB of resident Python
    ...
    ``hades debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "hades-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----HadesDebugBoundary9f3c"
...
def _capture_dump() -> str:
    """Run ``hades dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``hades dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``hades debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "hades debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `hades debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  hades debug delete <url>")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: hades debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'hades debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``hades debug`` call.
    ...
    # best-effort — any failure is swallowed so ``hades debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: hades debug <command>")
        print()
        print("Commands:")
