...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "hades"}:
        return "auto", []
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        try:
            ...
            # Tools/MCP servers (excluding the hades-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "hades-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Hades tool callback are always-on production
            ...
            if "hades-tools" in mig_report.migrated:
                msg_lines.append(
                    "Hades tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Hades runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Hades tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Hades runtime.")
