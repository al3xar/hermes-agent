...
# xAI's public API is OpenAI-compatible for the endpoints Hades commonly
...
class XAIGrokAdapter(UpstreamAdapter):
    ...
    auth_hint = "hades auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`hades auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`hades auth reset xai-oauth` or re-authenticate with "
                    "`hades auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `hades auth add xai-oauth --type oauth`."
            )
