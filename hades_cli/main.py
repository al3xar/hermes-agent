...
Usage:
    hades                     # Interactive chat (default)
    hades chat                # Interactive chat
    hades gateway             # Run gateway in foreground
    hades gateway start       # Start gateway as service
    hades gateway stop        # Stop gateway service
    hades gateway status      # Show gateway status
    hades gateway install     # Install gateway service
    hades gateway uninstall   # Uninstall gateway service
    ...
    hades cron list           # List cron jobs
    hades cron status         # Check if cron scheduler is running
    hades doctor              # Check configuration and dependencies
    hades honcho setup                    # Configure Honcho AI memory integration
    hades honcho status                   # Show Honcho config and connection status
    hades honcho sessions                 # List directory → session name mappings
    hades honcho map <name>               # Map current directory to a session name
    hades honcho peer                     # Show peer names and dialectic settings
    hades honcho peer --user NAME         # Set user peer name
    hades honcho peer --ai NAME           # Set AI peer name
    hades honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    hades honcho mode                     # Show current memory mode
    hades honcho mode [hybrid|honcho|local]  # Set memory mode
    hades honcho tokens                   # Show token budget settings
    hades honcho tokens --context N       # Set session.context() token cap
    hades honcho tokens --dialectic N     # Set dialectic result char cap
    hades honcho identity                 # Show AI peer identity representation
    hades honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    hades honcho migrate                  # Step-by-step migration guide: OpenClaw native → Hades + Honcho
    hades version             Show version
    hades update              Update to latest version
    hades uninstall           Uninstall Hades Agent
    hades acp                 Run as an ACP server for editor integration
    hades sessions browse     Interactive session picker with search
...
def _set_process_title() -> None:
    """Set the process title to 'hades' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``hades tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``hades.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("hades")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"hades", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"hades")
        # Windows: the .exe name is already ``hades.exe`` — nothing to do.
    except Exception:
        pass
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``HADES_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
...
# `entry.tsx`; this is just the earlier cousin. ``HADES_TUI_NO_EARLY_DISABLE``
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`hades --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``hades --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (hades tools, hades setup, hades model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'hades {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any hades module import.
#
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set HADES_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # /opt/data/profiles/coder).  If HADES_HOME points to the hades root
    # instead (e.g. systemd hardcodes HADES_HOME=/root/.hades), we must
    # still read active_profile — the user may have switched profiles via
    # `hades profile use` and the gateway should honour that choice.
    # See issue #22502.
    hades_home_env = os.environ.get("HADES_HOME", "")
    if profile_name is None and hades_home_env:
        if Path(hades_home_env).parent.name == "profiles":
            return
    ...
    if profile_name is not None:
        try:
            ...
            # A bug in profiles.py must NEVER prevent hades from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["HADES_HOME"] = hades_home
...
load_hades_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HADES_REDACT_SECRETS env
# var BEFORE hades_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    if _cfg_path.exists():
        ...
        if "HADES_REDACT_SECRETS" not in os.environ:
...
# Initialize centralized file logging early — all `hades` subcommands
...
if _FORCE_IPV4_EARLY:
    try:
        ...
        pass  # best-effort — don't crash if hades_constants not importable yet
...
def _has_any_provider_configured() -> bool:
    ...
    # Determine whether Hades itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_hades_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Only count these if Hades has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Hades to use their tokens.
    if _has_hades_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, hades_bin
        cli_args: the original CLI arguments (everything after 'hades')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    hades_bin = container_info["hades_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo hades {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo hades {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @hades/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@hades" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/hades-ink/package.json",
    "packages/hades-ink/index.js",
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``HADES_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
...
def _ensure_tui_node() -> None:
    ...
    ``HADES_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "HADES_HOME": hades_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(hades_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(has_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if has_cli_dir is None:
        has_cli_dir = Path(__file__).parent
    bundled = has_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (HADES_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()
    ...
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with HADES_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset HADES_TUI_DIR (e.g. `unset HADES_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @hades/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @hades/ink resolves through
        # packages/hades-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "hades-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _launch_tui(
    ...
    env["HADES_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["HADES_PYTHON_SRC_ROOT"] = os.environ.get(
        "HADES_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("HADES_PYTHON", sys.executable)
    env.setdefault("HADES_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["HADES_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["HADES_MODEL"] = model
        env["HADES_INFERENCE_MODEL"] = model
    if provider:
        env["HADES_TUI_PROVIDER"] = provider
        env["HADES_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["HADES_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["HADES_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["HADES_TUI_SKILLS"] = value
    if query:
        env["HADES_TUI_QUERY"] = query
    if image:
        env["HADES_TUI_IMAGE"] = image
    if checkpoints:
        env["HADES_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["HADES_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["HADES_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["HADES_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["HADES_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["HADES_ACCEPT_HOOKS"] = "1"
    ...
    # HADES_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `hades --tui` try to
    ...
    # still set HADES_TUI_RESUME themselves.
    env.pop("HADES_TUI_RESUME", None)
    if resume_session_id:
        env["HADES_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `hades update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``HADES_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``hades kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``hades kanban boards switch`` from another session can flip the file
...
def _sync_bundled_skills_quietly() -> None:
    ...
    dependency. Hades still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``HADES_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("HADES_TUI") == "1":
        return True
    try:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'hades sessions list' to see available sessions.")
                sys.exit(1)
        else:
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `hades gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: hades gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `hades gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  hades gateway")
        ...
        print()
        print("  Or install as a service: hades gateway install")
    else:
...
def cmd_postinstall(args):
    ...
    print("⚕ Hades post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``hades model``) and the setup wizard
    ...
    # PROVIDER_GROUPS in hades_cli/models.py). A multi-member group shows one
...
#
# Hades uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `hades model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `hades model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Hades only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``hades model`` flow, then route aux tasks to them here.
    """
    from hades_cli.config import load_config
...
def _prompt_custom_api_mode_selection(
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Hades URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def cmd_slack(args):
    ...
    Dispatches ``hades slack <subcommand>``. Currently supports:
...
def cmd_security(args):
    """Dispatch `hades security <subcmd>`."""
...
def cmd_import(args):
    """Restore a Hades backup from a zip file."""
...
# Critical files that every ``hades`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``hades update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "hades_cli/main.py",
    "hades_cli/config.py",
    "hades_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``hades`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="hades-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``hades update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``hades_cli/web_dist/`` (per vite.config.ts
    outDir: "../hades_cli/web_dist"), NOT to ``web/dist/``, so Python
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the hades venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — hades venv python3, no nix-shell overhead
    ...
    # a hades venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``hades update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``hades web``).
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (hades web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``hades update`` can unconditionally call ``hades desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``hades desktop`` (interactive launch) skips the build when the
...
def _desktop_build_needed(
    ...
    ``hades update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Hades.app/Contents/MacOS/Hades"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Hades.exe",
            release_dir / "win-ia32-unpacked" / "Hades.exe",
            release_dir / "win-arm64-unpacked" / "Hades.exe",
        ]
    else:
        candidates = [
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Hades.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Hades.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Hades.exe: Access is
    ...
    an unrelated "Hades" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Hades is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Hades.app/Contents/MacOS/Hades  ->  app bundle = .../Hades.app
    app = exe.parents[2]
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  hades gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Hades.exe locked on Windows, so the pack can't replace
            ...
            if not source_mode:
                ...
                # an in-place self-update (otherwise macOS reports "Hades is
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``hades dashboard`` processes other than ourselves.

    ``hades dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``hades update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``hades dashboard`` as
    ...
    environment variable ``HADES_DESKTOP_CHILD_PID`` on the spawned
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `hades update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``hades update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``hades update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: hades curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``hades dashboard`` processes.

    Called at the end of ``hades update`` (default ``reason``) and also
    from ``hades dashboard --stop`` (which overrides ``reason``).  The
    ...
    # backend child, it sets HADES_DESKTOP_CHILD_PID so that the update
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `hades update "
            f"--branch {branch}`, or update against main with `hades update`."
        )
        sys.exit(1)
    zip_url = (
    ...
    tmp_dir = tempfile.mkdtemp(prefix="hades-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"hades-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to hades-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"hades-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "hades-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Hades behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes hades completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Hades couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Hades couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Hades behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Hades couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Hades couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `hades update`
# =========================================================================
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Hades repository.")
        print("  This means you may miss updates from NousResearch/hades-agent.")
        print()
        try:
            response = (
...
def _invalidate_update_cache():
    ...
    ``hades update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``hades update`` mutates the
...
# survives and the next ``hades`` launch finishes the install instead of
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``hades update`` left half-done.
    ...
    protocol stream (``hades acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `hades update` was interrupted mid-install — "
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``hades update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hades_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "hades.exe",
        scripts_dir / "hades-gateway.exe",
    ]


def _detect_concurrent_hades_instances(
    ...
    The Hades Desktop Electron app spawns ``hades.EXE`` as a backend child,
    so during ``hades update`` the user-invoked process and the desktop's
    ...
    shims (``hades.exe`` / ``hades-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``hades update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``hades.exe``) is a separate process from the Python
    ...
    for shim in _hades_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated hades.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``hades update``
    ...
    #      hades.exe sitting *under* a non-Hades parent (e.g. a Hades
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "hades.exe"
    lines = ["✗ Another hades.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Hades Desktop, exit any open `hades` REPLs, and")
    lines.append("  stop the gateway (`hades gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `hades update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_hades_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``hades.exe``.
    ...
    when ``hades update`` runs, ``hades.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``hades.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next hades invocation by ``_cleanup_quarantined_exes``.
    ...
    3. Print a clear warning naming the most likely culprit (running
       Hades Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hades_exe_shims(scripts_dir):
        ...
        print(
            "    Close Hades Desktop, exit other `hades` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `hades update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_hades_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``hades.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``hades.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``hades`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_hades_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``hades.exe.old.*`` left by prior updates.

    Called early on every hades invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``hades update`` runs ``uv pip install -e .[all]`` which
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``hades.exe`` / ``hades-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_hades_exe`` for the rationale.
    """
    ...
    # hours later inside ``hades update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``hades update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``hades.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``hades`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print(
            ...
            "Run `hades update --force` after closing other hades processes."
        )
        return
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `hades update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``hades update`` to survive terminal loss.
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``hades update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``hades update`` in an SSH session or a terminal
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``hades update --gateway``) the update is already
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``hades update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``hades update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``hades update`` without requiring a reinstall.  The
    ...
    ``hades`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/hades.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v hades'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/hades, code at /usr/local/lib/hades-agent).
    fhs_link = Path("/usr/local/bin/hades")
...
def _run_pre_update_backup(args) -> None:
    ...
    HADES_HOME directories.  The ``--backup`` flag on ``hades update``
    ...
    print(f"  Restore:  hades import {out_path}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``hades update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `hades update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Hades via pip (for PyPI installs)."""
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # On Windows, abort early if another hades.exe is holding the venv shim
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_hades_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # orphan merge-conflict markers in hades_cli/config.py bricked
        # every user who ran ``hades update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``hades
        ...
        # the old bytecode (e.g. get_hades_home added to hades_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``hades`` launch finishes the
        ...
        # build.  ``hades desktop --build-only`` uses the content-hash stamp
        ...
        # modules in this process.  Reload hades_constants so that any lazy
        ...
        # check `hades curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``hades update`` gives users a predictable
        ...
        # When running as ``hades update --gateway`` (spawned by the gateway's
        ...
        try:
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: hades gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: hades -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Hades gateway unit files are still installed.
        # When both hades.service (from a pre-rename install) and the
        # current hades-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `hades update` surfaces the issue until the user migrates.
        try:
            from hades_cli.gateway import (
                has_legacy_hades_units,
                _find_legacy_hades_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_hades_units():
                print()
                print("⚠ Legacy Hades gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hades_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (hades.service) fight the current")
                print("  hades-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    hades gateway migrate-legacy")
                print()
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``hades -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `hades profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → hades -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.hades)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  hades profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     hades -p {name} chat")
                else:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="hades_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  hades -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      hades -p {plan.manifest.name} chat")
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `hades profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"hades -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        if data.get("hades_requires"):
            print(f"Requires:     Hades {data['hades_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hades_requires:
        print(f"  Requires: Hades {mf.hades_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``hades dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``hades dashboard --status``
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No hades dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `hades update`.
    ...
    if "HADES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
...
def cmd_logs(args):
    """View and filter Hades log files."""
...
# so that in ``hades -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``hades_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``hades -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``hades`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `hades --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for hades CLI."""
    # Cosmetic: make the process show up as 'hades' instead of 'python3.11'
    ...
    # Sweep stale ``hades.exe.old.*`` quarantine files left by previous
    # ``hades update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_hades_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``hades update``
    ...
    # ``hades skills install update``) merely defers recovery one launch;
    # under-matching (missing ``hades -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in hades_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in hades_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in hades_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in hades_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in hades_cli/subcommands/whatsapp.py)
    # =========================================================================
    build_whatsapp_parser(subparsers, cmd_whatsapp=cmd_whatsapp)

    # =========================================================================
    # slack command  (parser built in hades_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    # login command  (parser built in hades_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in hades_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in hades_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in hades_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in hades_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in hades_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    # hooks command  (parser built in hades_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in hades_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in hades_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in hades_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in hades_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in hades_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        ...
        "repo hades uses to snapshot working directories before "
    ...
    # =========================================================================
    # import command  (parser built in hades_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in hades_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in hades_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in hades_cli/subcommands/skills.py)
    # =========================================================================
    ...
    # =========================================================================
    # plugins command  (parser built in hades_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``hades --help``, ``hades version``, ``hades logs``,
    ...
    # =========================================================================
    # memory command  (parser built in hades_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in hades_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `hades computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `hades tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: hades computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: hades computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in hades_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch hades --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in hades_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in hades_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in hades_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in hades_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in hades_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in hades_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in hades_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in hades_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Hades-Setup.exe success screen tells users to
    # run `hades desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in hades_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in hades_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in hades_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``hades -c Pokemon Agent Dev`` → ``hades -c 'Pokemon Agent Dev'``
    ...
    # fails (e.g. 'hades -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (hades hooks list, cron
