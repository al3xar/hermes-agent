...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``hades config set`` (which rewrites
    ...
    ``config.yaml`` in place: hades never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``hades logs``
...
#   the next ``subprocess.run([...])`` Hades makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Hades itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``hades update``, the TUI build.
...
#   on every plugin install / ``hades update``.
...
# * ``HADES_HOME`` / ``HADES_PROFILE`` / ``HADES_CONFIG`` /
#   ``HADES_ENV`` — Hades runtime location flags. Writing these into
...
#
# IMPORTANT: ``HADES_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (HADES_GEMINI_CLIENT_ID,
# HADES_LANGFUSE_PUBLIC_KEY, HADES_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "GIT_EXEC_PATH",
    "GIT_SHELL",
    # Hades runtime location — never via dashboard env writer.
    # NOT a HADES_* blanket: integration credentials (HADES_GEMINI_*,
    # HADES_LANGFUSE_*, HADES_SPOTIFY_*, ...) ARE allowed.
    "HADES_HOME",
    "HADES_PROFILE",
    "HADES_CONFIG",
    "HADES_ENV",
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "(HADES_HOME, HADES_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `hades plugins enable
    # observability/langfuse` or `hades tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "HADES_LANGFUSE_ENV",
    "HADES_LANGFUSE_RELEASE",
    "HADES_LANGFUSE_SAMPLE_RATE",
    "HADES_LANGFUSE_MAX_CHARS",
    "HADES_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def is_managed() -> bool:
    """Check if Hades is running in package-manager-managed mode.

    Two signals: the HADES_MANAGED env var (set by the systemd service),
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade hades-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Hades was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.hades/.install_method`` file (written by installers)
    2. HADES_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/hades-agent`` image stamps ``docker``
    ...
    so ``hades update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_hades_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.hades/.install_method."""
    stamp = get_hades_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    ...
    ``uv tool install hades-agent`` places the install at
    ``.../uv/tools/hades-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``hades-agent`` happens to be uv-tool-installed on the machine while
    the *active* Hades is a regular pip/venv install, causing
    ``hades update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/hades-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade hades-agent"
    if method == "docker":
        return "docker pull nousresearch/hades-agent:latest"
    if method == "pip":
...
# hades_cli/main.py; lives here so the wording stays consistent and we
...
Hades Agent runs as a published image (nousresearch/hades-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/hades-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate hades-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/hades-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/hades-agent/tags
...
def format_managed_message(action: str = "modify this Hades installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            ...
            f"(HADES_MANAGED={env_hint}).\n"
            "Edit services.hades-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    ...
    Returns a dict with keys: backend, container_name, exec_user, hades_bin
    or None if container mode is not active, we're already inside the
    container, or HADES_DEV=1 is set.
    ...
    exec_user = info.get("exec_user", "hades")
    hades_bin = info.get("hades_bin", "/data/current-package/bin/hades")
...
def _resolve_hades_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the HADES_UID / HADES_GID env vars set by Docker deployments.

    Docker containers running Hades commonly set these to map the in-container
    ...
    subdirectories created at runtime by ``ensure_hades_home()`` (especially
...
def _chown_to_hades_uid(path) -> None:
    """Chown ``path`` to ``HADES_UID:HADES_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_hades_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_hades_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the hades group can
    ...
    Also applies ``HADES_UID``/``HADES_GID``-based ownership when those env
    ...
    try:
        ...
        pass
    _chown_to_hades_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Hades runs in a container with volume-mounted config files, forcing
...
def _secure_file(path):
    ...
    permissions.  Set HADES_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HADES_HOME if the user doesn't have one yet."""
...
def ensure_hades_home():
    ...
    if is_managed():
        ...
        try:
            _ensure_hades_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_hades_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HADES_HOME {home} does not exist. Run 'sudo nixos-rebuild switch' first."
        )
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Hades-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Hades
        ...
        # identity slot (SOUL.md). Empty by default. The HADES_ENVIRONMENT_HINT
    ...
    "terminal": {
        ...
        # skipped. When empty, Hades auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Hades sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Hades
        ...
        # bundled Hades image, which drops to the `hades` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Hades sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Hades should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``hades chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: hades sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Hades truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``hades kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``hades kanban decompose`` and the kanban
        ...
        # ``hades profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `hades model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hades` (and `hades chat`) launches by default:
        ...
        # REPL and `--tui` (or HADES_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `hades --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hades -c` muscle memory.  Default off so existing
        # users aren't surprised.  HADES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `hades --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/hades`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``HADES_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``HADES_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``HADES_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``HADES_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/hades``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Hades appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
    ...
    # response. If not, Hades feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Hades auto-pauses the goal and
    ...
    "skills": {
        ...
        # Substitute ${HADES_SKILL_DIR} and ${HADES_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `hades curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `hades update`, so pruning them only
        ...
        # user can roll back with `hades curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for hades-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (HADES_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # toggleable per-invocation via --accept-hooks or HADES_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `hades doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hades_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Hades to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via HADES_CRON_MAX_PARALLEL env var.
        "max_parallel_jobs": None,
    },
    # Kanban multi-agent coordination — controls the dispatcher loop that
    # spawns workers for ready tasks. The dispatcher ticks every N seconds
    # (default 60), reclaims stale claims, promotes dependency-satisfied
    # todos to ready, and fires `hades -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `hades` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `hades kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     hades-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Hades tools (terminal, read_file, write_file, patch,
    ...
    # update model picker lists without shipping a hades-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `hades model` invocation; network failures
    ...
    "gateway": {
        ...
        # the legacy native Hades agent loop.
        ...
        # user. Bridged to HADES_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Hades
        ...
        # to HADES_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # HADES_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to HADES_MEDIA_TRUST_RECENT_SECONDS.
    ...
    "sessions": {
        ...
        # default of ``hades sessions prune``.
        "retention_days": 90,
    ...
    # ``hades update`` behaviour.
    "updates": {
        # Run a full ``hades backup``-style zip of HADES_HOME before every
        # ``hades update``.  Backups land in ``<HADES_HOME>/backups/`` and
        # can be restored with ``hades import <path>``.  Off by default —
        # on large HADES_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `hades update` does with uncommitted local changes to the
    ...
    # `hades tools`. These settings tune the backing Responses API call.
    "x_search": {
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @hades:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. HADES_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #hades)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: hades-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Hades API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Hades API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Hades API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # HADES_TOOL_PROGRESS and HADES_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.hades.config`` entries, then checks
    ...
    try:
        ...
        # should never break `hades update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Hades versions) write ``models`` as
        ...
        try:
            ...
            return [
                ConfigIssue(
                    ...
                    "Run 'hades setup' to create a valid config",
                )
            ]
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("HADES_TOOL_PROGRESS")
            old_mode = get_env_value("HADES_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append(
                    "display.tool_progress=off (from HADES_TOOL_PROGRESS=false)"
                )
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(
                    f"display.tool_progress={old_mode.lower()} (from HADES_TOOL_PROGRESS_MODE)"
                )
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            ...
            if old_tz and old_tz.strip():
                ...
                results["config_added"].append(
                    f"timezone={old_tz.strip()} (from HADES_TIMEZONE)"
                )
            else:
                config["timezone"] = ""
    ...
    # settings in their `config.yaml`, and `hades curator status` has no
    ...
    #      on top of ensure_hades_home() — old profiles that predate this
    ...
    # metadata.hades.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _hades_home / "config.yaml"`` local variable
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`hades tools`, `hades setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `hades tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Hades env var names, avoiding false positives from values
...
def reload_env() -> int:
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Hades vars that are no longer in .env
...
def show_config():
    ...
    print(
        color("│              ⚕ Hades Configuration                    │", Colors.CYAN)
    )
    print(
        color(
            "└─────────────────────────────────────────────────────────┘", Colors.CYAN
    ...
    # Warn on stale HADES_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("HADES_MAX_ITERATIONS")
        if (
            ...
            print(
                color(
                    f"                ⚠ .env has stale HADES_MAX_ITERATIONS={_env_ghost} "
...
# ``_inject_profile_env_vars`` surfaces them in ``hades config`` UI so users
