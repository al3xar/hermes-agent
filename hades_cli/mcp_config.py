"""
MCP Server Management CLI — ``hades mcp`` subcommand.

Implements ``hades mcp add/remove/list/test/configure`` for interactive
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``hades mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import HadesTokenStorage
        return HadesTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # hades_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  hades mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  hades mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  hades mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: hades mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `hades mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  hades mcp add <name> --url <endpoint>')
        _info('  hades mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `hades mcp remove` + `hades mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `hades mcp login " + name + "`.")
            return
        if tools:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'hades mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``hades mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `hades plugin`.
        from hades_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("hades mcp                                    Open the catalog picker (default)")
        _info("hades mcp catalog                            List Nous-approved MCPs")
        _info("hades mcp install <name>                     Install a catalog MCP")
        _info("hades mcp serve                              Run as MCP server")
        _info("hades mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("hades mcp add <name> --command <cmd>         Add a stdio server")
        _info("hades mcp add <name> --preset <preset>       Add from a known preset")
        _info("hades mcp remove <name>                      Remove a server")
        _info("hades mcp list                               List configured servers")
        _info("hades mcp test <name>                        Test connection")
        _info("hades mcp configure <name>                   Toggle tools")
        _info("hades mcp login <name>                       Re-authenticate OAuth")
        print()
