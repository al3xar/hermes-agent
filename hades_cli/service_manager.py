...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``hades gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged hades
    ...
    detection always fails. Since every Hades runtime call inside the
    container drops to hades via ``s6-setuidgid``, that silent failure
...
#
# Per-profile gateways are registered dynamically when `hades profile create`
# runs inside the container (Phase 4). Static services (main-hades, dashboard)
...
# UID/GID of the in-image ``hades`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the hades user.
    ...
    (the ``hades`` user — UID 10000 — running every Hades runtime
    ...
    is dispatched as the hades user.
    ...
    with hades ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           hades:hades, 0755 (parent must already exist)
    ``svc_dir/event/``                     hades:hades, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 hades:hades, 0755
    ``svc_dir/supervise/event/``           hades:hades, 03730
    ``svc_dir/supervise/control``          hades:hades, 0660    (FIFO)
    ...
    slot's supervise/ was hades-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _HADES_UID, _HADES_GID)
        except PermissionError:
            # Running as the hades user already — directory is hades-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _HADES_UID, _HADES_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is hades-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _HADES_UID, _HADES_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``hades -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`hades profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-hades, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources HADES_HOME (and any extra env) via with-contenv —
             so e.g. ``-e HADES_HOME=/data/hades`` is honored at run
          ...
          4. Drops to the hades user and exec's
             ``hades -p <profile> gateway run`` (or just ``hades
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``hades gateway
        ...
        # guard.
        lines.append("export HADES_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "hades gateway run"
        else:
            gateway_cmd = f"hades -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid hades {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        — so a container started with ``-e HADES_HOME=/data/hades``
        gets its logs under /data/hades/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``hades logs`` reads and what
        ...
        return (
            ...
            f'mkdir -p "$log_dir"\n'
            f'chown -R hades:hades "$log_dir" 2>/dev/null || true\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid hades s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``hades gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with hades ownership
            ...
            # the hades user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is hades-owned (see ``_seed_supervise_skeleton``), and on
