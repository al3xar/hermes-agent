"""``hades slack ...`` CLI subcommands.

Today only ``hades slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ hades slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    for a Hades deployment — users can tweak them in the Slack UI after
    pasting.
    """
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hades_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$HADES_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Hades")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Hades"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Hades app\n"
