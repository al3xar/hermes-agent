"""``hades logs`` — view and filter Hades log files.
...
Usage examples::
    ...
    hades logs                    # last 50 lines of agent.log
    hades logs -f                 # follow agent.log in real time
    hades logs errors             # last 50 lines of errors.log
    hades logs gateway -n 100    # last 100 lines of gateway.log
    hades logs gui -f            # follow gui.log (dashboard/pty/ws)
    hades logs desktop -f        # follow desktop.log (Electron app boot/backend)
    hades logs --level WARNING    # only WARNING+ lines
    hades logs --session abc123   # filter by session ID substring
    hades logs --component tools  # only tool-related lines
    hades logs --since 1h         # lines from the last hour
    hades logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when Hades runs — try 'hades chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'hades chat' to generate logs)")
