"""MCP picker — interactive `hades mcp picker` (also the default `hades mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``hades mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `hades plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Hades session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `hades mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: hades mcp install <name>    Picker: hades mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Hades is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hades — run `hades update` "
...
def show_catalog() -> None:
    """`hades mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`hades mcp picker` (and default `hades mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`hades mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `hades mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
