"""Shell completion script generation for hades CLI.
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_hades_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
...
_hades_profiles() {{
    local profiles_dir="$HOME/.hades/profiles"
...
_hades_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_hades_profiles)" -- "$cur"))
        return
    fi
...
complete -F _hades_completion hades
"""
...
_hades_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.hades/profiles" ]]; then
        profiles+=($HOME/.hades/profiles/*(N/:t))
    fi
...
_hades() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_hades_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'hades command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "# Add to your config:",
        "#   hades completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hades_profiles",
        "    echo default",
        "    if test -d $HOME/.hades/profiles",
        "        for d in $HOME/.hades/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c hades -f -s p -l profile"
        " -d 'Profile name' -xa '(__hades_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__hades_profiles)' -d 'Profile name'"
                )
