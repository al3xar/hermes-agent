"""CLI subcommand: ``hades send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"hades send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"hades send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"hades send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"hades send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"hades send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `hades gateway setup`, or run the gateway once so")
...
def _load_hades_env() -> None:
    ...
    startup that ``hades send`` must replicate when invoked standalone:
    ...
    2. Bridge top-level simple values from ``~/.hades/config.yaml`` into
       ...
       via ``hades config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``hades send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "hades send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  hades send --to telegram \"hello\"\n"
            "  hades send --to discord:#ops --file report.md\n"
            "  hades send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "hades send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `hades send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Hades "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `hades send --list telegram`.",
    )
