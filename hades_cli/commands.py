...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("runtime", "View or toggle agent runtime (native | deepagents)",
               "Configuration", args_hint="[native|deepagents|show]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`has_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/hades`` subcommand mapping, and
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``hades skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``hades skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``hades skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/hades <verb>`` subcommand.
    ...
    can still reach them via ``/hades <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/hades`` is always reserved as the first entry so the
    legacy ``/hades <subcommand>`` form keeps working for anything that
    ...
    # Reserve /hades as the catch-all top-level command.
    entries.append(("hades", "Talk to Hades or run a subcommand", "[subcommand] [args]"))
    seen.add("hades")
...
def slack_app_manifest(request_url: str = "https://hades-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /hades handler.

    Maps both canonical names and aliases so /hades bg do stuff works
    the same as /hades background do stuff.

    Plugin-registered slash commands are included so ``/hades <plugin-cmd>``
