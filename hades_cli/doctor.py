...
load_hades_dotenv(hades_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized in {"google / gemini", "gemini"}:
        try:
            from hades_cli.auth import get_gemini_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``hades --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != hades_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'hades update', or set "
            "hades_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-hades and dashboard static services are up
    ...
    for static in ("main-hades", "dashboard"):
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `hades`.
    os.environ.setdefault("HADES_INTERACTIVE", "1")

    # Handle `hades doctor --ack <id>` as a fast path. Persist the ack and
    ...
    print(color("│                 🩺 Hades Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Detect drift between pyproject.toml and hades_cli/__init__.py versions
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.hades/.env or run 'hades setup')",
                            (
    ...
    if config_path.exists():
        ...
        # Detect stale HADES_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives HADES_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            ...
            env_ghost = load_env().get("HADES_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"HADES_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `hades setup` run)",
                )
                if should_fix:
                    if remove_env_value("HADES_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale HADES_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove HADES_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the HADES_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale HADES_MAX_ITERATIONS in .env shadows config.yaml — "
    ...
    for subdir_name in expected_subdirs:
        subdir_path = hades_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Hades a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Hades Agent Persona\n\n"
                "<!-- Edit this file to customize how Hades communicates. -->\n\n"
                "You are Hades, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = hades_home / "memories"
    ...
    state_db_path = hades_home / "state.db"
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        ...
                        "(or 'hades sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = hades_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `hades doctor` invocation.
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``hades doctor``.
    ...
    # never the legitimate source for `hades doctor`.
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "hades -p" in content:
                            _m = _re.search(r"hades -p (\S+)", content)
