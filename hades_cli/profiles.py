"""
Profile management for multiple isolated Hades instances.
...
Usage::
    ...
    hades profile create coder          # fresh profile + bundled skills
    hades profile create coder --clone  # also copy config, .env, SOUL.md, skills
    hades profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    hades -p coder chat                 # or via flag
    hades profile use coder             # set as sticky default
    hades profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See hades_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
#
# Rationale per item:
#   hades-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `hades profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `hades
...
# `hades skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Hades infrastructure directories that only the default profile
    ...
    is_default_source = source_resolved == _get_default_hades_home().resolve()
...
_RESERVED_NAMES = frozenset({
    "hades", "default", "test", "tmp", "root", "sudo",
})

# Hades subcommands that cannot be used as profile names/aliases
_HADES_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hades root, NOT to the current HADES_HOME
    ...
    """
    return _get_default_hades_home() / "profiles"


def _get_default_hades_home() -> Path:
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_hades_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``hades``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``hades`` profile inside ``~/.hades/``) or get refused
...
def get_profile_dir(name: str) -> Path:
    ...
    if canon == "default":
        return _get_default_hades_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, hades subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _HADES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a hades subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``hades -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``hades profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``hades profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Hades config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_hades_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``hades update`` skips re-seeding this profile's
    ...
    # Write the opt-out marker so seed_profile_skills() and `hades update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`hades profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `hades profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `hades -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``hades profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            ...
            "To remove everything, use: hades uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: hades profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    ...
    default_resolved = _get_default_hades_home().resolve()
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: hades profile import <archive> --name <name>"
        )
    ...
    if canon == "default":
        raise ValueError(
            ...
            "Specify a different name: hades profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="hades_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"hades_{old_name}"
    legacy_old_host = f"hades.{old_name}"
    new_host = f"hades_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_hades_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("hades_"):
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any hades modules
