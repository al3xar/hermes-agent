...
prefix (e.g. ``mission-control.tilos.com/hades/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /hades`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``HADES_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    "I set HADES_DASHBOARD_PUBLIC_URL but the OAuth callback is still
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/hades"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# HADES_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``HADES_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    if env_clean:
        return env_clean
    _warn_if_malformed("HADES_DASHBOARD_PUBLIC_URL env var", env_raw)
