"""
Dump command for hades CLI.

Outputs a compact, plain-text summary of the user's Hades setup
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.hades_build_sha`` by the Dockerfile's
    ``HADES_GIT_SHA`` build-arg (see ``hades_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``hades dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _count_skills(hades_home: Path) -> int:
...
def _cron_summary(hades_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = hades_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"hades_home:      {display_hades_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `hades auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `hades auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["hades-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(hades_home)}")
    lines.append(f"  skills:             {_count_skills(hades_home)}")
