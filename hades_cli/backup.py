"""
Backup and import commands for hades CLI.

`hades backup` creates a zip archive of the entire ~/.hades/ directory
(excluding the hades-agent repo and transient files).

`hades import` restores from a backup zip, overlaying onto the current
HADES_HOME root.
"""
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to hades root) should be skipped."""
...
def run_backup(args) -> None:
    ...
    hades_root = get_default_hades_root()

    if not hades_root.is_dir():
        print(f"Error: Hades home directory not found at {hades_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"hades-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"hades-backup-{stamp}.zip"
    ...
    for dirpath, dirnames, filenames in os.walk(hades_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(hades_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(hades_root)
    ...
    print(f"\nRestore with: hades import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Hades backup.
    ...
    # Look for telltale files that a hades home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Hades backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.hades/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a hades dir name
        if prefix in {".hades", "hades"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        has_config = (hades_root / "config.yaml").exists()
        has_env = (hades_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Hades configuration.")
        ...
        for member in members:
            ...
            target = hades_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(hades_root.resolve())
            except ValueError:
        ...
        profiles_dir = hades_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print(f"  Run: hades profile list  (after installing hades)")

        # Guidance
        print()
        if not (hades_root / "hades-agent").is_dir():
            print("Note: The hades-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: hades update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  hades -p {pname} gateway install")

        print("Done. Your Hades configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and hades backup --quick)
# ---------------------------------------------------------------------------
...
# — `hades update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(hades_home: Optional[Path] = None) -> Path:
    home = hades_home or get_hades_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    hades_home: Optional[Path] = None,
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(hades_home)
    if not root.exists():
        return []
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``hades update``.
    ...
    Args:
        ...
        hades_home: Override for the Hades home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(hades_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for hades backup --quick."""
...
def _write_full_zip_backup(out_path: Path, hades_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``hades_root`` to ``out_path``.
...
def _pre_update_backup_dir(hades_home: Optional[Path] = None) -> Path:
    home = hades_home or get_hades_home()
...
def create_pre_update_backup(
    ...
    (``hades update``) should continue even if the backup fails.
    """
    hades_root = hades_home or get_default_hades_root()
    if not hades_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(hades_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, hades_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `hades claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    # Reuses the shared backups/ directory so `hades import` and the
