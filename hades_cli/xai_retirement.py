...
Pure logic: walks a Hades config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `hades doctor` and a future `hades migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Hades config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Hades config and return retirement issues.
