"""``hades dashboard`` subcommand parser.

Extracted verbatim from ``hades_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    # for `hades dashboard` cmdlines and SIGTERM them directly — the same
    # path `hades update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running hades dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running hades dashboard processes and exit",
    )
    # Backward-compat shim: older Hades desktop app shells (<= 0.15.x) spawn the
    # backend as `hades dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Hades couldn't start" with no actionable cause. Accept and silently
    ...
    # `hades dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.hades/.env.
    # Nested subparser so bare `hades dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HADES_DASHBOARD_OAUTH_CLIENT_ID into ~/.hades/.env, and prints "
            "how to engage the login gate. Requires being logged in (hades setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://hades.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via HADES_DASHBOARD_PORTAL_URL. Mainly for "
