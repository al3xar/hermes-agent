...
# Without this, standalone invocations (e.g. after `hades update` reloads
# the module) fail with ModuleNotFoundError for hades_time et al.
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``hades tools`` config for the ``cron`` platform.
...
_hades_home: Path | None = None


def _get_hades_home() -> Path:
    """Resolve Hades home dynamically while preserving test monkeypatch hooks."""
    return _hades_home or get_hades_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hades_home = _get_hades_home()
    lock_dir = hades_home / "cron"
    return lock_dir, lock_dir / ".tick.lock"


@contextmanager
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific Hades profile.
    ...
    the scheduler's test/override hook and a context-local Hades home override
    both point at the resolved profile directory so _get_hades_home(),
    .env/config loading, script resolution, AIAgent construction, and downstream
    get_hades_home() callers agree on the same home.
    ...
    global _hades_home
    prior_override = _hades_home
    env_snapshot = os.environ.copy()

    from hades_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    override_token = None
    try:
        override_token = set_hades_home_override(profile_home)
        _hades_home = profile_home
        logger.info(
            "Job '%s': using Hades profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
        ...
        _hades_home = prior_override
        if override_token is not None:
            reset_hades_home_override(override_token)
...
def _get_script_timeout() -> int:
    ...
    if env_value:
        try:
            ...
            logger.warning("Invalid HADES_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * anything else — run with the current Python interpreter
    ...
    run_env["HADES_HOME"] = str(_get_hades_home())
    try:
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _hades_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_hades_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_hades_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # gateway message. Do not seed HADES_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads HADES_SESSION_PLATFORM
    #     and HADES_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via HADES_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     HADES_SESSION_PLATFORM == "telegram".
    ...
    #     HADES_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read HADES_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the HADES_CRON_AUTO_DELIVER_* vars set
    # below, so clearing HADES_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "HADES_CRON_AUTO_DELIVER_PLATFORM",
        "HADES_CRON_AUTO_DELIVER_CHAT_ID",
        "HADES_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_hades_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_hades_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["HADES_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["HADES_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["HADES_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
        ...
        try:
            # Do not inject HADES_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        # override via HADES_CRON_TIMEOUT env var.  0 = unlimited.
        #
        ...
        if _raw_cron_timeout:
            try:
                ...
                logger.warning(
                    "Invalid HADES_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_hades_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_hades_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _hades_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _hades_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set HADES_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        ...
        try:
            ...
            logger.warning("Invalid HADES_CRON_MAX_PARALLEL value; defaulting to unbounded")
        ...
        # Hades home override, scheduler _hades_home hook, and temporary
