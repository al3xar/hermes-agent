...
HADES_HOME_DIR="${HADES_HOME:-$HOME/.hades}"
HADES_CFG="${HADES_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Hades config has twozero_td MCP entry ──
if [[ ! -f "$HADES_CFG" ]]; then
    echo -e " ${FAIL} Hades config not found at ${HADES_CFG}"
    manual_steps+=("Create ${HADES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HADES_CFG" 2>/dev/null; then
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${HADES_CFG} manually"); }
    manual_steps+=("Restart Hades session to pick up config change")
fi
