...
4. Store the token in `~/.hades/.env` (or via `hades setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.hades/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
