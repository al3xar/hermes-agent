...
# from the Dockerfile never reaches `hades`. with-contenv repopulates
# the env from /run/s6/container_environment before exec'ing, which is
# what s6-supervised services use too (see main-hades/run).
#
# Routing:
#   no args                       → exec `hades` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `hades <args>` (subcommand passthrough)
#
# Drop to hades via s6-setuidgid, but skip it when already non-root.
set -e

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid hades "$@"; exec "$@"; }
...
# an arbitrary non-root, non-hades UID, the bootstrap was skipped and the
# baked image dirs (owned by the hades build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u hades)" ]; then
    cat >&2 <<EOF
[hades] ERROR: container started with --user $cur_uid (an arbitrary, non-hades UID) — not supported.
...
# to the hades user's home before dropping privileges so libraries that
...
_hades_orig_cwd="${HADES_ORIG_CWD:-$PWD}"
...
# the user's command so `hades chat` starts in the Docker -w
