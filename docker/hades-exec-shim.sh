#!/bin/sh
# shellcheck shell=sh
# /opt/hades/bin/hades — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `hades` user (UID 10000). When an operator runs `docker exec <c> hades ...`
...
# user runs `docker exec <c> hades login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Hades is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> hades
...
# Fix
# ---
# This shim sits at /opt/hades/bin/hades and is placed earliest on PATH.
# When invoked as root, it drops to the hades user (via s6-setuidgid)
...
# `docker exec --user hades`, kanban subagents, etc. — it short-circuits
...
# (/opt/hades/.venv/bin/hades), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set HADES_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the hades CLI. Default is to drop.

set -e

REAL=/opt/hades/.venv/bin/hades

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "hades-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user hades`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${HADES_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the hades user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hades-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "hades-shim: re-run with --user hades or set HADES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the hades user's home before dropping privileges. Without
