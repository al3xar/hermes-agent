...
#
# Wired into the image as /etc/cont-init.d/01-hades-setup by the
...
# Drop to hades via s6-setuidgid, but skip it when already non-root.
as_hades() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid hades "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/hades/
# .venv, ui-tui, node_modules) stay owned by the hades build UID (10000), so an
...
# into $HADES_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass HADES_UID/HADES_GID — or the PUID/PGID aliases — which the
...
# `id -u hades` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# hades UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u hades)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-hades UID).
...
and the baked image directories are owned by the hades user (UID $(id -u hades)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e HADES_UID=\$(id -u) -e HADES_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the hades user to that UID/GID at boot and chowns the data
...
# `s6-setuidgid hades mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom HADES_HOME paths whose parents only
# root can create (e.g. `HADES_HOME=/home/hades/.hades` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for HADES_UID/HADES_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  HADES_UID/
# HADES_GID still win when both are set.  See #15290, salvages #25872.
HADES_UID="${HADES_UID:-${PUID:-}}"
HADES_GID="${HADES_GID:-${PGID:-}}"

if [ -n "${HADES_UID:-}" ] && validate_uid_gid "$HADES_UID" && [ "$HADES_UID" != "$(id -u hades)" ]; then
    echo "[stage2] Changing hades UID to $HADES_UID"
    usermod -u "$HADES_UID" hades
fi
if [ -n "${HADES_GID:-}" ] && validate_uid_gid "$HADES_GID" && [ "$HADES_GID" != "$(id -g hades)" ]; then
    echo "[stage2] Changing hades GID to $HADES_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$HADES_GID" hades 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised hades user (UID 10000)
...
# hades process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes hades. Idempotent across container
...
#   - hades is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G hades 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] hades already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" hades 2>/dev/null; then
        echo "[stage2] Added hades to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group hades failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When HADES_UID is remapped or the top-level $HADES_HOME isn't owned by
# the runtime hades UID, restore ownership to hades — but ONLY for the
# directories hades actually writes to. The full $HADES_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of hades-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_hades_uid=$(id -u hades)
needs_chown=false
if [ "$(stat -c %u "$HADES_HOME" 2>/dev/null)" != "$actual_hades_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $HADES_HOME (targeted) to hades ($actual_hades_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $HADES_HOME: chown the directory itself (not its contents)
    # so hades can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown hades:hades "$HADES_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $HADES_HOME failed (rootless container?) — continuing"
    # Hades-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by hades (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$HADES_HOME/$sub" ]; then
            chown -R hades:hades "$HADES_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $HADES_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Hades-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime hades UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when HADES_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     hades UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R hades:hades` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $HADES_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $HADES_HOME ownership check
# above. `usermod -u <new> hades` re-chowns the hades home dir
# ($HADES_HOME == /opt/data) to the new UID as a side effect, so after a
# HADES_UID/PUID remap `stat $HADES_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/hades
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $HADES_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime hades UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_hades_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to hades ($actual_hades_uid)"
    chown -R hades:hades \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $HADES_HOME/profiles to hades on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> hades …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as hades and walks
...
# land root-owned, and the unprivileged hades runtime then hits
...
for f in \
    ...
    if [ -e "$HADES_HOME/$f" ]; then
        chown hades:hades "$HADES_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the hades runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$HADES_HOME/config.yaml" ]; then
    chown hades:hades "$HADES_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$HADES_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as hades user ---
# Run as hades via s6-setuidgid so dirs end up owned correctly (matters
...
seed_one() {
    ...
    if [ ! -f "$HADES_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_hades cp "$INSTALL_DIR/$src" "$HADES_HOME/$dest"
    fi
}
...
if [ -f "$HADES_HOME/.env" ]; then
    chown hades:hades "$HADES_HOME/.env" 2>/dev/null || true
...
# config-schema migrations that `hades update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set HADES_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$HADES_HOME/config.yaml" ]; then
    s6-setuidgid hades "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$HADES_HOME/auth.json" ] && [ -n "${HADES_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$HADES_AUTH_JSON_BOOTSTRAP" > "$HADES_HOME/auth.json"
    chown hades:hades "$HADES_HOME/auth.json" 2>/dev/null || true
...
# can set HADES_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# HADES_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$HADES_HOME/gateway_state.json" ] && \
        [ "${HADES_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$HADES_HOME/gateway_state.json"
    chown hades:hades "$HADES_HOME/gateway_state.json" 2>/dev/null || true
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_hades "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
...
# directory. agent-browser (the runtime CLI Hades spawns for the
...
# main-wrapper.sh propagates it into the supervised ``hades`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-hades, dashboard, etc.).
