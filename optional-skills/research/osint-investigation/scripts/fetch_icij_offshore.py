...
$HADES_OSINT_CACHE/icij/ (default: ~/.cache/hades-osint/icij/) and search
...
def _cache_dir() -> Path:
    ...
    return Path.home() / ".cache" / "hades-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "hades-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $HADES_OSINT_CACHE/icij or ~/.cache/hades-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
