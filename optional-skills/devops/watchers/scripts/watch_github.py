...
Usage (via cron with --no-agent):
    ...
    hades cron create hades-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HADES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hades-issues --repo NousResearch/hades-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.hades/.env to avoid the 60 req/hr
