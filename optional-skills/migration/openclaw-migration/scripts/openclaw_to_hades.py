#!/usr/bin/env python3
"""OpenClaw -> Hades migration helper.
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "messaging-settings": {
        ...
        "description": "Import Hades-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'hades cron'.",
    },
    "hooks-config": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Hades config.yaml")
    ensure_parent(path)
    path.write_text(
...
#
# Case-preserving: ``OpenClaw`` → ``Hades`` (prose), but lowercase matches
# like ``openclaw`` → ``hades`` (so filesystem paths like ``~/.openclaw``
# become ``~/.hades`` — the real Hades home — not the broken ``~/.Hades``).
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Hades`` but maps ``openclaw`` → ``hades`` so a
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Hades Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-hades/apply.ts "blocked by earlier
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-hades/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Hades session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Hades config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Hades
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Hades-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Hades-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: hades config set",
                        )
                    continue
        ...
        for oc_key, hades_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and hades_key not in secret_additions:
                secret_additions[hades_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, hades_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and hades_key not in secret_additions:
                        secret_additions[hades_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hades_config = load_yaml_file(destination)
        current_model = hades_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = hades_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                hades_config["model"] = {"default": model_str}
            dump_yaml_file(destination, hades_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Hades still uses "edge"
        ...
        hades_config = load_yaml_file(destination)
        existing_tts = hades_config.get("tts", {})
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Hades destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Hades-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Hades-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hades_cfg_path = self.target_root / "config.yaml"
        hades_cfg = load_yaml_file(hades_cfg_path)
        existing_mcp = hades_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            hades_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                hades_srv["command"] = srv["command"]
                if srv.get("args"):
                    hades_srv["args"] = srv["args"]
                if srv.get("env"):
                    hades_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    hades_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                hades_srv["url"] = srv["url"]
                if srv.get("headers"):
                    hades_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    hades_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                hades_srv["enabled"] = False
            if srv.get("timeout"):
                hades_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                hades_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                hades_srv["tools"] = {}
                if tools_cfg.get("include"):
                    hades_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    hades_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
        ...
        if added > 0 and self.execute:
            self.maybe_backup(hades_cfg_path)
            hades_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(hades_cfg_path, hades_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'hades cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = hades_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Hades reasoning_effort
        ...
        if compaction:
            compression = hades_cfg.get("compression") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'hades gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("HADES_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hades_cfg_path = self.target_root / "config.yaml"
        hades_cfg = load_yaml_file(hades_cfg_path)
        sr = hades_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hades_cfg_path = self.target_root / "config.yaml"
        hades_cfg = load_yaml_file(hades_cfg_path)
        custom_providers = hades_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(hades_cfg_path)
            hades_cfg["custom_providers"] = custom_providers
            dump_yaml_file(hades_cfg_path, hades_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if discord_cfg:
            hades_cfg_path = self.target_root / "config.yaml"
            hades_cfg = load_yaml_file(hades_cfg_path)
            discord_hades = hades_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_hades["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_hades["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                hades_cfg["discord"] = discord_hades
                dump_yaml_file(hades_cfg_path, hades_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hades_cfg_path = self.target_root / "config.yaml"
        hades_cfg = load_yaml_file(hades_cfg_path)
        browser_hades = hades_cfg.get("browser") or {}
        changed = False

        # Map fields that have Hades equivalents
        if browser.get("cdpUrl"):
            browser_hades["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_hades["headless"] = browser["headless"]
            changed = True

        if changed:
            hades_cfg["browser"] = browser_hades
            if self.execute:
                self.maybe_backup(hades_cfg_path)
                dump_yaml_file(hades_cfg_path, hades_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            hades_mode = mode_map.get(mode, "manual")
            hades_cfg.setdefault("approvals", {})["mode"] = hades_mode
            if self.execute:
                self.maybe_backup(hades_cfg_path)
                dump_yaml_file(hades_cfg_path, hades_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{hades_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Hades Migration Notes",
            "",
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Hades Config Not Overwritten)",
                "",
                "These items already existed in your Hades config. Re-run with",
        ...
        notes.extend([
            ...
            "",
            "**Strongly recommended:** Run `hades claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `hades claw cleanup` to fix it.",
            "",
            "## Hades-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `hades claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `hades setup` to configure any remaining settings",
            "- Run `hades mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `hades cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `hades cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `hades gateway install` if you need the gateway service",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Hades targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Hades Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if args.execute:
        ...
        print("    2. Run: hades mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: hades cron")
