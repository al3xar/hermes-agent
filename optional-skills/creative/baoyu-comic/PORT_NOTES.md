...
|--------|----------|--------|
| Metadata namespace | `openclaw` | `hades` (with `tags` + `homepage`) |
...
| File operations | Generic instructions | Hades file tools (`write_file`, `read_file`) |
...
If upstream adds a Hades-compatible PDF merge step (no extra npm deps), restore `scripts/` and reintroduce Step 8 in `workflow.md`.
