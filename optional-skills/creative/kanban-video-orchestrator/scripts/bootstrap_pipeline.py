...
../assets/, and writes a setup.sh that creates Hades profiles and fires the
initial kanban task.

Profile-config patching, SOUL.md-per-profile, TEAM.md task-graph convention,
and the `hades kanban create --workspace dir:` initial-task pattern are
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per Hades profile rules"
                        )
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for key in plan.get("api_keys_required", []):
        key_checks.append(f'check_key {key} hades {key} || exit 1')
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'hades profile create {t["profile"]} --clone 2>/dev/null || true'
        )
    ...
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.hades/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
