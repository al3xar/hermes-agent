...
the actual `setup.sh` that creates the project workspace, configures Hades
...
2. **Create profiles** — `hades profile create <name> --clone`
...
6. **Fire the initial kanban task** — `hades kanban create` assigned to the director
...
```bash
hades profile create director --clone 2>/dev/null || true
```
...
PyYAML must be installed in the user's Python (it ships with most Hades
...
```bash
hades kanban create "Direct production of <video title>" \
    --assignee director \
...
`~/.hades/.env` and macOS Keychain (if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    if grep -q "^${var}=" ~/.hades/.env 2>/dev/null && \
       [ -n "$(grep "^${var}=" ~/.hades/.env | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ~/.hades/.env or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY hades ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY hades OPENROUTER_API_KEY || exit 1
# ...
```
