...
hades kanban watch --tenant <project-slug>

# Snapshot of the board
hades kanban list --tenant <project-slug>
hades kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
hades kanban stats --tenant <project-slug>

# Visual dashboard (browser)
hades dashboard

# Inspect a specific task (includes comments + events)
hades kanban show <task-id>

# Follow a single task's event stream
hades kanban tail <task-id>
```

Verify available subcommands with `hades kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `hades kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `hades kanban block <id>` then `hades kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `hades kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `hades kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `hades profile list` (profile exists?), `hades status` (gateway/dispatcher up?), and verify tenant. |
...
hades kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
hades kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(hades kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
hades kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
hades kanban block <task-id>
hades kanban archive <task-id>

# Diagnose what happened
hades kanban show <task-id>      # task body, comments, recent events
hades kanban tail <task-id>      # follow the live event stream
hades kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `hades kanban create` for the director
...
while true; do
    clear
    hades kanban list --tenant <slug>
    echo "---"
    hades kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `hades kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `hades profile list` and start
