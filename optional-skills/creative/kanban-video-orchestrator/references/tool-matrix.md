...
toolsets it needs. Only references skills that ship in the public hades-agent
...
## Hades skills relevant to video production

### Visual / rendering skills (`hades-agent/skills/creative/`)
...
### Design / pre-production skills (`hades-agent/skills/creative/`)
...
### Audio / media skills (`hades-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`hades-agent/skills/devops/`)
...
These are **not** Hades skills but external CLIs / APIs that profiles invoke.
...
These are native Hades tools — not invoked via terminal but through their own
...
key is present in `~/.hades/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Hades profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.hades/.env` → environment variable.
...
When a new Hades-public video skill ships:
