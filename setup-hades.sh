...
# 5. Symlinks the 'hades' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/hades-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink hades into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up hades command..."
...
HADES_SKILLS_DIR="${HADES_HOME:-$HOME/.hades}/skills"
mkdir -p "$HADES_SKILLS_DIR"
...
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    ...
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$HADES_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  hades status        # Check configuration"
if is_termux; then
    echo "  hades gateway       # Run gateway in foreground"
else
    echo "  hades gateway install # Install gateway service (messaging + cron)"
fi
echo "  hades cron list     # View scheduled jobs"
echo "  hades doctor        # Diagnose issues"
echo ""
