...
Configuration surfaces (env wins over config.yaml when set non-empty):
  ...
  per-deploy values don't need to bake into ``config.yaml``:
      ...
      HADES_DASHBOARD_OAUTH_CLIENT_ID  — shape ``agent:{agent_instance_id}``
      HADES_DASHBOARD_PORTAL_URL       — defaults to
...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``hades-cli:`` prefix).
...
"state": …}`` and the route serializes those into the ``hades_session_pkce``
cookie.
...
Skip reasons:
  ...
  ("Set HADES_DASHBOARD_OAUTH_CLIENT_ID …") instead of the bare "no
...
# Production Portal URL. Override via HADES_DASHBOARD_PORTAL_URL for
...
class NousDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"hades_session_pkce\"]``
        ...
        cookie_payload = {
            "hades_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _verify_jwt(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # between HADES_DASHBOARD_PORTAL_URL / HADES_DASHBOARD_OAUTH_CLIENT_ID
...
def _resolve_client_id() -> str:
    ...
    Order:
      1. ``HADES_DASHBOARD_OAUTH_CLIENT_ID`` env var (when non-empty
...
def _resolve_portal_url() -> str:
    ...
    Order:
      1. ``HADES_DASHBOARD_PORTAL_URL`` env var (non-empty after strip).
...
def register(ctx) -> None:
    ...
    configured (either via ``HADES_DASHBOARD_OAUTH_CLIENT_ID`` env var
    ...
    can surface "Set HADES_DASHBOARD_OAUTH_CLIENT_ID …" instead of the
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            "HADES_DASHBOARD_OAUTH_CLIENT_ID is not set (and "
    ...
    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"HADES_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
