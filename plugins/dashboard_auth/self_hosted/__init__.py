...
A standards-compliant OpenID Connect Relying Party for the ``hades dashboard``
...
``hades_cli/dashboard_auth/routes.py``; this provider only:
...
same precedence convention as the ``nous`` plugin)::
    ...
    HADES_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Hades does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``HADES_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "HADES_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "HADES_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("HADES_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(HADES_DASHBOARD_OIDC_ISSUER + HADES_DASHBOARD_OIDC_CLIENT_ID) "
