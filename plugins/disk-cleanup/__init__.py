"""disk-cleanup plugin — auto-cleanup of ephemeral Hades session files.
...
def _extract_paths_from_terminal(args: Dict[str, Any], result: str) -> Set[str]:
    ...
    if isinstance(cmd, str) and cmd:
        # Tokenise the command — catches `touch /tmp/hades-x/test_foo.py`
        try:
...
All operations are scoped to HADES_HOME and /tmp/hades-*.
...
def register(ctx) -> None:
    ...
    ctx.register_command(
        ...
        description="Track and clean up ephemeral Hades session files.",
    )
