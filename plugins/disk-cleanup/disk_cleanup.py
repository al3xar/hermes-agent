...
Scope: strictly HADES_HOME and /tmp/hades-*
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HADES_HOME or ``/tmp/hades-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    hades_home = get_hades_home()
    try:
        path.resolve().relative_to(hades_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/hades-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("hades-"):
        return True
    return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under HADES_HOME (but leave HADES_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(hades_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == hades_home:
                continue
            try:
                rel_parts = dirpath.relative_to(hades_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    hades_home = get_hades_home()
    try:
        rel = path.resolve().relative_to(hades_home)
        ...
        # Path isn't under HADES_HOME (e.g. /tmp/hades-*) — fall through.
        pass
