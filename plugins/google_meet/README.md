...
```
┌─ gateway (Linux box, where hades runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `hades meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `hades meet` CLI |
...
| `cli.py` | `hades meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `hades meet node {run,list,approve,remove,status,ping}` |
...
hades meet install                                      # pip + Chromium
hades meet setup                                        # preflight
hades meet auth                                         # optional
hades meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
hades meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hades/.env
hades meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
hades meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
hades meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hades/.env
hades meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, hades will **not** switch your system audio input automatically — the
...
hades plugins enable google_meet
hades meet node run --display-name my-mac --host 0.0.0.0 --port 18789
