...
``$HADES_HOME/workspace/meetings/<meeting-id>/``. The main hades process
...
Run standalone for debugging::
    ...
    HADES_MEET_URL=https://meet.google.com/abc-defg-hij \\
    HADES_MEET_OUT_DIR=/tmp/meet-debug \\
    HADES_MEET_HEADED=1 \\
...
# Filenames the bot reads/writes in ``HADES_MEET_OUT_DIR``.
...
# and exposes ``window.__hadesMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__hadesMeetInstalled) return;
  window.__hadesMeetInstalled = true;
  window.__hadesMeetQueue = [];
  ...
  window.__hadesMeetDrain = () => {
    const out = window.__hadesMeetQueue.slice();
    window.__hadesMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "hades_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    # v2: optional realtime mode. Enabled when HADES_MEET_MODE=realtime.
    ...
    if not url or not _is_safe_meet_url(url):
        sys.stderr.write(
            "google_meet bot: refusing to launch — HADES_MEET_URL must be a "
            "meet.google.com URL. got: %r\n" % url
        )
        return 2
    if not out_dir_env:
        sys.stderr.write("google_meet bot: HADES_MEET_OUT_DIR is required\n")
        return 2
    ...
    if rt["enabled"]:
        if not realtime_api_key:
            state.set(error="realtime mode requested but no API key in HADES_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
            rt["enabled"] = False
        else:
            try:
    ...
    try:
        with sync_playwright() as pw:
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__hadesMeetDrain && window.__hadesMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__hadesMeetInstalled) {
