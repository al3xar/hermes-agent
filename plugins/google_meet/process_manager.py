...
def start(
    ...
    *,
    ...
    If a bot is already running for this hades install, leave it first —
    ...
    env = os.environ.copy()
    env["HADES_MEET_URL"] = url
    env["HADES_MEET_OUT_DIR"] = str(out)
    env["HADES_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["HADES_MEET_HEADED"] = "1"
    if auth_state:
        env["HADES_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["HADES_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if HADES_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["HADES_MEET_MODE"] = mode
    if realtime_model:
        env["HADES_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["HADES_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["HADES_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["HADES_MEET_REALTIME_KEY"] = realtime_api_key
