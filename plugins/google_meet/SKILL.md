...
hades meet install                 # pip deps + Chromium (transcribe only)
hades meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
hades meet auth                    # optional; skips guest-lobby wait
hades meet setup                   # preflight checks
```

`hades meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HADES_MEET_REALTIME_KEY in ~/.hades/.env
```
...
pip install playwright websockets && python -m playwright install chromium
hades plugins enable google_meet
hades meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
hades meet node approve my-mac ws://<mac-ip>:18789 <token>
hades meet node ping my-mac                   # confirm reachable
```

Run `hades meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `hades meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `HADES_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `hades_meet_src`). |
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HADES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `hades meet node approve`.
