...
``{title, url, description, position}`` rows every other Hades web
provider produces.
...
prefers Hades-managed xAI Grok OAuth (via ``hades auth``) and falls back
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``hades tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `hades auth` to sign in with "
