...
own pages inject it via ``window.__HADES_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``hades_cli/web_server.py``.

This means ``hades dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
def get_board(
    ...
    through to the active board (``HADES_KANBAN_BOARD`` env → on-disk
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``hades kanban
...
def list_diagnostics(
    ...
    AND for ``hades kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``hades kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``hades kanban specify <task_id>``.
    ...
    # HADES_KANBAN_BOARD env var — this endpoint runs in FastAPI's
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``hades kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def _active_profile_name() -> str:
    """Return the current Hades profile name for notify-sub ownership."""
    try:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``hades profile describe <name> --auto``. Non-OK
...
def decompose_task_endpoint(
    ...
    1:1 to ``hades kanban decompose <task_id>``.
    ...
    # HADES_KANBAN_BOARD env var would let concurrent requests for
