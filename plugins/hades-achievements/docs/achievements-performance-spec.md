# Hades Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: hades-achievements plugin
...
### `GET /api/plugins/hades-achievements/achievements`
...
### `POST /api/plugins/hades-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/hades-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/hades-achievements/overview`
...
- Persist plugin data under:
  - `~/.hades/plugins/hades-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.hades/plugins/hades-achievements/scan_checkpoint.json`
...
- `~/.hades/plugins/hades-achievements/`
