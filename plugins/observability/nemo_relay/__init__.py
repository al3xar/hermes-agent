"""nemo_relay — optional Hades plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atof_filename: str = "hades-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "hades-atif-{session_id}.json"
...
class _Runtime:
    def __init__(self, nemo_relay: Any, settings: _Settings) -> None:
        ...
        self._atof_subscriber_name = "hades.nemo_relay.atof"
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
            ...
            state.atif_subscriber_name = f"hades.nemo_relay.atif.{session_id}"
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("hades.subagent.stop", kwargs)
    ...
    def _run_managed_with_downstream_preservation(
        ...
        # execution so Hades retry classification still sees it. The LLM and tool
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("hades.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("hades.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("hades.turn.end", kwargs))
...
def on_post_api_request(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("hades.api.response.unmatched", kwargs)
            return
...
def on_api_request_error(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        span = state.llm_spans.pop(_api_key(kwargs), None)
        if span is None:
            runtime.mark("hades.api.error", kwargs)
            return
        runtime.nemo_relay.llm.call_end(
            span,
...
def on_post_tool_call(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("hades.tool.response.unmatched", kwargs)
            return
...
def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("hades.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("hades.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("HADES_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("HADES_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("HADES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("HADES_NEMO_RELAY_ATOF_FILENAME") or "hades-atof.jsonl",
        atof_mode=_env("HADES_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("HADES_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("HADES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("HADES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "hades-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("HADES_NEMO_RELAY_ATIF_AGENT_NAME") or "Hades Agent",
        atif_agent_version=_env("HADES_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("HADES_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _atif_subagent_export_mode() -> str:
    mode = _env("HADES_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # Hades wraps downstream execution failures in a local/private exception
...
def _is_relay_wrapped_callback_error(exc: Exception, callback_error: Exception | None) -> bool:
    ...
    # leading ``internal error:`` shape entirely, this returns False and Hades
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="hades-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
