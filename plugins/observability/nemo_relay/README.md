...
Optional Hades observability plugin that maps Hades observer hooks to
...
CLI UX. Instead, this plugin lets Hades emit NeMo Relay lifecycle events for
the work Hades already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Hades Agent can:

- Preserve Hades execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
hades plugins enable observability/nemo_relay
```

The `HADES_NEMO_RELAY_*` environment variables below only configure an
...
```bash
env HADES_HOME=/tmp/hades-nemo-relay-test \
  hades plugins enable observability/nemo_relay
```
...
`hades plugins enable ...` and the later `hades chat ...` run. If unset,
Hades uses the user's default home, usually `~/.hades`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export HADES_HOME=/tmp/hades-nemo-relay-test
hades plugins enable observability/nemo_relay
hades chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `hades` command you run is built from the
...
uv run hades plugins enable observability/nemo_relay
uv run hades chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/hades_agent-*.whl
...
The plugin can configure exporters directly from `HADES_NEMO_RELAY_*`
...
```bash
export HADES_NEMO_RELAY_ATOF_ENABLED=1
export HADES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export HADES_NEMO_RELAY_ATIF_ENABLED=1
export HADES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `HADES_NEMO_RELAY_ATOF_FILENAME`
- `HADES_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `HADES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `HADES_NEMO_RELAY_ATIF_AGENT_NAME`
- `HADES_NEMO_RELAY_ATIF_AGENT_VERSION`
- `HADES_NEMO_RELAY_ATIF_MODEL_NAME`
- `HADES_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Hades at it:

```bash
export HADES_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `HADES_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`HADES_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`HADES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Hades does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Hades keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Hades routes LLM and tool
...
For the full generic Hades middleware contract, see
...
export HADES_HOME=/tmp/hades-nemo-relay-docs/hades-home
mkdir -p "$HADES_HOME"
...
This run starts a parent Hades session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HADES_NEMO_RELAY_ATOF_ENABLED=1
export HADES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hades-nemo-relay-docs/subagent/atof
export HADES_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HADES_NEMO_RELAY_ATOF_MODE=overwrite
export HADES_NEMO_RELAY_ATIF_ENABLED=1
export HADES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hades-nemo-relay-docs/subagent/atif
export HADES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HADES_NEMO_RELAY_ATIF_AGENT_NAME='Hades Agent E2E'
export HADES_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export HADES_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"hades.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
message. Hades dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/hades-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/hades-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/hades-nemo-relay-docs/workdir/beta.txt
cd /tmp/hades-nemo-relay-docs/workdir

export HADES_NEMO_RELAY_ATOF_ENABLED=1
export HADES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hades-nemo-relay-docs/parallel/atof
export HADES_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HADES_NEMO_RELAY_ATOF_MODE=overwrite
export HADES_NEMO_RELAY_ATIF_ENABLED=1
export HADES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hades-nemo-relay-docs/parallel/atif
export HADES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export HADES_NEMO_RELAY_ATIF_AGENT_NAME='Hades Agent E2E'
export HADES_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Hades sessions map to `agent` scopes.
- Hades API request hooks map to `llm` scope start/end events.
- Hades tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Hades execution middleware to hand
...
```bash
export HADES_NEMO_RELAY_PLUGINS_TOML=/tmp/hades-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Hades routes execution
...
Hades provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Hades provider adapter next_call(...)

Hades tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Hades tool dispatcher next_call(...)
```
...
middleware for a local Hades run. This path requires a NeMo Relay runtime that
...
```bash
export HADES_HOME=/tmp/hades-middleware-test/hades-home
mkdir -p "$HADES_HOME" /tmp/hades-middleware-test/nemo-relay
...
cat > /tmp/hades-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/hades-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/hades-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Hades Middleware E2E"
agent_version = "local"
...
export HADES_NEMO_RELAY_PLUGINS_TOML=/tmp/hades-middleware-test/nemo-relay/plugins.toml
