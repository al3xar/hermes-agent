...
# ``model_tools`` import time, so ``hades status``, ``hades chat``, etc.
...
# ``hades_plugins.platforms__google_chat.adapter`` once the plugin
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a hades MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _hades_home = _get_hades_home()
        except (ModuleNotFoundError, ImportError):
            _hades_home = _Path.home() / ".hades"
        self._thread_count_store = _ThreadCountStore(
            _hades_home / "google_chat_thread_counts.json"
        )
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a hades MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``hades setup``.

    The setup wizard at ``hades_cli/gateway.py`` calls this for plugin
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. hades-chat-events) and a Pull subscription.")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``hades cron`` running as a
