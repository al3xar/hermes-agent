"""SimpleX Chat platform adapter (Hades plugin).
...
``plugins/platforms/simplex/``. The Hades plugin loader scans the
...
Optional environment variables:
    ...
    HADES_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``hades setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Hades — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hades setup gateway`` → SimpleX.
