"""
``hades photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Hades
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `hades photon ...` subcommands."""
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``hades photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    #    and persist both (runtime creds -> ~/.hades/.env, ids -> auth.json).
    try:
    ...
    print("  Start the gateway:  hades gateway start --platform photon")
    return 0
