...
Credential storage mirrors every other Hades channel:
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a hades dependency
...
# registers Hades as its own client_id.
...
# Default name of the project Hades provisions for the operator.
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of hades-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.hades/auth.json`` honouring the active Hades profile."""
    try:
