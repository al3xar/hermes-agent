...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Hades dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        ...
        # surfaces the missing-deps state in `hades setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Hades defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Hades-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Hades-Sidecar-Token": token}
...
def register(ctx) -> None:
    ...
    # gateway-setup hook and the `hades photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        # Surfaces Photon in `hades gateway setup` alongside every other
    ...
    # Register CLI subcommands — `hades photon ...`
