...
//
// Inbound  (gRPC -> Hades): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Hades -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Hades-Sidecar-Token: ${TOKEN}`):
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-hades-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Hades supplied them so a known-good
