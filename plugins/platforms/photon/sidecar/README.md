...
(`spectrum-ts`).  Hades is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Hades-Sidecar-Token`)
...
The Hades plugin's `hades photon setup` command runs `npm install`
here automatically.
