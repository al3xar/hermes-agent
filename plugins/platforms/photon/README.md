...
TypeScript-only, Hades runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` are loopback POSTs to the sidecar,
  authenticated with a shared `X-Hades-Sidecar-Token`.
...
`hades photon setup` does, in order:
...
There is no separate `login` command; like every other Hades channel,
...
Run `hades photon status` to see what's configured.
