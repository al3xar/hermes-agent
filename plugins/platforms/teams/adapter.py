...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Hades"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        hades_action = data.get("hades_action", "")
        session_key = data.get("session_key", "")

        if not hades_action or not session_key:
        ...
        choice = choice_map.get(hades_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="hades_approve",
                    data={**btn_data_base, "hades_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="hades_approve",
                    data={**btn_data_base, "hades_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="hades_approve",
                    data={**btn_data_base, "hades_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="hades_approve",
                    data={**btn_data_base, "hades_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Hades\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
