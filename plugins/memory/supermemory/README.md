...
|-----|---------|-------------|
| `container_tag` | `hades` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `hades-{identity}` → `hades-coder`). |
...
All Supermemory API calls send `x-sm-source: hades`, and document writes stamp
`metadata.sm_source: hades`. This is a **functional routing key, not telemetry**:
it groups Hades-written memories into a dedicated "Hades" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Hades profile:
...
For a profile named `coder`, this resolves to `hades-coder`. The default profile resolves to `hades-default`. Without `{identity}`, all profiles share the same container.
