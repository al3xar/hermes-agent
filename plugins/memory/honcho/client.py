...
Resolution order for config file:
  1. $HADES_HOME/honcho.json  (instance-local, enables isolated Hades instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Hades profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Hades profile.

    Resolution order:
      1. HADES_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``hades.<profile>``
      3. Fallback: ``"hades"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    default_path = _get_default_hades_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Hades system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.hades
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Hades profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.hades block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Hades session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Hades session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Hades session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'hades honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run hades honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `hades honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                hades_cfg = load_config()
                honcho_cfg = hades_cfg.get("honcho", {})
