...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/hades>
...
```bash
hades memory setup honcho   # configure Honcho directly (works on a fresh install)
hades memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
hades config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hades/.env
```

> `hades honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `hades memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HADES_HOME/honcho.json` | Profile-local (isolated Hades instances) |
| 2 | `~/.hades/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Hades profile: `hades` (default) or `hades_<profile>`.
...
**Deployment shapes** (`hades memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Hades to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Hades session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`hades-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`hades`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-hades-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening hades in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Hades session ID (timestamp + hex). Every `hades` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Hades profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`hades`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Hades profile: `hades` (default) or `hades_<profile>` (e.g. `hades -p coder` -> host key `hades_coder`). Older `hades.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `HADES_HONCHO_HOST` | Host key override |
...
|---------|-------------|
| `hades memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `hades honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `hades memory setup`) |
| `hades honcho status` | Show resolved config for active profile |
| `hades honcho enable` / `disable` | Toggle Honcho for active profile |
| `hades honcho mode <mode>` | Change recall or observation mode |
| `hades honcho peer --user <name>` | Update user peer name |
| `hades honcho peer --ai <name>` | Update AI peer name |
| `hades honcho tokens --context <N>` | Set context token budget |
| `hades honcho tokens --dialectic <N>` | Set dialectic max chars |
| `hades honcho map <name>` | Map current directory to a session name |
| `hades honcho sync` | Create host blocks for all Hades profiles |
