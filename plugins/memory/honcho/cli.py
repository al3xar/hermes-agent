...
Handles: hades honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "hades" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Hades profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'hades honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'hades honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `hades update` -- no output, no exceptions.
    """
    try:
        from hades_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Hades profile."""
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        hades_host.get("pinUserPeer"),
        hades_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in hades_host:
        aliases_src = hades_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in hades_host:
        prefix_src = hades_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(hades_host: dict) -> None:
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Hades persistent cross-session memory.")
    ...
    hades_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        current_host_key = hades_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            hades_host["apiKey"] = new_local_key
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'hades honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = hades_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        hades_host["peerName"] = new_peer

    current_ai = hades_host.get("aiPeer") or cfg.get("aiPeer", "hades")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hades_host["aiPeer"] = new_ai

    current_workspace = hades_host.get("workspace") or cfg.get("workspace", "hades")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        hades_host["workspace"] = new_workspace
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, hades_host)
    ...
    if current_shape == "single" and new_shape == "multi":
        peer_target = hades_host.get("peerName") or current_peer or "user"
        print(
    ...
    if new_shape == "single":
        _scrub_identity_mapping(hades_host)
        hades_host["pinPeerName"] = True
        print(f"  pinPeerName=true → all gateway users route to '{hades_host.get('peerName', '?')}'.")
        ...
        if prior_aliases:
            hades_host["userPeerAliases"] = prior_aliases
        ...
        if _new_prefix and not (prefix_from_root and _new_prefix == current_prefix):
            hades_host["runtimePeerPrefix"] = _new_prefix
        print("  Multi-user mode: each runtime ID → own peer. Use 'hades honcho status' to inspect.")
        ...
        if existing_aliases:
            hades_host["userPeerAliases"] = existing_aliases
        _prefix_default = current_prefix or ""
        _new_prefix = _prompt(
    ...
    # --- 4. Observation mode ---
    current_obs = hades_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        hades_host["observationMode"] = new_obs
    else:
        hades_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(hades_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        hades_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        hades_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = hades_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        hades_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = hades_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        hades_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                hades_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(hades_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            hades_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        hades_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        hades_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        hades_host["dialecticReasoningLevel"] = new_reasoning
    else:
        hades_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = hades_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        hades_host["sessionStrategy"] = new_strat

    hades_host["enabled"] = True
    hades_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        hades_config = load_config()
        hades_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(hades_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: hades config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    hades honcho status     -- show full config")
    print("    hades honcho mode       -- change recall/observation mode")
    print("    hades honcho tokens     -- tune context and dialectic budgets")
    print("    hades honcho peer       -- update peer names")
    print("    hades honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Hades profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: hades honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'hades honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'hades honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: hades honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        hades = hosts.get(_host_key(), {})
        user = hades.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = hades.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = hades.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = hades.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Hades' identity in Honcho. Seed with 'hades honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: hades honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: hades honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = hades.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = hades.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = hades.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Hades asks Honcho's AI peer a question")
        ...
        print("\n  Set with: hades honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'hades honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'hades honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    hades honcho identity --show        — show both peer representations")
        print("    hades honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Hades + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Hades\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Hades persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  hades honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'hades honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'hades honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  hades honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'hades'")
        print("  with Honcho configured and no prior session history.")
        print("  (Hades calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    hades honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'hades honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Hades, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'hades honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    hades honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  hades honcho identity <path/to/SOUL.md>")
    ...
    print("    Hades:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Hades:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Hades:   Honcho observes every message and updates representations")
    ...
    print("    Hades:   per-session by default — each run gets its own session")
    print("              Map a custom name:  hades honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. hades honcho setup              — configure API key (required)")
        print("  2. hades honcho migrate            — re-run this walkthrough")
    else:
        print("  1. hades honcho status             — verify Honcho connection")
        print("  2. hades                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. hades honcho identity --show    — verify AI peer representation")
        print("  4. hades honcho tokens             — tune context and dialectic budgets")
        print("  5. hades honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'hades memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``hades honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``hades honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to hades memory setup)",
    )
