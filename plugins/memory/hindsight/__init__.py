...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: hades)
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Hades can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    return {
        ...
        "banks": {
            "hades": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "hades"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Hades config."""
    profile = config.get("profile", "hades")
    return str(profile or "hades")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Hades profile name (from agent_identity)
      {workspace} — Hades workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``hades-{user}`` with no user becomes ``hades``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hades_home):
        ...
        import json
        from pathlib import Path
        config_dir = Path(hades_home) / "hindsight"
    ...
    def post_setup(self, hades_home: str, config: dict) -> None:
        ...
        from hades_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "hades")
        ...
        self.save_config(provider_config, hades_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(hades_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(hades_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "hades"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: hades-{profile}", "default": ""},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "hades"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "hades"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "hades", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "hades")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Hades created/used that client on the shared async loop,
