...
Hades spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
```bash
hindsight-embed -p hades ui start
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `hades` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `hades-{profile}` isolates memory per active Hades profile. Empty placeholders collapse cleanly (e.g. `hades-{user}` with no user becomes `hades`). |
