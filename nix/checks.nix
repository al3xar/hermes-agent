...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      hades-agent = self'.packages.default;
      hadesVenv = hades-agent.hadesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "hades-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${hadesVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "hades-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "hades-build-package" { } ''
          echo "PASS: package built at ${hades-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "hades-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "hades-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${hades-agent}/bin/hades || (echo "FAIL: hades binary missing"; exit 1)
          test -x ${hades-agent}/bin/hades-agent || (echo "FAIL: hades-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${hades-agent}/bin/hades version 2>&1 | grep -qi "hades" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hades-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in hades hades-agent hades-acp; do
            test -x ${hades-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hades-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking hades --help ==="
          ${hades-agent}/bin/hades --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${hades-agent}/bin/hades --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hades-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${hades-agent}/share/hades-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${hades-agent}/share/hades-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HADES_BUNDLED_SKILLS" ${hades-agent}/bin/hades || \
        ...
        bundled-plugins = pkgs.runCommand "hades-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${hades-agent}/share/hades-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${hades-agent}/share/hades-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HADES_BUNDLED_PLUGINS" ${hades-agent}/bin/hades || \
        ...
        bundled-locales = pkgs.runCommand "hades-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${hades-agent}/share/hades-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${hades-agent}/share/hades-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${hades-agent}/share/hades-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "HADES_BUNDLED_LOCALES" ${hades-agent}/bin/hades || \
            (echo "FAIL: HADES_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: HADES_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (HADES_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && HADES_BUNDLED_LOCALES=${hades-agent}/share/hades-agent/locales \
            ${hadesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with HADES_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${hadesVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${hadesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without HADES_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "hades-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${hades-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${hades-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HADES_TUI_DIR" ${hades-agent}/bin/hades || \
            (echo "FAIL: HADES_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: HADES_TUI_DIR set in wrapper"
        ...
        # Verify HADES_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        hades-node = pkgs.runCommand "hades-node-version" { } ''
          set -e
          echo "=== Checking HADES_NODE in wrapper ==="
          grep -q "HADES_NODE" ${hades-agent}/bin/hades || \
            (echo "FAIL: HADES_NODE not set in wrapper"; exit 1)
          echo "PASS: HADES_NODE present in wrapper"

          HADES_NODE=$(sed -n "s/^export HADES_NODE='\(.*\)'/\1/p" ${hades-agent}/bin/hades)
          test -x "$HADES_NODE" || (echo "FAIL: HADES_NODE=$HADES_NODE not executable"; exit 1)
          echo "PASS: HADES_NODE executable at $HADES_NODE"

          NODE_MAJOR=$("$HADES_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All HADES_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify HADES_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "hades-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(HADES_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking HADES_MANAGED guards ==="
          check_blocked "config set" ${hades-agent}/bin/hades config set model foo
          check_blocked "config edit" ${hades-agent}/bin/hades config edit
        ...
        extra-python-packages = let
          ...
          hadesWithExtra = hades-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "hades-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hadesWithExtra}/bin/hades || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hadesWithExtra}/bin/hades || \
          ...
          if grep -q "PYTHONPATH" ${hades-agent}/bin/hades; then
        ...
        extra-dependency-groups = let
          hadesWithGroups = hades-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "hades-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${hadesWithGroups}"
          echo "venv: ${hadesWithGroups.hadesVenv}"
        ...
        messaging-variant = pkgs.runCommand "hades-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.hadesVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "hades-config-roundtrip" {
          ...
          merge_and_load() {
            local hades_home="$1"
            export HADES_HOME="$hades_home"
            ${configMergeScript} ${nixSettings} "$hades_home/config.yaml"
            ${hadesVenv}/bin/python3 -c '
import json, sys
