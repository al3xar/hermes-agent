# nix/nixosModules.nix — NixOS module for hades-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: hades runs from /nix/store bind-mounted read-only into a
...
# and read by hades at startup — no container recreation needed for env changes.
#
# Tool resolution: the hades wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.hades-agent;
    effectivePackage =
    ...
    hades-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-hades-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "hades-config-attrs";
      description = "Hades YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "hades-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "hades-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'HADES_DOC_EOF'\n${value}\nHADES_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    # hades user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "hades-container-entrypoint" ''
      set -eu

      HADES_UID="''${HADES_UID:?HADES_UID must be set}"
      HADES_GID="''${HADES_GID:?HADES_GID must be set}"

      # ── Group: ensure a group with GID=$HADES_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$HADES_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="hades"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$HADES_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$HADES_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$HADES_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$HADES_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="hades"
        TARGET_HOME="/home/hades"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$HADES_UID" -g "$HADES_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$HADES_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$HADES_UID:$HADES_GID" "$TARGET_HOME"
      ...
      if [ -n "''${HADES_HOME:-}" ] && [ -d "$HADES_HOME" ]; then
        find "$HADES_HOME" \! -user "$HADES_UID" -exec chown "$HADES_UID:$HADES_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/hades-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/hades-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/hades ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/hades
        chmod 0440 /etc/sudoers.d/hades
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$HADES_UID" --regid="$HADES_GID" --init-groups "$@"
    ...
    # Default: /var/lib/hades/workspace → /data/workspace.
  ...
  in {
    options.services.hades-agent = with lib; {
      ...
      package = mkOption {
        type = types.package;
        default = hades-agent;
        description = "The hades-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/hades";
        description = "State directory. Contains .hades/ subdir (HADES_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Hades config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Hades reads this file on every startup via load_hades_dotenv().
        '';
      };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `hades gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the hades user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the hades plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Hades discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as hades (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in hades-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the hades CLI to environment.systemPackages and export
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          description = ''
            ...
            stateDir. These users are automatically added to the hades group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.hades-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the hades CLI to system PATH and export HADES_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.hades/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.HADES_HOME = "${cfg.stateDir}/.hades";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.hades-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the hades user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.hades-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed hades
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure hades is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.hades        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hades/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hades/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hades/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hades/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.hades/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."hades-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.hades
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.hades ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.hades ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.hades -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.hades/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.hades/$_subdir"
            chmod 2770 "${cfg.stateDir}/.hades/$_subdir"
            find "${cfg.stateDir}/.hades/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # hades-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.hades/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.hades/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hades/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.hades/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.hades/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hades/.managed
          chmod 0644 ${cfg.stateDir}/.hades/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.hades/.container-mode <<'HADES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hades_bin=${containerDataDir}/current-package/bin/hades
    HADES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hades/.container-mode
            chmod 0644 ${cfg.stateDir}/.hades/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.hades/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.hades";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.hades" ]; then
                  rm -f "${symlinkPath}"
                  echo "hades-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.hades -> stateDir/.hades for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.hades";
                target = "${cfg.stateDir}/.hades";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "hades-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.hades/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.hades/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.hades/auth.json
              fi
            ''}
          ''}
          ...
          # Hades reads $HADES_HOME/.env at startup via load_hades_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.hades/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'HADES_NIX_ENV_EOF'
    ${envFileContent}
    HADES_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.hades/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.hades/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.hades/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.hades-agent = {
          ...
          serviceConfig = {
            ...
            # $HADES_HOME/.env by the activation script. load_hades_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/hades"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the hades group can read/write them.
            UMask = "0007";
        ...
        systemd.services.hades-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              HADES_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              HADES_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env HADES_UID="$HADES_UID" \
                --env HADES_GID="$HADES_GID" \
                --env HADES_HOME=${containerDataDir}/.hades \
                --env HADES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/hades gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
