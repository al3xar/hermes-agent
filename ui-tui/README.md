...
Interpreter resolution order is: `HADES_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.hades/.hades_history` or under `HADES_HOME`.
...
```text
ui-tui/
  packages/hades-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      hades-ink.d.ts    type declarations for @hades/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent HadesCLI subprocess for slash commands
```
