...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.HADES_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Hades on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via HADES_TUI_TRUECOLOR=1.
