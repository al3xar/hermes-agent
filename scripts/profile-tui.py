#!/usr/bin/env python3
"""Drive the Hades TUI under HADES_DEV_PERF and summarize the pipeline.
...
launches `node dist/entry.js` directly with HADES_TUI_RESUME set so it
bypasses the hades_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HADES_PERF_LOG     (default ~/.hades/perf.log)
  HADES_PERF_NODE    (default node from $PATH)
  HADES_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests HADES_DEV_PERF wiring is broken).
"""
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — HADES_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["HADES_DEV_PERF"] = "1"
    env["HADES_DEV_PERF_MS"] = str(args.threshold_ms)
    env["HADES_DEV_PERF_LOG"] = str(log)
    env["HADES_TUI_RESUME"] = sid
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="HADES_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "hades-ink" / "src"
