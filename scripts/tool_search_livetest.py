...
# Force-isolate the test environment BEFORE any hades imports.
...
def setup_isolated_home(enabled: bool) -> Path:
    ...
    """
    home_dir = Path(tempfile.mkdtemp(prefix="hades_ts_live_"))
    hades_home = home_dir / ".hades"
    hades_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, hades_home / "auth.json")
    ...
    if real_env_file.exists():
        shutil.copy(real_env_file, hades_home / ".env")
        ...
        load_hades_dotenv(hades_home=str(Path.home() / ".hades"))
    ...
    (hades_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return hades_home
