...
# Python-driven tool session (e.g. Hades terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/hades-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/hades-agent.git"
HADES_HOME="${HADES_HOME:-$HOME/.hades}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${HADES_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$HADES_INSTALL_DIR"
...
#   code at /usr/local/lib/hades-agent, command at /usr/local/bin/hades,
#   data still at /root/.hades (HADES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   'hades update' runs never inject bundled skills either"
            ...
            echo "                   default (non-root):  ~/.hades/hades-agent"
            echo "                   default (root, Linux): /usr/local/lib/hades-agent"
            echo "  --hades-home PATH  Data directory (default: ~/.hades, or \$HADES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hades installs the code under"
            echo "  /usr/local/lib/hades-agent and links the command into"
            ...
            echo "  (default /root/.hades).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HADES_HOME/hades-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs hades setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `hades update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Hades-Setup) passes it so
...
# Decide where the repo checkout + venv live, and where the `hades` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HADES_HOME/hades-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HADES_HOME/hades-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/hades-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HADES_HOME/hades-agent — then preserve it)
#
# Always no-op when the user set --dir or $HADES_INSTALL_DIR.
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HADES_HOME/hades-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HADES_HOME/hades-agent/.git" ]; then
            INSTALL_DIR="$HADES_HOME/hades-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/hades-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/hades-agent"
...
get_hades_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/hades" ]; then
        echo "$link_dir/hades"
    else
        echo "hades"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://hades-agent.nousresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hades owns its own uv at $HADES_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hades_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hades update` stay in sync.
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/hades-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/hades-uv-installer.$$.sh")"
...
# string clears that floor; anything below it is replaced with the Hades-
...
install_node() {
    ...
    # the hades command uses (get_command_link_dir): /usr/local/bin for root
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="hades-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
...
setup_path() {
    ...
    # Create a user-facing shim for the hades command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $HADES_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$HADES_BIN"` self-recurse. (#21454)
...
SOUL_EOF
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        # skills_sync.py and `hades update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'hades update'." \
        ...
        log_info "  Future 'hades update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Hades Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Hades will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (hades --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `hades update` doesn't autostash every run.
...
maybe_start_gateway() {
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'hades whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HADES_CMD="$(get_hades_command_path)"
                $HADES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'hades whatsapp' to pair."
        fi
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HADES_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $HADES_CMD gateway start 2>/dev/null; then
            ...
            nohup $HADES_CMD gateway > "$HADES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
...
ensure_browser() {
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$HADES_HOME/node" --silent --ignore-scripts \
...
postinstall_mode() {
    ...
    HADES_CMD="$(command -v hades 2>/dev/null || echo "")"
    if [ -n "$HADES_CMD" ]; then
...
install_desktop() {
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Hades" ]; then
            app="$desktop_dir/release/linux-unpacked/Hades"
        elif [ -x "$desktop_dir/release/linux-unpacked/hades" ]; then
            app="$desktop_dir/release/linux-unpacked/hades"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Hades.app" \
            "$desktop_dir/release/mac/Hades.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Hades is damaged and can't be opened").
    ...
    # checkout stays clean for the next `hades update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$HADES_HOME/.install_method"
            ;;
        *)
