...
# - ensures ~/.hades/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-hades.sh
...
#   HADES_API_PORT=8642
#   HADES_API_HOST=127.0.0.1
#   HADES_API_MODEL_NAME='Hades Agent'
...
HADES_ENV_FILE="${HADES_ENV_FILE:-$HOME/.hades/.env}"
HADES_API_PORT="${HADES_API_PORT:-8642}"
HADES_API_HOST="${HADES_API_HOST:-127.0.0.1}"
HADES_API_CONNECT_HOST="${HADES_API_CONNECT_HOST:-127.0.0.1}"
HADES_API_MODEL_NAME="${HADES_API_MODEL_NAME:-Hades Agent}"
HADES_API_BASE_URL="http://${HADES_API_CONNECT_HOST}:${HADES_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-hades.sh"
LOG_DIR="$HOME/.hades/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$HADES_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$HADES_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.hades.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.hades</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.hades"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.hades"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-hades.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
...
[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-hades.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.hades/logs/openwebui.log
StandardError=append:%h/.hades/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-hades.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$HADES_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Hades API server is configured...'
  upsert_env API_SERVER_ENABLED true "$HADES_ENV_FILE"
  upsert_env API_SERVER_HOST "$HADES_API_HOST" "$HADES_ENV_FILE"
  upsert_env API_SERVER_PORT "$HADES_API_PORT" "$HADES_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$HADES_API_MODEL_NAME" "$HADES_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$HADES_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Hades gateway so API server settings take effect...'
  hades gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HADES_API_CONNECT_HOST}:${HADES_API_PORT}/health" >/dev/null; then
    log 'Hades API server did not answer on the first check. Trying to start gateway in the background...'
    nohup hades gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${HADES_API_CONNECT_HOST}:${HADES_API_PORT}/health" >/dev/null
  ...
  log "Hades API endpoint: ${HADES_API_BASE_URL}"
