...
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.hades', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.hades', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.hades', 'audio_cache');
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `hades_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
