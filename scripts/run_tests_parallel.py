...
Environment:
    HADES_TEST_WORKERS  Override worker count (default: os.cpu_count())
    HADES_TEST_PATHS    Override discovery roots (colon-sep, default: 'tests')
...
#                        nousresearch/hades-agent:test image, via
#                        ``HADES_TEST_IMAGE`` so the fixture skips
...
# via --file-timeout or HADES_TEST_FILE_TIMEOUT.
...
def _format_file(file: Path, repo_root: Path) -> str:
    ...
    ``/home/runner/work/hades-agent/hades-agent/tests/acp/test_auth.py``.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Parallel worker count (default: $HADES_TEST_WORKERS or cpu_count*2)",
    )
    parser.add_argument(
        "--paths",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: 600 (10 min), env: HADES_TEST_FILE_TIMEOUT."
        ),
    )
    parser.add_argument(
        "--slice",
        ...
        help=(
            ...
            "Env: HADES_TEST_SLICE (format: I/N)."
        ),
    )
    parser.add_argument(
        "paths_positional",
    ...
    # Parse --slice (or HADES_TEST_SLICE) early so we can exit on bad input
