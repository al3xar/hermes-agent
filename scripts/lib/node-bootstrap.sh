...
#   if [ "$HADES_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   HADES_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   HADES_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   HADES_HOME               (default: $HOME/.hades)
# ============================================================================

HADES_NODE_MIN_VERSION="${HADES_NODE_MIN_VERSION:-20}"
HADES_NODE_TARGET_MAJOR="${HADES_NODE_TARGET_MAJOR:-22}"
HADES_HOME="${HADES_HOME:-$HOME/.hades}"
HADES_NODE_AVAILABLE=false
...
_nb_have_modern_node() {
    ...
    [ "$(_nb_node_major)" -ge "$HADES_NODE_MIN_VERSION" ]
}
...
ensure_node() {
    ...
    _nb_try_fnm   && { HADES_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { HADES_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { HADES_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { HADES_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { HADES_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { HADES_NODE_AVAILABLE=true; return 0; }
