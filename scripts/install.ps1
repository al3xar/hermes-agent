...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HadesHome = $(if ($env:HADES_HOME) { $env:HADES_HOME } else { "$env:LOCALAPPDATA\hades" }),
    [string]$InstallDir = $(if ($env:HADES_HOME) { "$env:HADES_HOME\hades-agent" } else { "$env:LOCALAPPDATA\hades\hades-agent" }),
    ...
    # builds apps/desktop into a launchable Hades.exe.
    #
    # Why opt-in:
    #   * Hades-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Hades.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Hades.exe
    ...
    #     `hades desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/hades-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/hades-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $HadesHome)) {
        New-Item -ItemType Directory -Force -Path $HadesHome | Out-Null
    }
    $envFile = Join-Path $HadesHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $HadesHome "node"
...
function Install-Uv {
    # Hades owns its own uv at $HadesHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hades_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `hades update` stay in sync.
    $managedUv = Join-Path $HadesHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $HadesHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $HadesHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $HadesHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $HadesHome "bin"
...
function Install-Git {
    ...
    which Hades uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\hades\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Hades needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\hades\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HADES_GIT_BASH_PATH`` (User scope) so Hades can find it in a fresh
    ...
    # Download PortableGit into $HadesHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $HadesHome\git\ ..."
...
function Set-GitBashEnvVar {
    ...
    ``HADES_GIT_BASH_PATH`` (User env scope) so Hades can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $HadesHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $HadesHome\git\usr\bin\bash.exe
    $candidates += "$HadesHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$HadesHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    Write-Info "If needed, set HADES_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    # The portable zip path drops node.exe + npm into $HadesHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $HadesHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\hades-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$HadesHome\node") { Remove-Item -Recurse -Force "$HadesHome\node" }
                Move-Item $extractedDir.FullName "$HadesHome\node"
                ...
                $version = & "$HadesHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $HadesHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\hades-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    $stashName = "hades-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
            ...
            try {
                ...
                Write-Info "terminals, running hades processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/hades-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/hades-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/hades-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\hades-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\hades-agent-extract"
    ...
    # next `hades update` checkout aborts on a "dirty" tree the user never
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `hades.exe` exists
...
"@ 2>$null
    ...
    # isn't set, leaving venv\ empty and hades.exe broken with
    ...
    # here, not 30 seconds later when the user runs `hades`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `hades dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `hades dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `hades dashboard` should now work."
            } else {
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        $hadesBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so hades is globally available
    # On Windows, the hades.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$hadesBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$hadesBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $hadesBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HADES_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hades instead
    # of the Unix default ~/.hades
    $currentHadesHome = [Environment]::GetEnvironmentVariable("HADES_HOME", "User")
    if (-not $currentHadesHome -or $currentHadesHome -ne $HadesHome) {
        [Environment]::SetEnvironmentVariable("HADES_HOME", $HadesHome, "User")
        Write-Success "Set HADES_HOME=$HadesHome"
    }
    $env:HADES_HOME = $HadesHome
    
    # Update current session
    $env:Path = "$hadesBin;$env:Path"
    
    Write-Success "hades command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.hades-bootstrap-complete which tells the Hades
    ...
    $markerPath = Join-Path $InstallDir ".hades-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.hades directory structure
    New-Item -ItemType Directory -Force -Path "$HadesHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HadesHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$HadesHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.hades/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.hades/.env"
        }
    } else {
        Write-Info "~/.hades/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$HadesHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.hades/config.yaml from template"
        }
    } else {
        Write-Info "~/.hades/config.yaml already exists, keeping it"
    }
...
"@
        ...
        Write-Success "Created ~/.hades/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.hades/"
    
    # Seed bundled skills into ~/.hades/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.hades/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.hades/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$HadesHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.hades/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Hades-Setup.exe runs each -Stage NAME
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\hades-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\hades-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\hades-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Hades` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Hades.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_hades_desktop command
    # resolves apps/desktop/release/win-unpacked/Hades.exe directly,
    ...
    # The --dir mode produces an unpacked Hades.exe in
    ...
    # are NOT trying to work around). The Hades icon + product name are
    ...
    $buildLog = "$env:TEMP\hades-desktop-build-$(Get-Random).log"
    ...
    try {
        ...
        if ($code -ne 0) {
            ...
            # ENOENT on the final `electron` -> `Hades` rename: app-builder's
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Hades.exe",
        "$desktopDir\release\win-arm64-unpacked\Hades.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Hades.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Hades icon + identity are stamped onto Hades.exe by the
    ...
    #    Hades.exe. We deliberately do NOT point them at `hades desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Hades.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Hades.lnk')
        )
...
function Invoke-SetupWizard {
    ...
    # Run hades setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    $hadesCmd = "$InstallDir\venv\Scripts\hades.exe"
    if (-not (Test-Path $hadesCmd)) {
        $hadesCmd = "hades"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$HadesHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'hades whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$HadesHome\logs\gateway.log"
            Start-Process -FilePath $hadesCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$HadesHome\logs\gateway-error.log" `
                -WindowStyle Hidden
...
function Write-Completion {
    ...
    Write-Host "$HadesHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$HadesHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HadesHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HadesHome\hades-agent\"
    Write-Host ""
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Hades repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Hades-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Hades to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://hades-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
