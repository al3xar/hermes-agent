...
SSE streaming. By exposing this endpoint, hades-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  hades-agent   │
│  NextChat        │  │                                    │  gateway        │
...
2. Run `hades gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with hades-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns hades-agent as a model) |
...
Authorization: Bearer hades-api-key-here
...
But hades-agent sessions have persistent state (memory, skills, tool context).
...
requested model name to the configured hades-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of hades-agent's core prompt
...
Once implemented, hades-agent works as a drop-in backend for:
