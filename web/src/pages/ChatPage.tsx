/**
 * ChatPage — embeds `hades --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (hades_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { HADES_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${HADES_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__HADES_SESSION_TOKEN__ &&
    !window.__HADES_AUTH_REQUIRED__
  ...
  useEffect(() => {
    ...
    const gated = !!window.__HADES_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Hades TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[hades-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__HADES_AUTH_REQUIRED__``) this
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="hades-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
