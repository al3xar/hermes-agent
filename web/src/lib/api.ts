...
// (e.g. https://mission-control.tilos.com/hades/). The Python backend
// injects ``window.__HADES_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__HADES_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const HADES_BASE_PATH = readBasePath();
const BASE = HADES_BASE_PATH;
...
declare global {
  interface Window {
    __HADES_SESSION_TOKEN__?: string;
    __HADES_BASE_PATH__?: string;
    ...
    __HADES_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Hades-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__HADES_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``hades update``, ``hades gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__HADES_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__HADES_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("hades.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("hades.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__HADES_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("hades.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__HADES_SESSION_TOKEN__;
  if (injected) {
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__HADES_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Hades-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``hades_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__HADES_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateHades: () =>
    fetchJSON<ActionResponse>("/api/hades/update", { method: "POST" }),
  checkHadesUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/hades/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** hades-index only: whether the centralized index loaded. */
  available?: boolean;
}
