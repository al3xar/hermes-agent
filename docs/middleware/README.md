...
Hades middleware is the behavior-changing companion to observer hooks.
...
- Rewrite LLM provider request kwargs before Hades calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Hades retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Hades guardrails,
...
- `telemetry_schema_version`: currently `hades.observer.v1`
- `middleware_schema_version`: currently `hades.middleware.v1`
...
Hades stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Hades runs
them as a nested chain in registration order. Middleware failures are fail-open:
Hades logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Hades applies middleware in this order:
...
For each tool call, Hades applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Hades pre-execution path against the effective arguments:
...
```bash
hades plugins enable <plugin-name>
```
...
```bash
export HADES_HOME=/tmp/hades-middleware-test
mkdir -p "$HADES_HOME"
hades plugins enable <plugin-name>
hades chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run hades plugins enable <plugin-name>
uv run hades chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["hades_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/hades-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Hades expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Hades treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Hades preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Hades does not convert downstream
