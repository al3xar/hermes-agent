...
The correct mechanism is the existing subprocess path: `_spawn_hades_action`
...
```python
_spawn_hades_action(["-p", profile, "skills", "install", identifier], "skills-install")
```
...
spawned detached (`_spawn_hades_action` returns a PID immediately). So the
create flow is:
