# Hades Observer Hooks

Hades observer hooks are the read-only telemetry contract for plugins that
...
Hook callbacks are fail-open. Hades catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Hades only builds
