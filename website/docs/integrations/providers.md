...
This page covers setting up inference providers for Hades Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Hades.

## Inference Providers

You need at least one way to connect to an LLM. Use `hades model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `hades model` (OAuth, subscription-based) |
| **OpenAI Codex** | `hades model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `hades model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `hades model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `hades model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
...
| **xAI Grok OAuth (SuperGrok)** | `hades model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
...
| **Google Gemini (OAuth)** | `hades model` → "Google Gemini (OAuth)" (provider: `google-gemini-cli`, free tier supported, browser PKCE login) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.hades/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `hades model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `hades model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.hades/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `hades model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `hades model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `hades model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.hades/.env` (provider: `stepfun`) |
| **LM Studio** | `hades model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `hades model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
```bash
hades setup --portal     # fresh install — OAuth + provider + gateway in one command
hades model              # existing install — pick "Nous Portal" from the list
hades portal info        # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Hades Agent with Nous Portal guide](/guides/run-hades-with-nous-portal).

**Client identification.** Every Portal request from Hades Agent carries a `client=hades-client-v<version>` tag (e.g. `client=hades-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Hades traffic from other clients. No config required; the tag updates automatically when you `hades update`.

**JWT auth (automatic).** Hades prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Hades stores the resulting credentials in its own auth store under `~/.hades/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Hades marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `hades auth add codex-oauth` (or `hades model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Hades routes these tasks to your **main chat model** — the same model you picked in `hades model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `hades setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `hades model` or per-tool from `hades tools`. Inspect routing at any time with `hades portal info`.
:::

### Two Commands for Model Management

Hades has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`hades model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Hades chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `hades model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `hades model`, complete the provider setup, then start a new session.
...
When you authenticate via `hades model` → Anthropic OAuth (or via `hades auth add anthropic --type oauth`), Hades routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Hades — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
hades chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `hades model`
# Hades will use Claude Code's credential store directly when available
hades model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
hades chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
hades chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `hades model`, Hades prefers Claude Code's own credential store over copying the token into `~/.hades/.env`. That keeps refreshable Claude credentials refreshable.
...
Hades supports GitHub Copilot as a first-class provider with two modes:
...
```bash
hades chat --provider copilot --model gpt-5.4
```
...
If no token is found, `hades model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `hades model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `hades model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Hades
Hades sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Hades now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Hades therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
hades chat --provider copilot-acp --model copilot-acp
...
| `HADES_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `HADES_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# NovitaAI Model API
hades chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.hades/.env

# z.ai / ZhipuAI GLM
hades chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.hades/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
hades chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.hades/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
hades chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.hades/.env

# MiniMax (global endpoint)
hades chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.hades/.env

# MiniMax (China endpoint)
hades chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.hades/.env

# Qwen Cloud / DashScope (Qwen models)
hades chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.hades/.env

# Xiaomi MiMo
hades chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.hades/.env

# Tencent TokenHub (Hy3 Preview)
hades chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.hades/.env

# Arcee AI (Trinity models)
hades chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.hades/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
hades chat --provider gmi --model zai-org/GLM-5.1-FP8
...
When using the Z.AI / GLM provider, Hades automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.hades/.env` and pick xAI in `hades model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `hades model`, or run `hades auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Hades runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Hades automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `hades tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `hades doctor` and `hades chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `hades migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
hades migrate xai          # preview replacements
hades migrate xai --apply  # rewrite ~/.hades/config.yaml in place
```
...
# Short alias
hades chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `hades model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Hades auto-discovers the available models.
...
hades chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 hades chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `hades model`, sign in through the browser, and Hades persists the refresh token.
...
hades chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `HADES_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Hades exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
hades chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `hades model`, sign in through the browser, and Hades persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
hades chat   # uses api.minimax.io/anthropic endpoint
```
...
hades chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.hades/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 hades chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Hades automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
hades chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.hades/.env
```
...
```bash
# StepFun
hades chat --provider stepfun --model step-3.5-flash
...
```bash
# Use any available model
hades chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.hades/.env

# Short alias
hades chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
# → done — Hades auto-provisions your free tier on first request
```

Hades ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Tokens stored at `~/.hades/auth/google_oauth.json` (chmod 0600, atomic write, cross-process `fcntl` lock)
- Automatic refresh 60 s before expiry
- Headless environments (SSH, `HADES_HEADLESS=1`) → paste-mode fallback
...
- Responses translated back to OpenAI shape so the rest of Hades works unchanged
...
| Workspace / Standard / Enterprise account | Set `HADES_GEMINI_PROJECT_ID` or `GOOGLE_CLOUD_PROJECT` to your GCP project ID |
| VPC-SC-protected org | Hades detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Hades shows
...
```bash
HADES_GEMINI_CLIENT_ID=your-client.apps.googleusercontent.com
HADES_GEMINI_CLIENT_SECRET=...   # optional for Desktop clients
```
...
Hades Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Hades at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
:::warning hades model vs /model
**`hades model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Hades chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `hades model`, set up the new provider, then start a new session.
:::
...
When switching providers, Hades persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Hades.
:::
...
Supported parsers: `hades` (Qwen 2.5, Hades 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Hades to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Hades won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Hades 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Hades will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Hades) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.
...
Use that IP in your Hades config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Hades config.
...
If you're running Hades inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser hades` |
...
```bash
# Check what Hades thinks the context is
...
Hades auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Hades uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Hades with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
...
Then configure Hades with `hades model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Hades with `hades model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `hades model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Hades uses this to decide when to compress history and to validate API requests.
...
Hades uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`hades model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Hades uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `hades model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Hades will merge it into each chat-completions request for that endpoint:
...
The `hades model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so Hades routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `hades model` menu.
...
Together's `/v1/models` endpoint works, so `hades model` can auto-discover available models.
...
:::tip Troubleshooting
- `hades doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `hades model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `hades model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Hades at your instance (no API key needed):
   ```bash
   hades config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
Configure a chain of backup providers Hades tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `hades fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::
