...
- Your Hades install (`~/.hades/`) — Linux side. The installer already does this.
...
One exception: **MCP bridges that launch Windows binaries.** If you're using `chrome-devtools-mcp` through `cmd.exe` (see [MCP guide: WSL → Windows Chrome](/guides/use-mcp-with-hades#wsl2-bridge-hades-in-wsl-to-windows-chrome)), Windows may complain with a `UNC` warning if Hades's current working directory is `~`. In that case, start Hades from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
Hades's cwd is inside the Linux filesystem, and Windows `cmd.exe` doesn't know what to do with it. Start Hades from `/mnt/c/...` for that session, or use a wrapper that `cd`s to a Windows-reachable path before invoking the Windows executable.
...
- **[MCP guide → WSL → Windows Chrome](/guides/use-mcp-with-hades#wsl2-bridge-hades-in-wsl-to-windows-chrome)** — controlling your signed-in Windows Chrome from Hades in WSL.
