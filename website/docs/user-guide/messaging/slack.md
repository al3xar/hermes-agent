...
public HTTP endpoints, so your Hades instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Hades uses the modern
...
The fastest path is to paste a manifest Hades generates for you. It
...
### Option A: From a Hades-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   hades slack manifest --write
   ```
...
it will only work in DMs. Without `files:read`, Hades can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `hades-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Hades configuration issue.
:::
...
Hades uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
hades gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
hades gateway              # Foreground
hades gateway install      # Install as a user service
sudo hades gateway install --system   # Linux only: boot-time system service
```
...
Every Hades command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Hades command with its description.

Under the hood: Hades ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/hermes-agent/blob/main/hades_cli/commands.py)
...
When Hades adds new commands (e.g. after `hades update`), regenerate
...
### Legacy `/hades <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/hades btw run the tests` — Hades routes it the same way as `/btw
run the tests`. Free-form questions also work: `/hades what's the
...
As a workaround, Hades recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Hades treats it identically to the slash form and replies in the same
thread.
...
```bash
hades slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Hades behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Hades Agent what time is it?`). In channels, Hades replies in a thread attached to that message. |
| **Threads** | If you @mention Hades inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
slack:
  ...
  # must @mention the bot before Hades will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Hades will deliver scheduled messages,
...
Hades can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Hades also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Hades uses the correct workspace-specific client to respond.
...
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Hades now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
- Audit who has access to your Hades config directory
