...
Connect Hades to **iMessage** through [Photon][photon], a managed
...
both directions. Because the SDK is TypeScript-only, Hades runs it in a
...
hades photon setup --phone +15551234567
```
...
Photon uses the same authorization model as every other Hades
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Hades replies with a pairing code. Approve it with:

```bash
hades pairing approve photon <CODE>
```
...
By default Hades responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Hades` and
...
```bash
hades gateway start --platform photon
```
...
Send an iMessage to your assigned number and Hades will reply.
...
assigned iMessage line Hades uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run hades photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `hades photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `hades photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `hades photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Hades sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Hades wake words  | JSON list / comma / newline regex patterns for group mentions |
