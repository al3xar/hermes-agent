...
Connect Hades to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
All standard Hades commands work on Yuanbao:
...
4. Consider enabling verbose logging: `HADES_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Hades automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
HADES_LOG_LEVEL=debug hades gateway
```
...
```bash
hades chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
