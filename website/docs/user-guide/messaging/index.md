...
description: "Chat with Hades from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Hades](/guides/use-voice-mode-with-hades).
...
```mermaid
flowchart TB
    subgraph Gateway["Hades Gateway"]
...
```bash
hades gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
hades gateway              # Run in foreground
hades gateway setup        # Configure messaging platforms interactively
hades gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo hades gateway install --system   # Linux only: install a boot-time system service
hades gateway start        # Start the default service
hades gateway stop         # Stop the default service
hades gateway status       # Check default service status
hades gateway status --system         # Linux only: inspect the system service explicitly
```
...
hades pairing approve telegram XKGH5N7P

# Other pairing commands:
hades pairing list          # View pending + approved users
hades pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Hades appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
```bash
HADES_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
hades gateway install               # Install as user service
hades gateway start                 # Start the service
hades gateway stop                  # Stop the service
...
sudo hades gateway install --system
sudo hades gateway start --system
sudo hades gateway status --system
journalctl -u hades-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Hades will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Hades installations on the same machine (with different `HADES_HOME` directories), each gets its own systemd service name. The default `~/.hades` uses `hades-gateway`; other installations use `hades-gateway-<hash>`. The `hades gateway` commands automatically target the correct service for your current `HADES_HOME`.
:::

### macOS (launchd)

```bash
hades gateway install               # Install as launchd agent
hades gateway start                 # Start the service
hades gateway stop                  # Stop the service
hades gateway status                # Check status
...
The generated plist lives at `~/Library/LaunchAgents/ai.hades.gateway.plist`. It includes three environment variables:
...
- **HADES_HOME** — scopes the gateway to your Hades installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `hades gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HADES_HOME` directory gets its own launchd label. The default `~/.hades` uses `ai.hades.gateway`; other installations use `ai.hades.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `hades-cli` | Full access |
| Telegram | `hades-telegram` | Full tools including terminal |
| Discord | `hades-discord` | Full tools including terminal |
| WhatsApp | `hades-whatsapp` | Full tools including terminal |
| Slack | `hades-slack` | Full tools including terminal |
| Google Chat | `hades-google_chat` | Full tools including terminal |
| Signal | `hades-signal` | Full tools including terminal |
| SMS | `hades-sms` | Full tools including terminal |
| Email | `hades-email` | Full tools including terminal |
| Home Assistant | `hades-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `hades-mattermost` | Full tools including terminal |
| Matrix | `hades-matrix` | Full tools including terminal |
| DingTalk | `hades-dingtalk` | Full tools including terminal |
| Feishu/Lark | `hades-feishu` | Full tools including terminal |
| WeCom | `hades-wecom` | Full tools including terminal |
| WeCom Callback | `hades-wecom-callback` | Full tools including terminal |
| Weixin | `hades-weixin` | Full tools including terminal |
| BlueBubbles | `hades-bluebubbles` | Full tools including terminal |
| QQBot | `hades-qqbot` | Full tools including terminal |
| Yuanbao | `hades-yuanbao` | Full tools including terminal |
| Microsoft Teams | `hades-teams` | Full tools including terminal |
| API Server | `hades-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `hades-webhook` | Full tools including terminal |
