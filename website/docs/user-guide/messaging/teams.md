...
> Run `hades gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Hades strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create hades-bot --allow-anonymous
devtunnel port create hades-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host hades-bot
...
```bash
HADES_UID=$(id -u) HADES_GID=$(id -g) docker compose up -d gateway
```
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create hades-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs hades` for tracebacks |
