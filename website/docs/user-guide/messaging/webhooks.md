...
1. Enable via `hades gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `hades webhook subscribe`
...
Open a pull request on the repository. The webhook fires, Hades processes the event, and posts a review comment on the PR.
...
```bash
hades webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `hades webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
hades webhook remove github-issues
```

### Test a subscription

```bash
hades webhook test github-issues
hades webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `hades webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `hades gateway run`
