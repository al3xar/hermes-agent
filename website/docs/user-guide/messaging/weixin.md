...
Connect Hades to [WeChat](https://weixin.qq.com/) (微信), Tencent's personal messaging platform. The adapter uses Tencent's **iLink Bot API** for personal WeChat accounts — this is distinct from WeCom (Enterprise WeChat). Messages are delivered via long-polling, so no public endpoint or webhook is required.
...
QR login connects Hades to an **iLink bot identity** (e.g. `a5ace6fd482e@im.bot`), **not** a fully scriptable ordinary personal WeChat account. Consequences:
...
- The `WEIXIN_GROUP_POLICY` / `WEIXIN_GROUP_ALLOWED_USERS` settings below only take effect when iLink actually returns group events for your account type. If it doesn't, group messages will never reach Hades regardless of policy.
...
- Terminal QR rendering is included when Hades is installed with the `messaging` extra
...
- **QR code login** — scan-to-connect setup via `hades gateway setup`
...
Hades QR code with their own accounts; they must message the connected iLink
bot/contact through WeChat, and Hades will process the DM only if the sender's
Weixin user ID is present in `WEIXIN_ALLOWED_USERS`.

A practical setup flow is:

1. Pair Hades once with `hades gateway setup` and note the connected iLink bot
   account.
...
| `Weixin startup failed: WEIXIN_TOKEN is required` | Run `hades gateway setup` to complete QR login, or set `WEIXIN_TOKEN` manually |
| `Weixin startup failed: WEIXIN_ACCOUNT_ID is required` | Set `WEIXIN_ACCOUNT_ID` in your `.env` or run `hades gateway setup` |
| `Another local Hades gateway is already using this Weixin token` | Stop the other gateway instance first — only one poller per token is allowed |
| Session expired (`errcode=-14`) | Your login session has expired. Re-run `hades gateway setup` to scan a new QR code |
...
| Terminal QR code doesn't render | Reinstall with the messaging extra: `pip install hades-agent[messaging]`. Alternatively, open the URL printed above the QR |
