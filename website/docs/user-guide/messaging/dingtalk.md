...
Before setup, here's the part most people want to know: how Hades behaves once it's in your DingTalk workspace.

## How Hades Behaves

| Context | Behavior |
|---------|----------|
| **DMs (1:1 chat)** | Hades responds to every message. No `@mention` needed. Each DM has its own session. |
| **Group chats** | Hades responds when you `@mention` it. Without a mention, Hades ignores the message. |
| **Shared groups with multiple users** | By default, Hades isolates session history per user inside the group. Two people talking in the same group do not share one transcript unless you explicitly disable that. |
...
```bash
pip install "hades-agent[dingtalk]"
```
...
Hades can reply using DingTalk AI Cards instead of plain markdown messages. Cards provide a richer, more structured display and support streaming updates as the agent generates its response.
...
Hades automatically adds emoji reactions to your messages to show processing status:
...
**Fix**: Check that `hades gateway` is running. Look at the terminal output for error messages. Common issues: wrong credentials, app deactivated, `dingtalk-stream` or `httpx` not installed.
