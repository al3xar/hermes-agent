...
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Hades
...
> Run `hades gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Hades can read (e.g.,
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a hades restart)
...
- Principal: `hades-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Hades calls
...
- **App name**: whatever you want users to see ("Hades" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/hades-chat-events`.
...
`ADDED_TO_SPACE` event that Hades uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Hades
...
Install the dependencies the Google Chat adapter needs (no Hades extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Hades is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Hades detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Hades session.
...
3. Download the JSON. Move it onto the host that runs Hades.
...
hades -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Hades isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `hades gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Hades will only attach the SA bearer
