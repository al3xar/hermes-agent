...
Connect Hades to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Hades supports two WeCom integration modes:
...
> Run `hades gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Hades decrypts the message, queues it for the agent
...
(Use `hades gateway start` only after `hades gateway install` has registered the systemd/launchd service.)
...
fixing, restart `hades gateway run`.
...
Check `hades gateway run` logs for the bound host/port. If the adapter bound to
