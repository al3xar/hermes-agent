...
> Run `hades gateway setup` and pick **LINE** for a guided walk-through.
...
# devtunnel
devtunnel create hades-line --allow-anonymous
devtunnel port create hades-line -p 8646 --protocol https
devtunnel host hades-line
```
...
**"already in use by another profile".** The same channel access token is bound to another running Hades profile. Stop the other gateway or use a separate channel.
