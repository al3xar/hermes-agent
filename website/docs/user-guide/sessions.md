...
Hades stores session history so it can resume conversations, but it does not
...
Hades explicitly injects for that turn.
...
For example, if a user sends an image and asks Hades to make a meme from it,
Hades may inspect that image once with vision and run an image-processing
...
`hades sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`hades` or `hades chat`) |
...
hades -c "my project"   # → resumes "my project #3"
```
...
hades --resume 20250305_091523_a1b2c3d4
hades -r 20250305_091523_a1b2c3d4

# Resume by title
hades --resume "refactoring auth"

# Or with the chat subcommand
hades chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `hades sessions list`.

### Conversation Recap on Resume

When you resume a session, Hades displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Hades session." />
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `hades -r "<title>"` from the shell) and pick up where the platform left off.
...
Hades automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `hades sessions list` or `hades sessions browse`.
...
```bash
hades sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Hades creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`hades -c "my project"`), it automatically picks the most recent session in the lineage.
...
Hades provides a full set of session management commands via `hades sessions`:
...
# Filter by platform
hades sessions list --source telegram

# Show more sessions
hades sessions list --limit 50
```
...
hades sessions export backup.jsonl

# Export sessions from a specific platform
hades sessions export telegram-history.jsonl --source telegram

# Export a single session
hades sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
hades sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
hades sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
hades sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
hades sessions prune

# Custom age threshold
hades sessions prune --older-than 30

# Only prune sessions from a specific platform
hades sessions prune --source telegram --older-than 60

# Skip confirmation
hades sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`hades insights`](/reference/cli-commands#hades-insights).
...
When Hades cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Hades uses `group_sessions_per_user: true` in `config.yaml`. That means:
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Hades process in the same `HADES_HOME`
...
hades sessions delete <session_id>
