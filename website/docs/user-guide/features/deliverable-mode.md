...
persona in `~/.hades/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.hades/config.yaml` (switchable per session
via `/personality`).
...
If you use Hades' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.hades/config.yaml` under the `mcp_servers`
