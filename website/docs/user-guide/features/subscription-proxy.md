...
| Use case | "Use Hades as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Hades stores
the refresh token in `~/.hades/auth.json` — the same place all Hades
...
Model:      Hades-4-70B    # or Hades-4.3-36B, Hades-4-405B
```
...
interface in `hades_cli/proxy/adapters/`.
...
If you see `not logged in`, run `hades portal`. If you see
...
`hades_cli/proxy/adapters/<provider>.py` and registering it in
