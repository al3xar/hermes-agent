...
**Multi-agent profiles**: When multiple Hades instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
hades memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `hades` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Hades syncs them back at session init.
...
The `hades honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `hades memory setup honcho` (or run `hades memory setup` and pick it from the list); the `hades honcho` subcommand then appears on the next invocation.

```bash
hades memory setup honcho    # Configure Honcho directly (works before activation)
hades honcho status          # Connection status, config, and key settings
hades honcho setup           # Redirects to `hades memory setup` (post-activation alias)
hades honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
hades honcho peer            # Show or update peer names + dialectic reasoning level
hades honcho mode            # Show or set recall mode (hybrid/context/tools)
hades honcho tokens          # Show or set token budget for context and dialectic
hades honcho identity        # Seed or show the AI peer's Honcho identity
hades honcho sync            # Sync Honcho config to all existing profiles
hades honcho peers           # Show peer identities across all profiles
hades honcho sessions        # List known Honcho session mappings
hades honcho map             # Map current directory to a Honcho session name
hades honcho enable          # Enable Honcho for the active profile
hades honcho disable         # Disable Honcho for the active profile
hades honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `hades honcho`

If you previously used the standalone `hades honcho setup`:
...
No re-login or re-setup needed. Run `hades memory setup` and select "honcho" — the wizard detects your existing config.
