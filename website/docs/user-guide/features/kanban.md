...
description: "Durable SQLite-backed task board for coordinating multiple Hades profiles"
---
...
Hades Kanban is a durable task board, shared across all your Hades profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.hades/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
- **You (and scripts, and cron) drive the board through `hades kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/hades-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `HADES_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
- Workers spawned for a task see **only** their board's tasks — the
  dispatcher sets `HADES_KANBAN_BOARD` in the child env and every
...
hades kanban boards list

# Create a new board.
hades kanban boards create atm10-server \
    --name "ATM10 Server" \
...
hades kanban --board atm10-server list
hades kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
hades kanban boards switch atm10-server
hades kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
hades kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `HADES_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.hades/kanban/current` — the slug persisted by `hades kanban
   boards switch`.
4. `default`.
...
`hades dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<hades-home>/kanban/attachments/<task_id>/` for the default board, or
  `<hades-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `HADES_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
hades kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `hades kanban show t_abcd`.
...
Override the config flag at runtime via `HADES_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `hades gateway
...
until one comes up — `hades kanban create` warns about this at creation
time.

Running `hades kanban daemon` as a separate process is **deprecated**;
...
hades kanban create "nightly ops review" \
    --assignee ops \
...
```bash
hades kanban complete t_abc t_def t_hij --result "batch wrap"
hades kanban archive  t_abc t_def t_hij
hades kanban unblock  t_abc t_def
hades kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `hades kanban`.** When the dispatcher spawns a worker it sets `HADES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `hades kanban` CLI.
...
kanban_show()                                     # no args — uses HADES_KANBAN_TASK
...
### Why tools instead of shelling to `hades kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `hades kanban complete` *inside* the container, where `hades` isn't installed and `~/.hades/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.hades/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `hades chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HADES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/hades_cli/test_kanban_db.py -q"],
...
2. `cd $HADES_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
hades -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
hades -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
hades kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

hades kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
hades kanban create "Translate the docs site to French" \
...
```bash
hades -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
hades -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `hades kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `hades kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `hades profile create --description "..."`, `hades profile describe <name> --text "..."`, `hades profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `hades profile describe --auto`). |
...
If you run `hades dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.hades/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `hades -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard Hades dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
hades kanban init                                     # create kanban.db + print daemon hint
hades kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
hades kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
hades kanban show <id> [--json]
hades kanban assign <id> <profile>                    # or 'none' to unassign
hades kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
hades kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
hades kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
hades kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
hades kanban diagnostics [--json]                     # board health snapshot (alias: diag)
hades kanban link <parent_id> <child_id>
hades kanban unlink <parent_id> <child_id>
hades kanban claim <id> [--ttl SECONDS]
hades kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
hades kanban complete <id>... [--result "..."]
hades kanban block <id> "<reason>" [--ids <id>...]
hades kanban unblock <id>...
hades kanban archive <id>...

hades kanban tail <id>                                # follow a single task's event stream
hades kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
hades kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
hades kanban runs <id> [--json]                       # attempt history (one row per run)
hades kanban assignees [--json]                       # profiles on disk + per-assignee task counts
hades kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
hades kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `hades gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
hades kanban stats [--json]                           # per-status + per-assignee counts
hades kanban log <id> [--tail BYTES]                  # worker log from ~/.hades/kanban/logs/
hades kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
hades kanban notify-list [<id>] [--json]
hades kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
hades kanban context <id>                             # what a worker sees
hades kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
hades kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
hades kanban create "nightly backup audit" \
...
Every `hades kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `hades chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hades_cli.kanban.run_slash()` entry point that reuses the `hades kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `hades kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.hades/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`hades kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `hades kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/hades-kanban-v1-spec.pdf`.
...
```bash
hades kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$HADES_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
hades kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
hades kanban notify-list
hades kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
hades kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
hades kanban runs t_abcd
...
**Bulk close caveat.** `hades kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`hades kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `hades kanban heartbeat $TASK` to signal liveness during long operations. |
...
`hades kanban tail <id>` shows these for a single task. `hades kanban watch` streams them board-wide.
...
The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/hades-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
