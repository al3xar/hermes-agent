...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Hades automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Hades auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
hades auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
hades auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
hades auth add anthropic --type oauth
...
Run `hades auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `hades auth` | Interactive pool management wizard |
| `hades auth list` | Show all pools and credentials |
| `hades auth list <provider>` | Show a specific provider's pool |
| `hades auth add <provider>` | Add a credential (prompts for type and key) |
| `hades auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `hades auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `hades auth remove <provider> <index>` | Remove credential by 1-based index |
| `hades auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `hades auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `hades model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via hades model:
hades auth list
# Shows:
...
hades auth add Together.ai --api-key sk-together-second-key
```
...
Hades automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Hades PKCE OAuth | `~/.hades/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `hades auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `hades auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Hades can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Hades-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`hades_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`hades_cli/runtime_provider.py`** — Pool-aware credential resolution
