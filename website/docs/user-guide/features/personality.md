...
If you want to change who Hades is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Hades now seeds a default `SOUL.md` automatically in:

```text
~/.hades/SOUL.md
```

More precisely, it uses the current instance's `HADES_HOME`, so if you run Hades with a custom home directory, it will use:
...
- Hades creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Hades loads `SOUL.md` only from `HADES_HOME`
- Hades does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Hades falls back to a built-in default identity
...
If Hades loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HADES_HOME`, the personality belongs to the Hades instance itself.
...
- how Hades should handle uncertainty, disagreement, or ambiguity
...
## What Hades injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Hades falls back to a built-in default identity ("You are Hades Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Hades ships with built-in personalities you can switch to with `/personality`.
...
These are convenient overlays, but your global `SOUL.md` still gives Hades its persistent default personality unless the overlay meaningfully changes it.
...
- [SOUL.md Guide](/guides/use-soul-with-hades)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Hades speaks
- `display.skin` and `/skin` affect how Hades looks in the terminal
