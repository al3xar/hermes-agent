...
Schedule tasks to run automatically with natural language or cron expressions. Hades exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::tip
Cron jobs use whatever provider `hades model` selected. `hades setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Hades disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
hades cron create "every 2h" "Check server status"
hades cron create "every 1h" "Summarize new feed items" --skill blogwatcher
hades cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Hades will use the unified `cronjob` tool internally.
...
hades cron create "every 1d at 09:00" \
...
By default a cron job inherits whichever Hades profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `HADES_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:

```bash
# Pin a job to the `night-ops` profile regardless of where it was scheduled
hades cron create "every 1d at 03:00" \
...
Use `--profile default` to explicitly pin to the root Hades profile. The named profile must already exist; the scheduler refuses to create profiles on the fly. To clear a profile pin during `cron edit`, pass an empty string (`--profile ""` or `profile=""`) — the job reverts to running in whatever profile the scheduler itself is in.
...
:::note Serialization
Jobs with a `profile` set also run sequentially, for the same reason as `workdir`-pinned jobs: switching `HADES_HOME` is a process-global mutation, so two profile-pinned jobs running in parallel would race each other. Unpinned jobs still run in the normal parallel pool.
:::
...
```bash
hades cron edit <job_id> --schedule "every 4h"
hades cron edit <job_id> --prompt "Use the revised task"
hades cron edit <job_id> --skill blogwatcher --skill maps
hades cron edit <job_id> --add-skill maps
hades cron edit <job_id> --remove-skill blogwatcher
hades cron edit <job_id> --clear-skills
```
...
```bash
hades cron list
hades cron pause <job_id_or_name>
hades cron resume <job_id_or_name>
hades cron run <job_id_or_name>
hades cron remove <job_id_or_name>
hades cron edit <job_id_or_name> [...flags]
hades cron status
hades cron tick
```
...
```bash
hades gateway install     # Install as a user service
sudo hades gateway install --system   # Linux: boot-time system service for servers
hades gateway             # Or run in foreground
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.hades/cron/output/`), but no message is sent to the delivery target.
...
Or set the `HADES_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
- When Job 2 fires, Hades reads Job 1's most recent output from `~/.hades/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Hades skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `hades tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `hades tools` cron-platform config wins; otherwise Hades falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Hades should even invoke the agent. Emit a final stdout line of the form:
...
STATE="$HOME/.hades/scripts/.feed-changed.last"
...
Jobs may store `model` and `provider` as `null`. When those fields are omitted, Hades resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
