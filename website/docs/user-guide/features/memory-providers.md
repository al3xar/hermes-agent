...
```bash
hades memory setup      # interactive picker + configuration
hades memory status     # check what's active
hades memory off        # disable external provider
```

You can also select the active memory provider via `hades plugins` → Provider Plugins → Memory Provider.
...
When a memory provider is active, Hades automatically:
...
**Setup Wizard:**
```bash
hades memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `hades honcho setup` command still works (it now redirects to `hades memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$HADES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HADES_HOME/honcho.json` > `~/.hades/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/hades-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hades).
...
:::tip Migrating from `hades honcho`
If you previously used `hades honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Hades profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Hades profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Hades profile. Host key `hades` → default; `hades.<profile>` for others. |
...
```bash
hades profile create coder --clone
```

`--clone` creates a `hades.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Hades profile, creates host blocks for any profile without one, inherits settings from the default `hades` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/NousResearch/hades-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hades).
...
hades memory setup    # select "openviking"
# Or manually:
hades config set memory.provider openviking
...
**Setup:**
```bash
hades memory setup    # select "mem0"
# Or manually:
hades config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.hades/.env
```

**Config:** `$HADES_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `hades-user` | User identifier |
| `agent_id` | `hades` | Agent identifier |
...
**Setup:**
```bash
hades memory setup    # select "hindsight"
# Or manually:
hades config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.hades/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p hades ui start`
...
| `bank_id` | `hades` | Memory bank identifier |
...
See [plugin README](https://github.com/NousResearch/hades-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
hades memory setup    # select "holographic"
# Or manually:
hades config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.hades-memory-store`
...
**Setup:**
```bash
hades memory setup    # select "retaindb"
# Or manually:
hades config set memory.provider retaindb
...
# Then configure Hades
hades memory setup    # select "byterover"
# Or manually:
hades config set memory.provider byterover
```
...
# Or manually:
hades config set memory.provider supermemory
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `hades` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hades-{identity}` → `hades-coder`) to isolate memories per Hades profile
...
| **Requires** | `pip install hades-memori` + `hades-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install hades-memori
hades-memori install
hades config set memory.provider memori
hades memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `hades-memori` | Tool-aware memory + structured recall |
