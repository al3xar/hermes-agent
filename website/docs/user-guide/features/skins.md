...
|------|-------------|----------------|------------------|
| `default` | Classic Hades — gold and kawaii | `Hades Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `response_label` | Label on the response box header | ` ⚕ Hades ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default HADES_AGENT banner) | `""` |
...
## Hades Mod — Visual Skin Editor

[Hades Mod](https://github.com/cocktailpeanut/hades-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Hades Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/hades-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Hades skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
```bash
git clone https://github.com/cocktailpeanut/hades-mod.git
cd hades-mod/app
npm install
npm start
```
...
Hades Mod respects the `HADES_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `hades_cli/skin_engine.py`.
