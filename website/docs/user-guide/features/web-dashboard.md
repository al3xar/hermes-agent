...
:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `hades setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
hades dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
hades dashboard --host 0.0.0.0
...
```bash
pip install 'hades-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install hades-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `hades dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `hades dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `hades --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'hades-agent[web,pty]'`, or `[all]` covers both)
...
```bash
HADES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HADES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
HADES_DASHBOARD_BASIC_AUTH_SECRET=<32+ random bytes; openssl rand -base64 32>
```
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `hades config set` and the gateway read from.
:::
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `hades skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `hades mcp` reads from.
...
`hades mcp catalog` / `hades mcp install` use.
...
Connect Hades to any messaging platform from the browser — full parity with
`hades setup gateway`. The page lists every supported channel (Telegram,
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Hades versions, CPU core count + utilization, memory, disk usage of the Hades home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Hades version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `hades update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `hades portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `hades curator`.
...
| `GET /api/hades/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
|-------|-----------|----------|
| `hades dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `hades dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `hades dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
- **CLI — `hades dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `hades setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HADES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hades/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  hades dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes HADES_DASHBOARD_OAUTH_CLIENT_ID to ~/.hades/.env
  ```

- **GUI — the Local Dashboards page.** Open [`/local-dashboards`](https://portal.nousresearch.com/local-dashboards) in the Nous Portal to register, name, manage, and revoke self-hosted dashboards from the browser. Copy the resulting `agent:{id}` client ID into `HADES_DASHBOARD_OAUTH_CLIENT_ID` (env) or `dashboard.oauth.client_id` (config.yaml). This is also where you revoke a dashboard registered via the CLI.
...
|---------|-----------|--------|----------------|
| `HADES_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `hades dashboard register` |
...
Bundled providers reported these issues:
  • nous: HADES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
...
**1. Log in and register the dashboard.** `hades dashboard register` uses your existing Nous login to provision an OAuth client and writes `HADES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.hades/.env` for you:

```bash
hades setup            # if you're not already logged into Nous Portal
hades dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HADES_DASHBOARD_OAUTH_CLIENT_ID to ~/.hades/.env
```
...
```bash
hades dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://hades.example.com/auth/callback` and set `HADES_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
|---------|-----------|-------|
| `HADES_DASHBOARD_BASIC_AUTH_USERNAME` | `dashboard.basic_auth.username` | required to activate |
| `HADES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` | `dashboard.basic_auth.password_hash` | preferred (no plaintext at rest) |
| `HADES_DASHBOARD_BASIC_AUTH_PASSWORD` | `dashboard.basic_auth.password` | plaintext; **wins over a config `password_hash`** so you can rotate via env |
| `HADES_DASHBOARD_BASIC_AUTH_SECRET` | `dashboard.basic_auth.secret` | token-signing key |
| `HADES_DASHBOARD_BASIC_AUTH_TTL_SECONDS` | `dashboard.basic_auth.session_ttl_seconds` | access-token lifetime |
...
cat >> ~/.hades/.env <<EOF
HADES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HADES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH=$HASH
HADES_DASHBOARD_BASIC_AUTH_SECRET=$(openssl rand -base64 32)
EOF
chmod 600 ~/.hades/.env
```
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/hades/   # required
      client_id: hades-dashboard                              # required
...
| Env var | Overrides | Notes |
|---------|-----------|-------|
| `HADES_DASHBOARD_OIDC_ISSUER` | `dashboard.oauth.self_hosted.issuer` | OIDC issuer URL — required |
| `HADES_DASHBOARD_OIDC_CLIENT_ID` | `dashboard.oauth.self_hosted.client_id` | Public client id — required |
| `HADES_DASHBOARD_OIDC_SCOPES` | `dashboard.oauth.self_hosted.scopes` | Defaults to `openid profile email` |
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-hades.json` — it defines a `hades` realm, a **public PKCE client** (`hades-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-hades.json:/opt/keycloak/data/import/realm-hades.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/hades/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/hades`). The admin console is at
...
```bash
export HADES_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/hades"
export HADES_DASHBOARD_OIDC_CLIENT_ID="hades-dashboard"
export HADES_DASHBOARD_PUBLIC_URL="http://localhost:9119"
hades dashboard --host 0.0.0.0 --port 9119 --no-open
```

`HADES_DASHBOARD_PUBLIC_URL` tells the dashboard its OAuth callback is
...
> admin console (Clients → hades-dashboard → Settings). The same pattern works
...
For deploys behind reverse proxies that don't reliably forward those headers (manual nginx setups, on-prem ingresses, custom-domain deploys with partial proxy chains), set `dashboard.public_url` (or `HADES_DASHBOARD_PUBLIC_URL`) to the **complete public URL** the dashboard is reached at:

```yaml
dashboard:
  public_url: "https://dashboard.example.com/hades"
```
...
|---------|---------------|-------------|
| `dashboard.public_url` in `config.yaml` | `HADES_DASHBOARD_PUBLIC_URL` | Local dev / on-prem (canonical) |
| `HADES_DASHBOARD_PUBLIC_URL` env var | — | Hosting-platform secrets / CI |
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `hades_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `hades_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `hades_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `hades_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
```bash
# Quick env-var path.
HADES_DASHBOARD_OAUTH_CLIENT_ID=agent:test \
  hades dashboard --host 0.0.0.0
...
Hades Desktop can drive a Hades backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Hades Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`hades dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
hades dashboard --no-open --host 0.0.0.0 --port 9119
```

Prefer no plaintext at rest? Use `HADES_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` with a scrypt hash instead — see [Username/password provider](#usernamepassword-provider-no-oauth-idp) for the full surface.

If you run the dashboard as a systemd service, `~/.hades/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.hades/.env`, so the credentials are in the environment at boot.
...
- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/hades` are supported if you front it with a reverse proxy)
...
The session refreshes automatically and survives restarts when `HADES_DASHBOARD_BASIC_AUTH_SECRET` is set on the backend.

### Environment-variable override

Instead of the in-app setting, you can point the desktop at a backend with an env var before launching it. When `HADES_DESKTOP_REMOTE_URL` is set, it overrides the saved in-app URL (the Gateway settings panel shows an "env override" badge and disables editing); you still **Sign in** with your username and password from the panel.

| Env var | Value |
|---------|-------|
| `HADES_DESKTOP_REMOTE_URL` | `http://<backend-host>:9119` |

### Troubleshooting

- **"Remote gateway incomplete"** — you haven't entered a remote URL.
- **Sign-in fails with 401 / "Invalid credentials"** — the username or password doesn't match the backend's `HADES_DASHBOARD_BASIC_AUTH_USERNAME` / `HADES_DASHBOARD_BASIC_AUTH_PASSWORD`. The backend returns the same generic error for unknown user and wrong password, so check both. Confirm the gate with `curl -s http://<host>:9119/api/status | jq '.auth_required, .auth_providers'` — it should report `true` and include `"basic"`.
...
- **Signed out on every restart** — set `HADES_DASHBOARD_BASIC_AUTH_SECRET` to a stable value; otherwise the signing key is regenerated per boot.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `hades_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `hades update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `hades dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Hades Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Hades Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
