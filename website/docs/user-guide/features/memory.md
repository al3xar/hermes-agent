...
```bash
hades sessions list    # Browse past sessions
```
...
For deeper, persistent memory that goes beyond MEMORY.md and USER.md, Hades ships with 8 external memory provider plugins — including Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, and Supermemory.

External providers run **alongside** built-in memory (never replacing it) and add capabilities like knowledge graphs, semantic search, automatic fact extraction, and cross-session user modeling.

```bash
hades memory setup      # pick a provider and configure it
