...
If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Hades](/guides/use-voice-mode-with-hades).

## Prerequisites

Before using voice features, make sure you have:

1. **Hades Agent installed** — `pip install hades-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `hades model` or set your preferred provider credentials in `~/.hades/.env`
3. **A working base setup** — run `hades` to verify the agent responds to text before enabling voice

:::tip
The `~/.hades/` directory and default `config.yaml` are created automatically the first time you run `hades`. You only need to create `~/.hades/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `hades setup --portal` wires both up at once.
:::
...
pip install "hades-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "hades-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "hades-agent[tts-premium]"
...
Voice mode is available in both the **classic CLI** (`hades chat`) and the **TUI** (`hades --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.hades/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
hades                # Start the interactive CLI
```
...
1. Start the CLI with `hades` and enable voice mode with `/voice on`
...
```bash
hades gateway        # Start the gateway (connects to configured platforms)
hades gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@hadesbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.
...
```bash
hades gateway        # Start with existing configuration
```
...
If you are running Hades inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
