...
You can also point Hades at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `hades update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.hades` profile):

```bash
curl -fsSL https://hades-agent.nousresearch.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
hades profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
hades skills opt-out            # stop future seeding — nothing on disk is touched
hades skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
hades skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `hades update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `hades skills opt-in`) to re-enable.

:::note Safe by default
`hades skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Hades installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Hades to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hades/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
hades chat --toolsets skills -q "What skills do you have?"
hades chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.hades/cache/chart-q4-2025.png
...
When a missing value is encountered, Hades asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `hades setup` or `~/.hades/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `hades config migrate` prompts for unconfigured settings, and `hades config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
If you maintain skills outside of Hades — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Hades to scan those directories too.
...
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Hades ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
- `description` (optional) — short text shown in `/bundles` and `hades bundles list`.
...
hades bundles create research

# Overwrite an existing bundle
hades bundles create backend-dev --skill ... --force

# Delete a bundle
hades bundles delete backend-dev

# Re-scan ~/.hades/skill-bundles/ and report changes
hades bundles reload
```
...
```bash
hades skills browse                              # Browse all hub skills (official first)
hades skills browse --source official            # Browse only official optional skills
hades skills search kubernetes                   # Search all sources
hades skills search react --source skills-sh     # Search the skills.sh directory
hades skills search https://mintlify.com/docs --source well-known
hades skills inspect openai/skills/k8s           # Preview before installing
hades skills install openai/skills/k8s           # Install with security scan
hades skills install official/security/1password
hades skills install skills-sh/vercel-labs/json-render/json-render-react --force
hades skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
hades skills list --source hub                   # List hub-installed skills
hades skills check                               # Check installed hub skills for upstream updates
hades skills update                              # Reinstall hub skills with upstream changes when needed
hades skills audit                               # Re-scan all hub skills for security
hades skills uninstall k8s                       # Remove a hub skill
hades skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
hades skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
hades skills publish skills/my-skill --to github --repo owner/repo
hades skills snapshot export setup.json          # Export skill config
hades skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `hades skills search <query> --source skills-sh`. Hades resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Hades currently integrates with these skills ecosystems and discovery sources:
...
This is Vercel's public skills directory. Hades can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
hades skills search https://mintlify.com/docs --source well-known
hades skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
hades skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Hades can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
hades skills install openai/skills/k8s
hades skills tap add myorg/skills-repo
```
...
[Skills Hub](https://hades-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Hades-side changes required.
...
- Hades source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Hades supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Hades source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Hades can search and convert agent entries from LobeHub's public catalog into installable Hades skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Hades source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Hades integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Hades' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Hades source id: `browse-sh`
- Trust level: `community`

```bash
hades skills search airbnb --source browse-sh
hades skills inspect browse-sh/airbnb.com/search-listings-ddgioa
hades skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Hades fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
hades skills install https://sharethis.chat/SKILL.md
hades skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
hades skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `hades skills update` re-fetches from the same URL automatically when you want to refresh.
...
`hades skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
```bash
hades skills check          # Report which installed hub skills changed upstream
hades skills update         # Reinstall only the skills with updates available
hades skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Hades users add with `hades skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.hades.tags`, `version`, `author`, `platforms`, `metadata.hades.config`).
...
Hades discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Hades user can subscribe and install:

```bash
hades skills tap add my-org/hades-skills
hades skills search deploy
hades skills install my-org/hades-skills/deploy-runbook
```
...
The `hades skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `hades skills tap list` shows the effective path per tap.
...
```bash
hades skills install owner/repo/skills/my-workflow
```
...
## Bundled skill updates (`hades skills reset`)

Hades ships with a set of bundled skills in `skills/` inside the repo. On install and on every `hades update`, a sync pass copies those into `~/.hades/skills/` and records a manifest at `~/.hades/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Hades recomputes the hash of your local copy and compares it to the origin hash:
...
hades skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HADES_HOME`, so `hades -p coder skills reset <name>` only affects that profile.
:::
