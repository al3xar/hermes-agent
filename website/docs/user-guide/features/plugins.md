...
title: "Plugins"
description: "Extend Hades with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Hades has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
**→ [Build a Hades Plugin](/guides/build-a-hades-plugin)** — step-by-step guide with a complete working example.
...
Start Hades — your tools appear alongside built-in tools. The model can call them immediately.
...
```python
"""Minimal Hades plugin — registers a tool and a hook."""
...
Drop both files into `~/.hades/plugins/hello-world/`, restart Hades, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.hades/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HADES_ENABLE_PROJECT_PLUGINS=true` before starting Hades.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `hades <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `hades plugins install` |
...
| Project | `.hades/plugins/` | Project-specific plugins (requires `HADES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `hades_agent.plugins` entry_points | Distributed packages |
| Nix | `services.hades-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Hades also recognizes sub-category directories that route plugins to specialized discovery systems:
...
**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `hades plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.hades/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
hades plugins                    # interactive toggle (space to check/uncheck)
hades plugins enable <name>      # add to allow-list
hades plugins disable <name>     # remove from allow-list + add to disabled
```

After `hades plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.
...
Hades has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Hades also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Hades Plugin](/guides/build-a-hades-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Hades Plugin](/guides/build-a-hades-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Hades Plugin](/guides/build-a-hades-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `hades <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `HADES_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Hades auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `hades skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
On NixOS, plugins can be installed declaratively via the module options — no `hades plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
hades plugins                               # unified interactive UI
hades plugins list                          # table: enabled / disabled / not enabled
hades plugins install user/repo             # install from Git, then prompt Enable? [y/N]
hades plugins install user/repo --enable    # install AND enable (no prompt)
hades plugins install user/repo --no-enable # install but leave disabled (no prompt)
hades plugins update my-plugin              # pull latest
hades plugins remove my-plugin              # uninstall
hades plugins enable my-plugin              # add to allow-list
hades plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `hades plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `hades plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/guides/build-a-hades-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
