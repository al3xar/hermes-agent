...
description: "Connect Hades Agent to external tool servers via MCP — and control exactly which MCP tools Hades loads"
---

# MCP (Model Context Protocol)

MCP lets Hades Agent connect to external tool servers so the agent can use tools that live outside Hades itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Hades to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Hades tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Hades to see
...
4. Ask Hades to use the MCP-backed capability.
...
Hades will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Hades ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
hades mcp                # interactive picker (default)
hades mcp catalog        # plain-text list, scriptable
hades mcp install n8n    # install a catalog entry by name
```
...
`optional-mcps/` in the hades-agent repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Hades prompts at install time and writes the value to
...
- **OAuth** (third-party provider like Google/GitHub) — Hades points you at
  `hades auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Hades probes the MCP server to list every
...
written (if not). Re-run `hades mcp configure <name>` once the server is
reachable to refine.
...
the hades-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/NousResearch/hades-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Hades
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Hades`) for that entry instead of silently hiding it. Run `hades update`
to install the latest Hades when you see that.
...
MCPs are never auto-updated. Re-run `hades mcp install <name>` to refresh
after a Hades update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/NousResearch/hades-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints Hades connects to directly.
...
- you do not want Hades spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Hades handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, Hades prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.hades/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Hades runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal Hades prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.

See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `hades mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `hades mcp login` can list the tools and look like it worked, but every real tool call later times out. `hades mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `hades mcp login googledrive` — with the pre-registered client, Hades skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.hades/config.yaml` from inside a running Hades session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `hades mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Hades passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
Hades reads MCP config from `~/.hades/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Hades skips the server entirely |
...
For well-known MCP servers, `hades mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
hades mcp add codex --preset codex
```
...
You can pick any local name (`hades mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Hades registers MCP tools

Hades prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Hades sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Hades also registers utility tools around MCP resources and prompts:
...
- Hades only registers resource utilities if the MCP session actually supports resource operations
- Hades only registers prompt utilities if the MCP session actually supports prompt operations
...
If `enabled: false`, Hades skips the server completely and does not even attempt a connection.
...
You can also separately disable Hades-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Hades does not create an empty runtime MCP toolset for that server.
...
Hades discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Hades when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Hades receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Hades does not blindly pass your full shell environment.
...
Then verify your config and restart Hades.
...
Because Hades now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Hades may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Hades via the `sampling/createMessage` protocol. This allows an MCP server to ask Hades to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Hades as an MCP server

In addition to connecting **to** MCP servers, Hades can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Hades's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Hades
- You want a single MCP server that bridges to all of Hades's connected messaging platforms at once
- You already have a running Hades gateway with connected platforms
...
Add Hades to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Hades in a specific location:

```json
{
  "mcpServers": {
    "hades": {
      "command": "/home/user/.hades/hades-agent/venv/bin/hades",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Hades-specific channel browser:
...
- The embedded `hades mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Hades, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
