...
ACP is a good fit when you want Hades to behave like an editor-native coding agent instead of a standalone CLI or messaging bot.

## What Hades exposes in ACP mode

Hades runs with a curated `hades-acp` toolset designed for editor workflows. It includes:
...
Install Hades normally, then add the ACP extra:
...
For Zed registry installs, Zed launches Hades through the official ACP Registry entry. That entry uses a `uvx` distribution that runs:
...
Any of the following starts Hades in ACP mode:
...
Hades logs to stderr so stdout remains reserved for ACP JSON-RPC traffic.
...
```bash
hades acp --setup-browser           # interactive (prompts before ~400 MB download)
hades acp --setup-browser --yes     # accept the download non-interactively
```

This is the standalone command. The Zed registry's terminal-auth flow (`hades acp --setup`) also offers the browser bootstrap as a follow-up question after model selection, so most users never need to run `--setup-browser` directly.
...
If you want to define Hades manually, add it through VS Code settings under `acp.agents`:
...
4. Install it and start a new Hades external-agent thread.

Prerequisites:

- Configure Hades provider credentials first with `hades model`, or set them in `~/.hades/.env` / `~/.hades/config.yaml`.
...
```text
/path/to/hades-agent/acp_registry
```
...
ACP mode uses the same Hades configuration as the CLI:
...
Provider resolution uses Hades' normal runtime resolver, so ACP inherits the currently configured provider and credentials. Hades also advertises a terminal auth method (`--setup`) for first-run registry clients; this opens Hades' interactive model/provider setup.
...
- For manual/local development, verify the custom `agent_servers` command points to `hades acp`.
- Hades is installed and on your PATH.
