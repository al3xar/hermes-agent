...
Hades Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
```
hades computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `hades computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `hades tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
window-focus bug for UTM workflows). Hades refreshes the binary in two
places so you don't get stuck on a stale release:

- **`hades update`** — when you update Hades itself, if `cua-driver` is
...
- **`hades computer-use install --upgrade`** — manual force-refresh.
...
`hades computer-use status` shows the installed version next to the
binary path.
...
Hades applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.hades/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Hades applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `HADES_CUA_DRIVER_VERSION`
...
```
HADES_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
HADES_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
HADES_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`hades computer-use install` to fetch the cua-driver binary, or run
`hades tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/hades-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
