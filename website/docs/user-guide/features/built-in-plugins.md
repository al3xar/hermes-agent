...
Hades ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.hades/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Hades Plugin](/guides/build-a-hades-plugin) to write your own.
...
3. **Project** — `./.hades/plugins/<name>/` (requires `HADES_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `hades memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `hades plugins list` and the interactive `hades plugins` UI), but none load until you explicitly enable them:

```bash
hades plugins enable disk-cleanup
```
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Hades. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
hades plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `hades plugins enable <name>`.
...
| `hades-achievements` | dashboard tab | Steam-style collectible badges generated from your real Hades session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `hades memory` and `hades plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HADES_HOME` or `/tmp/hades-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `HADES_HOME` or `/tmp/hades-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `hades plugins enable disk-cleanup` (or check the box in `hades plugins`).

**Disabling again:** `hades plugins disable disk-cleanup`.
...
**Enabling:** `hades plugins enable security-guidance` (or check the box in `hades plugins`).

**Disabling again:** `hades plugins disable security-guidance`.
...
```bash
hades tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Hades and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
hades plugins enable observability/langfuse
```

Then put the credentials in `~/.hades/.env`:

```bash
HADES_LANGFUSE_PUBLIC_KEY=pk-lf-...
HADES_LANGFUSE_SECRET_KEY=sk-lf-...
HADES_LANGFUSE_BASE_URL=https://cloud.langfuse.com   # or your self-hosted URL
```
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Hades turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `HADES_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Hades session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `hades chat` session lives under one Langfuse session.

**Verify:**

```bash
hades plugins list                 # observability/langfuse should show "enabled"
hades chat -q "hello"              # check the Langfuse UI for a "Hades turn" trace
```
...
| `HADES_LANGFUSE_RELEASE` | — | Release/version tag |
| `HADES_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `HADES_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `HADES_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Hades-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Hades-prefixed wins when both are set.
...
**Disabling:** `hades plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
```bash
hades plugins enable google_meet
...
**Disabling:** `hades plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.hades/cache/google_meet/` until you remove them.

### hades-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Hades session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Hades feature changes.
...
- Unlock state is persisted to `$HADES_HOME/plugins/hades-achievements/state.json`
...
| Secret | Hidden until Hades detects the first related signal in your history |

**API** — routes mount under `/api/plugins/hades-achievements/`:
...
**State files** — live under `$HADES_HOME/plugins/hades-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Hades updates. |
...
**Enabling:** Nothing to enable — `hades-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `hades dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/hades-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.hades/plugins/hades-achievements/` that ships no dashboard. The plugin's state files under `$HADES_HOME/plugins/hades-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Hades plugin — see [Build a Hades Plugin](/guides/build-a-hades-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.hades/plugins/<name>/`
- Manifest source is reported as `bundled` in `hades plugins list`
