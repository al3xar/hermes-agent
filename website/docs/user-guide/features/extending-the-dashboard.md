...
The Hades web dashboard (`hades dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:
...
The CSS is injected as a single scoped `<style data-hades-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **Hades Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Hades Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Hades Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Hades plugins in `~/.hades/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__HADES_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.
...
(function () {
  ...
  window.__HADES_PLUGINS__.register("my-plugin", MyPage);
})();
```
...
Everything a plugin needs is on `window.__HADES_PLUGIN_SDK__`. Plugins should never import React directly.
...
```javascript
window.__HADES_PLUGINS__.registerSlot("my-plugin", "sidebar", MySidebar);
window.__HADES_PLUGINS__.registerSlot("my-plugin", "header-left", MyCrest);
```
...
window.__HADES_PLUGINS__.registerSlot("my-plugin", "sessions:top", PinnedSessionsBanner);
```
...
(function () {
  ...
  window.__HADES_PLUGINS__.register("session-notes", function () { return null; });

  // The real work.
  window.__HADES_PLUGINS__.registerSlot("session-notes", "sessions:top", Banner);
})();
```
...
A reference plugin (`example-dashboard` in [`hades-example-plugins`](https://github.com/NousResearch/hades-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
#### Accessing Hades internals

Backend routes run inside the dashboard process, so they can import from the hades-agent codebase directly:
...
| 3 | `./.hades/plugins/<name>/dashboard/` | `project` — only when `HADES_ENABLE_PROJECT_PLUGINS` is set |
...
…or restart `hades dashboard`.

#### Plugin load lifecycle

1. Dashboard loads. `main.tsx` exposes the SDK on `window.__HADES_PLUGIN_SDK__` and the registry on `window.__HADES_PLUGINS__`.
...
4. The plugin's IIFE runs and calls `window.__HADES_PLUGINS__.register(name, Component)` — and optionally `.registerSlot(name, slot, Component)` for each slot.
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/hades-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `hades-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NousResearch/hades-example-plugins.git

# Theme
cp hades-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.hades/dashboard-themes/

# Plugin
cp -r hades-example-plugins/strike-freedom-cockpit ~/.hades/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Hades Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
|--------|------|----------|
| `window.__HADES_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
| `window.__HADES_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__HADES_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
4. Open browser dev tools → Console — look for errors during the IIFE or `window.__HADES_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
5. Verify your bundle calls `window.__HADES_PLUGINS__.register(...)` with the **same name** as `manifest.json:name`.
...
2. Restart `hades dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
