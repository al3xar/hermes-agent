...
Hades runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Hades
...
1. Hades captures a baseline of current diagnostics for the file.
...
…). Hades auto-detects the binary on PATH or in
`<HADES_HOME>/lsp/bin/`.
...
`node_modules` tree — Hades installs both packages together when you
run `hades lsp install typescript` or auto-install fires on first
use.

## CLI

```
hades lsp status          # service state + per-server install status
hades lsp list            # registry, optionally --installed-only
hades lsp install <id>    # eagerly install one server
hades lsp install-all     # try every server with a known recipe
hades lsp restart         # tear down running clients
hades lsp which <id>      # print resolved binary path
```

`hades lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, Hades installs binaries into
...
shared location — the staging dir is fully Hades-owned and is
...
**`hades lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HADES_HOME>/lsp/bin/`. Run
`hades lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `hades lsp status`**
...
When `hades lsp status` shows a `Backend warnings` section, install
...
the rest of the session. Run `hades lsp restart` to clear the set;
