...
Hades has three hook systems that run custom code at key lifecycle points:
...
WEBHOOK_URL = "https://your-service.example.com/hades-events"
...
This tutorial shows how to build it yourself as a user-defined hook. Hades does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.
...
1. Run `hades cron list` and check if any scheduled jobs failed overnight.
...
```bash
hades logs --follow --level INFO | grep boot-md
```
...
See the **[Build a Plugin guide](/guides/build-a-hades-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
...
Declare shell-script hooks in your `cli-config.yaml` and Hades will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`hades_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.
...
Each time the event fires, Hades spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Hades-canonical
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Hades event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
Each unique `(event, command)` pair prompts the user for approval the first time Hades sees it, then persists the decision to `~/.hades/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `hades --accept-hooks chat`)
2. `HADES_ACCEPT_HOOKS=1` environment variable
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `hades hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

### The `hades hooks` CLI

| Command | What it does |
|---------|--------------|
| `hades hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `hades hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `hades hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `hades hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Re-run `hades hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
