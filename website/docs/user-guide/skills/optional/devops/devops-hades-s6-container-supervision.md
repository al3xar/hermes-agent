...
| Related skills | [`hades-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hades-agent), `hades-agent-dev` |
...
│   ├── 01-hades-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
...
└── CMD ("main program")               ← /opt/hades/docker/main-wrapper.sh
    └── routes user args: bare exec | hades subcommand | hades (no args)
...
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-hades-setup. |
...
| `hades_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `hades gateway start/stop/restart` and routes to s6 when running in a container. |
...
Trade-off: main hades is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to hades via `s6-setuidgid hades` at the top of run (unless you specifically need root).
...
Edit `S6ServiceManager._render_run_script` in `hades_cli/service_manager.py`. The function is also called by `hades_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/hades_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `hades -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `hades profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
