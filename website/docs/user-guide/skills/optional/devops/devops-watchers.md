...
| | |
|---|---|
| Source | Optional — install with `hades skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.hades/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $HADES_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name hades-issues --repo NousResearch/hades-agent --scope issues
```
