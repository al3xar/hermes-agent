...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. hades-agent@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hades skills install official/email/agentmail` |
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `hades-agent`)
   - The agent gets address: `hades-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "hades-outreach")
...
```
hades --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
