...
| | |
|---|---|
| Source | Optional — install with `hades skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Hades core capability.
...
Prefer Hades native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Hades native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Hades usage patterns
...
- Do not silently choose Parallel when Hades native tools are already sufficient.
