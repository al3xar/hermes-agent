...
| | |
|---|---|
| Source | Optional — install with `hades skills install official/security/1password` |
...
- `tmux` available for stable authenticated sessions during Hades terminal calls (desktop app flow only)
...
Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.hades/.env` (the skill will prompt for this on first load).
...
## Hades Execution Pattern (desktop app flow)

Hades terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/hades-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hades-op.sock"
