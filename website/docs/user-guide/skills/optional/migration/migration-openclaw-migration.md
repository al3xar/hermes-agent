...
Migrate a user's OpenClaw customization footprint into Hades Agent. Imports Hades-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hades skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Hades Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Hades`, `Memory`, `Persona`, `Import` |
| Related skills | [`hades-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hades-agent) |
...
```bash
hades claw migrate              # Full interactive migration
...
hades claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `hades setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hades.py` to:
...
- migrate Hades-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Hades working-directory configuration
- copy OpenClaw skills into `~/.hades/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hades workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hades/tts/`
- archive non-secret docs that do not have a direct Hades destination
...
- `scripts/openclaw_to_hades.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hades/skills/migration/openclaw-migration/scripts/openclaw_to_hades.py`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Hades workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Hades skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.hades/skills/migration/openclaw-migration/scripts/openclaw_to_hades.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hades/skills/migration/openclaw-migration/scripts/openclaw_to_hades.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.hades/skills/migration/openclaw-migration/scripts/openclaw_to_hades.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.hades/skills/migration/openclaw-migration/scripts/openclaw_to_hades.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.hades/skills/migration/openclaw-migration/scripts/openclaw_to_hades.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.hades/skills/migration/openclaw-migration/scripts/openclaw_to_hades.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
3. Do not silently overwrite non-empty Hades targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Hades destination. Unsupported auth blobs must still be reported as skipped.
...
- Hades persona state imported
- Hades memory files populated with converted OpenClaw knowledge
