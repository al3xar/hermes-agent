...
| | |
|---|---|
| Source | Optional — install with `hades skills install official/blockchain/hyperliquid` |
...
The script reads `~/.hades/.env` for two optional defaults:
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `~/.hades/.env`.
