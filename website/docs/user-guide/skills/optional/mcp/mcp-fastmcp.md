...
| | |
|---|---|
| Source | Optional — install with `hades skills install official/mcp/fastmcp` |
...
When the goal is Hades integration, either:
...
### Hades cannot see the deployed server

The server-building part may be correct while the Hades config is not. Load the `native-mcp` skill and configure the server in `~/.hades/config.yaml`, then restart Hades.
