...
| | |
|---|---|
| Source | Optional — install with `hades skills install official/autonomous-ai-agents/honcho` |
...
Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Hades profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hades#running-honcho-locally-with-hades

### Verify

```bash
hades honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Hades profile's AI peer
...
Honcho models conversations as interactions between **peers**. Hades creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Hades instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Hades run |
| `global` | Single session across all directories |

Manual override: `hades honcho map my-project-name`
...
Each Hades profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block hades.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `hades.coder` host block in `honcho.json`
...
```bash
hades honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Run `hades honcho setup`. Ensure `memory.provider: honcho` is in `~/.hades/config.yaml`.

### Memory not persisting across sessions
Check `hades honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `hades profile create <name> --clone`. For existing profiles: `hades honcho sync`.
...
| Command | Description |
|---------|-------------|
| `hades honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `hades honcho status` | Show resolved config, connection test, peer info for active profile |
| `hades honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `hades honcho disable` | Disable Honcho for the active profile |
| `hades honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `hades honcho peers` | Show peer identities across all profiles |
| `hades honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `hades honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `hades honcho sessions` | List known directory-to-session-name mappings |
| `hades honcho map <name>` | Map current working directory to a Honcho session name |
| `hades honcho identity` | Seed AI peer identity or show both peer representations |
| `hades honcho sync` | Create host blocks for all Hades profiles that don't have one yet |
| `hades honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Hades + Honcho |
| `hades memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `hades memory status` | Show active memory provider and config |
| `hades memory off` | Disable external memory provider |
