...
| | |
|---|---|
| Source | Optional — install with `hades skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${HADES_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HADES_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `hades`)    |
