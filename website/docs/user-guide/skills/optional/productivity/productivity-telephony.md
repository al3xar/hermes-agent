---
title: "Telephony — Give Hades phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Hades phone capabilities without core tool changes"
---
...
Give Hades phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `hades skills install official/productivity/telephony` |
...
This optional skill gives Hades practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.hades/.env`
...
It does **not** turn Hades into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
### 1) "I want Hades to own a real phone number"
Use **Twilio**.
...
```bash
hades skills search telephony
hades skills install official/productivity/telephony
```
...
3. Buy it and save it into `~/.hades/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Hades calling with your status update." --voice Polly.Joanna
```
...
Recommended Hades TTS -> Twilio Play workflow:
...
Example agent flow:
- Ask Hades to create the message audio with `text_to_speech`
...
Important note:
- Hades TTS is great for prerecorded outbound messages
...
- Hades STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
3. persist that number to `~/.hades/.env`
