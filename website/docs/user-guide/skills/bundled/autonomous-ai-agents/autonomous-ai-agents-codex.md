...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`hades-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hades-agent) |
...
For Hades itself, `model.provider: openai-codex` uses Hades-managed Codex
OAuth from `~/.hades/auth.json` after `hades auth add openai-codex`. For the
...
## Hades Gateway Caveat

When invoking the Codex CLI from a Hades gateway/service context (for example,
