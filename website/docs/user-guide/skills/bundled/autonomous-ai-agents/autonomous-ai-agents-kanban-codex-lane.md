...
description: "Use when a Hades Kanban worker wants to run Codex CLI as an isolated implementation lane while Hades keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Hades Kanban worker wants to run Codex CLI as an isolated implementation lane while Hades keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker), [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`hades-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hades-agent) |
...
- Hades can run the relevant tests itself after Codex exits.
...
1. Hades owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Hades board CLI as a substitute for the worker.
2. Hades owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Hades owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Hades with the repo's canonical wrapper.
4. Hades owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Hades owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${HADES_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Hades edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Hades can do the task directly.
...
Hades owns the Kanban lifecycle; do not call Hades kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Hades reconciliation.
...
- Explicit statement: Hades owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Hades will run afterward.
...
Hades must perform this checklist before accepting any Codex lane result:
...
- [ ] Hades ran the canonical tests itself, using `scripts/run_tests.sh` for Hades Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Hades-run tests.
- [ ] Accepted commits/diffs were applied to the Hades-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "hades"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Hades edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
3. Letting Codex own Kanban. Codex may summarize progress, but Hades writes board state.
...
- [ ] Hades reviewed `git diff` and safety-sensitive files.
- [ ] Hades ran canonical tests independently.
