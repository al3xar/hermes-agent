...
hades auth                 Interactive credential manager
hades auth add PROVIDER    Add OAuth or API-key credential (e.g. nous, openai-codex, qwen-oauth)
hades auth list            List stored credentials
hades auth remove PROVIDER Remove a stored credential
...
(`hades mcp install <name>`): `skill_view(name="hades-agent", file_path="references/native-mcp.md")`.
...
```bash
hades config set privacy.redact_pii true    # enable
hades config set privacy.redact_pii false   # disable (default)
```
...
```bash
hades config set approvals.mode smart       # recommended middle ground
hades config set approvals.mode off         # bypass everything (not recommended)
```
...
To keep the model away from network or media tools entirely, open `hades tools` and toggle per-platform. Takes effect on next session (`/reset`). See the Tools & Skills section above.
...
- **Use `hades chat -q` for fire-and-forget** — no PTY needed
...
the `cronjob` tool, the `hades cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `hades curator <verb>` — `status`, `run`, `pause`, `resume`,
...
without BOM. `hades config edit` writes without BOM; manual edits in
...
Config: `~/.hades/config.yaml` (settings), `~/.hades/.env` (API keys).
...
All handlers must return JSON strings. Use `get_hades_home()` for paths, never hardcode `~/.hades`.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `hades_cli/commands.py`
...
- Use `get_hades_home()` from `hades_constants` for all paths (profile-safe)
