...
> **Docker HOME pitfall:** In the official Hades Docker layout, `/opt/data` is `HADES_HOME`, but Hades tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Hades-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Hades tool calls will not see the credentials.
...
- Hades adaptation: reformatted for Hades skill conventions; safety guardrails preserved verbatim.
