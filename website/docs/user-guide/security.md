...
Before executing any command, Hades checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
| `timeout` | `60` | Seconds Hades waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `hades --yolo` or `hades chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `HADES_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `HADES_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Hades shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Hades refuses to run them **regardless** of:
...
| `pkill`/`killall` hades/gateway | Self-termination prevention |
...
The `HADES_EXEC_ASK=1` environment variable is automatically set when running the gateway.
...
:::tip
Use `hades config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Hades controls who can interact with the bot through a layered authorization system.
...
For more flexible authorization, Hades includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `hades pairing approve <platform> <code>` on the CLI
...
hades pairing approve telegram ABC12DEF

# Revoke a user's access
hades pairing revoke telegram 123456789

# Clear all pending codes
hades pairing clear-pending
```
...
When using the `docker` terminal backend, Hades applies strict security hardening to every container.
...
When loaded, Hades checks if these files exist in the active profile's `HADES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Hades infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Hades infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Hades integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Hades under WSL.
...
10. **Keep updated** — run `hades update` regularly for security patches
...
TERMINAL_SSH_USER=hades
TERMINAL_SSH_KEY=~/.ssh/hades_agent_key
```
...
Hades ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hades_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `hades doctor` for the full remediation.
- **`hades doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Hades installs these **lazily** on first use rather than eagerly under `hades-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `hades tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `hades tools`.
