...
One command — `hades setup --portal` — and you're ready to `hades chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Hades also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `hades --tui` — see the [TUI](tui.md) guide.
:::
...
hades chat --model "anthropic/claude-sonnet-4"

# With a specific provider
hades chat --provider nous        # Use Nous Portal
hades chat --provider openrouter  # Force OpenRouter

# With specific toolsets
hades chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
hades -s hades-agent-dev,github-auth
hades chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
hades --continue             # Resume the most recent CLI session (-c)
hades --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
hades chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
hades -w                         # Interactive mode in worktree
hades -w -z "Fix issue #123"     # Single query in worktree
```
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HADES_YOLO_MODE` is on (either `hades --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Hades to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Hades supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
```bash
hades -s hades-agent-dev,github-auth
hades chat -s github-pr-workflow -s github-auth
```

Hades loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Hades recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Hades — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Hades is working, Hades prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Hades to the background — just like any terminal process. The shell prints a confirmation:
...
Resume this session with:
  hades --resume 20260225_143052_a1b2c3
...
```bash
hades --continue                          # Resume the most recent CLI session
hades -c                                  # Short form
hades -c "my project"                     # Resume a named session (latest in lineage)
hades --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
hades --resume "refactoring auth"         # Resume by title
hades -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `hades sessions rename <id> <title>` from the command line. Use `hades sessions list` to browse past sessions.
...
Hades immediately confirms the task and gives you back the prompt:
