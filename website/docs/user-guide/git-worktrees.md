...
title: "Git Worktrees"
description: "Run multiple Hades agents safely on the same repository using git worktrees and isolated checkouts"
---
...
Hades treats the **current working directory** as the project root:

- CLI: the directory where you run `hades` or `hades chat`
...
git worktree add ../repo-feature feature/hades-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/hades-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Hades there:

```bash
cd ../repo-feature

# Start Hades in the worktree
hades
```
...
git worktree add ../repo-experiment-a feature/hades-a
git worktree add ../repo-experiment-b feature/hades-b
```
...
- Works on its own branch (`feature/hades-a` vs `feature/hades-b`).
...
- Hades checkpoint data under `~/.hades/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Hades experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/hades-checkpoints-docs`, `feature/hades-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Hades from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `hades -w` (Automatic Worktree Mode)

Hades has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `hades/hades-<hash>`).
...
```bash
hades -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `hades -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Hades session its own clean checkout.
