...
```bash
hades profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `hades profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Hades in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`hades profile export` / `import`](../reference/profile-commands.md#hades-profile-export) — that's what those are for.
...
```bash
hades profile create research-bot
...
Recipients who run `hades profile update research-bot` will pull the latest.
...
```bash
hades profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.hades/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
# GitHub shorthand
hades profile install github.com/you/research-bot

# Full HTTPS
hades profile install https://github.com/you/research-bot.git

# SSH
hades profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
hades profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
hades profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
hades profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
hades profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
hades profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Hades distribution.
...
```bash
hades profile info research-bot
```
...
`hades profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
hades profile update research-bot
```
...
```bash
hades profile delete research-bot
```
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `hades profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
hades profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `hades profile update pr-reviewer` and everyone's on the new version within minutes.
...
git remote add origin https://github.com/you/hades-polymarket-trader.git
git push -u origin main --tags

# Anyone
hades profile install github.com/you/hades-polymarket-trader --alias
```
...
You built Hades-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
hades profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
hades profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `hades profile info <name>` and hold off on updates until you're ready.
:::
...
hades profile info research-bot | grep Version
...
hades profile delete research-bot --yes
hades profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
hades profile update research-bot --force-config --yes
```
...
hades profile install github.com/yourname/forked-research-bot --alias
...
hades profile install ~/.hades/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
hades profile delete research-bot-test --yes
hades profile install ~/.hades/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `hades -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `hades profile` — not a parallel command tree.
...
- Reserved profile names (`hades`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`hades profile export` / `import`](../reference/profile-commands.md#hades-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Hades](../guides/use-soul-with-hades.md) — authoring personalities
