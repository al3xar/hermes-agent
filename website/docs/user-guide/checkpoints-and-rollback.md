...
| Command | Description |
|---------|-------------|
| `hades checkpoints` | Show total size, project count, per-project breakdown |
| `hades checkpoints status` | Same as bare `checkpoints` |
| `hades checkpoints list` | Alias for `status` |
| `hades checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `hades checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `hades checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Hades detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  ...
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/hades/<project-hash>`).
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `hades checkpoints prune` manually.
...
Hades responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.hades/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
  ...
  /home/you/code/hades-agent                                        20       2h ago  live
...
Clear with: hades checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
hades checkpoints prune --retention-days 3 --max-size-mb 200
```
...
- **Directory scope** — Hades skips overly broad directories (root `/`, home `$HOME`).
...
~/.hades/checkpoints/
  ...
  │   ├── refs/hades/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `hades checkpoints status` / `prune` / `clear` instead.
...
```bash
hades checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `hades chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `hades checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Hades session in its own worktree/branch, with checkpoints as an extra layer.
