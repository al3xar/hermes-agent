...
Copy-paste recipes for common automation patterns. Each template uses Hades's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`hades webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `hades webhook subscribe`) |
...
```bash
hades cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/hades-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/hades-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
hades webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
hades cron create "0 9 * * 1" \
  "Scan the NousResearch/hades-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/hades-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (hades_cli/commands.py, hades_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (hades_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
hades cron create "0 6 * * *" \
  "Run a dependency security audit on the hades-agent project.

1. cd ~/.hades/hades-agent && source .venv/bin/activate
...
```bash
hades webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
hades webhook subscribe alert-triage \
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Hades-Monitor/1.0"})
...
```bash
hades webhook subscribe github-issues \
  --events "issues" \
...
```bash
hades webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
hades webhook subscribe stripe-payments \
...
```bash
hades cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the hades-agent codebase.
