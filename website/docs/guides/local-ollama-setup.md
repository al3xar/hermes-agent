---
sidebar_position: 9
title: "Run Hades Locally with Ollama — Zero API Cost"
...
# Run Hades Locally with Ollama — Zero API Cost
...
You'll set up Hades Agent running entirely on your own hardware, using [Ollama](https://ollama.com) as the model backend. No API keys, no subscriptions, no data leaving your machine. Once configured, Hades works exactly like it does with OpenRouter or Anthropic — terminal commands, file editing, web browsing, delegation — but the model runs locally.

By the end, you'll have:

- Ollama serving one or more open-weight models
- Hades connected to Ollama as a custom endpoint
...
```bash
# ~/.hades/.env
HADES_API_TIMEOUT=1800   # 30 minutes — generous for slow local models
```
:::
...
Hades is an **agentic** assistant — it edits files, runs commands, and browses the web through tool calls. Models without tool-call support can only chat; they can't take actions. For the full Hades experience, use a model that supports tools (like `gemma4:31b`).
:::
...
You can pull several models and switch between them inside Hades with `/model`. Ollama loads the active model into memory on demand and unloads idle ones automatically.
:::
...
Hades will use the terminal tool, file operations, and your local model — no cloud calls.
...
By default, Ollama uses a 2048-token context. Hades requires at least 64,000 tokens for agentic work with tools:
...
Then update your Hades config to use `gemma4-64k` as the model name.
...
Once Hades works locally in the CLI, you can expose it as a Telegram or Discord bot — still running entirely on your hardware.
...
- **Hades has auto-repair** — it detects malformed tool calls and attempts to fix them automatically.
- **Set up a fallback** — if the local model fails 3 times, Hades falls back to a cloud provider.
...
- **Terminal commands** — Hades wraps the command, runs it, reads output regardless of model
