...
`hades claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Hades. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::tip
If your OpenClaw setup was multi-provider, `hades setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
hades claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.hades/` (by default a single restore-point archive is written before apply, under `~/.hades/backups/pre-migration-*.zip`; restorable with `hades import`). |
...
| What | OpenClaw source | Hades destination | Notes |
|------|----------------|-------------------|-------|
...
| Source | OpenClaw location | Hades destination |
|--------|------------------|-------------------|
...
Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Hades skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Hades destination | Notes |
|------|---------------------|-------------------|-------|
...
| What | OpenClaw config path | Hades config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Hades config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Hades field | Notes |
|----------------|-------------|-------|
...
| Platform | OpenClaw config path | Hades `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Hades path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `HADES_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Hades equivalent)
...
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Hades has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `hades cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `hades webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `hades honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `hades skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Hades profiles |
...
4. **Verify API keys** — run `hades status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hades-gateway`

6. **Check session policies** — run `hades config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `hades whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `hades claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `hades config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
