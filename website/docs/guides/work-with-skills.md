...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Hades new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Hades how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Hades installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Hades to use a specific skill, and it will load it via the `skill_view` tool.
...
hades skills install official/research/arxiv
...
For how to ship skills in your own plugin, see [Build a Hades Plugin → Bundle skills](/guides/build-a-hades-plugin#bundle-skills).
...
When a skill with config is first loaded, Hades prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
hades skills config gif-search

# View all skill config
hades config show | grep '^skills\.config'
```
...
```bash
hades chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.hades/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Hades may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Hades will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.
...
**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Hades to update the skill with what you learned. Skills that aren't maintained become liabilities.
