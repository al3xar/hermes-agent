...
:::tip
A $5/month VPS is plenty for running the gateway. Hades itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Hades Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_hades_bot`)
...
```
[Gateway] Starting Hades Gateway...
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `hades gateway install` to update the plist.
:::
...
```bash
hades gateway stop && hades gateway start
```
...
hades pairing revoke telegram 987654321
...
For a full guide, see [Use SOUL.md with Hades](/guides/use-soul-with-hades).
...
```bash
# From the CLI
hades cron list          # View all scheduled jobs
hades cron status        # Check if scheduler is running
