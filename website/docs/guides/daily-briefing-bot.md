...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `hades setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Hades spins up** a fresh agent session with your prompt
...
Hades will search the web, read through results, and produce something like:
...
Before creating cron jobs, ensure Hades has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Hades what you want:
...
Hades will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Hades to delegate each topic to a sub-agent:
...
Hades will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
