...
4. Fill in:
   - **Name:** `Hades Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `hades-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Hades ships a Graph auth smoke-test. From your Hades install:
...
3. Restart the gateway so the new secret is picked up: `hades gateway restart`.
