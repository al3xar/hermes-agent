...
title: "Build a hades Plugin"
description: "Step-by-step guide to building a complete hades plugin with tools, hooks, data files, and skills"
---

# Build a hades Plugin

This guide walks through building a complete hades plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
hades has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `hades_LOCAL_STT_COMMAND` to a shell template |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `hades skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
This tells hades: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** hades may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `hades my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but hades continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `hades_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
hades_PLUGINS_DEBUG=1 hades plugins list
```
...
```bash
hades logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `hades plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
...
When users run `hades plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing hades into installing arbitrary packages — only specs hades itself ships are eligible |
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), hades injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: hades-agent"}
...
Plugins can add their own `hades <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for hades my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: hades my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for hades my-plugin."""
...
After registration, users can run `hades my-plugin status`, `hades my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `hades name` in a terminal |
...
hades has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.hades/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `hades gateway status` without SDK instantiation.
...
hades also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.
...
hades connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).
...
hades skills search my-workflow --source myorg/skills-repo
hades skills install myorg/skills-repo/my-workflow
```
...
For STT, point `hades_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```bash
pip install hades-plugin-calculator
# Plugin auto-discovered on next hades startup
```
...
```python
# Wrong — will break if hades passes extra context
def handler(args):
    ...
