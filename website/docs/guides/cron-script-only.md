...
Hades calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Hades writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Hades:** *(writes `~/.hades/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
>
> **Hades:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `hades cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Hades:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
# 3. Verify
hades cron list
hades cron run <job_id>    # fire it once to test
```
...
```bash
hades cron create "every 5m"        # interval
hades cron create "every 2h"
hades cron create "0 9 * * *"       # standard cron: 9am daily
hades cron create "30m"             # one-shot: run once in 30 minutes
```
...
```bash
hades cron list                                    # see all jobs
hades cron pause <job_id>                          # stop firing, keep definition
hades cron resume <job_id>
hades cron edit <job_id> --schedule "every 10m"    # adjust cadence
hades cron edit <job_id> --agent                   # flip to LLM mode
hades cron edit <job_id> --no-agent --script …     # flip back
hades cron remove <job_id>                         # delete it
```
...
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When Hades might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Hades webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Hades being up. The in-gateway scheduler is the right choice when the thing being monitored is external.
