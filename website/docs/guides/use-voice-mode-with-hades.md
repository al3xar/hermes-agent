...
description: "A practical guide to setting up and using hades voice mode across CLI, Telegram, Discord, and Discord voice channels"
---
...
- you want hades sitting in a Discord voice channel for live conversation
...
## Step 1: make sure normal hades works first
...
hades supports both local and cloud speech stacks.
...
### If you use `hades setup`

If you choose NeuTTS in the setup wizard, hades checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. hades transcribes and responds
...
- asking hades to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full hades loop.

## Tuning CLI behavior

### Silence threshold

If hades starts/stops too aggressively, tune:
...
hades stays a normal chat bot, but can speak replies.
...
- you want hades to function like a portable research or ops assistant
...
hades joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- hades detects speech boundaries
- transcripts are posted in the associated text channel
- hades responds in text and audio
