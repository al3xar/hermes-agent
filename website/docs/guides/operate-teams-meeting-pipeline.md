...
```bash
hades teams-pipeline token-health
hades teams-pipeline token-health --force-refresh
```
...
```bash
hades teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
hades teams-pipeline maintain-subscriptions
hades teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Hades cron (recommended if you already run the Hades gateway)

Hades ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hades/scripts/`. First create the script:

```bash
mkdir -p ~/.hades/scripts
cat > ~/.hades/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec hades teams-pipeline maintain-subscriptions
EOF
...
Create `/etc/systemd/system/hades-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Hades Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/hades teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/hades-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Hades Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now hades-teams-pipeline-maintain.timer
systemctl list-timers hades-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/hades teams-pipeline maintain-subscriptions >> /var/log/hades/teams-pipeline-maintain.log 2>&1
```
...
```bash
hades teams-pipeline subscriptions   # should show expirationDateTime advanced
hades teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
hades teams-pipeline list
hades teams-pipeline list --status failed
hades teams-pipeline show <job-id>
```

### Replay a stored job

```bash
hades teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
hades teams-pipeline fetch --meeting-id <meeting-id>
hades teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `hades teams-pipeline maintain-subscriptions --dry-run`
- inspect `hades teams-pipeline list --status failed`
...
- run `hades teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `hades teams-pipeline run`
...
- [ ] `hades teams-pipeline validate` returns an OK snapshot
- [ ] `hades teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Hades cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
