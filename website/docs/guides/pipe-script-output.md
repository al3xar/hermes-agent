...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `hades send`."
---

# Pipe Script Output to Messaging Platforms

`hades send` is a small, scriptable CLI that pushes a message to any
messaging platform Hades is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | hades send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `hades
...
hades send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
hades send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
hades send --list

# Filter by platform
hades send --list telegram
```
...
Any platform Hades ships adapters for works as a target:
...
`hades send` resolves the message body in this order:

1. **Positional argument** — `hades send --to telegram "hi"`
2. **`--file PATH`** — `hades send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | hades send --to telegram`

When stdin is a TTY (no pipe), Hades does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  hades send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `hades send` reuses your Hades config, the same script works on
any host where Hades is installed — no need to export bot tokens into
...
`hades send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  hades send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | hades send \
...
```bash
./train.py --epochs 200 && \
  hades send --to telegram "training done" || \
  hades send --to telegram "training failed (exit $?)"
```
...
hades send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(hades send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `hades send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `hades send` calls
...
pointing at the gateway; start it with `hades gateway start` and retry.
...
"no channels discovered yet", start the gateway once (`hades gateway
...
| Approach | Multi-platform | Reuses Hades creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `hades send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`hades send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `hades send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `hades send` shares with cron delivery.
