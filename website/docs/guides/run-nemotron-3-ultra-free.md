...
[Download the Hades Desktop installer](https://hades-agent.nousresearch.com/desktop) for macOS or Windows, then open it. On first launch it finishes setting itself up (usually under a minute).

### 2. Connect Nous Portal

When the app opens, you'll see a "Let's get you set up" screen. Click **Nous Portal** (marked **Recommended**). Your browser opens — create a [Nous Portal](https://portal.nousresearch.com) account (or sign in), choose the **Free** plan, and authorize Hades. The app connects automatically.
...
Prefer to review first? Download [`install.sh`](https://hades-agent.nousresearch.com/install.sh), inspect it, then run it.
...
Select **Quick Setup**. Hades opens a browser tab and waits for you to finish the next steps.
...
- **Don't see the model in the list?** Make sure you finished the Nous Portal connection and that you're on the **Free** plan. In the CLI, `hades portal info` confirms you're logged in and routing through Nous.
...
- **[Run Hades Agent with Nous Portal](/guides/run-hades-with-nous-portal)** — Full Portal walkthrough: models, Tool Gateway, and verification
