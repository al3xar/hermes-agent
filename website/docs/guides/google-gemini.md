...
sidebar_position: 16
title: "Google Gemini"
description: "Use Hades Agent with Google Gemini — native AI Studio API, API-key setup, OAuth option, tool calling, streaming, and quota guidance"
---

# Google Gemini

Hades Agent supports Google Gemini as a native provider using the **Google AI Studio / Gemini API** — not the OpenAI-compatible endpoint. This lets Hades translate its internal OpenAI-shaped message and tool loop into Gemini's native `generateContent` API while preserving tool calling, streaming, multimodal inputs, and Gemini-specific response metadata.

Hades also supports a separate **Google Gemini (OAuth)** provider that uses the same Cloud Code Assist backend as Google's Gemini CLI. Use the API-key provider (`gemini`) for the lowest-risk official API path.

## Prerequisites

- **Google AI Studio API key** — create one at [aistudio.google.com/apikey](https://aistudio.google.com/apikey)
- **Billing-enabled Google Cloud project** — recommended for agent use. Gemini's free tier is too small for long-running agent sessions because Hades may make several model calls per user turn.
- **Hades installed** — no extra Python package is required for the native Gemini provider.

:::tip API key path
Set `GOOGLE_API_KEY` or `GEMINI_API_KEY`. Hades checks both names for the `gemini` provider.
:::
...
# → Hades checks your key tier and shows Gemini models
# → Select a model
...
After running `hades model`, your `~/.hades/config.yaml` will contain:
...
Hades detects this endpoint and creates its native Gemini adapter. Internally, Hades still keeps the agent loop in OpenAI-shaped messages, then translates each request to Gemini's native schema:
...
For Gemini 3 tool use, Hades preserves the `thoughtSignature` values attached to function-call parts and replays them on the next tool turn. That covers the validation-critical path for multi-step agent workflows.
...
Hades also has a `google-gemini-cli` provider:
...
This uses browser PKCE login and the Cloud Code Assist backend. It can be useful for users who want Gemini CLI-style OAuth, but Hades shows an explicit warning because Google may treat use of the Gemini CLI OAuth client from third-party software as a policy violation. For production or lowest-risk usage, prefer the API-key provider above.

## Available Models

The `hades model` picker shows Gemini models maintained in Hades' provider registry. Common choices include:
...
Model availability changes over time. If a model disappears or is not enabled for your key, run `hades model` again and pick one from the current list.
...
Google publishes moving aliases for the Pro and Flash Gemini families. `gemini-pro-latest` and `gemini-flash-latest` are useful when you want Google to advance the model automatically without changing your Hades config.
...
Google also exposes Gemma models through the Gemini API. Hades recognizes these as Google models, but hides very low-throughput Gemma entries from the default model picker so new users do not accidentally select an evaluation-tier model for a long-running agent session.
...
If you have not configured Gemini yet, exit the session and run `hades model` first. `/model` switches among already-configured providers and models; it does not collect new API keys.
...
For OAuth quota usage, run this inside a Hades session:
...
Gemini works with all Hades gateway platforms (Telegram, Discord, Slack, WhatsApp, LINE, Feishu, etc.). Configure Gemini as your provider, then start the gateway normally:
...
Hades could not find a usable API key. Add one of these to `~/.hades/.env`:
...
Then run `hades model` again.

### "This Google API key is on the free tier"

Hades probes Gemini API keys during setup. Free-tier quotas can be exhausted after a handful of agent turns because tool use, retries, compression, and auxiliary tasks may require multiple model calls.
...
The selected model is not available for your account, region, or key. Run `hades model` again and pick another Gemini model from the current list.

### Gemma model is not shown in `hades model`

Hades may hide low-throughput Gemma models from the picker by default. If you intentionally want to evaluate one, set the model ID directly in `~/.hades/config.yaml`.
...
The `google-gemini-cli` provider uses a Gemini CLI / Cloud Code Assist OAuth flow. Hades warns before starting it because this is distinct from the official AI Studio API-key path. Use `provider: gemini` with `GOOGLE_API_KEY` for the official API-key integration.

### Tool calling fails with schema errors

Upgrade Hades and rerun `hades model`. The native Gemini adapter sanitizes tool schemas for Gemini's stricter function-declaration format; older builds or custom endpoints may not.
