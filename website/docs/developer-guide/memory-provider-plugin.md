...
:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `hades plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hades_home (str): Active HADES_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `hades memory setup` | **Yes** |
| `save_config(values, hades_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
`get_config_schema()` returns a list of field descriptors used by `hades memory setup`:
...
the `messages` parameter; Hades will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hades_home` kwarg from `initialize()`, not hardcoded `~/.hades`:
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `hades my-provider status`, `hades my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `hades <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `hades --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: hades my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the hades my-provider argparse tree.
