...
The messaging gateway is the long-running process that connects Hades to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `hades_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
- **Explicit target delivery** — `send_message` tool specifying `telegram:-1001234567890`, or the [`hades send` CLI](/guides/pipe-script-output) wrapping the same tool for shell scripts
...
- `hades gateway start` / `hades gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.hades/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `hades gateway stop` stops only the current profile's gateway. `hades gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
