...
Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `hades tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `hades tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.hades/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `hades plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `hades tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `hades tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `hades tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `hades plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `hades tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `hades tools` model picker |
...
Drop a user plugin at `~/.hades/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `hades plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export HADES_HOME=/tmp/hades-imggen-test
...
hades plugins enable my-backend
...
# Exercise it
hades -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `hades tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
`my_backend_imggen_package` must expose a top-level `register` function. See [Distribute via pip](/guides/build-a-hades-plugin#distribute-via-pip) in the general plugin guide for the full setup.
