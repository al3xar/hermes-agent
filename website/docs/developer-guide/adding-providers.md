...
Hades can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `hades model` menu entries
...
1. `hades_cli/auth.py` decides how credentials are found.
2. `hades_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `hades_cli/models.py` and `hades_cli/main.py` make the provider show up in the CLI. (`hades_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `hades_cli/auth.py`
2. `hades_cli/models.py`
3. `hades_cli/runtime_provider.py`
4. `hades_cli/main.py`
...
:::tip
`hades_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `hades setup`.
:::
...
7. `hades model` menu includes the provider
8. `hades setup` wizard delegates to `main.py` automatically
...
- Provider-specific `hades model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `hades_cli/auth.py`
- `_PROVIDER_LABELS` in `hades_cli/models.py`
- `_PROVIDER_ALIASES` in both `hades_cli/auth.py` and `hades_cli/models.py`
- CLI `--provider` choices in `hades_cli/main.py`
...
## Step 2: Add auth metadata in `hades_cli/auth.py`
...
- What env vars should Hades check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `hades_cli/models.py`
...
## Step 4: Resolve runtime data in `hades_cli/runtime_provider.py`
...
Be careful with API-key precedence. Hades already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hades_cli/main.py`

A provider is not discoverable until it shows up in the interactive `hades model` flow.

Update these in `hades_cli/main.py`:
...
:::tip
`hades_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `hades model` and `hades setup` automatically.
:::
...
When you add a native provider, double-check that Hades is only sending fields that provider actually understands.
...
- `tests/hades_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/hades_cli/test_model_switch_custom_providers.py` (and adjacent `tests/hades_cli/test_model_switch_*.py`)
- `tests/hades_cli/test_setup_model_provider.py`
...
python -m pytest tests/hades_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/hades_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
- [ ] `ProviderConfig` added in `hades_cli/auth.py`
- [ ] aliases added in `hades_cli/auth.py` and `hades_cli/models.py`
- [ ] model catalog added in `hades_cli/models.py`
- [ ] runtime branch added in `hades_cli/runtime_provider.py`
- [ ] CLI wiring added in `hades_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `hades model` but not `hades setup`
