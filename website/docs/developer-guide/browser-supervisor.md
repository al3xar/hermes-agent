...
description: "How Hades detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---
...
(`hades-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Hades `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `hades-acp`, `hades-api-server`, and core toolsets (gated on CDP reachability)
- `hades_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
