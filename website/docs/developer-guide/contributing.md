...
- **Profile-safe paths**: Never hardcode `~/.hades` — use `get_hades_home()` from `hades_constants` for code paths and `display_hades_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/hermes-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
