...
- Building a custom/local tool without modifying Hades core? Start with [Build a Hades Plugin](../guides/build-a-hades-plugin.md)
...
- **Profile-safe paths**: Never hardcode `~/.hades` — use `get_hades_home()` from `hades_constants` for code paths and `display_hades_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/hades-agent/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Hades officially supports **Linux, macOS, WSL2, and native Windows (via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. If you're doing Windows-heavy dev, run the Windows-footgun lint (`scripts/check-windows-footguns.py`) before pushing.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/hades-agent/blob/main/LICENSE).
