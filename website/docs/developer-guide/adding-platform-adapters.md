...
This guide covers adding a new messaging platform to the Hades gateway. A platform adapter connects Hades to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core Hades code. Your plugin is a directory with two files:
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `hades config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hades-config) below).
...
| `hades config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `hades status` | Shows plugin platforms with `(plugin)` tag |
| `hades gateway setup` | Plugin platforms appear in setup menu |
| `hades tools` / `hades skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.hades/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `hades gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `hades cron run` separate from `hades gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `hades config`

`hades_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #hades)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
1. **`hades_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`hades_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`hades_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`hades_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`hades_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`hades_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"hades-newplat"` toolset definition with `_HADES_CORE_TOOLS`
2. **`toolsets.py`** — Add `"hades-newplat"` to the `"hades-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | hades-newplat toolset |
