...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, Hades prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. Hades never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.
...
- `prompt` (optional) — prompt text shown during `hades config migrate`; falls back to `description`
...
2. **Discovery:** `hades config migrate` scans all enabled skills, finds unconfigured settings, and prompts the user. Settings also appear in `hades config show` under "Skill Settings."
...
4. **Manual setup:** Users can also set values directly:
   ```bash
   hades config set skills.config.myplugin.path ~/my-data
   ```
...
When loaded, Hades checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
Prefer stdlib Python, curl, and existing Hades tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
| Token | Replaced with |
|---|---|
| `${HADES_SKILL_DIR}` | Absolute path to the skill's directory |
| `${HADES_SESSION_ID}` | The active session id (left in place if there is no session) |
...
To analyse the input, run:
    ...
    node ${HADES_SKILL_DIR}/scripts/analyse.js <input>
```
...
Git branch: !`git -C ${HADES_SKILL_DIR} rev-parse --abbrev-ref HEAD`
```
...
```bash
hades chat --toolsets skills -q "Use the X skill to do Y"
```
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo, is discoverable via `hades skills browse` (labeled "official"), and installs with built-in trust.

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a registry and share it via `hades skills install`.
...
```bash
hades skills tap add owner/repo
```
...
Hades can now consume third-party skills from multiple external discovery models:
