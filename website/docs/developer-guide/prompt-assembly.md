...
description: "How Hades builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Hades deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.hades.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
You are Hades, an AI assistant created by Nous Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hades_md(cwd_path)       # 1. .hades.md / HADES.md (walks to git root)
...
|----------|-------|-------------|-------|
| 1 | `.hades.md`, `HADES.md` | CWD up to git root | Hades-native project config |
...
- **YAML frontmatter stripped** — `.hades.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Hades concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.hades.md` / `HADES.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Hades already loads, rather than editing Python templates in place.
...
- Project context files such as `.hades.md`, `HADES.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Hades.
- Ephemeral overlays such as `HADES_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Hades assembles prompts for everyone, change Python and treat it as a code contribution
