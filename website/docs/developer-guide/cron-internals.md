...
description: "How Hades stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hades_cli/cron.py` | CLI `hades cron` subcommands |
...
In CLI mode, cron jobs only fire when `hades cron` commands are run or during active CLI sessions.
...
2. **Environment variable** — `HADES_CRON_SCRIPT_TIMEOUT`
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `hades cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `hades cron` CLI provides direct job management:

```bash
hades cron list                    # Show all jobs
hades cron create                  # Interactive job creation (alias: add)
hades cron edit <job_id>           # Edit job configuration
hades cron pause <job_id>          # Pause a running job
hades cron resume <job_id>         # Resume a paused job
hades cron run <job_id>            # Trigger immediate execution
hades cron remove <job_id>         # Delete a job
```
