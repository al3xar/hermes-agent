...
modifying Hades core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Hades Plugin](/guides/build-a-hades-plugin)
...
If your tool requires an API key, add it to `hades_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `hades_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `hades chat -q "Use the weather tool for London"`
