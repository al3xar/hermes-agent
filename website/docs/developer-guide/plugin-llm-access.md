...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Hades already has applies. The plugin never sees a
...
[`hades-example-plugins`](https://github.com/NousResearch/hades-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
hades-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/hades-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Hades provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Hades provider
        # (must be one Hades already knows about — same names as
        # `hades model` and config.yaml model.provider).
        ...
        # plugin sends — Hades does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
Existing `ctx.*` methods extend an existing Hades subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/hades-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/hades-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/hades-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/hades-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
