...
- **Anthropic** — Claude models (direct API, OAuth via `hades auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `hades model` or by editing `~/.hades/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Yes, natively.** Hades supports native Windows via the PowerShell installer — no WSL required. Run in PowerShell:

```powershell
iex (irm https://hades-agent.nousresearch.com/install.ps1)
```
...
WSL2 remains a fully supported alternative. To run Hades inside WSL2, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and use the standard install command:

```bash
curl -fsSL https://hades-agent.nousresearch.com/install.sh | bash
```

### I run Hades in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Hades inside WSL2
...
- let Hades use the resulting MCP browser tools

This is more reliable than trying to force Hades core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Hades](../guides/use-mcp-with-hades.md#wsl2-bridge-hades-in-wsl-to-windows-chrome)
...
Yes — Hades now has a tested Termux install path for Android phones.
...
Yes. Run `hades model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Hades minimum; set this to match your server's actual context window
```
...
Hades persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
Hades auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HADES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::
...
Yes. Import the `AIAgent` class and use Hades programmatically:
...
#### `hades: command not found` after installation
...
**Cause:** Hades requires Python 3.11 or newer.
...
**Cause:** Hades builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Hades runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Hades auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.hades/config.yaml`:
...
**Solution:** Exit your session and use `hades model` from your terminal to add new providers:
...
After adding a new provider via `hades model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `hades model` (from terminal) |
| Enter/change API keys | `hades model` (from terminal) |
...
# Or set directly
hades config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```
...
# Set a valid model
hades config set HADES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
hades chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `hades chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Hades detected the wrong context length for your model.
...
hades chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Hades may have the wrong context length for your model. Check what it detected:
...
**Cause:** Hades detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Hades never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `hades chat`
...
**Solution:**
- Verify your bot token is valid with `hades gateway setup`
...
pip install "hades-agent[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `hades gateway start` fails
...
tmux new -s hades 'hades gateway run'
# Reattach later: tmux attach -t hades

# Option 3: Background via nohup
nohup hades gateway run > ~/.hades/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'hades gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `hades gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
hades gateway install    # Re-snapshots your current PATH
hades gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.hades.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `hades chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `hades chat -t "terminal"`
...
cd ~/.hades/hades-agent && uv pip install -e ".[mcp]"
...
hades config show | grep -A 12 mcp_servers

# Restart Hades or reload MCP after config changes
hades chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Hades](/guides/use-mcp-with-hades)
...
:::warning
If an MCP server crashes mid-request, Hades will report a timeout. Check the server's own logs (not just Hades logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `hades profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `hades update`?

`hades update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `hades update` once — it covers every profile on the machine.
...
**Solution: Delegation config.** Hades can route subagents to a different model automatically. Set this in `~/.hades/config.yaml`:
...
Now when you tell Hades "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Hades do this?

**Current limitation:** Hades profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Hades reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `hades skills config` settings don't seem to take effect.

**Solution:** Use `hades skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`hades gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Hades creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Hades to another machine
...
2. On the **source machine**, create a full backup:
   ...
   hades backup
   ```
   This creates a zip of your entire `~/.hades/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hades-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/hades-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   hades import ~/hades-backup-<timestamp>.zip
   ```

4. On the new machine, run `hades setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
hades profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
hades profile import ./work-backup.tar.gz work
```
...
### `hades backup` vs `hades profile export`

| Feature | `hades backup` | `hades profile export` |
| :--- | :--- | :--- |
...
:::tip
`hades backup` produces a consistent snapshot even while Hades is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Hades-specific issue — it's a shell config permissions problem.
...
hades config show | head -20

# Re-run model selection
hades model

# Or test with a known-good model
hades chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/hades-agent/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Hades version (`hades --version`), and the full error message
