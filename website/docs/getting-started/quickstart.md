...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hades_cli/models.py` may change between releases) | `hades model` → MiniMax (OAuth) |
...
| **Google Gemini (OAuth)** | Gemini via the `google-gemini-cli` OAuth flow — no key needed | `hades model` → Google Gemini (OAuth) |
...
# ~/.hades/hades-agent on Linux/macOS or %LOCALAPPDATA%\hades\hades-agent on Windows):
...
Hades ships with a catalog of bundled skills already installed in `~/.hades/skills/`. You can add more from the Skills Hub, or write your own.
...
(If you installed without `[all]`, run `cd ~/.hades/hades-agent && uv pip install -e ".[acp]"` first.)
