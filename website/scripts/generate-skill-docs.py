...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/hades-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hades_meta = (fm.get("metadata") or {}).get("hades") or {}
    tags = hades_meta.get("tags") or []
    related = hades_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `hades skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        "",
        "Hades ships with a large built-in skill library copied into `~/.hades/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Hades also syncs bundled skills on `hades update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.hades/skills/` tree, it is still shipped with Hades; restore it with `hades skills reset <name> --restore`.",
        "",
