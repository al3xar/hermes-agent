...
在开始设置之前，先介绍大多数人最想了解的内容：Hades 进入服务器后的行为方式。

## Hades 的行为方式

| 上下文 | 行为 |
|---------|----------|
| **私信（DM）** | Hades 响应每条消息，无需 `@提及`。每个私信有独立的会话。 |
| **服务器频道** | 默认情况下，Hades 仅在被 `@提及` 时响应。如果你在频道中发帖但未提及它，Hades 会忽略该消息。 |
...
| **线程（Thread）** | Hades 在同一线程中回复。提及规则仍然适用，除非该线程或其父频道被配置为自由响应。线程的会话历史与父频道相互隔离。 |
| **多用户共享频道** | 默认情况下，Hades 为安全和清晰起见，在频道内按用户隔离会话历史。在同一频道中交谈的两个人不会共享同一份对话记录，除非你明确禁用该功能。 |
| **提及其他用户的消息** | 当 `DISCORD_IGNORE_NO_MENTION` 为 `true`（默认值）时，如果消息 @提及了其他用户但**未**提及机器人，Hades 保持沉默。这可防止机器人介入针对其他人的对话。如果你希望机器人响应所有消息而不管提及了谁，请设置为 `false`。此设置仅适用于服务器频道，不适用于私信。 |

:::tip
如果你想要一个普通的机器人帮助频道，让用户无需每次都 @标记就能与 Hades 对话，请将该频道添加到 `DISCORD_FREE_RESPONSE_CHANNELS`。
:::

### Discord Gateway（网关）模型

Hades 在 Discord 上不是无状态回复的 webhook（网络钩子）。它通过完整的消息网关运行，这意味着每条传入消息都会经过：
...
这一点很重要，因为在繁忙服务器中的行为取决于 Discord 路由和 Hades 会话策略两者。
...
因此，如果 Alice 和 Bob 都在 `#research` 中与 Hades 对话，即使他们使用的是同一个可见的 Discord 频道，Hades 默认也会将其视为独立的对话。
...
授权后，机器人将出现在你服务器的成员列表中（在你启动 Hades 网关之前，它会显示为离线）。
...
| `DISCORD_ALLOW_BOTS` | 否 | `"none"` | 控制机器人如何处理来自其他 Discord 机器人的消息。`"none"` — 忽略所有其他机器人。`"mentions"` — 仅接受 `@提及` Hades 的机器人消息。`"all"` — 接受所有机器人消息。 |
...
| `HADES_DISCORD_TEXT_BATCH_DELAY_SECONDS` | 否 | `0.6` | 适配器在刷新排队文本块之前等待的宽限窗口。用于平滑流式输出。 |
| `HADES_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` | 否 | `2.0` | 当单条消息超过 Discord 长度限制时，分割块之间的延迟。 |
...
行为：
- 精确的线程/频道 ID 匹配优先。
- 如果消息到达线程或论坛帖子内，且该线程没有明确条目，Hades 会回退到父频道/论坛 ID。
...
为 `true` 时：Alice 和 Bob 在 `#research` 中交谈，各自与 Hades 有独立的对话。为 `false` 时：整个频道共享一份对话记录和一个正在运行的 agent 槽位。
...
Hades 自动将已安装的技能注册为**原生 Discord 应用命令**。这意味着技能会出现在 Discord 的自动补全 `/` 菜单中，与内置命令并列。
...
无需额外配置——通过 `hades skills install` 安装的任何技能都会在下次网关重启时自动注册为 Discord 斜杠命令。

### 禁用斜杠命令注册

如果你针对同一个 Discord 应用运行多个 Hades 网关（例如测试环境 + 生产环境），只有其中一个应该拥有全局斜杠命令注册——否则最后启动的那个会覆盖之前的注册，导致注册状态不稳定。在"从属"网关上关闭斜杠注册：
...
Discord 的每次上传大小限制取决于服务器的加成等级（免费 25 MB，最高 500 MB）。如果 Hades 收到 HTTP 413，适配器会回退到指向本地缓存路径的链接，而不是静默失败。
...
- **Discord 语音频道**：Hades 还可以加入语音频道，聆听用户说话，并在频道中回话。

完整的设置和操作指南，请参阅：
- [语音模式](/user-guide/features/voice-mode)
- [与 Hades 使用语音模式](/guides/use-voice-mode-with-hades)

## 论坛频道

Discord 论坛频道（类型 15）不接受直接消息——论坛中的每个帖子都必须是线程。Hades 自动检测论坛频道，并在需要发送消息时创建新的线程帖子，因此 `send_message`、TTS、图片、语音消息和文件附件都无需 agent 进行特殊处理即可正常工作。
...
**原因**：Hades 网关未运行，或 token 不正确。

**解决方法**：检查 `hades gateway` 是否正在运行。验证 `.env` 文件中的 `DISCORD_BOT_TOKEN`。如果你最近重置了 token，请更新它。
...
默认情况下，Hades 会阻止机器人 ping `@everyone`、`@here` 和角色提及，即使其回复中包含这些 token 也不例外。这可防止措辞不当的 prompt 或回显的用户内容向整个服务器发送垃圾消息。个人 `@user` ping 和回复引用 ping（"回复……"小标签）保持启用，以便正常对话仍然有效。
