...
Teams 将 @提及作为普通消息投递，其中包含 `<at>BotName</at>` 标签，Hades 在处理前会自动去除这些标签。
...
devtunnel port create hades-bot -p 3978 --protocol https  # 如已修改 TEAMS_PORT，请替换 3978
...
| 隧道 URL 在重启后变更 | 使用命名隧道（`devtunnel create hades-bot`）时，devtunnel URL 是持久的。ngrok 和 cloudflared 每次运行都会生成新 URL（除非你有付费计划）——URL 变更时请用 `teams app update` 更新机器人端点 |
| Teams 显示"此机器人未响应" | Webhook 返回了错误。检查 `docker logs hades` 中的错误堆栈 |
