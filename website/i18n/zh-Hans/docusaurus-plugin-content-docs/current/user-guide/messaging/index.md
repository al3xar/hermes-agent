...
sidebar_position: 1
title: "消息网关"
description: "通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Yuanbao、Microsoft Teams、LINE、Webhooks 或任何兼容 OpenAI 的前端与 Hades 对话 — 架构与配置概览"
---

# 消息网关

通过 Telegram、Discord、Slack、WhatsApp、Signal、SMS、Email、Home Assistant、Mattermost、Matrix、DingTalk、Feishu/Lark、WeCom、Weixin、BlueBubbles（iMessage）、QQ、Yuanbao、Microsoft Teams、LINE、ntfy 或浏览器与 Hades 对话。网关是一个单一后台进程，连接所有已配置的平台，管理会话，运行 cron 任务，并传递语音消息。

完整的语音功能集——包括 CLI 麦克风模式、消息中的语音回复以及 Discord 语音频道对话——请参阅 [Voice Mode](/user-guide/features/voice-mode) 和 [Use Voice Mode with Hades](/guides/use-voice-mode-with-hades)。
...
```bash
hades gateway setup        # 交互式配置所有消息平台
```
...
```bash
hades gateway              # 在前台运行
hades gateway setup        # 交互式配置消息平台
hades gateway install      # 安装为用户服务（Linux）/ launchd 服务（macOS）
sudo hades gateway install --system   # 仅 Linux：安装开机启动的系统服务
hades gateway start        # 启动默认服务
hades gateway stop         # 停止默认服务
hades gateway status       # 检查默认服务状态
hades gateway status --system         # 仅 Linux：显式检查系统服务
```
...
# 其他配对命令：
hades pairing list          # 查看待审核和已批准的用户
hades pairing revoke telegram 123456789  # 撤销访问权限
```
...
第一次在任意平台向繁忙的 agent 发送消息时，Hades 会在繁忙确认中附加一行提示，说明该配置项（`"💡 First-time tip — …"`）。该提示每次安装只触发一次——由 `onboarding.seen.busy_input_prompt` 下的标志锁定。删除该键可再次看到提示。
...
```bash
hades gateway install               # 安装为用户服务
hades gateway start                 # 启动服务
hades gateway stop                  # 停止服务
hades gateway status                # 检查状态
...
除非你确实有此需要，否则避免同时安装用户和系统网关单元。Hades 检测到两者同时存在时会发出警告，因为 start/stop/status 行为会变得不明确。

:::info 多个安装
如果你在同一台机器上运行多个 Hades 安装（使用不同的 `HADES_HOME` 目录），每个安装都有自己的 systemd 服务名称。默认的 `~/.hades` 使用 `hades-gateway`；其他安装使用 `hades-gateway-<hash>`。`hades gateway` 命令会自动针对当前 `HADES_HOME` 对应的正确服务。
:::

### macOS（launchd）

```bash
hades gateway install               # 安装为 launchd agent
hades gateway start                 # 启动服务
hades gateway stop                  # 停止服务
hades gateway status                # 检查状态
...
生成的 plist 文件位于 `~/Library/LaunchAgents/ai.hades.gateway.plist`。它包含三个环境变量：
...
- **HADES_HOME** — 将网关限定到你的 Hades 安装。

:::tip 安装后 PATH 变更
launchd plist 是静态的——如果你在配置网关后安装了新工具（例如通过 nvm 安装新版 Node.js，或通过 Homebrew 安装 ffmpeg），请重新运行 `hades gateway install` 以捕获更新后的 PATH。网关会检测到过时的 plist 并自动重新加载。
:::

:::info 多个安装
与 Linux systemd 服务类似，每个 `HADES_HOME` 目录都有自己的 launchd 标签。默认的 `~/.hades` 使用 `ai.hades.gateway`；其他安装使用 `ai.hades.gateway-<suffix>`。
:::
...
|----------|---------|--------------|
| CLI | `hades-cli` | 完全访问 |
| Telegram | `hades-telegram` | 完整工具，包括终端 |
| Discord | `hades-discord` | 完整工具，包括终端 |
| WhatsApp | `hades-whatsapp` | 完整工具，包括终端 |
| Slack | `hades-slack` | 完整工具，包括终端 |
| Google Chat | `hades-google_chat` | 完整工具，包括终端 |
| Signal | `hades-signal` | 完整工具，包括终端 |
| SMS | `hades-sms` | 完整工具，包括终端 |
| Email | `hades-email` | 完整工具，包括终端 |
| Home Assistant | `hades-homeassistant` | 完整工具 + HA 设备控制（ha_list_entities、ha_get_state、ha_call_service、ha_list_services） |
| Mattermost | `hades-mattermost` | 完整工具，包括终端 |
| Matrix | `hades-matrix` | 完整工具，包括终端 |
| DingTalk | `hades-dingtalk` | 完整工具，包括终端 |
| Feishu/Lark | `hades-feishu` | 完整工具，包括终端 |
| WeCom | `hades-wecom` | 完整工具，包括终端 |
| WeCom Callback | `hades-wecom-callback` | 完整工具，包括终端 |
| Weixin | `hades-weixin` | 完整工具，包括终端 |
| BlueBubbles | `hades-bluebubbles` | 完整工具，包括终端 |
| QQBot | `hades-qqbot` | 完整工具，包括终端 |
| Yuanbao | `hades-yuanbao` | 完整工具，包括终端 |
| Microsoft Teams | `hades-teams` | 完整工具，包括终端 |
| API Server | `hades-api-server` | 完整工具（去除 `clarify`、`send_message`、`text_to_speech`——程序化访问没有交互用户） |
| Webhooks | `hades-webhook` | 完整工具，包括终端 |
