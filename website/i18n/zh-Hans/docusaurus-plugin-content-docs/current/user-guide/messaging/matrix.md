...
Hades 兼容任何 Matrix homeserver——Synapse、Conduit、Dendrite 或 matrix.org。

在开始设置之前，先了解大多数人最想知道的：Hades 连接后的行为方式。

## Hades 的行为方式

| 场景 | 行为 |
|---------|----------|
| **私聊（DM）** | Hades 响应每条消息，无需 `@提及`。每个 DM 有独立的会话。设置 `MATRIX_DM_MENTION_THREADS=true` 可在 DM 中被 `@提及` 时创建线程。 |
| **房间** | 默认情况下，Hades 需要 `@提及` 才会响应。设置 `MATRIX_REQUIRE_MENTION=false` 或将房间 ID 添加到 `MATRIX_FREE_RESPONSE_ROOMS` 可开启自由响应模式。房间邀请会被自动接受。 |
| **线程** | Hades 支持 Matrix 线程（MSC3440）。在线程中回复时，Hades 会将线程上下文与主房间时间线隔离。机器人已参与的线程无需提及即可响应。 |
| **自动线程** | 默认情况下，Hades 会为其在房间中响应的每条消息自动创建线程，以保持对话隔离。设置 `MATRIX_AUTO_THREAD=false` 可禁用此功能。设置 `MATRIX_DM_AUTO_THREAD=true`（默认 false）可同时为私聊消息自动创建线程——这与 `MATRIX_DM_MENTION_THREADS` 不同，后者仅在私聊中 @提及 Bot 时才创建线程。 |
| **多用户共享房间** | 默认情况下，Hades 在房间内按用户隔离会话历史。同一房间中的两个人不会共享同一对话记录，除非你明确禁用该功能。 |
...
2. 选择一个用户名，例如 `hades`——完整的用户 ID 将是 `@hades:your-server.org`。

### 方式 B：使用 matrix.org 或其他公共 Homeserver

1. 前往 [Element Web](https://app.element.io) 创建新账户。
2. 为机器人选择一个用户名（例如 `hades-bot`）。

### 方式 C：使用你自己的账户

你也可以以自己的用户身份运行 Hades。这意味着机器人以你的名义发帖——适合个人助手场景。

## 第二步：获取访问令牌

Hades 需要访问令牌（access token）来向 homeserver 进行身份验证。有两种方式：
...
你可以不提供访问令牌，而是提供机器人的用户 ID 和密码。Hades 会在启动时自动登录。这种方式更简单，但密码会存储在你的 `.env` 文件中。
...
:::tip
你可以在后台运行 `hades gateway`，或将其作为 systemd 服务以持续运行。详情请参阅部署文档。
:::

## 端对端加密（E2EE）

Hades 支持 Matrix 端对端加密，你可以在加密房间中与机器人聊天。
...
每次启动时，如果设置了 `MATRIX_RECOVERY_KEY`，Hades 会从 homeserver 的安全密钥存储中导入交叉签名密钥并对当前设备进行签名。此操作是幂等的，可以永久启用。
...
Hades 在启动时会检测到此情况并拒绝启用 E2EE，日志显示：`device XXXX has stale one-time keys on the server signed with a previous identity key`。
...
**原因**：主机系统时钟超前于实际时间。Matrix 适配器应用了 5 秒启动宽限过滤器（`event_ts < startup_ts - 5`）以忽略初始同步中重放的事件。当系统时钟超前时，每个传入事件看起来都"早于启动时间"，在到达消息处理器之前就被丢弃——机器人看起来已连接但从不回复。参见 [#12614](https://github.com/NousResearch/hades-agent/issues/12614)。
...
如果你之前使用 `MATRIX_ENCRYPTION=true` 运行 Hades，并正在升级到使用新的基于 SQLite 的加密存储的版本，机器人的加密身份已发生变化。你的 Matrix 客户端（Element）可能缓存了旧的设备密钥，并拒绝与机器人共享加密会话。
...
:::tip
**新安装不受影响。** 此迁移仅在你之前使用旧版 Hades 配置了可用的 E2EE 并正在升级时才需要。
...
Matrix E2EE 需要 `libolm`，而该库无法在 macOS ARM64（Apple Silicon）上编译。`hades-agent[matrix]` extra 仅限 Linux。如果你在 macOS 上，代理模式允许你在 Linux 虚拟机的 Docker 容器中运行 E2EE，而实际的 agent 在 macOS 上原生运行，可完整访问你的本地文件、记忆和技能。
...
| 设置 | 说明 |
|---------|-------------|
| `GATEWAY_PROXY_URL` | 远程 Hades API 服务器的 URL（例如 `http://192.168.1.100:8642`） |
...
:::tip
会话连续性通过 `X-Hades-Session-Id` 请求头维护。主机的 API 服务器按此 ID 跟踪会话，因此对话在消息之间持续存在，就像使用本地 agent 一样。
:::
...
**解决方法**：同步循环在出错时每 5 秒自动重试。检查 Hades 日志中与同步相关的警告。如果机器人持续落后，请确保你的 homeserver 有足够的资源。

### 机器人离线

**原因**：Hades gateway 未运行，或连接失败。

**解决方法**：检查 `hades gateway` 是否正在运行。查看终端输出中的错误消息。常见问题：homeserver URL 错误、访问令牌过期、homeserver 不可达。
...
- **媒体支持**：Hades 可以发送和接收图片、音频、视频和文件附件。媒体通过 Matrix 内容仓库 API 上传到你的 homeserver。
