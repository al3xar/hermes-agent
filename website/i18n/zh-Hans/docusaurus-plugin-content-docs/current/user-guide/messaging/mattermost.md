...
无需额外的 Mattermost 库——适配器使用 `aiohttp`，该库已作为 Hades 的依赖项包含在内。

在开始配置之前，先了解大多数人最关心的部分：Hades 进入你的 Mattermost 实例后的行为方式。

## Hades 的行为方式

| 场景 | 行为 |
|---------|----------|
| **私信（DM）** | Hades 响应每一条消息，无需 `@提及`。每个私信有独立的会话。 |
| **公开/私有频道** | Hades 仅在被 `@提及` 时响应。未被提及时，Hades 忽略消息。 |
| **线程（Thread）** | 若设置 `MATTERMOST_REPLY_MODE=thread`，Hades 在你的消息下方以线程形式回复。线程上下文与父频道隔离。 |
| **多用户共享频道** | 默认情况下，Hades 在频道内按用户隔离会话历史。同一频道中的两个人不会共享同一份对话记录，除非你明确禁用该设置。 |

:::tip
如果你希望 Hades 以线程对话方式回复（嵌套在原始消息下方），请设置 `MATTERMOST_REPLY_MODE=thread`。默认值为 `off`，即在频道中发送普通消息。
:::
...
:::tip
你也可以使用**个人访问 token** 代替机器人账户。前往**个人资料** → **安全** → **个人访问 Token** → **创建 Token**。如果你希望 Hades 以你自己的用户身份发帖而非独立的机器人用户，这种方式很有用。
:::
...
:::tip
你可以在后台运行 `hades gateway`，或将其配置为 systemd 服务以持续运行。详情参见部署文档。
:::
...
`MATTERMOST_REPLY_MODE` 设置控制 Hades 发布响应的方式：

| 模式 | 行为 |
|------|----------|
| `off`（默认） | Hades 在频道中发送普通消息，与普通用户一样。 |
| `thread` | Hades 在你的原始消息下方以线程形式回复。在大量来回交流时保持频道整洁。 |
...
**解决方法**：检查 `hades gateway` 是否正在运行。查看终端输出中的错误信息。常见问题：URL 错误、token 过期、Mattermost 服务器无法访问。
