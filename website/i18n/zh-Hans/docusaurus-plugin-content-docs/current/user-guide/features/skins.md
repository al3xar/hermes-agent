...
| 皮肤 | 描述 | Agent 品牌 | 视觉特征 |
|------|------|-----------|---------|
| `default` | 经典 Hades — 金色与 kawaii 风格 | `Hades Agent` | 暖金色边框，cornsilk 文字，spinner 中的 kawaii 面孔。熟悉的双蛇杖横幅。简洁亲切。 |
...
| `response_label` | 响应框标题上的标签 | ` ⚕ Hades ` |
...
| `banner_logo` | 字符串 | Rich 标记 ASCII 艺术 logo（替换默认的 HADES_AGENT 横幅） | `""` |
...
[Hades Mod](https://github.com/cocktailpeanut/hades-mod) 是一个社区构建的 Web UI，用于可视化创建和管理皮肤。无需手写 YAML，提供带实时预览的点击式编辑器。
...
- 列出所有内置和自定义皮肤
- 将任意皮肤在可视化编辑器中打开，涵盖所有 Hades 皮肤字段（颜色、spinner、品牌、工具前缀、工具 emoji）
...
Hades Mod 遵循 `HADES_HOME` 环境变量，因此也适用于[配置文件](/user-guide/profiles)。

## 操作说明

- 内置皮肤从 `hades_cli/skin_engine.py` 加载。
- 未知皮肤自动回退到 `default`。
