...
如需包含推荐配置和实际使用模式的实践指南，请参阅 [使用 Hades 的语音模式](/guides/use-voice-mode-with-hades)。

## 前提条件

使用语音功能前，请确保已完成以下准备：

1. **已安装 Hades Agent** — `pip install hades-agent`（参见 [安装](/getting-started/installation)）
2. **已配置 LLM 提供商** — 运行 `hades model` 或在 `~/.hades/.env` 中设置首选提供商的凭据
3. **基础设置正常** — 运行 `hades` 验证 Agent 能够响应文字消息，再启用语音功能

:::tip
`~/.hades/` 目录和默认的 `config.yaml` 会在首次运行 `hades` 时自动创建。只需手动创建 `~/.hades/.env` 来存放 API 密钥。
:::

:::tip Nous Portal 同时覆盖两项
付费的 [Nous Portal](/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`hades setup --portal` 可一次性完成两项配置。
:::
...
语音模式在**经典 CLI**（`hades chat`）和 **TUI**（`hades --tui`）中均可使用。两者行为完全一致 — 相同的斜杠命令、相同的 VAD（语音活动检测）静音检测、相同的流式 TTS、相同的幻觉过滤器。TUI 额外将崩溃诊断日志转发至 `~/.hades/logs/`，以便在异常音频后端出现按键录音失败时提供完整堆栈跟踪，而非静默消失。
...
1. 使用 `hades` 启动 CLI，并通过 `/voice on` 启用语音模式
...
```bash
hades gateway        # 启动 gateway（连接到已配置的平台）
hades gateway setup  # 首次配置的交互式设置向导
```
...
**服务器频道：** Bot 仅在被 @提及时响应（例如 `@hadesbyt4 你好`）。请确保从提及弹窗中选择 **Bot 用户**，而非同名角色。
