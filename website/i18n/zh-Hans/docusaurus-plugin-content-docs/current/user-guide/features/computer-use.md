...
使用 `hades computer-use status` 验证安装结果。

**方式二：通过交互式界面启用工具集。**

1. 运行 `hades tools`，选择 `🖱️ Computer Use (macOS)` → `cua-driver (background)`。
2. 安装程序将运行上游安装脚本（与方式一相同）。

安装完成后，无论采用哪种方式，继续执行以下步骤：

3. 在提示时授予 macOS 权限：
   - **系统设置 → 隐私与安全性 → 辅助功能** → 允许终端（或 Hades 应用）。
...
cua-driver 项目会定期发布修复（例如 v0.1.6 修复了 UTM 工作流中的 Safari 窗口焦点问题）。Hades 在两处刷新二进制文件，避免你停留在过时版本：

- **`hades update`** — 更新 Hades 本身时，如果 `cua-driver` 在 PATH 中，更新结束时会重新运行上游安装程序。对非 macOS 用户及未安装 cua-driver 的用户无操作。
- **`hades computer-use install --upgrade`** — 手动强制刷新。无论 cua-driver 是否已安装，都会重新运行上游安装程序。在不等待下次 Agent 更新的情况下获取最新修复时使用此命令。

`hades computer-use status` 会在二进制路径旁显示已安装的版本号。
...
- **私有 SPI 风险。** Apple 可能在任何 OS 更新中更改 SkyLight 的符号接口。如需在 macOS 版本升级时保持可复现性，请通过 `HADES_CUA_DRIVER_VERSION` 环境变量固定驱动版本。
...
**`computer_use backend unavailable: cua-driver is not installed`** — 运行 `hades computer-use install` 获取 cua-driver 二进制文件，或运行 `hades tools` 并启用 Computer Use 工具集。
...
- [通用技能：`macos-computer-use`](https://github.com/NousResearch/hades-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
