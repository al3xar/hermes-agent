...
sidebar_position: 17
title: "扩展 Dashboard"
description: "为 Hades Web Dashboard 构建主题和插件——调色板、字体排版、布局、自定义标签页、shell 插槽、页面级插槽以及后端 API 路由"
---

# 扩展 Dashboard

Hades Web Dashboard（`hades dashboard`）在设计上支持换肤和扩展，无需 fork 代码库。对外暴露三个层次：
...
CSS 在主题应用时以单个带作用域的 `<style data-hades-theme-css>` 标签注入，主题切换时清除。**每个主题上限为 32 KiB。**
...
| 主题 | 调色板 | 字体排版 | 布局 |
|-------|---------|------------|--------|
| **Hades Teal**（`default`） | 深青色 + 奶油色 | 系统字体栈，15px | 0.5rem 圆角，comfortable |
| **Hades Teal (Large)**（`default-large`） | 同 default | 系统字体栈，18px，行高 1.65 | 0.5rem 圆角，spacious |
...
引用 Google Fonts 的主题（除 Hades Teal 外均如此）会按需加载样式表——首次切换时会向 `<head>` 注入一个 `<link>` 标签。
...
Dashboard 插件是一个包含 `manifest.json`、预构建 JS bundle，以及可选的 CSS 文件和带 FastAPI 路由的 Python 文件的目录。插件与其他 Hades 插件一起存放在 `~/.hades/plugins/<name>/`——dashboard 扩展是该插件目录内的 `dashboard/` 子文件夹，因此一个插件可以从单次安装中同时扩展 CLI/gateway 和 dashboard。

插件不打包 React 或 UI 组件，而是使用暴露在 `window.__HADES_PLUGIN_SDK__` 上的 **Plugin SDK**。这使插件 bundle 保持极小体积（通常只有几 KB），并避免版本冲突。
...
插件所需的一切均在 `window.__HADES_PLUGIN_SDK__` 上。插件不应直接导入 React。
...
参考插件（[`hades-example-plugins`](https://github.com/NousResearch/hades-example-plugins/tree/main/example-dashboard) 中的 `example-dashboard`）提供了一个向 `sessions:top` 注入横幅的实时演示——安装它可端到端了解该模式。
...
后端路由在 dashboard 进程内运行，因此可以直接从 hades-agent 代码库导入：
...
| 3 | `./.hades/plugins/<name>/dashboard/` | `project`——仅在设置 `HADES_ENABLE_PROJECT_PLUGINS` 时生效 |
...
1. Dashboard 加载。`main.tsx` 在 `window.__HADES_PLUGIN_SDK__` 上暴露 SDK，在 `window.__HADES_PLUGINS__` 上暴露注册表。
2. `App.tsx` 调用 `usePlugins()` → 获取 `GET /api/dashboard/plugins`。
3. 对于每个 manifest：注入 CSS `<link>`（如已声明），然后通过 `<script>` 标签加载 JS bundle。
4. 插件的 IIFE 运行并调用 `window.__HADES_PLUGINS__.register(name, Component)`——以及可选的 `.registerSlot(name, slot, Component)` 用于每个插槽。
...
[`strike-freedom-cockpit`](https://github.com/NousResearch/hades-example-plugins/tree/main/strike-freedom-cockpit) 插件（伴随仓库 `hades-example-plugins`）是一个完整的换肤演示。它将主题 YAML 与仅插槽插件配对，在不 fork dashboard 的情况下生成驾驶舱风格的 HUD。
...
打开 dashboard，从主题切换器中选择 **Strike Freedom**。驾驶舱侧边栏出现，徽标显示在顶栏，标语替换底栏。切换回 **Hades Teal**，插件仍然安装但不可见（`sidebar` 插槽仅在 `cockpit` 布局变体下渲染）。
...
| 全局变量 | 类型 | 提供方 |
|--------|------|----------|
| `window.__HADES_PLUGIN_SDK__` | object | `registry.ts` — React、hooks、UI 组件、API 客户端、工具函数。 |
| `window.__HADES_PLUGINS__.register(name, Component)` | function | 注册插件的主组件。 |
| `window.__HADES_PLUGINS__.registerSlot(name, slot, Component)` | function | 注册到命名 shell 插槽。 |
...
4. 打开浏览器开发工具 → Console——查找 IIFE 执行期间的错误或 `window.__HADES_PLUGINS__ is undefined`（表示 SDK 未初始化，通常是更早的 React 渲染崩溃导致）。
5. 验证你的 bundle 以与 `manifest.json:name` **相同的名称**调用 `window.__HADES_PLUGINS__.register(...)`。
...
2. 重启 `hades dashboard`——插件 API 路由在启动时挂载一次，**不会**在重新扫描时挂载。
