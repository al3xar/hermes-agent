...
运行 `hades model` 并选择 Nous Portal 作为提供商时，Hades 会主动询问是否启用 Tool Gateway：
...
网关使用 Nous Portal 凭据认证（在 `hades model` 完成后写入 `~/.hades/auth.json`）。
...
在 `hades tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.nousresearch.com/manage-subscription) 或通过 `hades tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `hades setup terminal` 或在 `config.yaml` 中配置。
