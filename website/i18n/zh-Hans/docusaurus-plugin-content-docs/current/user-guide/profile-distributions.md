...
……他们就拥有了完整的 agent。填入自己的 API 密钥（`.env.EXAMPLE` → `.env`），即可运行 `my-research-agent chat`，或通过 Telegram / Discord / Slack / 任何 gateway 平台与其交互。当你推送新版本时，他们运行 `hades profile update my-research-agent` 即可拉取你的更改——他们的记忆和会话保持不变。
...
权衡之处：接收方需要安装 git。在 2026 年运行 Hades 的任何机器上，这已是既成事实。
...
- **你只想在自己的机器上备份一个 profile。** 使用 [`hades profile export` / `import`](../reference/profile-commands.md#hades-profile-export)——那正是这两个命令的用途。
...
运行 `hades profile update research-bot` 的接收方将拉取最新版本。
...
`hades profile list` 还会显示 `Distribution` 列，让你一眼看出哪些 profile 来自仓库，哪些是手动构建的：
...
在笔记本上的任何迭代（`git commit && push`）都可以通过 `hades profile update research-bot` 同步到工作站。记忆按机器独立保存——笔记本记住自己的对话，工作站记住自己的，互不干扰。
...
当负责人发布 v1.1（更好的 SOUL、新技能）时，工程师运行 `hades profile update pr-reviewer`，所有人在几分钟内就能用上新版本。
...
你在 Hades 之上构建了产品——也许是合规监控框架、客服技术栈、特定领域的研究平台。你想以产品形式分发它。
...
:::note
Git ref 固定（`#v1.2.0`）已在规划中，但不在初始版本中——目前安装时跟踪默认分支。通过 `hades profile info <name>` 查看已安装版本，在准备好之前暂缓更新。
:::
...
来自分发的 cron 任务**不会自动调度**——安装器会打印 `hades -p <name> cron list`，你需要显式启用它们。SOUL.md 和技能在你开始与 profile 对话后立即生效，因此如果你从不熟悉的来源安装，请在第一次运行前阅读它们。
...
- `install`、`update`、`info` 位于 `hades profile` 下——不是独立的命令树。
...
- 保留的 profile 名称（`hades`、`test`、`tmp`、`root`、`sudo`）在安装时会被拒绝，以避免与常见二进制文件冲突。

## 另请参阅

- [Profiles：运行多个 Agent](./profiles.md) — 基础概念
- [Profile 命令参考](../reference/profile-commands.md) — 每个标志、每个选项
- [`hades profile export` / `import`](../reference/profile-commands.md#hades-profile-export) — 本地备份 / 恢复（非分发）
- [在 Hades 中使用 SOUL](../guides/use-soul-with-hades.md) — 编写个性
