...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/autonomous-ai-agents/hades-agent` |
| 版本 | `2.1.0` |
| 作者 | Hades Agent + Teknium |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `hades`, `setup`, `configuration`, `multi-agent`, `spawning`, `cli`, `gateway`, `development` |
...
:::info
以下是 Hades 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::

# Hades Agent

Hades Agent 是 Nous Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Hades 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。

Hades 的差异化特性：

- **通过 skill 自我提升** — Hades 通过将可复用流程保存为 skill 来从经验中学习。当它解决复杂问题、发现工作流或被纠正时，可以将该知识持久化为 skill 文档，加载到未来的会话中。skill 随时间积累，使 agent 在你的特定任务和环境中表现越来越好。
...
- **Profiles（配置文件）** — 运行多个独立的 Hades 实例，各自拥有隔离的配置、会话、skill 和记忆。
- **可扩展** — 插件、MCP 服务器、自定义工具、webhook 触发器、cron 调度以及完整的 Python 生态系统。

人们将 Hades 用于软件开发、研究、系统管理、数据分析、内容创作、家庭自动化，以及任何受益于具有持久上下文和完整系统访问权限的 AI agent 的场景。

**此 skill 帮助你高效使用 Hades Agent** — 包括设置、配置功能、生成额外的 agent 实例、排查问题、找到正确的命令和设置，以及在需要扩展或贡献时理解系统的工作原理。

**文档：** https://hades-agent.nousresearch.com/docs/
...
# 单次查询
hades chat -q "What is the capital of France?"
...
```
hades setup [section]      Interactive wizard (model|terminal|gateway|tools|agent)
hades model                Interactive model/provider picker
hades config               View current config
hades config edit          Open config.yaml in $EDITOR
hades config set KEY VAL   Set a config value
hades config path          Print config.yaml path
hades config env-path      Print .env path
hades config check         Check for missing/outdated config
hades config migrate       Update config with new options
hades auth                 交互式凭据管理器
hades auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
hades auth list            列出已存储的凭据
hades auth remove PROVIDER 移除已存储的凭据
hades doctor [--fix]       Check dependencies and config
hades status [--all]       Show component status
```

### 工具与 Skill

```
hades tools                Interactive tool enable/disable (curses UI)
hades tools list           Show all tools and status
hades tools enable NAME    Enable a toolset
hades tools disable NAME   Disable a toolset

hades skills list          List installed skills
hades skills search QUERY  Search the skills hub
hades skills install ID    Install a skill (ID can be a hub identifier OR a direct https://…/SKILL.md URL; pass --name to override when frontmatter has no name)
hades skills inspect ID    Preview without installing
hades skills config        Enable/disable skills per platform
hades skills check         Check for updates
hades skills update        Update outdated skills
hades skills uninstall N   Remove a hub skill
hades skills publish PATH  Publish to registry
hades skills browse        Browse all available skills
hades skills tap add REPO  Add a GitHub repo as skill source
```
...
hades mcp test NAME        Test connection
hades mcp configure NAME   Toggle tool selection
```

### Gateway（消息平台）

```
hades gateway run          Start gateway foreground
hades gateway install      Install as background service
hades gateway start/stop   Control the service
hades gateway restart      Restart the service
hades gateway status       Check status
hades gateway setup        Configure platforms
```
...
平台文档：https://hades-agent.nousresearch.com/docs/user-guide/messaging/

### 会话

```
hades sessions list        List recent sessions
hades sessions browse      Interactive picker
hades sessions export OUT  Export to JSONL
hades sessions rename ID T Rename a session
hades sessions delete ID   Delete a session
hades sessions prune       Clean up old sessions (--older-than N days)
hades sessions stats       Session store statistics
```

### Cron 任务

```
hades cron list            List jobs (--all for disabled)
hades cron create SCHED    Create: '30m', 'every 2h', '0 9 * * *'
hades cron edit ID         Edit schedule, prompt, delivery
hades cron pause/resume ID Control job state
hades cron run ID          Trigger on next tick
hades cron remove ID       Delete a job
hades cron status          Scheduler status
```

### Webhook

```
hades webhook subscribe N  Create route at /webhooks/<name>
hades webhook list         List subscriptions
hades webhook remove NAME  Remove a subscription
hades webhook test NAME    Send a test POST
```

### Profiles

```
hades profile list         List all profiles
hades profile create NAME  Create (--clone, --clone-all, --clone-from)
hades profile use NAME     Set sticky default
hades profile delete NAME  Delete a profile
hades profile show NAME    Show details
hades profile alias NAME   Manage wrapper scripts
hades profile rename A B   Rename a profile
hades profile export NAME  Export to tar.gz
hades profile import FILE  Import from archive
```

### 凭证池

```
hades auth add             Interactive credential wizard
hades auth list [PROVIDER] List pooled credentials
hades auth remove P INDEX  Remove by provider + index
hades auth reset PROVIDER  Clear exhaustion status
```

### 其他

```
hades insights [--days N]  Usage analytics
hades update               Update to latest version
hades pairing list/approve/revoke  DM authorization
hades plugins list/install/remove  Plugin management
hades honcho setup/status  Honcho memory integration (requires honcho plugin)
hades memory setup/status/off  Memory provider config
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://hades-agent.nousresearch.com/docs/reference/slash-commands)。命令注册表的权威来源是 `hades_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
~/.hades/hades-agent/     Source code (if git-installed)
```
...
使用 `hades config edit` 或 `hades config set section.key value` 编辑。
...
完整配置参考：https://hades-agent.nousresearch.com/docs/user-guide/configuration

### 提供商

支持 20+ 个提供商。通过 `hades model` 或 `hades setup` 设置。
...
完整提供商文档：https://hades-agent.nousresearch.com/docs/integrations/providers

### Toolset

通过 `hades tools`（交互式）或 `hades tools enable/disable NAME` 启用/禁用。
...
常见的"为什么 Hades 对我的输出/工具调用/命令做了 X？"开关——以及更改它们的确切命令。其中大多数需要新会话（聊天中的 `/reset`，或启动新的 `hades` 调用），因为它们在启动时只读取一次。

### 工具输出中的密钥脱敏

密钥脱敏**默认关闭** — 工具输出（终端 stdout、`read_file`、网页内容、子 agent 摘要等）不经修改直接传递。如果用户希望 Hades 在 API key、token 和密钥进入对话上下文和日志之前自动屏蔽它们：

```bash
hades config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export HADES_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `hades config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
```bash
hades config set privacy.redact_pii true    # 启用
hades config set privacy.redact_pii false   # 禁用（默认）
```

### 命令审批提示

默认情况下（`approvals.mode: manual`），Hades 在运行被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）之前会提示用户。模式如下：
...
```bash
hades config set approvals.mode smart       # 推荐的折中方案
hades config set approvals.mode off         # 绕过一切（不推荐）
```
...
要完全阻止模型访问网络或媒体工具，打开 `hades tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
将额外的 Hades 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。

### 何时使用此方式 vs delegate_task

| | `delegate_task` | 生成 `hades` 进程 |
|-|-----------------|--------------------------|
...
```
terminal(command="hades chat -q 'Research GRPO papers and write summary to ~/research/grpo.md'", timeout=300)

# 长任务后台运行：
terminal(command="hades chat -q 'Set up CI/CD for ~/myapp'", background=true)
```

### 交互式 PTY 模式（通过 tmux）

Hades 使用 prompt_toolkit，需要真实终端。使用 tmux 进行交互式生成：

```
# 启动
terminal(command="tmux new-session -d -s agent1 -x 120 -y 40 'hades'", timeout=10)
...
```
# Agent A：后端
terminal(command="tmux new-session -d -s backend -x 120 -y 40 'hades -w'", timeout=10)
terminal(command="sleep 8 && tmux send-keys -t backend 'Build REST API for user management' Enter", timeout=15)

# Agent B：前端
terminal(command="tmux new-session -d -s frontend -x 120 -y 40 'hades -w'", timeout=10)
...
```
# 恢复最近的会话
terminal(command="tmux new-session -d -s resumed 'hades --continue'", timeout=10)

# 恢复特定会话
terminal(command="tmux new-session -d -s resumed 'hades --resume 20260225_143052_a1b2c3'", timeout=10)
```
...
- **fire-and-forget 使用 `hades chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`hades cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
用户文档：https://hades-agent.nousresearch.com/docs/user-guide/features/cron

### Curator（skill 生命周期）

agent 创建的 skill 的后台维护。跟踪使用情况，将闲置 skill 标记为过时，归档过时的 skill，保留运行前的 tar.gz 备份以防数据丢失。

- **CLI：** `hades curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
用户文档：https://hades-agent.nousresearch.com/docs/user-guide/features/curator

### Kanban（多 agent 工作队列）

用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `hades kanban <verb>` 驱动；调度器生成的 worker 看到由 `HADES_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
- **隔离：** 看板是硬边界（worker 在环境中固定 `HADES_KANBAN_BOARD`）；租户是看板内用于工作区路径和记忆键隔离的软命名空间。

用户文档：https://hades-agent.nousresearch.com/docs/user-guide/features/kanban

---

## Windows 特有问题

Hades 在 Windows 上原生运行（PowerShell、cmd、Windows Terminal、git-bash mintty、VS Code 集成终端）。大多数功能开箱即用，但 Win32 和 POSIX 之间有一些差异曾给我们带来麻烦——遇到新问题时请在此记录，以免下一个人（或下一个会话）重新踩坑。
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`hades config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。

### `execute_code` / 沙箱

**WinError 10106**（"无法加载或初始化请求的服务提供商"）来自沙箱子进程——它无法创建 `AF_INET` socket，因此回退的 loopback-TCP RPC 在 `connect()` 之前失败。根本原因通常**不是**损坏的 Winsock LSP；而是 Hades 自身的环境清理器从子进程环境中删除了 `SYSTEMROOT` / `WINDIR` / `COMSPEC`。Python 的 `socket` 模块需要 `SYSTEMROOT` 来定位 `mswsock.dll`。通过 `tools/code_execution_tool.py` 中的 `_WINDOWS_ESSENTIAL_ENV_VARS` 允许列表修复。如果仍然遇到此问题，在 `execute_code` 块内 echo `os.environ` 以确认 `SYSTEMROOT` 已设置。完整诊断方案见 `references/execute-code-sandbox-env-windows.md`。

### 测试/贡献

**`scripts/run_tests.sh` 在 Windows 上无法直接使用** — 它查找 POSIX venv 布局（`.venv/bin/activate`）。Hades 安装的 venv 位于 `venv/Scripts/`，也没有 pip 或 pytest（为减小安装体积而精简）。解决方案：将 `pytest + pytest-xdist + pyyaml` 安装到系统 Python 3.11 用户站点，然后设置 `PYTHONPATH` 直接调用 pytest：
...
**正斜杠几乎在所有地方都有效。** `C:/Users/...` 被每个 Hades 工具和大多数 Windows API 接受。在代码和日志中优先使用正斜杠——避免在 bash 中转义反斜杠。
...
### 工具不可用
1. `hades tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `hades doctor` — 检查配置和依赖
2. `hades auth` — 重新认证 OAuth 提供商（或 `hades auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `hades model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `hades skills list` — 验证已安装
2. `hades skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `hades -s name`
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `hades config edit` 或[配置文档](https://hades-agent.nousresearch.com/docs/user-guide/configuration) |
| 可用工具 | `hades tools list` 或[工具参考](https://hades-agent.nousresearch.com/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://hades-agent.nousresearch.com/docs/reference/slash-commands) |
| Skill 目录 | `hades skills browse` 或[Skill 目录](https://hades-agent.nousresearch.com/docs/reference/skills-catalog) |
| 提供商设置 | `hades model` 或[提供商指南](https://hades-agent.nousresearch.com/docs/integrations/providers) |
| 平台设置 | `hades gateway setup` 或[消息文档](https://hades-agent.nousresearch.com/docs/user-guide/messaging/) |
| MCP 服务器 | `hades mcp list` 或[MCP 指南](https://hades-agent.nousresearch.com/docs/user-guide/features/mcp) |
| Profiles | `hades profile list` 或[Profiles 文档](https://hades-agent.nousresearch.com/docs/user-guide/profiles) |
| Cron 任务 | `hades cron list` 或[Cron 文档](https://hades-agent.nousresearch.com/docs/user-guide/features/cron) |
| 记忆 | `hades memory status` 或[记忆文档](https://hades-agent.nousresearch.com/docs/user-guide/features/memory) |
| 环境变量 | `hades config env-path` 或[环境变量参考](https://hades-agent.nousresearch.com/docs/reference/environment-variables) |
| CLI 命令 | `hades --help` 或[CLI 参考](https://hades-agent.nousresearch.com/docs/reference/cli-commands) |
| Gateway 日志 | `~/.hades/logs/gateway.log` |
| 会话文件 | `~/.hades/sessions/` 或 `hades sessions browse` |
| 源代码 | `~/.hades/hades-agent/` |

---

## 贡献者快速参考

面向偶尔贡献者和 PR 作者。完整开发者文档：https://hades-agent.nousresearch.com/docs/developer-guide/
...
1. 在 `hades_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- POSIX 文件模式（0o600 等）→ `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")`（参见 `tests/hades_cli/test_auth_toctou_file_modes.py`）
- `signal.SIGALRM` → 仅 Unix（参见 `tests/conftest.py::_enforce_test_timeout`）
...
- 所有路径使用 `hades_constants` 中的 `get_hades_home()`（profile 安全）
