...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/software-development/hades-agent-skill-authoring` |
| 版本 | `1.0.0` |
| 作者 | Hades Agent |
| 许可证 | MIT |
| 平台 | linux, macos, windows |
| 标签 | `skills`, `authoring`, `hades-agent`, `conventions`, `skill-md` |
...
2. **仓库内（本 skill 讨论此情况）：** `/home/bb/hades-agent/skills/<category>/<name>/SKILL.md` — 已提交，随包一起发布。使用 `write_file` + `git add`。`skill_manage(action='create')` **不**针对此目录树。

## 使用时机

- 用户要求你"在此分支 / 仓库 / 提交中"添加一个 skill
- 你正在提交一个应随 hades-agent 一起发布的可复用工作流
- 你正在编辑 `/home/bb/hades-agent/skills/` 下的现有 skill（小改动用 `patch`，重写用 `write_file`；`skill_manage` 对仓库内 skill 的 `patch` 仍有效，但 `create` 无效）
...
`metadata.hades.related_skills` 在加载时会合并两个目录树（仓库内 `skills/` 和 `~/.hades/skills/`）。你**可以**从仓库内 skill 引用用户本地 skill，但对于全新克隆仓库的其他用户，该引用无法解析。仓库内 skill 优先只引用仓库内 skill。如果某个频繁被引用的 skill 仅存在于 `~/.hades/skills/`，请考虑将其提升到仓库中。
...
- [ ] `name`、`description`、`version`、`author`、`license`、`metadata.hades.{tags, related_skills}` 均已填写
- [ ] Name ≤ 64 个字符，小写加连字符
