...
| | |
|---|---|
| 来源 | 可选 — 通过 `hades skills install official/research/osint-investigation` 安装 |
...
:::info
以下是 Hades 在触发该 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::
...
# 之后在本地搜索。缓存 30 天，存储于
# $HADES_OSINT_CACHE/icij/（默认：~/.cache/hades-osint/icij/）。
...
# 设置 HADES_OSINT_UA=your-app/1.0 (your@email) 以标识自身
