...
| | |
|---|---|
| 来源 | 可选 — 通过 `hades skills install official/research/darwinian-evolver` 安装 |
...
调用来调用它（纯聚合方式）。**不得**将上游类导入 Hades 本身。
...
7. **AGPL 协议。** 永远不要在 Hades 核心中使用 `from darwinian_evolver import ...`。`~/.hades/skills/...` 下的自定义驱动脚本属于用户侧，没有问题。
