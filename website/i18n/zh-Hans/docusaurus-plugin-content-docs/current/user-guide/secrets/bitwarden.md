...
2. Hades 将该单一令牌以 `BWS_ACCESS_TOKEN` 的形式存储在 `~/.hades/.env` 中。
3. 每次 `hades`（或 gateway，或 cron 任务）启动时，在加载 `~/.hades/.env` 之后，Hades 会调用 `bws secret list <project_id>` 并将返回的密钥写入 `os.environ`。
4. 默认情况下，Hades **覆盖**环境中已有的值，因此 Bitwarden 是唯一可信来源——在 Web 应用中轮换一次密钥，每个 Hades 进程在下次启动时即可获取最新值。如果希望 `.env` 优先，可在配置中将 `override_existing: false`。
...
2. 创建或选择一个**项目**（例如"Hades keys"）。
3. 将提供商密钥添加为 secret。secret 的**名称**将成为环境变量名——使用 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等。
4. **Machine accounts → New machine account → My Hades machine** → **Projects** 标签页 → 授予对你的项目的 Read 权限。
...
此后，每次调用 `hades` 都会在启动时拉取最新 secret。进程中首次应用 secret 时，stderr 会显示一行摘要信息。

## CLI

| 命令 | 功能 |
|---|---|
| `hades secrets bitwarden setup` | 交互式向导（安装二进制文件、提示输入令牌、选择项目、测试拉取） |
| `hades secrets bitwarden status` | 显示配置、二进制版本及令牌是否存在 |
| `hades secrets bitwarden sync` | 演习模式：立即拉取 secret 并显示将应用的内容 |
| `hades secrets bitwarden sync --apply` | 拉取并导出到当前 shell 的环境中 |
| `hades secrets bitwarden install` | 仅下载固定版本的 `bws` 二进制文件（无需认证） |
| `hades secrets bitwarden disable` | 将 `enabled` 设为 `false`；保留令牌和项目 ID |
...
| `cache_ttl_seconds` | `300` | 进程内拉取结果的复用时长。设为 `0` 可禁用缓存。缓存按进程隔离；新的 `hades` 调用从头开始。 |
...
Bitwarden 永远不会阻塞 Hades 启动。如果出现任何问题，stderr 会显示一行警告，Hades 继续使用 `.env` 中已有的凭据：

| 现象 | 原因 | 修复方法 |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | 配置中已启用，但令牌已从 `.env` 中清除 | 重新运行 `hades secrets bitwarden setup` |
...
- 即使 `override_existing: true`，Hades 也会拒绝让 Bitwarden 覆盖引导令牌本身。如果你将 `BWS_ACCESS_TOKEN` 作为 secret 存储在项目中，应用时会静默跳过。
- `bws` 二进制文件的下载会与同一 GitHub release 中发布的 SHA-256 校验和进行验证。不匹配时将中止安装。
- 固定版本（撰写本文时为 `bws v2.0.0`）通过向本仓库提交 PR 的方式更新——Hades 不会将 `bws` 自动升级到"最新版本"，因为上游 release 的结构可能发生变化。
...
适合使用此功能的场景：多机器集群、共享开发机、gateway VPS，或任何需要跨多个 Hades 安装进行集中轮换和吊销管理的场景。