...
:::warning 仅限内置核心工具
本页面用于向仓库本身添加 **Hades 内置工具**。
如果你想要个人专用、项目本地或其他自定义工具，而不修改 Hades 核心，请使用插件方式：

- [插件](/user-guide/features/plugins)
- [构建 Hades 插件](/guides/build-a-hades-plugin)
...
如果你的工具需要 API 密钥，将其添加到 `hades_cli/config.py`：
...
- [ ] 可选：已将 API 密钥添加到 `hades_cli/config.py` 的 `OPTIONAL_ENV_VARS`
- [ ] 可选：已添加到 `toolset_distributions.py` 以支持批量处理
- [ ] 已通过 `hades chat -q "Use the weather tool for London"` 测试