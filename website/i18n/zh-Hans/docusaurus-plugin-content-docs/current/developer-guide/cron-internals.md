...
sidebar_position: 11
title: "Cron 内部机制"
description: "Hades 如何存储、调度、编辑、暂停、加载技能以及投递 cron 任务"
---
...
| `hades_cli/cron.py` | CLI `hades cron` 子命令 |
...
在 CLI 模式下，cron 任务仅在运行 `hades cron` 命令或活跃 CLI 会话期间触发。
...
2. **环境变量** — `HADES_CRON_SCRIPT_TIMEOUT`
...
调度器使用跨进程文件锁（Unix 上的 `fcntl.flock`，Windows 上的 `msvcrt.locking`）防止重叠的 tick 对同一批到期任务执行两次——即使在 gateway 的进程内 ticker 与独立的 `hades cron` / 手动 `tick()` 调用之间也如此。若无法获取锁，`tick()` 立即返回 0。

## CLI 接口

`hades cron` CLI 提供直接的任务管理功能：

```bash
hades cron list                    # 显示所有任务
hades cron create                  # 交互式创建任务（别名：add）
hades cron edit <job_id>           # 编辑任务配置
hades cron pause <job_id>          # 暂停运行中的任务
hades cron resume <job_id>         # 恢复已暂停的任务
hades cron run <job_id>            # 触发立即执行
