...
ACP 适配器将 Hades 的同步 `AIAgent` 封装为异步 JSON-RPC stdio 服务器。
...
`acp_adapter/tools.py` 将 Hades 工具映射到 ACP 工具类型，并构建面向编辑器的内容。
...
因此 ACP 通告并使用当前配置的 Hades provider/凭据。它还始终通告一个终端 setup 认证方法（`hades-setup`，参数 `--setup`），以便首次运行的 registry 客户端在启动正常 ACP 会话前可以打开 Hades 的交互式模型/provider 配置。
...
- `tests/acp/` — ACP 测试套件
- `toolsets.py` — `hades-acp` toolset 定义
- `hades_cli/main.py` — `hades acp` CLI 子命令
- `pyproject.toml` — `[acp]` 可选依赖 + `hades-acp` 脚本