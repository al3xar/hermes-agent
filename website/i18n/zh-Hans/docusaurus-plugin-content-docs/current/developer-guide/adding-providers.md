...
Hades 已经可以通过自定义 provider 路径与任何 OpenAI 兼容的端点通信。除非你需要为某个服务提供一流的用户体验，否则不要添加内置 provider：

- provider 专属的认证或 token 刷新
- 精选的模型目录
- setup / `hades model` 菜单条目
...
1. `hades_cli/auth.py` 决定如何查找凭据。
2. `hades_cli/runtime_provider.py` 将其转换为运行时数据：
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `hades_cli/models.py` 和 `hades_cli/main.py` 使 provider 在 CLI 中可见。（`hades_cli/setup.py` 自动委托给 `main.py`——无需在此处做任何修改。）
...
:::tip
`hades_cli/setup.py` **无需**修改。setup 向导将 provider/model 选择委托给 `main.py` 中的 `select_provider_and_model()`——在那里添加的任何 provider 都会自动出现在 `hades setup` 中。
:::
...
7. `hades model` 菜单包含该 provider
8. `hades setup` 向导自动委托给 `main.py`
...
- 带有特定认证流程的 provider 专属 `hades model` 菜单条目
...
- `hades_cli/auth.py` 中的 `PROVIDER_REGISTRY`
- `hades_cli/models.py` 中的 `_PROVIDER_LABELS`
- `hades_cli/auth.py` 和 `hades_cli/models.py` 中的 `_PROVIDER_ALIASES`
- `hades_cli/main.py` 中的 CLI `--provider` 选项
- setup / 模型选择分支
- 辅助模型默认值
- 测试
...
## 第 2 步：在 `hades_cli/auth.py` 中添加认证元数据
...
## 第 3 步：在 `hades_cli/models.py` 中添加模型目录和别名
...
## 第 4 步：在 `hades_cli/runtime_provider.py` 中解析运行时数据
...
注意 API key 优先级。Hades 已经包含避免将 OpenRouter key 泄露给无关端点的逻辑。新 provider 应同样明确地指定哪个 key 对应哪个 base URL。

## 第 5 步：在 `hades_cli/main.py` 中接线 CLI

在交互式 `hades model` 流程中出现之前，provider 是不可发现的。

在 `hades_cli/main.py` 中更新以下内容：
...
:::tip
`hades_cli/setup.py` 无需修改——它调用 `main.py` 中的 `select_provider_and_model()`，因此你的新 provider 会自动出现在 `hades model` 和 `hades setup` 中。
:::
...
添加原生 provider 时，仔细检查 Hades 只向该 provider 发送它实际理解的字段。
...
- [ ] 在 `hades_cli/auth.py` 中添加 `ProviderConfig`
- [ ] 在 `hades_cli/auth.py` 和 `hades_cli/models.py` 中添加别名
- [ ] 在 `hades_cli/models.py` 中添加模型目录
- [ ] 在 `hades_cli/runtime_provider.py` 中添加运行时分支
- [ ] 在 `hades_cli/main.py` 中添加 CLI 接线（setup.py 自动继承）
...
### 7. 更新了 `hades model` 但未更新 `hades setup`
