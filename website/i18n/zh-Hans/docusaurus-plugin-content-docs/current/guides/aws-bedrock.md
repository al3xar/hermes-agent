...
- **boto3** — 通过 `pip install hades-agent[bedrock]` 安装
- **IAM 权限** — 至少需要：
...
:::tip EC2 / ECS / Lambda
在 AWS 计算环境中，为实例附加带有 `AmazonBedrockFullAccess` 的 IAM 角色即可。无需 API 密钥，无需 `.env` 配置——Hades 会自动检测实例角色。
:::
...
运行 `hades model` 后，你的 `~/.hades/config.yaml` 将包含以下内容：
...
Hades 通过 Bedrock 控制平面自动发现可用模型。你可以自定义发现行为：
...
Bedrock 模型使用**推理配置文件 ID** 进行按需调用。`hades model` 选择器会自动显示这些 ID，并将推荐模型置于顶部：
...
Bedrock 可与所有 Hades gateway 平台配合使用（Telegram、Discord、Slack、飞书等）。将 Bedrock 配置为提供商后，正常启动 gateway 即可：
...
你已触及 Bedrock 单模型速率限制。Hades 会自动进行退避重试。如需提高限额，请在 [AWS Service Quotas 控制台](https://console.aws.amazon.com/servicequotas/)申请配额提升。

## 一键 AWS 部署

如需在 EC2 上通过 CloudFormation 进行全自动部署：

**[sample-hades-agent-on-aws-with-bedrock](https://github.com/JiaDe-Wu/sample-hades-agent-on-aws-with-bedrock)** — 自动创建 VPC、IAM 角色、EC2 实例并配置 Bedrock。一键即可在任意区域完成部署。