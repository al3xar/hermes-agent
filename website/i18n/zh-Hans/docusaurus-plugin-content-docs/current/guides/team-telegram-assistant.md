...
:::tip
一台 $5/月的 VPS 足以运行 gateway（网关）。Hades 本身很轻量——花钱的是 LLM API 调用，而那些调用发生在远端。
:::
...
2. **发送 `/newbot`**——BotFather 会询问两件事：
   - **显示名称**——用户看到的名字（例如 `Team Hades Assistant`）
   - **用户名**——必须以 `bot` 结尾（例如 `myteam_hades_bot`）
...
sudo hades gateway install --system   # 仅 Linux：开机启动的系统服务
```
...
:::tip macOS PATH
launchd plist 在安装时捕获你的 Shell PATH，以便 gateway 子进程能找到 Node.js 和 ffmpeg 等工具。如果之后安装了新工具，请重新运行 `hades gateway install` 以更新 plist。
:::
...
完整指南请参阅[在 Hades 中使用 SOUL.md](/guides/use-soul-with-hades)。
...
```bash
# 通过 CLI
hades cron list          # 查看所有定时任务
hades cron status        # 检查调度器是否运行
