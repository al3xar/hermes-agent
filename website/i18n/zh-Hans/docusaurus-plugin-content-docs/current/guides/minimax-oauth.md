...
Hades Agent 通过基于浏览器的 OAuth 登录流程支持 **MiniMax**，使用与 [MiniMax 门户](https://www.minimax.io) 相同的凭据。无需 API 密钥或信用卡——登录一次，Hades 即可自动刷新您的会话。
...
```bash
hades auth add minimax-cn --type oauth   # 如果您的中国账户支持 OAuth
# 或更简单的方式：
...
Hades 将打印验证 URL 和用户码——在任意设备上打开该 URL，并在提示时输入用户码。

## OAuth 流程

Hades 针对 MiniMax OAuth 端点实现了 PKCE 设备码流程：

1. Hades 生成 PKCE verifier/challenge 对和一个随机 state 值。
...
4. Hades 轮询 `{base_url}/oauth/token`，直到令牌到达（或超过截止时间）。
...
```bash
hades config set model MiniMax-M2.7
hades config set provider minimax-oauth
```
...
```bash
hades --provider minimax-oauth    # 规范名称
hades --provider minimax-portal   # 别名
hades --provider minimax-global   # 别名
hades --provider minimax_oauth    # 别名（下划线形式）
```
...
要将 `minimax-oauth` 设为活跃 provider，请在 `config.yaml` 中设置 `model.provider: minimax-oauth`（使用 `hades setup` 进行引导式配置），或在单次调用时传入 `--provider minimax-oauth`：
...
Hades 在每次会话启动时，若 access token 距过期不足 60 秒则刷新令牌。如果 access token 已经过期（例如长时间离线后），刷新将在下一次请求时自动触发。如果刷新失败并返回 `refresh_token_reused` 或 `invalid_grant`，Hades 会将会话标记为需要重新登录。

当刷新失败为终态（HTTP 4xx、`invalid_grant`、授权已撤销等）时，Hades 将 refresh token 标记为失效并在本地隔离，避免持续重放注定失败的交换。Agent 会显示一条"需要重新认证"的消息，并在您再次登录之前保持等待。

**解决方法：** 再次运行 `hades auth add minimax-oauth` 以开始全新登录。下一次成功交换后隔离状态将自动清除。

### 授权超时

设备码流程有有限的过期窗口。如果您未在规定时间内批准登录，Hades 将抛出超时错误。

**解决方法：** 重新运行 `hades auth add minimax-oauth`（或 `hades model`）。流程将重新开始。

### State 不匹配（可能的 CSRF）

Hades 检测到授权服务器返回的 `state` 值与其发送的值不匹配。

**解决方法：** 重新运行登录。如果问题持续，请检查是否有代理或重定向正在修改 OAuth 响应。

### 从远程服务器登录

如果 `hades` 无法打开浏览器窗口，请使用 `--no-browser`：

```bash
hades auth add minimax-oauth --no-browser
```

Hades 将打印 URL 和用户码。在任意设备上打开该 URL 并在那里完成流程。

### 运行时出现"未登录 MiniMax OAuth"错误

auth 存储中没有 `minimax-oauth` 的凭据。您尚未登录，或凭据文件已被删除。

**解决方法：** 运行 `hades model` 并选择 MiniMax (OAuth)，或运行 `hades auth add minimax-oauth`。
