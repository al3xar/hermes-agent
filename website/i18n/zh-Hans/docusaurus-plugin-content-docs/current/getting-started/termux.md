...
这并不妨碍 Hades 作为手机原生 CLI agent 正常工作——只是意味着推荐的移动端安装有意比桌面/服务器安装更精简。

---

## 方式一：一行安装命令

Hades 现已内置 Termux 感知的安装路径：
...
- 将 `hades` 链接到 `$PREFIX/bin`，使其保留在 Termux PATH 中
...
### 5. 将 `hades` 添加到 Termux PATH

```bash
ln -sf "$PWD/venv/bin/hades" "$PREFIX/bin/hades"
```

`$PREFIX/bin` 在 Termux 中已默认在 PATH 中，因此这样做可以让 `hades` 命令在新 shell 中持续可用，无需每次重新激活虚拟环境。
...
### `hades doctor` 提示缺少 ripgrep 或 Node
