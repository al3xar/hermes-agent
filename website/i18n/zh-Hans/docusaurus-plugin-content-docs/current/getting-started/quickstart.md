...
本指南带你从零开始搭建一个能够应对实际使用的 Hades 环境。完成安装、选择 provider（服务提供商）、验证对话正常运行，并了解出现问题时的处理方法。
...
| 目标 | 先做这步 | 再做这步 |
|---|---|---|
| 只想让 Hades 在本机跑起来 | `hades setup` | 运行一次真实对话并验证有响应 |
| 已知道要用哪个 provider | `hades model` | 保存配置，然后开始聊天 |
| 想搭建机器人或长期运行的服务 | CLI 正常后运行 `hades gateway setup` | 接入 Telegram、Discord、Slack 或其他平台 |
| 想使用本地或自托管模型 | `hades model` → 自定义 endpoint | 验证 endpoint、模型名称和上下文长度 |
| 想要多 provider 故障转移 | 先运行 `hades model` | 基础对话正常后再添加路由和故障转移 |

**经验法则：** 如果 Hades 无法完成一次正常对话，暂时不要添加更多功能。先让一次完整对话跑通，再逐步叠加 gateway、cron、skills、语音或路由。
...
```bash
pip install hades-agent
hades postinstall     # 可选：安装 Node.js、浏览器、ripgrep、ffmpeg 并运行 setup
```
...
这是最重要的配置步骤。使用 `hades model` 以交互方式完成选择：
...
| Provider | 说明 | 配置方式 |
|----------|-----------|---------------|
| **Nous Portal** | 订阅制，零配置 | 通过 `hades model` 进行 OAuth 登录 |
| **OpenAI Codex** | ChatGPT OAuth，使用 Codex 模型 | 通过 `hades model` 进行设备码认证 |
| **Anthropic** | 直接使用 Claude 模型——Max 计划 + 额外用量积分（OAuth），或按 token 付费的 API key | `hades model` → OAuth 登录（需要 Max + 额外积分），或 Anthropic API key |
...
| **MiniMax (OAuth)** | 通过浏览器 OAuth 使用 MiniMax-M2.7，无需 API key | `hades model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot 订阅（GPT-5.x、Claude、Gemini 等） | 通过 `hades model` 进行 OAuth，或设置 `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent 后端（在本地启动 `copilot` CLI） | `hades model`（需要 `copilot` CLI + `copilot login`） |
...
:::tip
你可以随时通过 `hades model` 切换 provider——没有锁定。所有支持的 provider 完整列表及配置详情，请参阅 [AI Providers](../integrations/providers.md)。
:::
...
:::tip 选择你的界面
Hades 提供两种终端界面：经典的 `prompt_toolkit` CLI，以及更新的 [TUI](../user-guide/tui.md)（支持模态覆盖层、鼠标选择和非阻塞输入）。两者共享相同的会话、斜杠命令和配置——分别用 `hades` 和 `hades --tui` 试试看。
:::
...
- `hades tools` — 按平台调整工具访问权限
- `hades skills` — 浏览并安装可复用的工作流
...
```bash
hades config set terminal.backend docker    # Docker 隔离
hades config set terminal.backend ssh       # 远程服务器
```

### 语音模式

```bash
# 在 Hades 安装目录下运行（curl 安装器在 Linux/macOS 上将其放置于
# ~/.hades/hades-agent，在 Windows 上为 %LOCALAPPDATA%\hades\hades-agent）：
...
```bash
hades skills search kubernetes
...
（如果安装时未包含 `[all]`，请先运行 `cd ~/.hades/hades-agent && uv pip install -e ".[acp]"`。）
...
| 现象 | 可能原因 | 解决方法 |
|---|---|---|
| Hades 启动但回复为空或异常 | Provider 认证或模型选择有误 | 重新运行 `hades model`，确认 provider、模型和认证信息 |
| 自定义 endpoint "可用"但返回乱码 | base URL、模型名称有误，或实际上不兼容 OpenAI | 先用独立客户端验证该 endpoint |
| Gateway 启动但无法收到消息 | Bot token、白名单或平台配置不完整 | 重新运行 `hades gateway setup` 并检查 `hades gateway status` |
| `hades --continue` 找不到旧会话 | 切换了 profile 或会话从未保存 | 检查 `hades sessions list`，确认你在正确的 profile 下 |
| 模型不可用或出现异常的故障转移行为 | Provider 路由或故障转移设置过于激进 | 在基础 provider 稳定之前关闭路由 |
| `hades doctor` 标记配置问题 | 配置值缺失或已过期 | 修复配置，在添加功能前重新测试普通对话 |
...
|---------|-------------|
| `hades` | 开始聊天 |
| `hades model` | 选择 LLM provider 和模型 |
| `hades tools` | 配置每个平台启用的工具 |
| `hades setup` | 完整配置向导（一次性配置所有内容） |
| `hades doctor` | 诊断问题 |
| `hades update` | 更新到最新版本 |
| `hades gateway` | 启动消息 gateway |
| `hades --continue` | 恢复上次会话 |
