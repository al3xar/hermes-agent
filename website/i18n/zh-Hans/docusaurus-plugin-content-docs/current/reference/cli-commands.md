...
sidebar_position: 1
title: "CLI 命令参考"
description: "Hades 终端命令及命令族的权威参考"
---
...
|--------|-------------|
| `--version`, `-V` | 显示版本并退出。 |
| `--profile <name>`, `-p <name>` | 选择本次调用使用的 Hades profile（配置文件）。覆盖 `hades profile use` 设置的粘性默认值。 |
...
| `--tui` | 启动 [TUI](../user-guide/tui.md) 而非经典 CLI。等同于 `HADES_TUI=1`。 |
...
| 命令 | 用途 |
|---------|---------|
| `hades chat` | 与 agent 进行交互式或单次聊天。 |
| `hades model` | 交互式选择默认 provider 和模型。 |
| `hades fallback` | 管理主模型出错时依次尝试的 fallback provider。 |
| `hades gateway` | 运行或管理消息 gateway 服务。 |
| `hades proxy` | 本地 OpenAI 兼容代理，附加 OAuth provider 凭据。参见 [订阅代理](../user-guide/features/subscription-proxy.md)。 |
| `hades lsp` | 管理 Language Server Protocol 集成（为 write_file/patch 提供语义诊断）。 |
| `hades setup` | 全部或部分配置的交互式设置向导。 |
| `hades whatsapp` | 配置并配对 WhatsApp 桥接。 |
| `hades slack` | Slack 辅助工具（当前功能：生成将每条命令注册为原生斜杠命令的 app manifest）。 |
| `hades auth` | 管理凭据——添加、列出、删除、重置、设置策略。处理 Codex/Nous/Anthropic 的 OAuth 流程。 |
| `hades login` / `logout` | **已弃用** — 请改用 `hades auth`。 |
| `hades status` | 显示 agent、auth 和平台状态。 |
| `hades cron` | 检查并触发 cron 调度器。 |
| `hades kanban` | 多 profile 协作看板（任务、链接、调度器）。 |
| `hades webhook` | 管理用于事件驱动激活的动态 webhook 订阅。 |
| `hades hooks` | 检查、审批或删除 `config.yaml` 中声明的 shell 脚本 hook。 |
| `hades doctor` | 诊断配置和依赖问题。 |
| `hades security audit` | 对 venv、plugin 依赖和固定 MCP 服务器进行按需供应链审计（OSV.dev）。 |
| `hades dump` | 可直接复制粘贴的设置摘要，用于支持/调试。 |
| `hades debug` | 调试工具——上传日志和系统信息以获取支持。 |
| `hades backup` | 将 Hades 主目录备份为 zip 文件。 |
| `hades checkpoints` | 检查/修剪/清除 `~/.hades/checkpoints/`（`/rollback` 使用的影子存储）。不带参数运行可查看状态概览。 |
| `hades import` | 从 zip 文件恢复 Hades 备份。 |
| `hades logs` | 查看、跟踪和过滤 agent/gateway/错误日志文件。 |
| `hades config` | 显示、编辑、迁移和查询配置文件。 |
| `hades pairing` | 审批或撤销消息配对码。 |
| `hades skills` | 浏览、安装、发布、审计和配置 skill。 |
| `hades bundles` | 将多个 skill 归组到单个 `/<name>` 斜杠命令下。参见 [Skill Bundles](../user-guide/features/skills.md#skill-bundles)。 |
| `hades curator` | 后台 skill 维护——状态、运行、暂停、固定。参见 [Curator](../user-guide/features/curator.md)。 |
| `hades memory` | 配置外部 memory provider。当对应 provider 激活时，特定于 plugin 的子命令（如 `hades honcho`）会自动注册。 |
| `hades acp` | 将 Hades 作为 ACP 服务器运行，用于编辑器集成。 |
| `hades mcp` | 管理 MCP 服务器配置，并将 Hades 作为 MCP 服务器运行。 |
| `hades plugins` | 管理 Hades Agent plugin（安装、启用、禁用、删除）。 |
| `hades portal` | Nous Portal 状态、订阅链接和 Tool Gateway 路由。参见 [Tool Gateway](../user-guide/features/tool-gateway.md)。 |
| `hades tools` | 按平台配置已启用的工具。 |
| `hades computer-use` | 安装或检查 cua-driver 后端（macOS Computer Use）。 |
| `hades sessions` | 浏览、导出、修剪、重命名和删除会话。 |
| `hades insights` | 显示 token/费用/活动分析。 |
| `hades claw` | OpenClaw 迁移辅助工具。 |
| `hades dashboard` | 启动用于管理配置、API 密钥和会话的 Web 控制台。 |
| `hades profile` | 管理 profile——多个隔离的 Hades 实例。 |
| `hades completion` | 打印 shell 补全脚本（bash/zsh/fish）。 |
| `hades version` | 显示版本信息。 |
| `hades update` | 拉取最新代码并重新安装依赖（git 安装），或检查 PyPI 并执行 `pip install --upgrade`（pip 安装）。`--check` 预览而不安装；`--backup` 在拉取前对 `HADES_HOME` 进行快照。 |
| `hades uninstall` | 从系统中删除 Hades。 |
...
### `hades -z <prompt>` — 脚本化单次调用

对于程序化调用方（shell 脚本、CI、cron、通过管道传入 prompt 的父进程），`hades -z` 是最纯粹的单次入口：**单个 prompt 输入，最终响应文本输出，stdout 和 stderr 上不输出任何其他内容。** 无横幅、无 spinner、无工具预览、无 `Session:` 行——只有 agent 的最终回复纯文本。
...
| 标志 | 等效环境变量 | 用途 |
|---|---|---|
| `-m` / `--model <model>` | `HADES_INFERENCE_MODEL` | 覆盖本次运行的模型 |
...
相同的 agent、相同的工具、相同的 skill——只是剥离了所有交互式/装饰性层。如果你还需要在记录中包含工具输出，请改用 `hades chat -q`；`-z` 专门用于"我只需要最终答案"的场景。

## `hades model`

交互式 provider + 模型选择器。**这是添加新 provider、设置 API 密钥和运行 OAuth 流程的命令。** 从终端运行——不要在活跃的 Hades 聊天会话内部运行。
...
:::warning hades model 与 /model——了解区别
**`hades model`**（从终端运行，在任何 Hades 会话外部）是**完整的 provider 设置向导**。它可以添加新 provider、运行 OAuth 流程、提示输入 API 密钥并配置端点。

**`/model`**（在活跃的 Hades 聊天会话中输入）只能**在已设置好的 provider 和模型之间切换**。它无法添加新 provider、运行 OAuth 或提示输入 API 密钥。

**如果需要添加新 provider：** 先退出 Hades 会话（`Ctrl+C` 或 `/quit`），然后从终端提示符运行 `hades model`。
:::
...
如果你只配置了 OpenRouter，`/model` 将只显示 OpenRouter 模型。要添加其他 provider（Anthropic、DeepSeek、Copilot 等），请退出会话并从终端运行 `hades model`。
:::
...
| 选项 | 说明 |
|--------|-------------|
| `--all` | 在 `start` / `restart` / `stop` 时：对**每个 profile** 的 gateway 执行操作，而不仅限于活跃的 `HADES_HOME`。当你并行运行多个 profile 并希望在 `hades update` 后全部重启时很有用。 |
| `--no-supervise` | 在 `run` 时：在 s6-overlay Docker 镜像内部，跳过 s6 自动监管，退回到 pre-s6 前台语义——gateway 作为容器主进程运行，无自动重启。在 s6 镜像之外为空操作。等同于设置 `HADES_GATEWAY_NO_SUPERVISE=1`。 |

:::tip WSL 用户
使用 `hades gateway run` 而非 `hades gateway start`——WSL 的 systemd 支持不稳定。用 tmux 包裹以保持持久运行：`tmux new -s hades 'hades gateway run'`。详见 [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hades-gateway-start-fails)。
:::
...
| `--reset` | 在设置前将配置重置为默认值。 |
| `--reconfigure` | 向后兼容别名——在已有安装上裸运行 `hades setup` 现在默认执行此操作。 |
...
关于 gateway 本身的配置，请参阅 [Tool Gateway](../user-guide/features/tool-gateway.md)。关于一键设置路径，请参阅上方的 `hades setup --portal`。
...
```bash
hades slack manifest              # 将 manifest 打印到 stdout
hades slack manifest --write      # 写入 ~/.hades/slack-manifest.json
hades slack manifest --slashes-only  # 仅输出 features.slash_commands 数组
```
...
| `--name NAME` | `Hades` | Slack 中的机器人显示名称。 |
| `--description DESC` | 默认简介 | Slack app 目录中显示的机器人描述。 |
| `--slashes-only` | 关闭 | 仅输出 `features.slash_commands`，用于合并到手动维护的 manifest 中。 |

`hades update` 后重新运行 `hades slack manifest --write` 以获取新增命令。


## `hades login` / `hades logout` *（已弃用）*

:::caution
`hades login` 已被移除。请使用 `hades auth` 管理 OAuth 凭据，使用 `hades model` 选择 provider，或使用 `hades setup` 进行完整的交互式设置。
:::
...
```bash
hades auth                                              # 交互式向导
hades auth list                                         # 显示所有池
hades auth list openrouter                              # 显示特定 provider
hades auth add openrouter --api-key sk-or-v1-xxx        # 添加 API 密钥
hades auth add anthropic --type oauth                   # 添加 OAuth 凭据
hades auth remove openrouter 2                          # 按索引删除
hades auth reset openrouter                             # 清除冷却时间
hades auth status anthropic                             # 显示某 provider 的认证状态
hades auth logout anthropic                             # 登出并清除已存储的认证状态
hades auth spotify                                      # 通过 PKCE 将 Hades 与 Spotify 认证
```
...
| 标志 | 用途 |
|------|---------|
| `--board <slug>` | 操作特定看板。默认为当前看板（通过 `hades kanban boards switch`、`HADES_KANBAN_BOARD` 环境变量或 `default` 设置）。 |

**这是人工/脚本操作界面。** 调度器生成的 agent worker 通过专用的 `kanban_*` [toolset](/user-guide/features/kanban#how-workers-interact-with-the-board)（`kanban_show`、`kanban_complete`、`kanban_block`、`kanban_create`、`kanban_link`、`kanban_comment`、`kanban_heartbeat`；编排器 profile 还可使用 `kanban_list` 和 `kanban_unblock`）驱动看板，而非调用 `hades kanban`。Worker 的环境中固定了 `HADES_KANBAN_BOARD`，因此物理上无法看到其他看板。
...
hades kanban list                  # 显示 atm10-server 的任务
...
看板解析顺序（优先级从高到低）：`--board <slug>` 标志 → `HADES_KANBAN_BOARD` 环境变量 → `~/.hades/kanban/current` 文件 → `default`。

所有操作也可作为 gateway 中的斜杠命令使用（`/kanban …`），参数界面相同——包括 `boards` 子命令和 `--board` 标志。

完整设计——与 Cline Kanban / Paperclip / NanoClaw / Gemini Enterprise 的对比、八种协作模式、四个用户故事、并发正确性证明——请参阅仓库中的 `docs/hades-kanban-v1-spec.pdf` 或 [Kanban 用户指南](/user-guide/features/kanban)。
...
输出整个 Hades 设置的紧凑纯文本摘要。专为复制粘贴到 Discord、GitHub issue 或 Telegram 寻求支持而设计——无 ANSI 颜色、无特殊格式，只有数据。
...
| 部分 | 详情 |
|---------|---------|
| **Header** | Hades 版本、发布日期、git commit hash |
...
:::tip
`hades dump` 专为分享而设计。交互式诊断请使用 `hades doctor`。可视化概览请使用 `hades status`。
:::
...
报告包含系统信息（操作系统、Python 版本、Hades 版本）、近期 agent 和 gateway 日志（每文件 512 KB 限制）以及脱敏的 API 密钥状态。密钥始终脱敏——不会上传任何密钥。
...
```bash
hades debug share              # 上传调试报告，打印 URL
hades debug share --lines 500  # 包含更多日志行
hades debug share --expire 30  # 粘贴保留 30 天
hades debug share --local      # 在终端打印报告（不上传）
```
...
创建 Hades 配置、skill、会话和数据的 zip 归档。备份不包含 hades-agent 代码库本身。

| 选项 | 说明 |
|--------|-------------|
| `-o`, `--output <path>` | zip 文件的输出路径（默认：`~/hades-backup-<timestamp>.zip`）。 |
| `-q`, `--quick` | 快速快照：仅包含关键状态文件（config.yaml、state.db、.env、auth、cron 任务）。比完整备份快得多。 |
| `-l`, `--label <name>` | 快照标签（仅与 `--quick` 配合使用）。 |

备份使用 SQLite 的 `backup()` API 进行安全复制，因此即使 Hades 正在运行也能正确工作（WAL 模式安全）。
...
- `hades-agent` 代码本身（这是用户数据备份，不是仓库快照）。

### 示例

```bash
hades backup                           # 完整备份到 ~/hades-backup-*.zip
hades backup -o /tmp/hades.zip        # 完整备份到指定路径
hades backup --quick                   # 仅状态快速快照
hades backup --quick --label "pre-upgrade"  # 带标签的快速快照
```
...
| 子命令 | 说明 |
|------------|-------------|
| `status`（默认） | 显示总大小、项目数量和每个项目的详情。裸 `hades checkpoints` 等同于此。 |
| `list` | `status` 的别名。 |
...
```bash
hades checkpoints                                  # 状态概览
hades checkpoints prune --retention-days 3         # 激进清理
hades checkpoints prune --max-size-mb 200          # 一次性收紧大小上限
hades checkpoints clear-legacy -f                  # 删除 v1 归档目录
hades checkpoints clear -f                         # 清除所有内容
```
...
将之前创建的 Hades 备份恢复到 Hades 主目录。归档中的所有文件会覆盖 Hades 主目录中的现有文件；`--force` 仅跳过当目标已有 Hades 安装时触发的确认提示。
...
### 示例
```bash
hades import ~/hades-backup-20260423.zip           # 覆盖现有配置前提示确认
hades import ~/hades-backup-20260423.zip --force   # 不提示直接覆盖
```
...
查看、跟踪和过滤 Hades 日志文件。所有日志存储在 `~/.hades/logs/`（非默认 profile 存储在 `<profile>/logs/`）。
...
Hades 使用 Python 的 `RotatingFileHandler`。旧日志会自动轮转——查找 `agent.log.1`、`agent.log.2` 等。`hades logs list` 子命令显示所有日志文件，包括已轮转的。
...
hades skills install https://sharethis.chat/SKILL.md                     # 直接 URL（单文件 SKILL.md）
hades skills install https://example.com/SKILL.md --name my-skill        # frontmatter 无名称时覆盖名称
hades skills check
hades skills update
hades skills config
...
- `--source well-known` 允许你将 Hades 指向暴露 `/.well-known/skills/index.json` 的站点。
...
在全新安装时，第一次计划运行会延迟一个完整的 `interval_hours`（默认 7 天）——gateway 不会在 `hades update` 后的第一次 tick 时立即执行 curator。使用 `hades curator run --dry-run` 在此之前预览。
...
| `add` | 选择 provider + 模型（与 `hades model` 相同的选择器）并追加到链末尾 |
...
:::info Provider 特定子命令
当外部 memory provider 处于活跃状态时，它可能会注册自己的顶级 `hades <provider>` 命令用于 provider 特定管理（例如 Honcho 激活时的 `hades honcho`）。未激活的 provider 不暴露其子命令。运行 `hades --help` 查看当前已连接的命令。
:::
...
将 Hades 作为 ACP（Agent Client Protocol）stdio 服务器启动，用于编辑器集成。
...
管理 MCP（Model Context Protocol）服务器配置，并将 Hades 作为 MCP 服务器运行。

| 子命令 | 说明 |
|------------|-------------|
| `serve [-v\|--verbose]` | 将 Hades 作为 MCP 服务器运行——向其他 agent 暴露对话。 |
...
参见 [MCP 配置参考](./mcp-config-reference.md)、[在 Hades 中使用 MCP](../guides/use-mcp-with-hades.md) 和 [MCP 服务器模式](../user-guide/features/mcp.md#running-hades-as-an-mcp-server)。
...
统一的 plugin 管理——通用 plugin、memory provider 和 context engine 集于一处。不带子命令运行 `hades plugins` 会打开包含两个部分的复合交互界面：
...
参见 [Plugins](../user-guide/features/plugins.md) 和 [构建 Hades Plugin](../guides/build-a-hades-plugin.md)。
...
`hades computer-use install` 是安装 `computer_use` toolset 使用的 [cua-driver](https://github.com/trycua/cua) 二进制文件的稳定入口。它运行与首次启用 Computer Use 时 `hades tools` 调用的相同上游安装程序，因此如果 toolset 切换未触发安装（例如在已配置用户的设置中），可以安全地用于重新运行安装。

`hades update` 在更新结束时，如果 cua-driver 在 PATH 中，会自动重新运行上游安装程序，因此大多数用户不需要手动调用 `--upgrade`。当上游发布了你现在就想要的修复，而不想等待下次 Hades 更新时，使用此选项。
...
将 OpenClaw 设置迁移到 Hades。从 `~/.openclaw`（或自定义路径）读取并写入 `~/.hades`。自动检测旧版目录名（`~/.clawdbot`、`~/.moltbot`）和配置文件名（`clawdbot.json`、`moltbot.json`）。

| 选项 | 说明 |
|--------|-------------|
| `--dry-run` | 预览将迁移的内容而不写入任何内容。 |
| `--preset <name>` | 迁移预设：`full`（所有兼容设置）或 `user-data`（排除基础设施配置）。两种预设都不导入密钥——需要显式传入 `--migrate-secrets`。 |
| `--overwrite` | 在冲突时覆盖现有 Hades 文件（默认：当计划有冲突时拒绝应用）。 |
| `--migrate-secrets` | 在迁移中包含 API 密钥。即使在 `--preset full` 下也需要显式指定。 |
| `--no-backup` | 跳过迁移前对 `~/.hades/` 的 zip 快照（默认情况下，在应用前会将单个还原点归档写入 `~/.hades/backups/pre-migration-*.zip`；可用 `hades import` 恢复）。 |
...
迁移涵盖 30+ 个类别，包括 persona、memory、skill、模型 provider、消息平台、agent 行为、会话策略、MCP 服务器、TTS 等。条目要么**直接导入**到 Hades 等效项，要么**归档**以供手动审查。
...
启动 Web 控制台——基于浏览器的界面，用于管理配置、API 密钥和监控会话。需要 `pip install hades-agent[web]`（FastAPI + Uvicorn）。内嵌浏览器 Chat 标签页始终可用，但额外需要 `pty` extra（`pip install 'hades-agent[web,pty]'`）以及 POSIX PTY 环境（如 Linux、macOS 或 WSL2）。完整文档请参阅 [Web 控制台](/user-guide/features/web-dashboard)。
...
| `--stop` | — | 停止正在运行的 `hades dashboard` 进程并退出。 |
| `--status` | — | 列出正在运行的 `hades dashboard` 进程并退出。 |
...
管理 profile——多个隔离的 Hades 实例，每个实例拥有自己的 config、会话、skill 和主目录。
...
将 shell 补全脚本打印到 stdout。在 shell profile 中 source 输出内容，即可对 Hades 命令、子命令和 profile 名称进行 Tab 补全。
...
```bash
hades update [--check] [--backup] [--restart-gateway]
```

拉取最新的 `hades-agent` 代码并在 venv 中重新安装依赖，然后重新运行安装后 hook（MCP 服务器、skill 同步、补全安装）。可在运行中的安装上安全执行。

**pip 安装：** `hades update` 自动检测基于 pip 的安装——查询 PyPI 获取最新版本并运行 `pip install --upgrade hades-agent`，而非 `git pull`。PyPI 发布跟踪标记版本（主要/次要版本），而非 `main` 上的每个 commit。使用 `--check` 查看是否有更新的 PyPI 版本可用，而不安装。
...
- **配对数据快照。** 即使 `--backup` 关闭，`hades update` 也会在 `git pull` 前对 `~/.hades/pairing/` 和 Feishu 评论规则进行轻量快照。如果拉取覆盖了你正在编辑的文件，可以用 `hades backup restore --state pre-update` 回滚。
- **旧版 `hades.service` 警告。** 如果 Hades 检测到预重命名的 `hades.service` systemd 单元（而非当前的 `hades-gateway.service`），会打印一次性迁移提示，帮助你避免循环重启问题。
...
| 命令 | 说明 |
|---------|-------------|
| `hades version` | 打印版本信息。 |
| `hades update` | 拉取最新变更并重新安装依赖。 |
| `hades uninstall [--full] [--yes]` | 删除 Hades，可选择删除所有 config/数据。 |
