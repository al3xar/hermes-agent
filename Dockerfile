...
# the sealed venv.
#
# The DeepAgents runtime ([deepagents] extra — deepagents SDK + langchain
# integrations + langfuse tracing) is baked into the published image for the
# same reason as hindsight/matrix/google-chat: its SDKs lazy-install into the
# immutable image venv at first use rather than the mounted /opt/data volume,
# so it is lost on every container recreate / image update and runtime
# `hermes --runtime=deepagents` then fails with
# `ModuleNotFoundError`. Baked here in the python_deps stage so the sealed
# install carries it without a first-boot network fetch.
#
...
RUN python3 -m pm.build_env --source /opt/hermes --python /usr/local/bin/python3 \
    ...
    --extra azure-identity --extra hindsight --extra matrix --extra deepagents \
    --extra google-chat
