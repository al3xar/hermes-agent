...
# subprocesses, git, bun, etc.) that would otherwise accumulate when hades
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# hades process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main hades process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Hades WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via HADES_UID at runtime
RUN useradd -u 10000 -m -d /opt/data hades
...
#
# ui-tui/packages/hades-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/hades-ink/ ui-tui/packages/hades-ink/
...
# lock on the @hades/ink entry, tripped the TUI launcher's
...
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all --extra messaging --extra anthropic --extra bedrock --extra azure-identity --extra hindsight --extra deepagents

# ---------- Source code ----------
# .dockerignore excludes node_modules, so the installs above survive.
COPY --chown=hades:hades . .
...
# ---------- Permissions ----------
# Make install dir world-readable so any HADES_UID can read it at runtime.
# The venv needs to be traversable too.
# node_modules trees additionally need to be writable by the hades user
# so the runtime `npm install` triggered by _tui_need_npm_install() in
# hades_cli/main.py succeeds (see #18800). /opt/hades/web is build-time
# only (HADES_WEB_DIST points at hades_cli/web_dist) and is intentionally
# not chowned here.
# /opt/hades/gateway is runtime-writable: Python may create __pycache__ and
# gateway state artifacts beneath the package after services drop privileges,
# especially when the hades UID is remapped at boot (#27221).
# The .venv MUST remain hades-writable so lazy_deps.py can install
# remaining optional platform packages and future pin bumps at first use.
# Without this, `uv pip install` fails with EACCES and adapters silently
# fail to load.  See tools/lazy_deps.py.
USER root
RUN chmod -R a+rX /opt/hades && \
    chown -R hades:hades /opt/hades/.venv /opt/hades/ui-tui /opt/hades/gateway /opt/hades/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the hades user via
# `s6-setuidgid hades` in its run script. If HADES_UID is unset, services
# run as the default hades user (UID 10000).

# ---------- Link hades-agent itself (editable) ----------
...
# container always returns nothing — meaning `hades dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the HADES_GIT_SHA build-arg to
# /opt/hades/.hades_build_sha at build time, and have
# hades_cli/build_info.py read it at runtime.  Both `hades dump` and
...
ARG HADES_GIT_SHA=
RUN if [ -n "${HADES_GIT_SHA}" ]; then \
        printf '%s\n' "${HADES_GIT_SHA}" > /opt/hades/.hades_build_sha && \
        chown hades:hades /opt/hades/.hades_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-hades + dashboard.
...
# `exec hades`. Wired in as cont-init.d/01- so it
...
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/hades/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-hades-setup && \
    chmod +x /etc/cont-init.d/01-hades-setup
...
# `docker exec <c> hades ...` they default to root, and any file the
# command writes under $HADES_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/hades/bin/hades, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid hades`
# when invoked as root. Non-root callers (supervised processes,
# `--user hades`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/hades/.venv/bin/hades). See the shim source for
# the opt-out env var (HADES_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/hades-exec-shim.sh /opt/hades/bin/hades

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> hades ...` and any
# subprocess that doesn't activate the venv first still find hades.
#
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. hades
# subcommand vs. no-args), drops to the hades user via s6-setuidgid,
