...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'hades-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/hades-runner-test',
    sourceRepoRoot: null,
    hadesHome: '/tmp/hades-runner-test',
    logRoot: '/tmp/hades-runner-test',
