 ...
 *     `X-Hades-Session-Token` header; WS uses `?token=`.
...
//   - hades_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - hades_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / hades #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-hades_session_at', '__Secure-hades_session_at', 'hades_session_at']
const RT_COOKIE_VARIANTS = ['__Host-hades_session_rt', '__Secure-hades_session_rt', 'hades_session_rt']
...
/**
 * True if any cookie in `cookies` is a hades session ACCESS-token cookie
