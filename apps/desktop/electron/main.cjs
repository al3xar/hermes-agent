...
async function spawnPoolBackend(profile, entry) {
  ...
  child.once('exit', (code, signal) => {
    ...
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`Hades backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })
...
async function startHades() {
  ...
  connectionPromise = (async () => {
    ...
    hadesProcess = spawn(backend.command, backend.args, {
      cwd: hadesCwd,
      env: {
        ...process.env,
...
ipcMain.handle('hades:setting:defaultProjectDir:get', async () => ({
  ...
  defaultLabel: path.join(app.getPath('home'), 'hades-projects')
}))
...
async function getUninstallSummary() {
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'hades_cli.main', 'uninstall', '--gui-summary'], {
        cwd: agentRoot,
