...
const USER_DATA_OVERRIDE = process.env.HADES_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.HADES_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with HADES_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[hades] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[hades] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[hades] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[hades] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// HADES_HOME — the user-facing root for everything Hades-related. Mirrors
// scripts/install.ps1's $HadesHome and scripts/install.sh's $HADES_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\hades (matches install.ps1)
...
// %LOCALAPPDATA%\hades yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// HADES_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
...
function resolveHadesHome() {
  if (process.env.HADES_HOME) return path.resolve(process.env.HADES_HOME)
...
const HADES_HOME = resolveHadesHome()
// ACTIVE_HADES_ROOT — the canonical mutable Hades install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_HADES_ROOT = path.join(HADES_HOME, 'hades-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_HADES_ROOT, 'venv')
...
// boots, going straight to `resolveHadesBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_HADES_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_HADES_ROOT, '.hades-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Hades profile the desktop launches its
// local backend as. When set, startHades() passes `hades --profile <name>
...
// hadesDesktop.updates.setBranch().
...
const BOOT_FAKE_MODE = process.env.HADES_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.HADES_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// Seed the native About panel with the live Hades version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `hades update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveHadesVersion(),
...
const MEDIA_PROTOCOL = 'hades-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(hades-media://…)`.
...
let mainWindow = null
let hadesProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by hadesProcess +
// connectionPromise + startHades(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.HADES_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.HADES_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startHades() calls (e.g. the renderer's
...
function rememberLog(chunk) {
  ...
  hadesLog.push(...lines)
  if (hadesLog.length > 300) {
    hadesLog.splice(0, hadesLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-hades-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('hades:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the hades:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('hades:bootstrap:event', ev)
}
...
function isHadesSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'hades_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.HADES_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Hades deps (notably pywinpty's Rust-built
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\hades\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'hades', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'hades', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\hades\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'hades', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'hades', 'git', 'bin', 'git.exe'))
  }
...
function recentHadesLog() {
  return hadesLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's hades root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_HADES_ROOT.
// HADES_DESKTOP_HADES_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.HADES_DESKTOP_HADES_ROOT && path.resolve(process.env.HADES_DESKTOP_HADES_ROOT),
    !IS_PACKAGED && isHadesSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isHadesSourceRoot(ACTIVE_HADES_ROOT) ? ACTIVE_HADES_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_HADES_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('hades:updates:progress', merged)
  }
}
...
// HADES_HOME/hades-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_hades_home). That binary owns
// ALL repo mutation — running `hades update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'hades-setup.exe' : 'hades-setup'
...
// Path to the venv shim whose lock decides whether `hades update` can write
// fresh entry points. On Windows this is the file the running backend
// `hades.exe` holds open; on POSIX it's never mandatory-locked.
function venvHadesShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'hades.exe')
    : path.join(updateRoot, 'venv', 'bin', 'hades')
}
...
// only signals the direct child, so on Windows a backend `hades.exe` that
// spawned its own grandchildren (a `hades` REPL, a pty terminal session, the
...
// `hadesProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `hades.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a hades
// REPL, a pty terminal, the gateway) keep `hades.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (hadesProcess && Number.isInteger(hadesProcess.pid)) pids.push(hadesProcess.pid)
  ...
  if (hadesProcess && !hadesProcess.killed) {
    try {
      hadesProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvHadesShimPath(updateRoot)
...
// `hades update`). Instead we spawn the staged Hades-Setup binary with
// --update and quit, so it can run `hades update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri hades-setup. Unlike Windows
      ...
      // whole update itself: `hades update` (backend) + `hades desktop
    ...
    if (!updater) {
      ...
      // `hades desktop`, never the Tauri installer that self-copies
      // hades-setup.exe into HADES_HOME). They DO have a working `hades`
      ...
      // checkout they're on — bare `hades update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'hades update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `hades update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `hades update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, hadesRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Hades updater…', percent: 100 })
    ...
    // hades.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `hades update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: HADES_HOME,
      env: {
        ...process.env,
        HADES_HOME,
        PATH: [path.join(HADES_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the hades CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `hades` on PATH.
function resolveHadesCliBinary(updateRoot) {
  const venvHades = path.join(updateRoot, 'venv', 'bin', 'hades')
  if (fileExists(venvHades)) return venvHades
  return findOnPath('hades') || null
}
...
// macOS/Linux in-app update: backend (`hades update`) + OS-aware GUI rebuild
// (`hades desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const hades = resolveHadesCliBinary(updateRoot)
  if (!hades) {
    emitUpdateProgress({ stage: 'manual', message: 'hades update', percent: null })
    return { ok: true, manual: true, command: 'hades update', hadesRoot: updateRoot }
  }

  // Put the Hades-managed Node and the venv on PATH so `hades desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(HADES_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    HADES_HOME,
  ...
  // `hades update` reaps stale `hades dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads HADES_DESKTOP_CHILD_PID and excludes
  ...
  if (hadesProcess && Number.isInteger(hadesProcess.pid)) {
    desktopChildPids.push(hadesProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.HADES_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  const updated = await runStreamedUpdate(hades, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'hades update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'hades update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(hades, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Hades.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Hades.app')
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.hades-update-new"; then
    rm -rf "$DST.hades-update-old" 2>/dev/null || true
    mv "$DST" "$DST.hades-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.hades-update-new" "$DST"
    rm -rf "$DST.hades-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `hades-desktop-update-${Date.now()}.sh`)
  try {
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `hades
  ...
  return isHadesSourceRoot(ACTIVE_HADES_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.HADES_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set HADES_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: hades desktop --force-build`
  )
  return candidates[0]
}
...
function resolveHadesCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Hades.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveHadesCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'hades_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_HADES_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Hades at ${ACTIVE_HADES_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'hades_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_HADES_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_HADES_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveHadesBackend(dashboardArgs) {
  // 1. Explicit override -- HADES_DESKTOP_HADES_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.HADES_DESKTOP_HADES_ROOT && path.resolve(process.env.HADES_DESKTOP_HADES_ROOT)
  if (overrideRoot && isHadesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Hades source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `hades` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isHadesSourceRoot.)
  if (!IS_PACKAGED && isHadesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Hades source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_HADES_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\hades\hades-agent (Windows) or ~/.hades/hades-agent.
  ...
  //    to spawning hades. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `hades update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `hades` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    HADES_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.HADES_DESKTOP_IGNORE_EXISTING !== '1') {
    let hadesCommand = null
    const hadesOverride = process.env.HADES_DESKTOP_HADES

    if (hadesOverride) {
      const resolvedOverride = findOnPath(hadesOverride)
      if (resolvedOverride) {
        hadesCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(hadesOverride, { isWsl: IS_WSL })) {
        hadesCommand = hadesOverride
      } else {
        rememberLog(`Ignoring Windows Hades override under WSL: ${hadesOverride}`)
      }
    } else {
      hadesCommand = findOnPath('hades')
    }

    if (hadesCommand) {
      if (looksLikeDesktopAppBinary(hadesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Hades CLI: ${hadesCommand}`)
        hadesCommand = null
      }
    }

    if (hadesCommand) {
      // Smoke-test the candidate before trusting it. A `hades` shim
      ...
      const shellForProbe = isCommandScript(hadesCommand)
      if (verifyHadesCli(hadesCommand, { shell: shellForProbe })) {
        return {
          label: `existing Hades CLI at ${hadesCommand}`,
          command: hadesCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Hades CLI at ${hadesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  if (python) {
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\hades\hades-agent\venv.
    if (canImportHadesCli(python)) {
      return {
        kind: 'python',
  ...
  //    resolveHadesBackend was the old "no payload" path and forced the
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveHadesBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Hades bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
      ...
      // Latch the failure so subsequent startHades() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // hades:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveHadesBackend(backend.args))
  }
  ...
  if (!isHadesSourceRoot(ACTIVE_HADES_ROOT)) {
    throw new Error(
      `Hades install at ${ACTIVE_HADES_ROOT} is missing or incomplete. ` +
  ...
  // %LOCALAPPDATA%\hades\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `hades` on PATH, this check still helps.
  ...
  if (!fileExists(venvPython)) {
    ...
    // isHadesSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Hades venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Hades at ${ACTIVE_HADES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Hades-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('hades:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveHadesCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('hades:preview-file-changed', payload)
}
...
async function waitForHades(baseUrl, token) {
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('hades:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('hades:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Hades...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('hades:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('hades:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('hades:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'hades:desktop:zoomLevel'
...
//
// Hosted Hades gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``hades_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / hades #37247).
//     Both are set as HttpOnly cookies (``hades_session_at`` ~15 min,
//     ``hades_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:hades-remote-oauth'
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.HADES_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.HADES_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by HADES_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (hades #37247), and the
...
//   2. env override (HADES_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.HADES_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.HADES_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'HADES_DESKTOP_REMOTE_URL is set but HADES_DESKTOP_REMOTE_TOKEN is not. ' +
...
function globalRemoteActive() {
  if (process.env.HADES_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Hades
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startHades())
...
// startHades() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetHadesConnection() nulls hadesProcess.
  const dying = hadesProcess && !hadesProcess.killed ? hadesProcess : null
  resetHadesConnection()
...
// profile to startHades() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startHades() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForHades(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: hadesLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  const backend = await ensureRuntime(resolveHadesBackend(dashboardArgs))
  const hadesCwd = resolveHadesCwd()
  ...
  const child = spawn(backend.command, backend.args, hiddenWindowsChildOptions({
      ...
      env: {
        ...
        HADES_HOME,
        ...backend.env,
        HADES_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        HADES_DESKTOP: '1',
        HADES_WEB_DIST: webDist
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  await Promise.race([waitForHades(baseUrl, token), startFailed])
  ready = true
...
async function startHades() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startHades() call re-throws the same error
  ...
  connectionPromise = (async () => {
    ...
    // resolves HADES_HOME the same way `hades -p <name>` does on the CLI. An
    ...
    hadesProcess.stdout.on('data', rememberLog)
    hadesProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    hadesProcess.once('error', error => {
    ...
    hadesProcess.once('exit', (code, signal) => {
    ...
    await Promise.race([waitForHades(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startHades().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('hades:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Hades…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('hades:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetHadesConnection only
...
ipcMain.handle('hades:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('hades:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('hades:window:openSession', async (_event, sessionId) => {
...
ipcMain.handle('hades:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startHades() call restarts the
...
ipcMain.handle('hades:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startHades() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('hades:bootstrap:cancel', async () => {
...
ipcMain.handle('hades:boot-progress:get', async () => bootProgressState)
ipcMain.handle('hades:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('hades:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('hades:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('hades:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('hades:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('hades:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('hades:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('hades:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('hades:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('hades:profile:set', async (_event, name) => {
...
ipcMain.on('hades:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('hades:requestMicrophoneAccess', async () => {
...
ipcMain.handle('hades:api', async (_event, request) => {
...
ipcMain.handle('hades:notify', (_event, payload) => {
...
ipcMain.handle('hades:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('hades:readFileText', async (_event, filePath) => {
...
ipcMain.handle('hades:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('hades:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('hades:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('hades:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('hades:saveClipboardImage', async () => {
...
ipcMain.handle('hades:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('hades:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('hades:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('hades:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('hades:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveHadesCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('hades:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveHadesCwd()
}))

ipcMain.handle('hades:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('hades:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('hades:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('hades:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('hades:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('hades:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('hades:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: hadesLog.slice(-200) }))
...
function terminalShellCommand() {
  // HADES_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.HADES_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // Let a hades/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from HADES_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.HADES_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `hades:terminal:${id}:${suffix}`
}
...
ipcMain.handle('hades:fs:readDir', async (_event, dirPath) => {
...
ipcMain.handle('hades:fs:gitRoot', async (_event, startPath) => {
...
ipcMain.handle('hades:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Hades.')
  }
...
ipcMain.handle('hades:terminal:write', (_event, id, data) => {
...
ipcMain.handle('hades:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('hades:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('hades:updates:check', async () =>
...
ipcMain.handle('hades:updates:apply', async (_event, payload) =>
...
ipcMain.handle('hades:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('hades:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Hades version (the one `release.py` bumps in
// hades_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Hades version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveHadesVersion() {
  try {
...
// Re-resolve the live Hades version and push it into the native About panel
// just before showing it, so an in-place `hades update` is reflected without
...
ipcMain.handle('hades:version', async () => ({
  appVersion: resolveHadesVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  hadesRoot: resolveUpdateRoot()
}))
...
// the actual removal via `hades uninstall …` so the cross-platform PATH /
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_HADES_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isHadesSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(py, ['-m', 'hades_cli.main', 'uninstall', '--gui-summary'], hiddenWindowsChildOptions({
        cwd: agentRoot,
...
async function runDesktopUninstall(mode) {
  ...
  try {
    await releaseBackendLock(ACTIVE_HADES_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `hades-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `hades-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('hades:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('hades:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('hades:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('hades:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))
