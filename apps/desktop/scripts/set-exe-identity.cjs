...
// so EVERY packed build — first install, `hades desktop`, the installer's
...
// path (which rebuilds via `hades desktop --build-only`, never install.ps1)
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  console.log('[set-exe-identity] done — Hades icon + identity stamped')
}
