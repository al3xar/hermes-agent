...
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Hades.app')
    return {
      appPath,
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Hades.exe'),
...
// Default HADES_HOME for non-sandboxed runs -- matches main.cjs's
// resolveHadesHome(). On Windows it's %LOCALAPPDATA%\hades; elsewhere
...
const DEFAULT_HADES_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'hades')
  }
  return path.join(os.homedir(), '.hades')
})()
const VENV_ROOT = path.join(DEFAULT_HADES_HOME, 'hades-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'hades-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.HADES_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Hades-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Hades-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Hades-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Hades-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.HADES_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.HADES_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const hadesHome = path.join(sandbox, 'hades-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(hadesHome, { recursive: true })
  ...
  env.HADES_DESKTOP_CWD = cwd
  env.HADES_DESKTOP_IGNORE_EXISTING = '1'
  env.HADES_DESKTOP_TEST_MODE = 'fresh-install'
  env.HADES_DESKTOP_USER_DATA_DIR = userDataDir
  env.HADES_HOME = hadesHome
  delete env.HADES_DESKTOP_HADES
  delete env.HADES_DESKTOP_HADES_ROOT
  ...
  console.log(`  HADES_HOME: ${hadesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hadesHome, 'hades-agent', 'venv') }
}
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'hades-agent', 'hades_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Hades
...
Fast rerun (skip rebuild if the packaged app already exists):
  HADES_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
