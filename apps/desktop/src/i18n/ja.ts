...
export const ja = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHadesDesktop: 'Hades Desktop を起動中…'
    },
    errors: {
  ...
  settings: {
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: 'Hades がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
    ...
    gateway: {
      ...
      envOverrideDesc:
        '保存された設定を使用するには HADES_DESKTOP_REMOTE_URL と HADES_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      localTitle: 'ローカルゲートウェイ',
      localDesc: 'ローカルホストでプライベートな Hades バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      ...
      remoteUrlDesc: 'リモートダッシュボードバックエンドのベース URL。/hades などのパスプレフィックスもサポートしています。',
  ...
  commandCenter: {
    ...
    hadesActiveSessions: (version, count) => `Hades ${version} · アクティブセッション ${count}`,
  ...
  updates: {
    ...
    applyingBody: 'Hades アップデーターが独自のウィンドウで引き継ぎ、完了後に Hades を再度開きます。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Hades が自動的に再接続します。',
    applyingClose: 'Hades は更新を適用するために閉じます。',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.hades/config.yaml) に追加されます。Hades はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
