...
import { getHadesConfigRecord, type HadesConfigRecord, saveHadesConfig } from '@/hades'
...
export interface I18nConfigClient {
  getConfig: () => Promise<HadesConfigRecord>
  saveConfig: (config: HadesConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.hadesDesktop?.api) {
      return Promise.resolve({})
    }

    return getHadesConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.hadesDesktop?.api) {
...
export function getConfigDisplayLanguage(config: HadesConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: HadesConfigRecord, locale: Locale): HadesConfigRecord {
