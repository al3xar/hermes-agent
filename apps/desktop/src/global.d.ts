...
declare global {
  interface Window {
    hadesDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<HadesConnection>
      ...
      api: <T>(request: HadesApiRequest) => Promise<T>
      notify: (payload: HadesNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<HadesReadFileTextResult>
      selectPaths: (options?: HadesSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<HadesPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<HadesPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: HadesTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<HadesReadDirResult>
      gitRoot?: (path: string) => Promise<string | null>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: HadesTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<HadesTerminalSession>
        write: (id: string, data: string) => Promise<boolean>
      }
      onClosePreviewRequested?: (callback: () => void) => () => void
      onOpenUpdatesRequested?: (callback: () => void) => () => void
      onWindowStateChanged?: (callback: (payload: HadesWindowState) => void) => () => void
      onPreviewFileChanged: (callback: (payload: HadesPreviewFileChanged) => void) => () => void
...
export interface HadesTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface HadesTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `hades update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  hadesRoot?: string
}
...
export interface HadesConnection {
  baseUrl: string
...
export interface HadesTitleBarTheme {
  background: string
  foreground: string
}

export interface HadesWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface HadesApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface HadesNotification {
  title?: string
  body?: string
  silent?: boolean
}

export interface HadesPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface HadesReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface HadesPreviewWatch {
  id: string
  path: string
}

export interface HadesReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface HadesReadDirResult {
  entries: HadesReadDirEntry[]
  error?: string
}

export interface HadesPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface HadesSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
