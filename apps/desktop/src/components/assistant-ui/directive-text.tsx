...
const HADES_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type HadesRefType = (typeof HADES_REF_TYPES)[number]
...
const ICON_PATHS: Record<HadesRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as HadesRefType] ?? ICON_FALLBACK
...
const HADES_DIRECTIVE_RE = new RegExp(
...
export const hadesDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(HADES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as HadesRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: HadesRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Hades directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => hadesDirectiveFormatter.parse(cleanedText), [cleanedText])
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.hadesDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.hadesDesktop?.readFileDataUrl(id)
