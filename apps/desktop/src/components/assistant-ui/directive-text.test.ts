...
import { formatRefValue, hadesDirectiveFormatter } from './directive-text'
...
describe('hadesDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = hadesDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = hadesDirectiveFormatter.parse('@file:src/main.tsx the entry point')
