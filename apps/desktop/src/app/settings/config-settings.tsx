...
import {
  getElevenLabsVoices,
  getHadesConfigDefaults,
  getHadesConfigRecord,
  getHadesConfigSchema,
  saveHadesConfig
} from '@/hades'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, HadesConfigRecord } from '@/types/hades'
...
export function ConfigSettings({
  ...
  const { t } = useI18n()
  const c = t.settings.config
  const [config, setConfig] = useState<HadesConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<HadesConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getHadesConfigRecord(), getHadesConfigDefaults(), getHadesConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  const updateConfig = (next: HadesConfigRecord) => {
