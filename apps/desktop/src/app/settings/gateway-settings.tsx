...
export function GatewaySettings() {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.hadesDesktop
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.hadesDesktop.applyConnectionConfig(payload())
        : await window.hadesDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.hadesDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.hadesDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.hadesDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.hadesDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.hadesDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.hadesDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.hadesDesktop?.getConnectionConfig) {
    return (
      <EmptyState
  ...
  return (
    <SettingsContent>
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/hades"
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.hadesDesktop?.revealLogs()} size="sm" variant="textStrong">
              <FileText />
              {g.openLogs}
            </Button>
          }
