        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.hadesDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Hades to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.hadesDesktop?.watchPreviewFile ||
      !window.hadesDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.hadesDesktop.onPreviewFileChanged(payload => {
    ...
    void window.hadesDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.hadesDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.hadesDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:hades-preview')
