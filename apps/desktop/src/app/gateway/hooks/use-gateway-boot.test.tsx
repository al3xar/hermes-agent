...
// bug that drives the REAL useGatewayBoot hook + REAL HadesGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshHadesConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { hadesDesktop?: unknown }).hadesDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { hadesDesktop?: unknown }).hadesDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForHades) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startHades()'s remote branch awaits waitForHades() for 45s before it
    ...
    ;(window as { hadesDesktop?: unknown }).hadesDesktop = desktop
    ...
    // After ~45s waitForHades gives up and getConnection rejects → boot()
