...
import type { HadesConnection } from '@/global'
import { HadesGateway } from '@/hades'
...
import type { RpcEvent } from '@/types/hades'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.hadesDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: HadesGateway | null) => void
  refreshHadesConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: HadesConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Hades...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Hades…". The probe is a no-op for a healthy or local backend.
        ...
        await callbacksRef.current.refreshHadesConfig().catch(() => undefined)
    ...
    const gateway = new HadesGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshHadesConfig()
