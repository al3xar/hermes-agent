...
const HADES_PATHS_MIME = 'application/x-hades-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(HADES_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(HADES_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.hadesDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      fontSize: 11,
      lineHeight: 1.12,
      // Full-screen TUIs (hades --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[hades-terminal] WebGL unavailable; falling back to DOM', err)
      }
