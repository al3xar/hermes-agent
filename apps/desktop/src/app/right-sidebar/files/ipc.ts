...
import type { HadesReadDirEntry, HadesReadDirResult } from '@/global'

export type ProjectTreeEntry = HadesReadDirEntry
...
async function gitRootFor(start: string) {
  if (!window.hadesDesktop?.gitRoot) {
    return null
  }
  ...
  if (!cached) {
    cached = window.hadesDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.hadesDesktop?.readDir || !window.hadesDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.hadesDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.hadesDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: HadesReadDirEntry) {
...
async function filterIgnored(entries: HadesReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<HadesReadDirResult> {
  if (!window.hadesDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.hadesDesktop.readDir(dirPath)
