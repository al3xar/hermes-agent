...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active Hades profile'],
  ...
  ['/version', 'Show Hades Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownHadesSlashCommand(normalized)
}
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownHadesSlashCommand(normalized)
}
...
function isKnownHadesSlashCommand(command: string): boolean {
