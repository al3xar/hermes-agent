...
describe('gatewayMediaDataUrl', () => {
  ...
  beforeEach(() => {
    api.mockClear()
    vi.stubGlobal('window', { hadesDesktop: { api } })
  })
  ...
  it('requests the encoded gateway path and returns the data URL', async () => {
    const url = await gatewayMediaDataUrl('/home/u/.hades/images/a b.png')

    expect(url).toBe('data:image/png;base64,ZHVtbXk=')
    expect(api).toHaveBeenCalledWith({
      path: '/api/media?path=%2Fhome%2Fu%2F.hades%2Fimages%2Fa%20b.png'
    })
  })
})
