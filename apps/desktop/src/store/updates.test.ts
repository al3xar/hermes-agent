...
const checkHadesUpdateSpy = vi.fn()
const updateHadesSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/hades', () => ({
  checkHadesUpdate: (...args: unknown[]) => checkHadesUpdateSpy(...args),
  updateHades: (...args: unknown[]) => updateHadesSpy(...args),
...
describe('checkBackendUpdates', () => {
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkHadesUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'hades update',
      message: null,
    ...
    expect(checkHadesUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkHadesUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateHadesSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkHadesUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'hades update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateHadesSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkHadesUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
