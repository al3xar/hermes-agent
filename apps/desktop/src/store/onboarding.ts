...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/hades'
...
const CONFIGURED_CACHE_KEY = 'hades-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'hades-onboarding-skipped-v1'
const POLL_MS = 2000
...
async function fetchProviderDefaultModel(
  ...
  // `hades model` does (for Nous it honors the user's free/paid tier, so a
...
async function openSignInUrl(url: string) {
  if (window.hadesDesktop?.openExternal) {
    try {
      await window.hadesDesktop.openExternal(url)
