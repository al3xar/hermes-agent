import type { ConnectionState, GatewayEvent } from '@hades/shared'
...
const isOpen = (gateway: HadesGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<HadesGateway | null>(null)
...
let primaryGateway: HadesGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: HadesGateway | null, profile = 'default'): void {
...
export function activeGateway(): HadesGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<HadesGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
