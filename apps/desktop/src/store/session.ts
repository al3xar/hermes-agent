...
import type { SessionInfo, UsageStats } from '@/types/hades'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'hades.desktop.workspace-cwd'
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.hadesDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.hadesDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<HadesConnection | null>(null)
...
export const setConnection = (next: Updater<HadesConnection | null>) => updateAtom($connection, next)
