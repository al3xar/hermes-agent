...
import { getProfiles, setApiRequestProfile } from '@/hades'
...
import type { ProfileInfo } from '@/types/hades'
...
const PROFILE_ORDER_STORAGE_KEY = 'hades.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'hades.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.hadesDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.hadesDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'hades.desktop.showAllProfiles'
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.hadesDesktop?.touchBackend?.(target).catch(() => undefined)
}
