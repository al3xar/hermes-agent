...
export const $hadesHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, hadesHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_hades_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $hadesHome.set(hadesHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Hades" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Hades-Setup.exe --update);
...
export async function launchHadesDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_hades_desktop', { installRoot })
}
