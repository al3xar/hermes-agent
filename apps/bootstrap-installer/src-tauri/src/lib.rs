...
/// desktop app handing off via `Hades-Setup.exe --update`).
///
...
pub fn run() {
    ...
    // Hades is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Hades installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Hades is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Hades" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Hades" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_hades_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hades_home().join("hades-agent");
                if bootstrap::hades_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_hades_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "hades already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            update::start_update,
            // Hand-off
            bootstrap::launch_hades_desktop,
            // Diagnostics
            paths::get_log_path,
        ...
        .expect("error while running Hades Setup");
}
