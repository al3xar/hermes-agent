...
//! Electron desktop's resolveHadesHome() ALL use ~/.hades on macOS — there
...
pub fn hades_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\hades — matches scripts/install.ps1's $HadesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("hades");
        }
    }

    // macOS + Linux + fallback: ~/.hades (matches Python get_hades_home(),
    // install.sh, and the Electron desktop's resolveHadesHome()).
...
pub fn bootstrap_cache_dir() -> PathBuf {
    hades_home().join("bootstrap-cache")
}
...
/// hades-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\hades\hades-setup.exe`; on other
...
pub fn copy_self_to_hades_home() -> std::io::Result<()> {
...
/// We don't always know ACTIVE_HADES_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".hades-bootstrap-complete")
}
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[hades-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("HADES_BOOTSTRAP_LOG")
...
pub fn get_hades_home() -> String {
    hades_home().to_string_lossy().into_owned()
}
