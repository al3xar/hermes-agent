...
//!
//! Driven when the installer is launched as `Hades-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Hades desktop process to fully exit (so the venv
//!      shim is free; otherwise `hades update` aborts with exit code 2),
//!   2. run `hades update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in hades_cli/main.py),
//!   3. run `hades desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `hades update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_hades) and
...
/// `hades update` exit code meaning "another hades process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// hades_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release the venv shim
/// before giving up and letting `hades update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Hades"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let hades_home = crate::paths::hades_home();
    let install_root = hades_home.join("hades-agent");
    ...
    let hades = resolve_hades(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the hades CLI under {}. Is Hades installed? \
    ...
    // async on Windows. If it still holds the venv shim, `hades update`
    // aborts with exit 2. Give it a bounded window to clear.
    wait_for_venv_free(&install_root, &app).await;

    // ---- stage 1: hades update -----------------------------------------
    // Pass --branch so `hades update` targets the branch this installer was
    ...
    // without --branch, `hades update` switches the checkout to `main` (a
    ...
    // --force skips `hades update`'s Windows running-exe guard (which would
    ...
    // time `hades update` runs there is no legitimate hades.exe to protect,
    // and the guard would only produce a false "Hades is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `hades update` lazily imports
    ...
    // second `hades update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Hades" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Hades is still running. Close all Hades windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "hades update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::hades_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: hades desktop --build-only ----------------------------
    // `hades update` deliberately does NOT build apps/desktop (it installs
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `hades desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Hades manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_hades_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
async fn wait_for_venv_free(install_root: &Path, app: &AppHandle) {
    let shim = venv_hades(install_root);
    let deadline = Instant::now() + DESKTOP_EXIT_WAIT;

    emit_log(app, Some("update"), LogStream::Stdout, "[update] waiting for Hades to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend hades.exe (or a grandchild it spawned)
            ...
            // every hades.exe except ourselves, then give the OS a beat to
            // unload the image.
            emit_log(
                app,
                Some("update"),
                LogStream::Stdout,
                "[update] Hades still holding the venv shim; force-killing stragglers…",
            );
            force_kill_other_hades();
...
/// Force-kill any `hades.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `hades.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside `wait_for_venv_free`,
/// which completes BEFORE we spawn `venv\Scripts\hades.exe update`. At this
/// point no update-driven hades.exe exists yet, so the only hades.exe images
...
/// Spawn `hades <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv hades shim under an install root, regardless of existence.
fn venv_hades(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("hades.exe")
    } else {
        install_root.join("venv").join("bin").join("hades")
    }
}

/// Resolve the hades CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `hades` on PATH.
fn resolve_hades(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_hades(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "hades.exe" } else { "hades" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        ...
        hades_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        hades_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_hades_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Hades.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.hades-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.hades-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::hades_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_hades_is_under_install_root() {
        let root = Path::new("/x/hades-agent");
        let shim = venv_hades(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Hades.app"]),
            Some(PathBuf::from("/Applications/Hades.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Hades.app");
        let tmp = base.join("Hades.app.hades-update-new");
        let old = base.join("Hades.app.hades-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Hades.app");
        let tmp = base.join("Hades.app.hades-update-new"); // intentionally absent
        let old = base.join("Hades.app.hades-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Hades.app");
        let tmp = base.join("Hades.app.hades-update-new"); // absent
        let old = base.join("Hades.app.hades-update-old");
        write_marker(&target, "OLD");
