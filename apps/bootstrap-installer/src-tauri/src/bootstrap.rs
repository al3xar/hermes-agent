...
/// Spawn the locally-built Hades desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_hades_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_hades_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Hades desktop at {}. The desktop build step \
             may have been skipped or failed. Run `hades desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Hades desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Hades
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_hades_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Hades.exe"),
            ("win-arm64-unpacked", "Hades.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Hades.app/Contents/MacOS", "Hades"),
            ("mac-arm64/Hades.app/Contents/MacOS", "Hades"),
        ]
    } else {
        &[("linux-unpacked", "hades")]
    };
...
pub(crate) fn resolve_hades_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_hades_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Hades.app/Contents/MacOS/Hades -> .../Hades.app
...
/// path so a bare re-open just opens Hades instead of re-running setup.
pub(crate) fn hades_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".hades-bootstrap-complete").exists()
        && resolve_hades_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_hades_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Hades desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_hades_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::hades_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $HadesHome which Stage-Repository clones
    // the repo INTO at $HadesHome\hades-agent. Mirrors hades_constants.
    let hades_home = args
        .hades_home
        .clone()
        .unwrap_or_else(|| crate::paths::hades_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&hades_home).join("hades-agent");

    // Copy ourselves to HADES_HOME/hades-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_hades_home() {
...
async fn run_install_script(
    ...
    args: &[String],
    hades_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, hades_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Hades"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Hades.app
    ...
    // what the updater ditto's over /Applications/Hades.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_hades_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_hades_desktop_app(&root)
    ...
    #[test]
    fn resolve_hades_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_hades_desktop_app(&root).is_none(),
