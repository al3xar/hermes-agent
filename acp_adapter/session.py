...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Hades itself is running in WSL.

    Windows ACP clients can launch ``hades acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Hades from a Windows workspace while the ACP process runs
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Hades runtime provider configuration.
    ...
    def _get_db(self):
        ...
        later (e.g. by the test fixture ``_isolate_hades_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=hades_home / "state.db")
            return self._db_instance
        except Exception:
