"""CLI entry point for the hades-agent ACP adapter.
...
def _load_env() -> None:
    ...
    loaded = load_hades_dotenv(hades_home=hades_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", hades_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--version", action="store_true", help="Print Hades version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    from acp_adapter.server import HadesACPAgent  # noqa: F401

    print("Hades ACP check OK")
...
def _run_setup() -> None:
    ...
    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "hades", "model"]
        hades_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``hades postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting hades-agent ACP adapter")
    ...
    import acp
    from .server import HadesACPAgent
