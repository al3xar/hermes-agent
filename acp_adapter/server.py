...
try:
    from hades_cli import __version__ as HADES_VERSION
except Exception:
    HADES_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Hades running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Hades cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Hades/OpenAI-compatible user content payload."""
...
class HadesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Hades AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Hades version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Hades maps edit approval
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Hades estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_hades_session_id: str,
        previous_hades_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.hades.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_hades_session_id,
                previous_hades_session_id=previous_hades_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_hades_session_id: Optional[str] = None,
        previous_hades_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Hades changes it.

        When the internal Hades head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_hades_session_id`` so the
        attached ``_meta.hades.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        meta = self._provenance_meta(
            session_id,
            current_hades_session_id or session_id,
            previous_hades_session_id,
        )
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="hades-agent", version=HADES_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Hades setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Hades
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Hades on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # not the executor's. Also set HADES_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue HADES_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind HADES_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("HADES_INTERACTIVE")
            ...
            try:
                ...
                # Restore HADES_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("HADES_INTERACTIVE", None)
                else:
                    os.environ["HADES_INTERACTIVE"] = previous_interactive
                # Restore HADES_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("HADES_SESSION_ID", None)
                else:
        ...
        try:
            # Snapshot the internal Hades DB session id before the turn so we
            ...
            pre_turn_hades_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (HADES_SESSION_KEY in
        ...
        # _meta.hades.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_hades_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_hades_id
            and pre_turn_hades_id
            and post_turn_hades_id != pre_turn_hades_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_hades_session_id=post_turn_hades_id,
                    previous_hades_session_id=pre_turn_hades_id,
                )
            except Exception:
                logger.debug(
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Hades Agent v{HADES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Hades has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
