"""ACP auth helpers — detect and advertise Hades authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "hades-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Hades runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Hades can resolve any runtime provider credentials."""
...
def build_auth_methods() -> list[Any]:
    ...
    not have Hades provider credentials configured yet, so Hades always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Hades using the currently configured "
