...
Resolution order:
  1. ``HADES_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.hades/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Hades never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Hades requires 3.9+)
...
def reset_cache() -> None:
    ...
    config edit or ``HADES_TIMEZONE`` update) to force ``get_timezone()`` /
