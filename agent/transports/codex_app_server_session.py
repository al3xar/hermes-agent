...
Owns one Codex thread per Hades session. Drives `turn/start`, consumes
...
_HADES_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Hades/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Hades session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _HADES_TO_CODEX_PERMISSION_PROFILE.get(
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            ...
            client_version=_get_hades_version(),
        )
    ...
    def _handle_server_request(self, req: dict) -> None:
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own hades-tools callback we
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Hades approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Hades returns 'once', 'session', 'always', or 'deny'.
...
def _get_hades_version() -> str:
    """Best-effort Hades version string for codex's userAgent line."""
    try:
