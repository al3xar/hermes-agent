...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    hades_home = _hades_home_path()
    hades_root = _hades_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(hades_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(hades_root / ".anthropic_oauth.json"),
...
def get_safe_write_root() -> Optional[str]:
    """Return the resolved HADES_WRITE_SAFE_ROOT path, or None if unset."""
    root = os.getenv("HADES_WRITE_SAFE_ROOT", "")
    if not root:
        return None
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Hades control-plane files: block both the ACTIVE profile's view
    # (hades_home) AND the global root view. Without the root pass, a
    ...
    hades_dirs = []
    for base in (_hades_home_path(), _hades_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in hades_dirs:
                hades_dirs.append(real)
        except Exception:
            continue

    for base_real in hades_dirs:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Hades path.

    Three categories are blocked:

      * Internal Hades cache files under ``HADES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HADES_HOME and the global Hades
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    # Hades root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hades_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Hades cache file "
    ...
    for hd in hades_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Hades credential store "
    ...
    for hd in hades_dirs:
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Hades MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Hades profiles are separate HADES_HOME dirs under
...
#
# Reference: May 2026 incident where a hades-security profile session
# edited skills under both ``~/.hades/profiles/hades-security/skills/``
...
def _resolve_active_profile_name() -> str:
    ...
    """
    try:
        home_real = _hades_home_path().resolve()
        root_real = _hades_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Hades scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Hades entirely. Caller is expected to surface the warning to the
...
# ``…/sandboxes/<backend>/<task>/home/.hades/…`` segment and warns
...
# (the agent's view inside the container is plain ``/root/.hades/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.hades`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.hades/…`` and returns the
    index where the inner Hades-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .hades / <thing>
        if i + 5 >= len(parts):
            continue
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    ...
    Otherwise returns a dict with:
      ...
      * ``inner_path``: the portion under the mirror's ``.hades`` (what the
...
# that prefix: the agent sees plain ``/root/.hades/…``.  The root:root
