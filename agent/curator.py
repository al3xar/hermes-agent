...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `hades update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``hades update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``hades curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `hades curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    ...
    "to find: hades-config-*, hades-dashboard-*, gateway-*, codex-*, "
...
def _reports_root() -> Path:
    ...
    ``ensure_hades_home()`` pre-creates this dir on every CLI launch and
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every hades install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: hades curator status
        keep an umbrella stable: hades curator pin document-tools
    ...
    lines.append("full report: hades curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: hades curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "safety and can be restored via `hades curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Restore any via `hades curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `hades curator restore <name>`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `hades curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `hades update` only if "
        ...
        # recorded in state so `hades curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``hades model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `hades model` → auxiliary
