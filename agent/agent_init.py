...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: hades config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    # report cumulative micros spent.  Surfaced behind HADES_DEV_CREDITS.
    ...
    setup_logging(hades_home=_ra()._hades_home)
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `hades model` to "
                        "select a provider, or run `hades setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff HADES_KANBAN_TASK is set).
    ...
    agent.logs_dir = hades_home / "sessions"
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "hades"
                    except Exception:
                        pass
