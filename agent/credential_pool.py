...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``hades model`` / ``hades auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Hades process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``hades`` invocation that defaults to the active provider
        (e.g. setup wizard, ``hades auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("hades_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Hades
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("hades_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Hades process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Hades profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `hades model` / `hades auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `hades model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``hades auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `hades auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `hades auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Hades PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `hades setup` ("Claude
        ...
        # Claude Code CLI, hades_pkce creds from a previous OAuth login)
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"hades_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_hades_oauth_credentials

        for source_name, creds in (
            ("hades_pkce", read_hades_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `hades auth add nous --label <name>`).  Fall back to the
        ...
        # Hades auth store or env vars, so resolve them here.
        ...
        # standard `hades auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `hades auth remove openai-codex` marks
        ...
        # Hades owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``hades model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``hades auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `hades auth remove <provider> <N>` for an
