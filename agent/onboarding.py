...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Hades starts and finds ``~/.openclaw/``.

    Points users at ``hades claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``hades claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "run `hades claw cleanup` (renames it to ~/.openclaw.pre-migration — "
