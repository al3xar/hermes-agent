"""OpenAI-compatible shim that forwards Hades requests to `copilot --acp`.

This adapter lets Hades treat the GitHub Copilot ACP server as a chat-style
backend. Each request starts a short-lived ACP session, sends the formatted
conversation as a single prompt, collects text chunks, and converts the result
back into the minimal shape Hades expects from an OpenAI client.
"""
...
def _format_messages_as_prompt(
    ...
    if model:
        sections.append(f"Hades requested model hint: {model}")
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            if proc.poll() is not None and stderr_text:
                if _is_gh_copilot_deprecation_message(stderr_text):
                    raise RuntimeError(
                        "Hades ACP mode requires the NEW GitHub Copilot CLI "
                        ...
                        "point Hades at it explicitly:\n"
                        "  export HADES_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
                        "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\n"
                        "directly with a Copilot subscription token) via `hades setup`.\n\n"
                        f"Original error:\n{stderr_text}"
                    )
