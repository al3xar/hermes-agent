...
_HADES_MD_NAMES = (".hades.md", "HADES.md")


def _find_hades_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.hades.md`` or ``HADES.md``.
...
HADES_AGENT_HELP_GUIDANCE = (
    ...
    "Hades itself — configuring, setting up, using, extending, or troubleshooting "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Hades "
...
# container / remote host rather than on the machine where Hades itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Hades runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Hades itself is running. The host OS, home, and cwd "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Hades "
    ...
    # Hades desktop GUI — any agent running under the desktop app should know
    # it. HADES_DESKTOP marks the backend powering the chat; HADES_DESKTOP_TERMINAL
    # marks a hades launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("HADES_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("HADES_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Hades desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Hades
...
def build_skills_system_prompt(
    ...
    _platform_hint = (
        os.environ.get("HADES_PLATFORM")
        or get_session_env("HADES_SESSION_PLATFORM")
        or ""
    )
...
def _load_hades_md(cwd_path: Path) -> str:
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".hades.md")
    except Exception as e:
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .hades.md / HADES.md  (walk to git root)
