"""Unified removal contract for every credential source Hades reads from.

Hades seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.hades/.env
    claude_code   — ~/.claude/.credentials.json
    hades_pkce   — ~/.hades/.anthropic_oauth.json
    ...
    manual        — user ran `hades auth add`
...
unify here is **removal**:
    ...
    ``hades auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), hades_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"  The pool entry is now suppressed — Hades will ignore "
            f"{env_var} until you run `hades auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Hades stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `hades auth add anthropic` to re-enable if needed.",
    ])


def _remove_hades_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Hades Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `hades auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `hades auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``hades auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``hades auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `hades model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    via ``hades auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `hades auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `hades auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Hades stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `hades auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="hades_pkce",
        remove_fn=_remove_hades_pkce,
