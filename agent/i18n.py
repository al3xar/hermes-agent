...
Language resolution order:
    ...
    2. ``HADES_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``HADES_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    if override:
        ...
        if candidate.is_dir():
            return candidate
        logger.warning(
            "HADES_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a hades dependency
