...
# Matches ${HADES_SKILL_DIR} / ${HADES_SESSION_ID} tokens in SKILL.md.
# Tokens that don't resolve (e.g. ${HADES_SESSION_ID} with no session) are
# left as-is so the user can debug them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(HADES_SKILL_DIR|HADES_SESSION_ID)\}")
...
def substitute_template_vars(
    ...
    """Replace ${HADES_SKILL_DIR} / ${HADES_SESSION_ID} in skill content.
    ...
    def _replace(match: re.Match) -> str:
        token = match.group(1)
        if token == "HADES_SKILL_DIR" and skill_dir_str:
