...
Hades pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.hades/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hades_home>/bin/bws`` on
  first use.  Hades pins one version (``_BWS_VERSION``) and downloads
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``hades`` invocations don't
  hammer the API.
* Failures NEVER block Hades startup.  Missing binary, no network,
...
# In-process cache so repeated load_hades_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, "_CachedFetch"] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `hades chat -q ...`
...
# <hades_home>/cache/bws_cache.json. The file holds only the secret VALUES,
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under hades_home/cache/.

    `home_path` is what `load_hades_dotenv()` already resolved; falling back
    to `$HADES_HOME` / `~/.hades` keeps direct callers working too.
    """
...
def _hades_bin_dir() -> Path:
    """Where Hades stores its managed binaries.  Profile-aware."""
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<hades_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _hades_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``hades secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _hades_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="hades-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<hades_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_hades_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `hades secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `hades secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `hades secrets bitwarden setup` to install."
        )
        return result
