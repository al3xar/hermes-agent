...
Hades keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). Hades typically makes 3-10 API calls per user turn, "
...
def build_gemini_request(
    *,
    ...
    if max_tokens is not None:
        ...
        # finishReason=MAX_TOKENS, truncating tool calls mid-stream (Hades
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "(get one at https://aistudio.google.com/app/apikey), or run `hades setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "hades-agent (gemini-native)",
        }
