...
Every Hades request that hits the Nous Portal — main agent loop, auxiliary
...
Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):
    ...
    [
        "product=hades-agent",
        "client=hades-client-v<__version__>",
    ]
...
def _hades_version() -> str:
    """Return the current Hades release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``hades_cli`` cannot be imported (should
...
def hades_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=hades-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=hades-client-v{_hades_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=hades-agent", hades_client_tag()]
