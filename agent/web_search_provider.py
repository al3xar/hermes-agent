...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``hades tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``hades tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``hades tools`` picker.

        Used by ``hades_cli/tools_config.py`` to inject this provider as a
