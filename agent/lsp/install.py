...
appropriate.  All installs go to a Hades-owned bin staging dir,
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``hades lsp
  status``.
...
def hades_lsp_bin_dir() -> Path:
    """Return the Hades-owned bin staging dir for LSP servers."""
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(hades_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hades_lsp_bin_dir().parent  # <HADES_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a hades-owned target dir.
    ...
    console script.
    """
    pip_target = hades_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = hades_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``hades lsp status`` CLI to give users a quick
