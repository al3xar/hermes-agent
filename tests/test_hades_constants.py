...
class TestGetDefaultHadesRoot:
    """Tests for get_default_hades_root() — Docker/custom deployment awareness."""

    def test_no_hades_home_returns_native(self, tmp_path, monkeypatch):
    ...
    def test_hades_home_is_native(self, tmp_path, monkeypatch):
    ...
    def test_hades_home_is_profile(self, tmp_path, monkeypatch):
    ...
    def test_hades_home_is_docker(self, tmp_path, monkeypatch):
    ...
    def test_hades_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any HADES_HOME outside ~/.hades is treated as the root."""
        custom = tmp_path / "my-hades-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_no_hades_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
    ...
    def test_no_hades_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
...
class TestGetHadesHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When HADES_HOME is unset on Windows, use %LOCALAPPDATA%\\hades."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("HADES_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
