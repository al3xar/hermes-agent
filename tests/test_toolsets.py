...
class TestToolsetConsistency:
    ...
    def test_hades_platforms_share_core_tools(self):
        """All hades-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on hades-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["hades-cli", "hades-telegram", "hades-discord", "hades-whatsapp", "hades-slack", "hades-signal", "hades-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hades_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hades-whatsapp")

    def test_hades_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hades-api-server")
