...
def scenario(name):
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"hades_atyp_{name}_")
            os.environ["HADES_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("hades_cli", "plugins", "gateway")):
                    del sys.modules[m]
    ...
    try:
        ...
        try:
            ...
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE hades_home: {resolved}")
...
@scenario("hades_home_with_spaces")
def _(home, kb):
    ...
    weird = tempfile.mkdtemp(prefix="hades with spaces ")
...
@scenario("hades_home_via_symlink")
def _(home, kb):
    ...
    real = tempfile.mkdtemp(prefix="hades_real_")
    link1 = real + "_link1"
    link2 = real + "_link2"
