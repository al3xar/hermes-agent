...
Reads HADES_KANBAN_TASK from env, heartbeats periodically, does short
work, completes via the CLI. Designed to be spawned by the dispatcher
exactly the way `hades chat -q` would be, minus the LLM cost.
"""
...
def main():
    ...
    subprocess.run(
        ["hades", "kanban", "heartbeat", tid, "--note", "started"],
    ...
    for i in range(3):
        time.sleep(0.3)
        subprocess.run(
            ["hades", "kanban", "heartbeat", tid, "--note", f"progress {i+1}/3"],
            check=True, capture_output=True,
        )
