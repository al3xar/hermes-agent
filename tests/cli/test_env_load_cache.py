...
The cache exists to keep `hades tools` → "All Platforms" fast: every
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(config_mod, "ensure_hades_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "hades-test-no-such-env-xyz123.env"
