...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under hades_home/profiles/<name>/."""
    p = hades_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `hades profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    if state is not None:
        (hades_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (hades_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (hades_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        hades_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        hades_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    so `hades gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        hades_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    the root profile. It must call `hades gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    ...
    assert "hades gateway run" in run
...
def test_legacy_gateway_run_env_no_supervise_does_not_seed_s6_state(
    ...
    scandir = tmp_path / "run-service"; scandir.mkdir()
    monkeypatch.setenv("HADES_GATEWAY_NO_SUPERVISE", "1")
