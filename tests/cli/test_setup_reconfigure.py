...
- Bare `hades setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `hades setup --quick` runs the narrower "fill in missing items" flow.
- `hades setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("hades_cli.setup.ensure_hades_home", {}),
...
class TestExistingInstallDefault:
    """Bare `hades setup` on an existing install = full reconfigure wizard."""
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`hades setup --reconfigure` behaves the same as bare `hades setup`."""
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["hades", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["hades", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["hades", "setup"])
        try:
            main()
        except SystemExit:
            pass
