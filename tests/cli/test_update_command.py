...
Verifies that ``HadesCLI._handle_update_command`` correctly:
...
def _make_self(modal_response):
    ...
    so we don't need a full ``HadesCLI`` construction.
...
def _call(self_):
    ...
    return HadesCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        ...
        patch(
            ...
            return_value="Use `brew upgrade hades-agent` to update.",
        ),
    ):
        result = _call(self_)

    out = capsys.readouterr().out
    assert "brew upgrade hades-agent" in out
