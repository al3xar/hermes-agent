...
def isolated_kanban_home(monkeypatch):
    ...
    for mod in list(sys.modules.keys()):
        if mod.startswith("hades_cli") or mod.startswith("hades_state") or mod == "hades_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: hades kanban dispatch must pass kanban.max_in_progress from
