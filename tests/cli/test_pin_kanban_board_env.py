...
board into `HADES_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`hades kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `hades kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `HADES_KANBAN_BOARD` and restore it after the test.
...
def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("HADES_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("hades_cli.main")
...
def test_pin_swallows_resolution_failures(monkeypatch):
    ...
    assert "HADES_KANBAN_BOARD" not in main_mod.os.environ
