...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("HADES_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("HADES_HOME", str(home))
    monkeypatch.delenv("HADES_KANBAN_DB", raising=False)
    monkeypatch.delenv("HADES_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("HADES_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("HADES_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """HADES_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import hades_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("HADES_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("HADES_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("HADES_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("HADES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("HADES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "HADES_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Hades profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="hades-task", assignee="daily")
...
def test_scratch_workspace_created_under_hades_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``HADES_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("HADES_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
# spawned with `hades -p <profile>` must read/write the same kanban.db
...
class TestSharedBoardPaths:
    ...
    def _set_home(self, monkeypatch, tmp_path, hades_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        monkeypatch.delenv("HADES_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_hades(
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # Worker's perspective (profile activated by `hades -p coder`).
    ...
    def test_docker_custom_hades_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
    ...
    def test_explicit_override_via_hades_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: HADES_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".hades"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HADES_HOME", str(profile_home))
        monkeypatch.setenv("HADES_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_KANBAN_HOME", "   ")
    ...
    def test_hades_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # HADES_KANBAN_DB pins the file path directly and beats both
        # HADES_KANBAN_HOME and the `get_default_hades_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HADES_HOME", str(default_home))
        monkeypatch.setenv("HADES_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("HADES_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows HADES_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_hades_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # HADES_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".hades"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("HADES_HOME", str(default_home))
        monkeypatch.setenv("HADES_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("HADES_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows HADES_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HADES_HOME", str(default_home))
        monkeypatch.setenv("HADES_KANBAN_DB", "   ")
        monkeypatch.setenv("HADES_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject HADES_KANBAN_DB
        # and HADES_KANBAN_WORKSPACES_ROOT into the worker env so the
        ...
        env = captured["env"]
        assert env["HADES_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["HADES_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["HADES_KANBAN_TASK"] == "t_dispatch_env"
        assert env["HADES_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    with _patch("hades_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="hades_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'hades kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_hades_argv()
#
# Workers spawned by the dispatcher must use a `hades` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["hades", ...]`
...
def test_resolve_hades_argv_prefers_path_shim(monkeypatch):
    """When `hades` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    ...
    argv = kb._resolve_hades_argv()
    assert argv == ["/usr/local/bin/hades"]


def test_resolve_hades_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import hades_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("HADES_BIN", ".\\hades.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_hades_argv() == [os.path.abspath(".\\hades.exe")]


def test_resolve_hades_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "hades.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("HADES_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_hades_argv() == [sys.executable, "-m", "hades_cli.main"]


def test_resolve_hades_argv_honors_hades_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like HADES_BIN lets service managers pin the executable."""
    import shutil
    import hades_cli.kanban_db as kb
    ...
    monkeypatch.setenv("HADES_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_hades_argv() == [str(shim)]


def test_resolve_hades_argv_hades_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare HADES_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import hades_cli.kanban_db as kb

    cwd_hades = tmp_path / "hades"
    cwd_hades.write_text("wrong\n", encoding="utf-8")
    cwd_hades.chmod(cwd_hades.stat().st_mode | stat.S_IXUSR)
    path_hades = tmp_path / "bin" / "hades"
    path_hades.parent.mkdir()
    path_hades.write_text("right\n", encoding="utf-8")
    path_hades.chmod(path_hades.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_hades.parent))
    monkeypatch.setenv("HADES_BIN", "hades")

    assert kb._resolve_hades_argv() == [str(path_hades)]


def test_resolve_hades_argv_hades_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare HADES_BIN does not accept current-directory shadow executables."""
    import sys
    import hades_cli.kanban_db as kb

    (tmp_path / "hades.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_hades_argv_hades_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved HADES_BIN batch shim is not used as worker argv[0]."""
    import sys
    import hades_cli.kanban_db as kb
...
def test_resolve_hades_argv_hades_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved HADES_BIN command names do not delegate cwd search to Popen."""
    import sys
    import hades_cli.kanban_db as kb
...
def test_resolve_hades_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m hades_cli.main`.

    Pins the correct module name (NOT `hades` — there is no top-level
    `hades` package). Regression for #23198: the original PR shipped
    `python -m hades` which fails with `No module named hades` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_hades_argv_module_actually_runs():
