...
subparser, placing the flag BEFORE the subcommand (e.g. 'hades --yolo chat')
...
def _build_parser():
    """Build the hades argument parser from the real code.
...
class TestYoloEnvVar:
    """Verify --yolo sets HADES_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `hades gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `hades <argv>` must exit 0 (help) rather than
