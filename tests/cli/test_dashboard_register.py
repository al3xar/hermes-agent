"""Tests for ``hades dashboard register``.
...
class TestHappyPath:
    def _run(self, *, args, account_token="tok_abc", portal="https://portal.nousresearch.com",
        ...
        # (idempotency key) and once for HADES_DASHBOARD_PORTAL_URL. Route by
        ...
        def fake_get_env(key):
            if key == "HADES_DASHBOARD_OAUTH_CLIENT_ID":
    ...
    def test_writes_client_id_and_posts_generated_name(self, capsys):
        ...
        assert saved["HADES_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-1"
        assert "HADES_DASHBOARD_PORTAL_URL" not in saved
    ...
    def test_custom_redirect_uri_is_forwarded(self, capsys):
        captured: dict = {}
        self._run(
            args=_ns(redirect_uri="https://hades.example.com/auth/callback"),
            captured=captured,
        )
        assert (
            captured["body"]["custom_redirect_uri"]
            == "https://hades.example.com/auth/callback"
        )
    ...
    def test_non_default_portal_is_persisted(self, capsys):
        ...
        assert (
            saved["HADES_DASHBOARD_PORTAL_URL"]
...
class TestIdempotentRerun(TestHappyPath):
    ...
    ``existing_client_id`` (the HADES_DASHBOARD_OAUTH_CLIENT_ID a prior run
    ...
    def test_stale_id_falls_through_to_create_prints_registered(self, capsys):
        ...
        assert saved["HADES_DASHBOARD_OAUTH_CLIENT_ID"] == "agent:selfhost-new"
...
class TestCustomPortalPersistence:
    """`--portal-url` / HADES_DASHBOARD_PORTAL_URL is persisted to .env.
    ...
    def _run(self, *, args, portal, existing_portal):
        ...
        HADES_DASHBOARD_PORTAL_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "HADES_DASHBOARD_PORTAL_URL":
        ...
        with patch(
            ...
            # The ambient process env may carry HADES_DASHBOARD_PORTAL_URL
            # (e.g. staging dev shells); drop it so `custom_portal_supplied`
            # is driven solely by the args.portal_url under test.
            dr.os.environ.pop("HADES_DASHBOARD_PORTAL_URL", None)
            dr.cmd_dashboard_register(args)
        return saved
    ...
    def test_explicit_custom_url_persisted_when_var_absent(self, capsys):
        ...
        assert saved["HADES_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
    ...
    def test_explicit_custom_url_updates_existing_in_place(self, capsys):
        ...
        assert (
            saved["HADES_DASHBOARD_PORTAL_URL"] == "https://new-preview.example.com"
        )
    ...
    def test_explicit_custom_url_persisted_even_when_equals_default(self, capsys):
        ...
        assert (
            saved["HADES_DASHBOARD_PORTAL_URL"] == "https://portal.nousresearch.com"
        )
...
class TestPublicUrlPersistence:
    """`--redirect-uri` derives & persists HADES_DASHBOARD_PUBLIC_URL in .env.

    --redirect-uri is the full public callback (e.g.
    https://hades.example.com/auth/callback). At serve time the dashboard auth
    layer reconstructs that callback by appending "/auth/callback" to
    HADES_DASHBOARD_PUBLIC_URL, so the value that's actually consumed is the
    ORIGIN (scheme://host). We derive the origin from the supplied redirect URI
    and persist THAT as HADES_DASHBOARD_PUBLIC_URL — the var the runtime reads
    ...
    def _run(self, *, args, existing_public=None):
        ...
        HADES_DASHBOARD_PUBLIC_URL (None = not present in .env).
        """
        response = {
            "client_id": "agent:selfhost-1",
            "id": "selfhost-1",
        ...
        def fake_get_env_value(key, *a, **kw):
            if key == "HADES_DASHBOARD_PUBLIC_URL":
    ...
    def test_origin_derived_from_full_callback_path(self, capsys):
        ...
        assert saved["HADES_DASHBOARD_PUBLIC_URL"] == "https://hades.example.com"
        # The full callback path must NOT be persisted verbatim (would double
        # the path at serve time).
        assert "/auth/callback" not in saved["HADES_DASHBOARD_PUBLIC_URL"]

    def test_origin_preserves_port(self, capsys):
        saved = self._run(
            args=_ns(redirect_uri="https://hades.example.com:8443/auth/callback"),
            existing_public=None,
        )
        assert saved["HADES_DASHBOARD_PUBLIC_URL"] == "https://hades.example.com:8443"
    ...
    def test_public_url_updates_existing_in_place(self, capsys):
        ...
        assert saved["HADES_DASHBOARD_PUBLIC_URL"] == "https://new.example.com"

    def test_public_url_equal_to_existing_is_noop(self, capsys):
        # Derived origin already matches what's stored → no redundant write.
        saved = self._run(
            args=_ns(redirect_uri="https://hades.example.com/auth/callback"),
            existing_public="https://hades.example.com",
        )
        assert "HADES_DASHBOARD_PUBLIC_URL" not in saved
    ...
    def test_public_url_persisted_alongside_portal_url(self, capsys):
        ...
        response = {
            ...
            "name": "dreamy_tesla",
            "kind": "SELF_HOSTED",
            "custom_redirect_uri": "https://hades.example.com/auth/callback",
            "created_at": "2026-06-04T12:00:00.000Z",
        }
        saved: dict = {}
        ...
        with patch(
            ...
            dr.cmd_dashboard_register(
                _ns(
                    ...
                    redirect_uri="https://hades.example.com/auth/callback",
                )
            )
        assert saved["HADES_DASHBOARD_PORTAL_URL"] == "https://preview.example.com"
        assert saved["HADES_DASHBOARD_PUBLIC_URL"] == "https://hades.example.com"
