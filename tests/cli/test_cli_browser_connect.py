...
class TestChromeDebugLaunch:
    ...
    def test_windows_launch_uses_browser_found_on_path(self):
        ...
        with patch("hades_cli.browser_connect.shutil.which", side_effect=lambda name: r"C:\Chrome\chrome.exe" if name == "chrome.exe" else None), \
            ...
            assert HadesCLI._try_launch_chrome_debug(9333, "Windows") is True
    ...
    def test_windows_launch_falls_back_to_common_install_dirs(self, monkeypatch):
        ...
        with patch("hades_cli.browser_connect.shutil.which", return_value=None), \
             ...
             patch("subprocess.Popen", side_effect=fake_popen):
            assert HadesCLI._try_launch_chrome_debug(9222, "Windows") is True
    ...
    def test_launch_tries_next_browser_when_first_candidate_fails(self):
        ...
        with patch("hades_cli.browser_connect.get_chrome_debug_candidates", return_value=[brave, chrome]), \
             patch("subprocess.Popen", side_effect=fake_popen):
            assert HadesCLI._try_launch_chrome_debug(9222, "Linux") is True
    ...
    def test_connect_context_note_allows_expected_browser_use(self, monkeypatch):
        ...
        with patch("hades_cli.cli_commands_mixin.is_browser_debug_ready", return_value=True), \
