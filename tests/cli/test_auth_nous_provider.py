...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("HADES_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_hades_ca_bundle_env_falls_back(self, monkeypatch):
        from hades_cli.auth import _resolve_verify

        monkeypatch.setenv("HADES_CA_BUNDLE", "/nonexistent/hades-ca.pem")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    monkeypatch.setenv("HADES_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    hades_home = tmp_path / "hades"
    _setup_nous_auth(hades_home, access_token="at-123")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    hades_home = tmp_path / "hades"
    _setup_nous_auth(hades_home, access_token="at-fresh")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    hades_home = tmp_path / "hades"
    _setup_nous_auth(hades_home, refresh_token="refresh-old")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `hades model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return hades_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        hades_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `hades auth add nous`
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Hades is not logged into Nous Portal").
    ...
    # doesn't raise "Hades is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `hades auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Hades must surface an
    ...
    re-authenticate" string led users to report this as a Hades persistence
    ...
    assert "hades auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect HADES_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("HADES_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without HADES_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    monkeypatch.delenv("HADES_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `hades auth add nous
    ...
    hades_home = tmp_path / "hades"
    hades_home.mkdir(parents=True, exist_ok=True)
    (hades_home / "auth.json").write_text(
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "hades-cli"
