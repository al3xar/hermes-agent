...
Verifies the contract documented in Phase 6 v2 of the plan:
  ...
  - ``set_session_cookies(refresh_token="")`` does NOT emit the
    ``hades_session_rt`` cookie (contract V1: no RT to persist).
...
class TestApi401Envelope:
    ...
    def test_login_url_drops_next_for_deep_api_path(self, gated_app):
        ...
        from ``next=`` entirely; the SPA's own ``hades.lastLocation``
...
class TestTransparentRefreshOnAccessTokenEviction:
    ...
    the browser deletes ``hades_session_at`` the instant the token lapses,
    while ``hades_session_rt`` lives for 30 days. From that moment the
    ...
    def _build_rt_only_app(self):
        ...
        """
        import time as _t
        from tests.cli.conftest_dashboard_auth import _sign
