...
Regression: the old implementation wrote ``hades_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``hades sessions browse``, but CWD-resident
...
def hades_home(tmp_path, monkeypatch):
    ...
    # Clear any cached hades_home computation
    import hades_constants
    if hasattr(hades_constants, "_hades_home_cache"):
        hades_constants._hades_home_cache = None
    return home
...
def test_save_conversation_writes_under_hades_home(hades_home, tmp_path, monkeypatch, capsys):
    ...
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "hades_constants"]:
    ...
    cli.HadesCLI.save_conversation(stub)

    # File must NOT be in CWD
    cwd_leak = list(work.glob("hades_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.hades/sessions/saved/
    saved_dir = hades_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("hades_conversation_*.json"))
...
def test_save_conversation_empty_history_does_nothing(hades_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "hades_constants"]:
        sys.modules.pop(mod, None)
    import cli
