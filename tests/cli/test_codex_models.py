...
def test_get_codex_model_ids_prioritizes_default_and_cache(tmp_path, monkeypatch):
    ...
    # backend still accepts it via the OAuth-backed CLI/Hades route.
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in hades_cli/setup.py line 873.
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    ...
    monkeypatch.setattr(
        "hades_cli.auth.get_codex_auth_status",
        lambda: {"logged_in": True, "source": "hades-auth-store"},
    )
    monkeypatch.setattr(
        "hades_cli.auth.resolve_codex_runtime_credentials",
...
def _make_cli(model="anthropic/claude-opus-4.6", **kwargs):
    """Create a HadesCLI with minimal mocking."""
    ...
    with (
        ...
        cli = HadesCLI(model=model, **kwargs)
    return cli
...
class TestNormalizeModelForProvider:
    ...
    def test_default_model_replaced(self):
        ...
        with (
            patch("cli.get_tool_definitions", return_value=[]),
            patch.dict("os.environ", {"LLM_MODEL": "", "HADES_MAX_ITERATIONS": ""}, clear=False),
            patch.dict(_cli_mod.__dict__, {"CLI_CONFIG": _clean_config}),
        ):
            from cli import HadesCLI
            cli = HadesCLI()
