...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "hades gateway stop",
        "hades gateway start",
        "hades  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "HADES GATEWAY RESTART",           # uppercase
    ])
    def test_hades_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.hades.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.hades.gateway.plist",
        "launchctl stop ai.hades.gateway",
    ...
    @pytest.mark.parametrize("text", [
        "kill hades gateway process",
        "pkill -f hades.*gateway",
    ])
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_hades_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade hades then run hades gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.hades.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify hades gateway stop/restart refuse when _HADES_GATEWAY=1."""
