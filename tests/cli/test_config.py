"""Tests for hades_cli configuration management."""
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``hades logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        with patch.dict(os.environ, {"HADES_HOME": str(tmp_path)}):
            ...
            # stands out at hades startup before logging is configured)
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """HADES_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `hades setup`) lets a
        ...
        via config.yaml; HADES_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from hades_cli.config import OPTIONAL_ENV_VARS
        assert "HADES_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``HADES_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Hades
    process on next startup via the dotenv → ``os.environ`` chain in
    ``hades_cli/env_loader.py``.
    ...
    def _hades_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "HADES_SPOTIFY_CLIENT_ID",
            "HADES_QWEN_BASE_URL",
            "HADES_MAX_ITERATIONS",
        ],
    )
    def test_hades_integration_keys_still_writable(self, allowed_key):
        """``HADES_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``HADES_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``HADES_*``) keep working. The denylist
