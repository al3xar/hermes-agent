...
The Python launcher (`hades --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Hades session
...
class TestEarlyMouseDisable:
    def test_writes_disable_sequence_when_tui_flag_in_argv(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["hades", "--tui", "-c", "abc"])
        monkeypatch.delenv("HADES_TUI", raising=False)
        monkeypatch.delenv("HADES_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_writes_disable_sequence_when_hades_tui_env_set(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["hades"])
        monkeypatch.setenv("HADES_TUI", "1")
        monkeypatch.delenv("HADES_TUI_NO_EARLY_DISABLE", raising=False)
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.delenv("HADES_TUI", raising=False)
        monkeypatch.setenv("HADES_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_skips_when_stdout_is_not_a_tty(self, monkeypatch):
        # `hades --tui … >log` or CI capture: pipe is fd 1, not a TTY. The
