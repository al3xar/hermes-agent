...
class TestSystemdServiceRefresh:
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "hades gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`hades gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `hades gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `hades gateway restart`)."""
        unit_path = tmp_path / "hades-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``/tmp/pytest-of-.../hades_test`` HADES_HOME — silently breaking
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "hades gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_hades_home_for_target_user", lambda home: "/home/alice/.hades")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("HADES_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("HADES_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("HADES_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.hades.gateway.plist"
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup hades gateway run" in out
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_hades_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: hades gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup hades gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.hades.gateway"])
            ),
        )
...
class TestSystemUnitHadesHome:
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'HADES_HOME=/home/alice/.hades' in unit
        assert '/root/.hades' not in unit
    ...
    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("HADES_HOME", "/root/.hades/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'HADES_HOME=/home/alice/.hades/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_hades_home(self, monkeypatch):
    ...
    def test_user_unit_unaffected_by_change(self):
        ...
        hades_home = str(gateway_cli.get_hades_home().resolve())
        assert f'HADES_HOME={hades_home}' in unit


class TestHadesHomeForTargetUser:
    """Unit tests for _hades_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("HADES_HOME", raising=False)

        result = gateway_cli._hades_home_for_target_user("/home/alice")
        assert result == "/home/alice/.hades"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._hades_home_for_target_user("/home/alice")
        assert result == "/home/alice/.hades/profiles/coder"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "hades gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_hades_home_returns_empty(self, tmp_path, monkeypatch):
        ...
        result = gateway_cli._profile_arg(str(hades_home))
        assert result == ""
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.hades.gateway-orcha.plist"
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        assert "WorkingDirectory=/home/alice/.hades" in unit
        assert "WorkingDirectory=/home/alice/.hades/hades-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'hades gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'hades gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'hades gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "hades gateway run" in out


class TestLegacyHadesUnitDetection:
    """Tests for _find_legacy_hades_units / has_legacy_hades_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``hades.service`` unit behind when the service was
    ...
    The detector must flag ``hades.service`` ONLY when it actually runs our
    ...
    def test_detects_legacy_hades_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "hades.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_hades_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "hades.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_hades_units() is True

    def test_detects_legacy_hades_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "hades.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_hades_gateway_coder(self, tmp_path, monkeypatch):
        ...
        assert results == []
        assert gateway_cli.has_legacy_hades_units() is False

    def test_ignores_unrelated_hades_service(self, tmp_path, monkeypatch):
        """Third-party ``hades.service`` that isn't ours stays untouched.

        If a user has some other package named ``hades`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "hades.service").write_text(
            "[Unit]\nDescription=Some Other Hades\n[Service]\n"
            "ExecStart=/opt/other-hades/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_hades_units() == []
        assert gateway_cli.has_legacy_hades_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "hades.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "hades.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          ...
          - python path/to/hades_cli/main.py gateway run
          - hades gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m hades_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hades/hades_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/hades gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hades/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"hades.service" if i == 0 else f"hades.service"  # same name
            # Test each variant fresh
            (user_dir / "hades.service").write_text(
                f"[Unit]\nDescription=Old Hades\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_hades_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "hades.service" in out
        assert "hades gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "hades.service"
...
class TestRemoveLegacyHadesUnits:
    """Tests for remove_legacy_hades_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_hades_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_hades_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop hades.service" in c for c in cmds_joined)
        assert any("--user disable hades.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo hades gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop hades.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable hades.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "hades.service"
        system_legacy = system_dir / "hades.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_hades_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `hades gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        # public, just confirm that `hades gateway --help` shows it.
        import subprocess
        import sys
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_hades_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_hades_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_hades_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_hades_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``hades gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        ...
        assert "sudo hades gateway uninstall --system" in out
        assert "hades gateway install" in out
...
class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``hades gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    def test_stable_working_dir_uses_hades_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".hades"
        home.mkdir()
    ...
    def test_user_unit_workingdirectory_is_hades_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".hades"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_hades_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_hades_home(self, tmp_path, monkeypatch):
        import re
