...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `hades tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr("hades_cli.tools_config._xai_credentials_present", lambda: True)
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `hades tools`,
    ...
    # User explicitly opted into spotify but not x_search via `hades tools`.
    config = {"platform_toolsets": {"cli": ["hades-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[hades-cli, spotify]`` (composite + configurable) must keep the full
    ``hades-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``hades-cli`` on the floor,
    ...
    for ts in (
        ...
        assert ts in enabled, f"{ts} should be enabled when hades-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["hades-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``hades-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["hades-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # auto-recovered by _get_platform_tools so saving via `hades tools`
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (hades-cli, hades-telegram, etc.) must NOT
    ...
    ``hades tools``, but hades-cli stays in the config and re-enables
    ...
    # hades-cli must NOT survive — it's a platform default, not an MCP server
    assert "hades-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_hades_telegram():
    """Same bug for Telegram — hades-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser",
    ...
    assert "hades-telegram" not in saved
    assert "web" in saved
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr(
        "hades_cli.nous_subscription.managed_nous_tools_enabled", lambda: True
    )
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `hades tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's HADES_CUA_DRIVER_CMD override."""

    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with (
        patch.dict("os.environ", {"HADES_CUA_DRIVER_CMD": "custom-cua"}),
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with (
        patch.dict("os.environ", {"HADES_CUA_DRIVER_CMD": "   "}),
        patch("shutil.which", side_effect=fake_which),
    ):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`hades tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["hades-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "hades-_test_platform"},
    }

    with mock_patch(
        "hades_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}
    ):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'hades-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `hades tools`,
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `hades tools`, the toolset appears."""
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `hades tools` → "reconfigure existing" shows
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr(
        "hades_cli.tools_config._run_post_setup", lambda key: called.append(key)
    )
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr("hades_cli.nous_subscription.ensure_nous_portal_access", _boom)
...
#    reported as added/removed by `hades tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `hades tools` printed ``- kanban``
