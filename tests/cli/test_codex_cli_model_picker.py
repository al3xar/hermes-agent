...
Note: auto-import from ~/.codex/auth.json was removed in #12360 — Hades
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `hades auth openai-codex`. The old
...
@pytest.fixture()
def hades_auth_only_env(tmp_path, monkeypatch):
...
def test_normal_path_still_works(hades_auth_only_env):
...
def test_codex_picker_uses_live_codex_catalog(hades_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Hades
    auth store."""
    hades_home = tmp_path / ".hades"
    hades_home.mkdir()
