...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("hades/hades-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned hades/* and pr-* branches."""

    def test_prunes_orphaned_hades_branch(self, git_repo):
        """hades/hades-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "hades/hades-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "hades/hades-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "hades/hades-deadbeef" in result.stdout
        ...
        assert "hades/hades-deadbeef" in orphaned
        ...
        assert "hades/hades-deadbeef" not in result.stdout
