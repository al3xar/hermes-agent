...
This saves 500-650ms on ``hades --help``, ``hades version``,
``hades logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``hades --help`` in-process and parse the subcommand block.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["hades", "--tui", "chat"], "chat"),
        (["hades", "-w", "logs"], "logs"),
        (["hades", "chat", "hello world"], "chat"),
        (["hades", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["hades", "-m", "gpt5", "chat"], "chat"),
        (["hades", "--model", "gpt5", "chat", "hi"], "chat"),
        (["hades", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["hades", "-z", "hello world"], None),
        (["hades", "-z", "hello", "chat"], "chat"),
        (["hades", "--model=gpt5", "chat"], "chat"),     # inline form
        (["hades", "--", "chat"], "chat"),               # -- terminator
        (["hades", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["hades", "some-plugin-cmd"], "some-plugin-cmd"),
        (["hades", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["hades"],                          # bare → chat
        ["hades", "--help"],                # top-level help
        ["hades", "-h"],
        ["hades", "version"],               # known built-in
        ["hades", "logs"],
        ["hades", "gateway", "run"],
        ["hades", "--tui"],
        ["hades", "-w", "--tui"],
        ["hades", "chat", "hi"],
        ["hades", "help"],                  # accepted built-in-ish
        ["hades", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["hades", "meet", "join"],          # potential google_meet plugin
        ["hades", "honcho", "status"],      # potential memory plugin
        ["hades", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``hades help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"hades_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
