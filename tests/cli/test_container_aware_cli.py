...
def container_env(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("HADES_DEV", raising=False)

    container_mode = hades_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=hades-agent\n"
        "exec_user=hades\n"
        "hades_bin=/data/current-package/bin/hades\n"
    )
    return hades_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    assert info["exec_user"] == "hades"
    assert info["hades_bin"] == "/data/current-package/bin/hades"
...
def test_get_container_exec_info_skipped_when_hades_dev(container_env, monkeypatch):
    """Returns None when HADES_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("HADES_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_hades_dev_zero(container_env, monkeypatch):
    """HADES_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("HADES_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        hades_home = Path(tmpdir) / ".hades"
        hades_home.mkdir()
        (hades_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("hades_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_hades_home": lambda: hades_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HADES_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "hades-custom"
    assert info["exec_user"] == "myuser"
    assert info["hades_bin"] == "/opt/hades/bin/hades"
