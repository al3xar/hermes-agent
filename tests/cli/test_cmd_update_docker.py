"""Tests for ``hades update`` / ``--check`` inside the Docker container.
...
fix, ``hades update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Hades, not an update to the running container, so the message
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``hades update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull nousresearch/hades-agent:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``hades update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull nousresearch/hades-agent:latest" in msg
