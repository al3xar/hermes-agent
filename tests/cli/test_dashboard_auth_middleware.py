...
def test_full_login_round_trip_unlocks_gated_api(gated_app):
    ...
    assert any("hades_session_at" in c for c in set_cookies)
    assert any("hades_session_rt" in c for c in set_cookies)
...
def _complete_stub_login(client) -> None:
    ...
    client's cookie jar holds ``hades_session_at`` / ``hades_session_rt``
...
def test_logout_clears_cookies_and_redirects_to_login(gated_app):
    ...
    assert any(
        c.startswith("hades_session_at=") and "Max-Age=0" in c
        for c in set_cookies
    )
    assert any(
        c.startswith("hades_session_rt=") and "Max-Age=0" in c
        for c in set_cookies
    )
...
def _mint_stub_at(stub: StubAuthProvider) -> str:
    ...
    state = dict(
        seg.split("=", 1)
        for seg in ls.cookie_payload["hades_session_pkce"].split(";")
        if "=" in seg
    )["state"]
    verifier = dict(
        seg.split("=", 1)
