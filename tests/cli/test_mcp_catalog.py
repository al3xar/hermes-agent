...
@pytest.fixture(autouse=True)
def _isolate_hades_home(tmp_path, monkeypatch):
    """Redirect all config I/O to a temp HADES_HOME."""
    hh = tmp_path / "hades-home"
    hh.mkdir()
    ...
    # mcp_catalog grabs get_hades_home() lazily through hades_constants
    monkeypatch.setattr(
...
class TestCatalogDiagnostics:
    ...
    def test_picker_surfaces_future_manifest_warning(self, catalog_dir, capsys, monkeypatch):
        ...
        entries so users running headless or after `hades setup` know to update."""
        body = _basic_manifest()
        ...
        assert "requires a newer Hades" in out
