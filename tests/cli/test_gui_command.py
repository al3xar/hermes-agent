"""Tests for ``hades gui`` desktop launcher wiring."""
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    hades_root = tmp_path / "custom-hades"
    ...
    with patch("hades_cli.main.shutil.which", return_value="/usr/bin/npm"), \
        ...
        cli_main.cmd_gui(_ns(
            ...
            hades_root=str(hades_root),
            cwd=str(cwd),
        ))

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["HADES_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["HADES_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["HADES_DESKTOP_HADES_ROOT"] == str(hades_root)
    assert launch_env["HADES_DESKTOP_CWD"] == str(cwd)
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"hades"}', encoding="utf-8")
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    with patch("hades_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("hades_cli.main.subprocess.run", side_effect=[pack_fail, pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "Hades.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "Hades.exe"
