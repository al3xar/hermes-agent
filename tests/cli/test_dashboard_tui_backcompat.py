"""Regression test: `hades dashboard --tui` must not hard-crash.

Older Hades desktop app shells (<= 0.15.x) spawn the backend as::

    hades dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Hades couldn't
