...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # hades model, not via upgrade prompts.
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in HADES_OVERLAYS
        overlay = HADES_OVERLAYS["gmi"]
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a HadesAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("HadesAgent/")

    def test_gmi_profile_declares_hades_user_agent(self):
        """The GMI plugin sets a HadesAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("HadesAgent/"), (
            f"expected GMI profile User-Agent to start with 'HadesAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["hades", "chat", "--provider", "gmi"])
