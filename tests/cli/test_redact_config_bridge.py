...
`HADES_REDACT_SECRETS` at module-import time. `hades_cli/main.py` at
...
in .env), the toggle was silently ignored in both `hades chat` and
`hades gateway run`.

Fix: bridge `security.redact_secrets` from config.yaml → `HADES_REDACT_SECRETS`
env var in `hades_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    probe = textwrap.dedent(
        ...
        os.environ.pop("HADES_REDACT_SECRETS", None)
        sys.path.insert(0, %r)
    ...
    env.pop("HADES_REDACT_SECRETS", None)
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    """.env HADES_REDACT_SECRETS takes precedence over config.yaml."""
    hades_home = tmp_path / ".hades"
    hades_home.mkdir()
    (hades_home / "config.yaml").write_text(
        textwrap.dedent(
    ...
    (hades_home / ".env").write_text("HADES_REDACT_SECRETS=true\n")
