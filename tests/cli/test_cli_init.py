"""Tests for HadesCLI initialization -- catches configuration bugs
...
def _make_cli(env_overrides=None, config_overrides=None, **kwargs):
    """Create a HadesCLI instance with minimal mocking."""
    import importlib
    ...
    clean_env = {"LLM_MODEL": "", "HADES_MAX_ITERATIONS": ""}
    if env_overrides:
    ...
    with patch.dict(sys.modules, prompt_toolkit_stubs), \
        ...
        with patch.object(_cli_mod, "get_tool_definitions", return_value=[]), \
            ...
            return _cli_mod.HadesCLI(**kwargs)
...
class TestMaxTurnsResolution:
    ...
    def test_env_var_max_turns(self):
        ...
        cli_obj = _make_cli(env_overrides={"HADES_MAX_ITERATIONS": "42"})
        assert cli_obj.max_turns == 42

    def test_invalid_env_var_max_turns_falls_back_to_default(self):
        """Invalid env values should not crash CLI init."""
        cli_obj = _make_cli(env_overrides={"HADES_MAX_ITERATIONS": "not-a-number"})
...
class TestFallbackChainInit:
    def test_merges_new_and_legacy_fallback_config(self):
        cli = _make_cli(config_overrides={
            ...
            "fallback_model": {"provider": "nous", "model": "Hades-4"},
        })
        assert cli._fallback_model == [
            {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
            {"provider": "nous", "model": "Hades-4"},
        ]
...
class TestHistoryDisplay:
    ...
    def test_resume_updates_hades_session_id_env_and_context(self, tmp_path):
        ...
        _VAR_MAP["HADES_SESSION_ID"].set("current_session")
        ...
        try:
            ...
            assert get_session_env("HADES_SESSION_ID") == "target_session"
        finally:
            cli._session_db.close()
            os.environ.pop("HADES_SESSION_ID", None)
            _VAR_MAP["HADES_SESSION_ID"].set(_UNSET)
...
class TestRootLevelProviderOverride:
    ...
    def test_model_provider_wins_over_root_provider(self, tmp_path, monkeypatch):
        ...
        import cli
        monkeypatch.setattr(cli, "_hades_home", hades_home)
