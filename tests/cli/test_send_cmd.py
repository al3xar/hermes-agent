"""Tests for the ``hades send`` CLI subcommand.
...
def test_load_hades_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``hades send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``hades config set``) lives
    ...
    hades_home.mkdir()
    (hades_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    send_cmd._load_hades_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_hades_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    hades_home = tmp_path / ".hades"
    hades_home.mkdir()
    (hades_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_hades_env_handles_missing_files(tmp_path, monkeypatch):
