...
def _make_agent(hades_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = hades_home / "hades-agent"
    (agent_root / "hades_cli").mkdir(parents=True)
    (agent_root / "hades_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(hades_home: Path) -> None:
    """Create the source-built GUI artifacts a `hades desktop` run produces."""
    desktop = hades_home / "hades-agent" / "apps" / "desktop"
    ...
    (hades_home / "hades-agent" / "node_modules").mkdir(parents=True)
    (hades_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(hades_home: Path) -> None:
    (hades_home / "config.yaml").write_text("x: 1\n")
    (hades_home / ".env").write_text("KEY=secret\n")
    (hades_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    hades_home = tmp_path / ".hades"
    hades_home.mkdir()
    assert gu.agent_is_installed(hades_home) is False
    _make_agent(hades_home)
    assert gu.agent_is_installed(hades_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    hades_home = tmp_path / ".hades"
    (hades_home / "hades-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(hades_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    hades_home = tmp_path / ".hades"
    _make_gui_build(hades_home)
    artifacts = gu.source_built_gui_artifacts(hades_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(hades_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(hades_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    hades_home = tmp_path / ".hades"
    agent_root = _make_agent(hades_home)
    _make_gui_build(hades_home)
    _make_user_data(hades_home)
    ...
    removed = gu.uninstall_gui(hades_home)
    ...
    assert not (hades_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "hades_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (hades_home / "config.yaml").exists()
    assert (hades_home / ".env").exists()
    assert (hades_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    hades_home = tmp_path / ".hades"
    _make_agent(hades_home)
    userdata = tmp_path / "Hades-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(hades_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(hades_home)
    ...
    assert summary["hades_home"] == str(hades_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Hades"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Hades"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Hades"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Hades"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(uninstall, "get_hades_home", lambda: hades_home)
...
def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    ...
    hades_home = tmp_path / ".hades"
    (hades_home / "hades-agent" / "hades_cli").mkdir(parents=True)
    (hades_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
...
def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_hades_home", lambda: hades_home)
    ...
    assert not (desktop / "release").exists()
    assert not (hades_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "hades_cli").exists()
    assert (hades_home / "config.yaml").exists()
