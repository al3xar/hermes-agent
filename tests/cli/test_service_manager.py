...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the hades user. The new
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `hades_cli` package succeeds — gateway_windows is
...
# unprivileged hades user from every lifecycle op. These tests run
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "hades -p coder gateway run" in run_text
    assert "s6-setuidgid hades" in run_text
    ...
    # in hades_cli/gateway.py::_gateway_command_inner.
    assert "export HADES_S6_SUPERVISED_CHILD=1" in run_text
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid hades hades -p coder gateway run" in run_text
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "hades profile create typo" in msg
...
#
# `hades gateway stop` inside a container dispatches through
