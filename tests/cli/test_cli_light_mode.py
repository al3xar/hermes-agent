...
class TestLightModeDetection:
    def test_hades_light_env_true_forces_light(self, cli_mod, monkeypatch):
        monkeypatch.setenv("HADES_LIGHT", "1")
        assert cli_mod._detect_light_mode() is True

    def test_hades_light_env_false_forces_dark(self, cli_mod, monkeypatch):
        monkeypatch.setenv("HADES_LIGHT", "0")
    ...
    def test_theme_hint_light(self, cli_mod, monkeypatch):
        monkeypatch.delenv("HADES_LIGHT", raising=False)
        monkeypatch.delenv("HADES_TUI_LIGHT", raising=False)
...
class TestSkinConfigHook:
    ...
    def test_hook_installed(self, cli_mod):
        ...
        assert getattr(SkinConfig, "_hades_light_mode_hook_installed", False) is True
