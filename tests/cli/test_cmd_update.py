...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/tmp/hades-launcher-venv")
        ...
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/tmp/hades-launcher-venv"
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_on_fork_checks_upstream_when_origin_up_to_date(
        ...
        origin but behind NousResearch/hades-agent silently misses updates.
        """
        from hades_cli import main as hm
        ...
        with patch.object(
            hm,
            "_get_origin_url",
            return_value="https://github.com/example/hades-agent.git",
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.hades"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.hades/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.hades/profiles/work"))
...
class TestCmdUpdateBranchFlag:
    """``hades update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``hades update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``hades update --branch=<non-main>`` must refuse on the ZIP fallback path.
