...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.hades/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.hades            (has its own auth.json fixture)
    * Profile     -> tmp_path/.hades/profiles/coder   (active, HADES_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Hades is not
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    hades_home = tmp_path / "classic"
    hades_home.mkdir()
    monkeypatch.setenv("HADES_HOME", str(hades_home))

    _write(hades_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    ...
    # sees tmp_path/home/.hades as the "real home" — which is NOT equal
    ...
    _write(hades_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
