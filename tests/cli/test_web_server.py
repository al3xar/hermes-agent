...
@pytest.fixture
def _install_example_plugin(_isolate_hades_home):
    ...
    real bundled plugins — kanban, hades-achievements, model providers
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Hades vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Hades env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints HADES_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        ...
        monkeypatch.setenv("HADES_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("HADES_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_hades_home):
    ...
    def test_update_hades_returns_docker_guidance_without_spawning(self, monkeypatch):
        import hades_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn hades update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_hades_action", fail_spawn)
        web_server._ACTION_PROCS.pop("hades-update", None)
        web_server._ACTION_RESULTS.pop("hades-update", None)

        resp = self.client.post("/api/hades/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "hades-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/hades-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/hades-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/hades-agent:latest" in line for line in status_data["lines"])

    def test_update_hades_spawns_on_non_docker_install(self, monkeypatch):
        import hades_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_hades_action", fake_spawn)
        web_server._ACTION_PROCS.pop("hades-update", None)
        web_server._ACTION_RESULTS.pop("hades-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "hades-update"}
        assert calls == [(["update"], "hades-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("hades-update", None)
        web_server._ACTION_RESULTS.pop("hades-update", None)
        web_server._ACTION_PROCS["hades-update"] = proc

        resp = self.client.get("/api/actions/hades-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "hades-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["hades-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["headers"]["User-Agent"].startswith("HadesDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "hades_pair123_bot",
                "deep_link": "https://t.me/newbot/HadesSetupBot/hades_pair123_bot",
                "qr_payload": "https://t.me/newbot/HadesSetupBot/hades_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "hades_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/HadesSetupBot/hades_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/HadesSetupBot/hades_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "hades_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_hades_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "hades_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/HadesSetupBot/hades_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/HadesSetupBot/hades_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "hades_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_hades_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``hades gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "hades_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/HadesSetupBot/hades_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/HadesSetupBot/hades_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "hades_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "hades_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/HadesSetupBot/hades_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/HadesSetupBot/hades_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "hades_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/HadesSetupBot/hades_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/HadesSetupBot/hades_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/hades_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "hades-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "hades-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `hades model`), not the first curated paid entry."""
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_hades_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        ...
        (hades_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = hades_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_hades_for_default_profile(self):
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "hades setup"
    ...
    def test_profiles_create_with_clone_from_default_copies_default_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        cloned_skill = get_hades_home() / "profiles" / "cloned" / "skills" / "custom" / "new-skill" / "SKILL.md"
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``hades_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in hades_cli/web_server.py."
        )
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_hades_home, _install_example_plugin):
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, hades-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (hades-achievements is loaded by default).
        resp = self.client.get("/api/plugins/hades-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``hades --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        assert env["HADES_TUI_INLINE"] == "1"
        assert env["HADES_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_hades_home
    ):
        import hades_cli.main as main_mod
        ...
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/hades-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/hades-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"hades-ws-ok" in buf:
                    break
            assert b"hades-ws-ok" in buf
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        HADES_TUI_SIDECAR_URL env var pointing back at /api/pub on the
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    ...
    gateway_url = env.get("HADES_TUI_GATEWAY_URL", "")
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_hades_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("HADES_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under HADES_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_hades_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("HADES_DESKTOP", raising=False)
