...
1. ``hades_cli.web_server._discover_dashboard_plugins`` opted into
   the untrusted ``./.hades/plugins/`` source via
...
class TestProjectPluginsEnvGate:
    ...
    def project_plugin(self, tmp_path, monkeypatch):
        """Plant a project-source plugin under CWD's ``.hades/plugins``
...
class TestEndToEndPocBlocked:
    ...
    def test_full_chain_blocked(self, tmp_path, monkeypatch):
        ...
        for call in spec.call_args_list:
            ...
            assert module_name != "hades_dashboard_plugin_evil"
            assert target != payload_py
            assert "evil-repo" not in target.parts
        assert "hades_dashboard_plugin_evil" not in sys.modules
