"""Tests for hades_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestIsSkillDisabled:
    ...
    @patch("hades_cli.config.load_config")
    @patch.dict("os.environ", {"HADES_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """HADES_SESSION_PLATFORM should be used when HADES_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_hades_platform_takes_precedence(self, tmp_path, monkeypatch):
        """HADES_PLATFORM should win over HADES_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        monkeypatch.setenv("HADES_PLATFORM", "telegram")
        monkeypatch.setenv("HADES_SESSION_PLATFORM", "telegram")
