"""Test that HADES_SESSION_ID is exposed as an env var and ContextVar."""
...
def _cleanup_env():
    """Remove HADES_SESSION_ID before/after each test."""
...
def test_session_id_env_set_on_init():
    """AIAgent.__init__ sets HADES_SESSION_ID in the environment."""
    agent = AIAgent(
        api_key="test-key",
...
def test_session_id_env_uses_provided_id():
    """When session_id is passed explicitly, HADES_SESSION_ID reflects it."""
...
def test_session_id_contextvar_set():
    ...
    assert get_session_env("HADES_SESSION_ID") == custom_id
