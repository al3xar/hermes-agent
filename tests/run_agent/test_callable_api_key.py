...
Hades preserves the callable end-to-end so the SDK refreshes tokens
...
class TestCliEnsureRuntimeCredentialsCallable:
    ...
    ``HadesCLI`` instance) — the predicate change is the load-bearing
    ...
    def test_callable_predicate_present_in_cli_runtime_validation(self):
        ...
        src = (Path(__file__).resolve().parent.parent.parent
               / "hades_cli" / "cli_agent_setup_mixin.py").read_text()
...
class TestInlinedDisplayMasks:
    ...
    def test_cli_show_config_handles_callable(self):
        """``cli.HadesCLI.show_config`` previously did
        ...
        assert "is_token_provider(self.api_key)" in src, (
            "cli.HadesCLI.show_config must guard self.api_key via "
            "is_token_provider so callable Entra ID providers don't "
            "crash /config."
        )
        assert '"Microsoft Entra ID"' in src, (
            "cli.HadesCLI.show_config must print the static "
