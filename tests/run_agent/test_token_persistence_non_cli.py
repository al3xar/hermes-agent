...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    hades_state = ModuleType("hades_state")
    hades_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "hades_state", hades_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Hades"}, "task-id"))
