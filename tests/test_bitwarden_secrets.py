...
binary download are exercised manually by `hades secrets bitwarden
...
def hades_home(tmp_path, monkeypatch):
    """Point Hades at an isolated home directory."""
    home = tmp_path / ".hades"
    home.mkdir()
...
def test_install_bws_rejects_malicious_member(hades_home, monkeypatch):
...
def test_install_bws_happy_path(hades_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(hades_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(hades_home, monkeypatch):
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under hades_home/cache/."""
    home = tmp_path / ".hades"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
