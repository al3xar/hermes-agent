...
Older versions of ``install.sh`` created ``$command_link_dir/hades`` as a
symlink to the pip-generated entry point at ``$HADES_BIN`` (i.e.
``venv/bin/hades``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/hades" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$HADES_BIN" "$@"`` then self-recursed and ``hades`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/hades``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/hades\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/hades"')
    cat_idx = block.find('cat > "$command_link_dir/hades" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/hades before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    ``HADES_BIN`` and ``command_link_dir`` pointed at this fixture. The fix
    requires that, after the run:

      * ``venv/bin/hades`` still contains its original pip-script body
      * ``local_bin/hades`` is a regular file (not a symlink) holding the shim
    """
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/hades was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/hades must be replaced with a regular file, not "
