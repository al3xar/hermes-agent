...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hades_env(self):
        ...
        # Note: hades internal .env is in ~/.hades/.env which is NOT a project-local
...
class TestCacheFileReadBlocking:
    """Internal Hades cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        hades_home = tmp_path / ".hades"
        cache = hades_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._hades_home_path", return_value=hades_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Hades cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        hades_home = tmp_path / ".hades"
        hub = hades_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_hades_home(self, tmp_path):
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        hades_home = tmp_path / ".hades"
        cache = hades_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
