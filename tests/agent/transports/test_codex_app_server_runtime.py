...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Hades passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("HADES_HOME", "/users/alice/.hades/profiles/backend-worker")
        monkeypatch.setenv("HADES_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "HADES_KANBAN_DB",
            "/users/alice/.hades/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.hades/kanban/boards/smoke"]'
            in cmd
        )
