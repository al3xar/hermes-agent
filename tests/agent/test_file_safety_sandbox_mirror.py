...
Those paths look like ``…/sandboxes/<backend>/<task>/home/.hades/…`` and
they accumulate divergent copies of authoritative profile state (SOUL.md,
config.yaml, memories/*.md) because the host Hades process never reads
...
class TestClassifySandboxMirrorTarget:
    ...
    def test_sandboxes_segment_without_home_hades_returns_none(self, tmp_path):
    ...
    def test_sandboxes_segment_with_home_but_no_hades_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-hades`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``…/sandboxes/<backend>/<task>`` without ``home/.hades/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_hades_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_hades_home_path", lambda: tmp_path / "profiles" / "group1")
