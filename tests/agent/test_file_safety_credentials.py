...
Regression for https://github.com/NousResearch/hades-agent/issues/17656 —
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_hades_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "hades_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_hades_home_path", lambda: home)
    return home
...
def test_arbitrary_hades_home_file_not_blocked(fake_home):
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal Hades cache file" in err
...
def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    ...
    sibling.mkdir()
    traversal = sibling / ".." / "hades_home" / "auth.json"
...
def test_identically_named_hades_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Hades-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(fs, "_hades_home_path", lambda: profile)
