...
def test_hades_client_tag_includes_current_version():
    """The client tag must reflect hades_cli.__version__ verbatim."""
    from hades_cli import __version__
    from agent.portal_tags import hades_client_tag

    assert hades_client_tag() == f"client=hades-client-v{__version__}"


def test_hades_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import hades_client_tag

    tag = hades_client_tag()
    assert tag.startswith("client=hades-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import hades_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=hades-agent" in tags
    assert hades_client_tag() in tags
    assert len(tags) == 2
