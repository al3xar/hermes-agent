...
from hades_cli.profiles import _get_default_hades_home
...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "hades"
        assert config.workspace_id == "hades"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"hades": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"hades": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"hades": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            ...
            result = config.resolve_session_name("/home/user/hades-agent/subdir")
...
class TestResolveConfigPath:
    def test_prefers_hades_home_when_exists(self, tmp_path):
        hades_home = tmp_path / "hades"
        hades_home.mkdir()
        local_cfg = hades_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        ...
        # _get_default_hades_home() must resolve back to ~/.hades — that's
        ...
        assert _get_default_hades_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_hades_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "hades_coder"
        assert profile_host_key("default") == "hades"

    def test_default_returns_hades(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("HADES_HONCHO_HOST", None)
            os.environ.pop("HADES_HOME", None)
            assert resolve_active_host() == "hades"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"HADES_HONCHO_HOST": "hades.coder"}):
            assert resolve_active_host() == "hades.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HADES_HONCHO_HOST", None)
            with patch("hades_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "hades_coder"

    def test_default_profile_returns_hades(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HADES_HONCHO_HOST", None)
            with patch("hades_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "hades"

    def test_custom_profile_returns_hades(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HADES_HONCHO_HOST", None)
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="hades_coder")
        assert config.host == "hades_coder"
        assert config.workspace_id == "hades"  # shared workspace
        assert config.ai_peer == "hades_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="hades")
        assert config.host == "hades"
        assert config.workspace_id == "hades"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "hades": {"aiPeer": "hades", "peerName": "alice"},
                "hades_coder": {
                    "aiPeer": "hades_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="hades_coder", config_path=config_file,
        )
        assert config.host == "hades_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "hades_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="hades_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "hades_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "hades.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="hades_dreamer",
        ...
        assert config.host == "hades_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "hades_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"hades": {"enabled": True, "aiPeer": "hades"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"hades": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_hades_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="hades",
    ...
    def test_hades_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="hades",
