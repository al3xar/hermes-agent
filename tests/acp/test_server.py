"""Tests for acp_adapter.server — HadesACPAgent ACP server."""
...
from acp_adapter.server import HadesACPAgent, HADES_VERSION
...
def agent(mock_manager):
    """HadesACPAgent backed by a mock session manager."""
    return HadesACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.version == HADES_VERSION
        ...
        assert payloads == [
            {
                ...
                "description": (
                    ...
                    "Use this when Hades has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Hades provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = HadesACPAgent(session_manager=manager)
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "HadesACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("HadesACPAgent")
...
class TestPrompt:
    ...
    @pytest.mark.asyncio
    async def test_prompt_propagates_hades_session_id_env(self, agent, monkeypatch):
        """ACP must propagate the originating session id to the agent loop
        via ``HADES_SESSION_ID`` so tools that want to stamp side-effects
        ...
        assert captured["env"] == new_resp.session_id, (
            "HADES_SESSION_ID must be set to the originating ACP session id "
        ...
        assert os.environ.get("HADES_SESSION_ID") is None, (
            "HADES_SESSION_ID must be restored after the agent call so "
            "a re-used executor thread doesn't leak the id into the next "
            "session's tools"
        )

    @pytest.mark.asyncio
    async def test_prompt_restores_prior_hades_session_id(self, agent, monkeypatch):
        """If the env already had HADES_SESSION_ID set (e.g. nested
        agent loops), the prior value must be restored after the inner
        prompt completes — not popped, not left at the inner id."""
        monkeypatch.setenv("HADES_SESSION_ID", "outer-sess")
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert HADES_VERSION in result
