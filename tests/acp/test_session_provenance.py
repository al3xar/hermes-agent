...
def test_root_session_no_compression(db):
    ...
    assert prov["currentHadesSessionId"] == "root1"
    assert prov["rootHadesSessionId"] == "root1"
    assert prov["parentHadesSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_hades_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentHadesSessionId"] == "old"
    assert prov["rootHadesSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousHadesSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootHadesSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootHadesSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_hades_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousHadesSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"hades"}
    assert "sessionProvenance" in meta["hades"]
    assert meta["hades"]["sessionProvenance"]["currentHadesSessionId"] == "root1"
