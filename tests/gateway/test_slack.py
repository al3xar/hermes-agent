...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /hades, /btw, /stop,
        ...
        for expected in ("/hades", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@hades`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@hades"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@hades")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /hades subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /hades btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_hades_prefix_still_works(self, adapter):
        """Backward compat: /hades btw foo must still route to /btw foo.

        Old workspace manifests only declared /hades as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_hades_freeform_question(self, adapter):
        """/hades <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/hades",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_hades_slash_stashes_context(self, adapter):
        """Legacy /hades <subcommand> also stashes context."""
        command = {
            "command": "/hades",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_hades_question_does_not_stash_context(self, adapter):
        """Free-form /hades <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/hades",
