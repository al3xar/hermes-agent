...
class TestBlueBubblesHelpers:
    ...
    def test_default_mention_patterns_match_hades_variants(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter.require_mention is True
        assert adapter._message_matches_mention_patterns("Hades, summarize this")
        ...
        assert not adapter._message_matches_mention_patterns("antihades should not match")
    ...
    def test_custom_mention_patterns_override_defaults(self, monkeypatch):
        ...
        assert not adapter._message_matches_mention_patterns("Hades what is next?")

    def test_clean_mention_text_strips_leading_wake_word(self, monkeypatch):
        adapter = _make_adapter(monkeypatch, require_mention=True)

        assert adapter._clean_mention_text("Hades, summarize this") == "summarize this"
        assert adapter._clean_mention_text("Hades agent: summarize this") == "summarize this"
        assert adapter._clean_mention_text("please ask Hades about this") == "please ask Hades about this"
