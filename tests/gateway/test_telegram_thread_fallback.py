...
Supergroup forum topics route with ``message_thread_id``. Hades-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """Hades-created DM topic lanes should still attempt scoped typing.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_hades_dm_topics():
    """Hades-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
