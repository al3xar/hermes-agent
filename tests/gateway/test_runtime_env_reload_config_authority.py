...
HADES_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env HADES_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_keeps_env_max_iterations_when_config_omits_key(
    ...
    hades_home = tmp_path / ".hades"
    hades_home.mkdir()
    (hades_home / "config.yaml").write_text(yaml.safe_dump({"agent": {}}), encoding="utf-8")
    (hades_home / ".env").write_text("HADES_MAX_ITERATIONS=123\n", encoding="utf-8")
