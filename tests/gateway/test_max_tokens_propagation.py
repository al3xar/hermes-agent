...
Precedence verified here:
    HADES_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    ...
    These tests deliberately re-import ``hades_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("HADES_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("hades_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (hades_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("hades_cli", "gateway")):
                del sys.modules[mod]
...
def test_env_override_beats_everything(isolated_home, monkeypatch):
    """HADES_MAX_TOKENS is the internal override mechanism (highest priority)."""
    write_cfg, fresh_gateway = isolated_home
    monkeypatch.setenv("HADES_MAX_TOKENS", "2048")
    write_cfg(
        """
        model:
          default: glm-5.1
