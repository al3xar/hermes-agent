...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="HadesBot"),
    )
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello Hades", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello Hades",
    )
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_hades_when_only_mentions(adapter):
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    hades_dir.mkdir()
    config_path = hades_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("HADES_HOME", str(hades_dir))
