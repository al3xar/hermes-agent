...
def _make_enabled_plugin(hades_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <hades_home>/plugins/<name> and opt it in."""
    plugin_dir = hades_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    hades_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        hades_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HADES_HOME", str(tmp_path / "hades_empty"))
