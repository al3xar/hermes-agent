...
Override the image with ``HADES_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("HADES_TEST_IMAGE", "hades-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``HADES_TEST_IMAGE`` env var to point at a pre-built
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged hades user
# ---------------------------------------------------------------------------
#
# Background: every Hades runtime path inside the container drops to UID
# 10000 (the ``hades`` user) via ``s6-setuidgid hades``. ``docker exec``
...
# fails (PermissionError swallowed) for hades, so a test that ran as root
...
# helpers below run every probe under ``-u hades`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: hades).
