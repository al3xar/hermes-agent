...
package-lock.json. Without HADES_TUI_DIR set, ``_make_tui_argv`` falls
...
The fix is ``ENV HADES_TUI_DIR=/opt/hades/ui-tui`` in the Dockerfile, which
...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the hades user, return stdout."""
    inner = (
        "source /opt/hades/.venv/bin/activate && "
        "cd /opt/hades && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the hades user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "hades", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_hades_tui_dir_env_is_set(built_image: str) -> None:
    """HADES_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$HADES_TUI_DIR"'],
        capture_output=True, text=True, timeout=60,
    )
    assert r.returncode == 0, r.stderr[-2000:]
    assert r.stdout.strip() == "/opt/hades/ui-tui", (
        f"HADES_TUI_DIR={r.stdout.strip()!r} (expected /opt/hades/ui-tui)"
    )
...
def test_prebuilt_bundle_present_and_no_runtime_install(built_image: str) -> None:
    ...
    # With HADES_TUI_DIR set, _make_tui_argv returns the prebuilt path BEFORE
