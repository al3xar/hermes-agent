"""Regression test: ``hades dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``hades dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$HADES_GIT_SHA`` build-arg to ``/opt/hades/.hades_build_sha`` and
``hades_cli/build_info.py`` reads it as a fallback.
...
is absent and ``hades dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/hades/.hades_build_sha`` exists in the image, assert that
  ``hades dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's hades user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"hades dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/hades/.hades_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/hades/.hades_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    """When the image was built with ``HADES_GIT_SHA``, dump must surface it.
    ...
    # CI path: build-arg was set, baked file exists.  ``hades dump``
