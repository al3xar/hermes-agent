...
class TestHadesConstantsFallback:
    """Tests for _hades_home.py fallback when hades_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_hades_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _hades_home.py with hades_constants blocked."""
        monkeypatch.setitem(sys.modules, "hades_constants", None)
        spec = importlib.util.spec_from_file_location("_hades_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_hades_home_env_var(self, monkeypatch, tmp_path):
        """When hades_constants is missing, HADES_HOME comes from env var."""
        monkeypatch.setenv("HADES_HOME", str(tmp_path / "custom-hades"))
        module = self._load_helper(monkeypatch)
        assert module.get_hades_home() == tmp_path / "custom-hades"

    def test_fallback_defaults_to_dot_hades(self, monkeypatch):
        """When hades_constants is missing and HADES_HOME unset, default to ~/.hades."""
        monkeypatch.delenv("HADES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_hades_home() == Path.home() / ".hades"

    def test_fallback_ignores_empty_hades_home(self, monkeypatch):
        """Empty/whitespace HADES_HOME is treated as unset."""
        monkeypatch.setenv("HADES_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_hades_home() == Path.home() / ".hades"

    def test_fallback_display_hades_home_shortens_path(self, monkeypatch):
        """Fallback display_hades_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("HADES_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_hades_home() == "~/.hades"

    def test_fallback_display_hades_home_profile_path(self, monkeypatch):
        """Fallback display_hades_home() handles profile paths under ~/."""
        monkeypatch.setenv("HADES_HOME", str(Path.home() / ".hades/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_hades_home() == "~/.hades/profiles/coder"

    def test_fallback_display_hades_home_custom_path(self, monkeypatch):
        """Fallback display_hades_home() returns full path for non-home locations."""
        monkeypatch.setenv("HADES_HOME", "/opt/hades-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_hades_home() == "/opt/hades-custom"

    def test_delegates_to_hades_constants_when_available(self):
        """When hades_constants IS importable, _hades_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_hades_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import hades_constants
        assert module.get_hades_home is hades_constants.get_hades_home
        assert module.display_hades_home is hades_constants.display_hades_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Hades Docker image, whose venv is built with
    ...
    def test_returns_false_when_no_pip_and_no_uv(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "hades-agent[google]" in out
