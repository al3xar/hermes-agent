...
def load_module():
    spec = importlib.util.spec_from_file_location("openclaw_to_hades", SCRIPT_PATH)
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `hades cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # hades_config_mod  — print statements in the post-migration summary that
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "hades_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Hades``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Hades prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Hades to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Hades config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Hades uses tools well"
    # All-lowercase matches → lowercase ``hades``; this preserves the
    # real filesystem path ``~/.hades`` (Hades home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "hades should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Hades remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "hades prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Hades was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "hades uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Hades said to ask Hades about Hades settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.hades`` (the real Hades home), not the broken
    ``.Hades``.
    ...
    rewritten to ``~/.Hades/config.yaml`` — a path that doesn't exist —
    ...
    assert mod.rebrand_text("use .openclaw directory") == "use .hades directory"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Hades" in result
