...
- Unlimited timeout (HADES_CRON_TIMEOUT=0)
- Backward compat: HADES_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """HADES_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """HADES_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("HADES_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """HADES_CRON_TIMEOUT=0 yields None (unlimited)."""
    ...
    def test_timeout_invalid_value_falls_back_to_default(self, monkeypatch):
        """HADES_CRON_TIMEOUT=abc should fall back to 600s, not raise ValueError."""
        monkeypatch.setenv("HADES_CRON_TIMEOUT", "abc")
        raw = os.getenv("HADES_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
    ...
    def test_timeout_empty_string_uses_default(self, monkeypatch):
        """HADES_CRON_TIMEOUT='' (empty) should use the 600s default."""
        monkeypatch.setenv("HADES_CRON_TIMEOUT", "")
        raw = os.getenv("HADES_CRON_TIMEOUT", "").strip()
        _cron_timeout = self._parse_cron_timeout(raw)
        assert _cron_timeout == 600.0
...
class TestSysPathOrdering:
    ...
    def test_hades_time_importable(self):
        """hades_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _hades_now
        assert callable(_hades_now)

    def test_hades_constants_importable(self):
        """hades_constants should be importable from cron context."""
