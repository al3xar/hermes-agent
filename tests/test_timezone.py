...
class TestHadesTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_hades_time_cache()
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When HADES_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) HADES_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("HADES_TIMEZONE=" + os.environ.get("HADES_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "HADES_TIMEZONE=NOT_SET" in result["output"], (
            "HADES_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When HADES_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("HADES_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=hades_tz) which shifted
        absolute time when system-local tz != Hades tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hades_tz(self):
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        ...
        Scenario: system is Asia/Kolkata (UTC+5:30), Hades is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Hades tz
        ...
        # Use a Hades timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _hades_now's wall time — this would
