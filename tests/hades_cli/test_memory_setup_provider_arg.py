"""Tests for `hades memory setup [provider]` routing.
...
``hades memory setup honcho``) without the interactive picker — which matters
because the per-provider ``hades <provider>`` subcommand is only registered
