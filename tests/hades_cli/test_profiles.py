...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.hades/profiles)
    * HADES_HOME  -> tmp_path/.hades  (so get_hades_home() agrees)
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["hades", "test", "tmp", "root", "sudo"])
...
class TestGetProfileDir:
    ...
    def test_default_returns_hades_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes hades-agent, .worktrees,
...
class TestNoSkillsOptOut:
    """Tests for `hades profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `hades update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestGetActiveProfileName:
    ...
    def test_default_hades_home_returns_default(self, profile_env):
        # HADES_HOME points to tmp_path/.hades which is the default
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("hades")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        ...
        assert "hades -p mybot" in content
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "hades -p redqueen" in content
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "hades.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["hades_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["hades_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "hades.ssi_health": {"workspace": "hades", "enabled": True}
            }
        }))
        ...
        assert "hades.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["hades_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["hades_heimdall"]["workspace"] == "hades"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "hades.ssi_health": {"aiPeer": "ssi_health"},
                "hades_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["hades.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["hades_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".hades_history",
        ...
        excluded_prefixes = [
            "default/hades-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.hades_history",
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultHadesHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_hades_home()."""
    ...
    def test_default_hades_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_hades_home()
    ...
    def test_default_hades_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_hades_home() returns HADES_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("HADES_HOME", str(docker_home))
