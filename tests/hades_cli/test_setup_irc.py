"""Tests for IRC gateway configuration via `hades setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Hades install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Hades install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#hades")
            monkeypatch.setenv("IRC_NICKNAME", "hades-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `hades setup gateway` experience with IRC present."""
