...
that also has ``uv tool install hades-agent`` does not get misclassified.
"""
...
class TestIsUvToolInstall:
    def test_returns_true_when_sys_prefix_matches_uv_tool_layout(self):
        ...
        with patch.object(config.sys, "prefix", "/home/user/.local/share/uv/tools/hades-agent"):
    ...
    def test_returns_true_when_sys_executable_matches_uv_tool_layout(self):
        ...
        with patch.object(config.sys, "prefix", "/some/unrelated/venv"), \
             patch.object(
                 config.sys,
                 "executable",
                 "/home/user/.local/share/uv/tools/hades-agent/bin/python",
             ):
            assert config.is_uv_tool_install() is True
    ...
    def test_does_not_consult_uv_tool_list(self):
        ...
        ``uv tool install hades-agent`` somewhere on disk. Copilot review on
...
class TestRecommendedUpdateCommandForUvTool:
    ...
    def test_uv_pip_install_keeps_legacy_recommendation(self):
        """Existing behavior: uv is on PATH but Hades is a regular pip install."""
        from hades_cli import config
...
class TestCmdUpdatePipUsesUvTool:
    ...
    def test_runs_uv_pip_install_when_not_uv_tool(self, mock_run):
        """Existing behavior preserved when uv is present but Hades isn't a tool install."""
        from hades_cli.main import _cmd_update_pip
...
class TestCmdUpdatePipInstallLayouts:
    ...
    def test_pipx_managed_uses_pipx_upgrade(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/home/u/.local/pipx/venvs/hades-agent")
    ...
    def test_venv_exports_virtualenv_and_omits_system(self, mock_run, monkeypatch):
        ...
        monkeypatch.setattr(hm.sys, "prefix", "/home/u/.hades/hades-agent/venv")
        monkeypatch.setattr(hm.sys, "base_prefix", "/usr")
        ...
        assert mock_run.call_args.kwargs["env"]["VIRTUAL_ENV"] == "/home/u/.hades/hades-agent/venv"
