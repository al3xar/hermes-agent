"""Tests for SIGHUP protection and stdout mirroring in ``hades update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``hades_cli/main.py``.  These exist so
that ``hades update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        if hasattr(_cfg, "_HADES_HOME_CACHE"):
            _cfg._HADES_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "hades update started" in contents
        finally:
            _finalize_update_output(state)
