...
The build SHA is written by the Dockerfile's ``HADES_GIT_SHA`` build-arg
into ``<project_root>/.hades_build_sha``.  These tests cover the read-side
...
def test_get_build_sha_returns_none_when_file_absent(tmp_path):
    ...
    missing = tmp_path / ".hades_build_sha"  # never created
...
def test_get_build_sha_reads_baked_file(tmp_path):
    ...
    sha_file = tmp_path / ".hades_build_sha"
