...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (HADES_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via HADES_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in HADES_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from hades_cli.providers import HADES_OVERLAYS
        assert "bedrock" in HADES_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from hades_cli.providers import HADES_OVERLAYS
        assert HADES_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from hades_cli.providers import HADES_OVERLAYS
        assert HADES_OVERLAYS["bedrock"].auth_type == "aws_sdk"
