...
def _setup_hades_auth(hades_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_hades_auth(hades_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    hades_home = tmp_path / "hades"
    _setup_hades_auth(hades_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_save_codex_tokens_syncs_credential_pool(tmp_path, monkeypatch):
    ...
    auth = json.loads((hades_home / "auth.json").read_text())
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    fix landed would have run ``hades auth add openai-codex`` as a
    ...
    clobbered independent accounts added via ``hades auth add openai-codex``.
    ...
    (hades_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # Independent account from `hades auth add openai-codex` —
...
def test_save_codex_tokens_does_not_overwrite_independent_manual_entries(tmp_path, monkeypatch):
    ...
    Regression for #39236: ``hades auth add openai-codex`` for accounts B and C
    ...
    (hades_home / "auth.json").write_text(json.dumps({
        ...
        "credential_pool": {
            "openai-codex": [
                ...
                # ``hades auth add openai-codex`` (account B and account C).
...
def test_save_codex_tokens_still_refreshes_legacy_manual_alias(tmp_path, monkeypatch):
    ...
    ``hades auth add openai-codex`` as a workaround when there was no
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    ...
    _save_codex_tokens({"access_token": "hades-at", "refresh_token": "hades-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Hades store
    assert not (codex_home / "auth.json").exists()

    # Hades auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "hades-at"


def test_resolve_returns_hades_auth_store_source(tmp_path, monkeypatch):
    hades_home = tmp_path / "hades"
    _setup_hades_auth(hades_home)
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run hades auth".
    """
    from hades_cli.auth import (
    ...
    assert "hades auth" not in rendered
