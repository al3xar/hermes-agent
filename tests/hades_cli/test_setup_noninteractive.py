...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `hades setup` should reach setup.py, which prints headless guidance."""
        from hades_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("hades_cli.setup.ensure_hades_home"),
            patch("hades_cli.setup.load_config", return_value={}),
            patch("hades_cli.setup.get_hades_home", return_value="/tmp/.hades"),
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `hades` should not prompt for input when no provider exists and stdin is headless."""
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`hades setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["hades", "setup", "tts"])
