...
class TestSlackNativeSlashes:
    ...
    def test_hades_catchall_is_first(self):
        """``/hades`` must be reserved as the first slot so the legacy
        ``/hades <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "hades"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /hades <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        anything dropped remains reachable via ``/hades <command>``."""
        names = {n for n, _d, _h in slack_native_slashes()}
        assert "btw" in names
        assert "bg" in names
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `hades slack manifest`)."""
...
class TestPluginCommandEnumeration:
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/hades metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
