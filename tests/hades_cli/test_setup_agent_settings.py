...
def test_setup_agent_settings_uses_displayed_max_iterations_value(tmp_path, monkeypatch, capsys):
    ...
    .env `HADES_MAX_ITERATIONS` fallback was removed because it was
...
def test_setup_agent_settings_prefers_config_over_stale_env(tmp_path, monkeypatch, capsys):
    ...
    Regression guard for the bug where `.env HADES_MAX_ITERATIONS=60`
    from an old `hades setup` run shadowed `agent.max_turns: 500` in
    ...
    monkeypatch.setattr(
        ...
        lambda key: "60" if key == "HADES_MAX_ITERATIONS" else "",
    )
    monkeypatch.setattr("hades_cli.setup.prompt", lambda *args, **kwargs: next(prompt_answers))
    monkeypatch.setattr("hades_cli.setup.prompt_choice", lambda *args, **kwargs: 4)
    monkeypatch.setattr("hades_cli.setup.save_env_value", lambda *args, **kwargs: None)
    ...
    assert "HADES_MAX_ITERATIONS" in removed_keys
