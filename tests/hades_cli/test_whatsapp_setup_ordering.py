...
Before the fix, ``hades whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``hades gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HADES_HOME", str(hades))
...
def _env_value(hades_home: Path, key: str) -> str | None:
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``hades whatsapp`` with an existing paired session and
