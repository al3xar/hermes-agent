...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `hades doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "HADES_HOME", hades_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("HADES_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_hades_home(self, tmp_path, provider=""):
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_hades_home(tmp_path, provider)
...
def test_run_doctor_accepts_hades_provider_ids_that_catalog_aliases(
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale HADES_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        hades_home = tmp_path / ".hades"
        hades_home.mkdir(parents=True)
        (hades_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"HADES_MAX_ITERATIONS={ghost}\n")
        (hades_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "HADES_HOME", hades_home)
        monkeypatch.setattr(doctor_mod, "get_hades_home", lambda: hades_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("HADES_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("HADES_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale HADES_MAX_ITERATIONS" in out
        env_after = (hades_home / ".env").read_text(encoding="utf-8")
        assert "HADES_MAX_ITERATIONS" not in env_after
