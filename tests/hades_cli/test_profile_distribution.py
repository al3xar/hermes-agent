...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/hades_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_hades_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Hades"):
                check_hades_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Hades distribution" in out
...
class TestInstall:
    ...
    def test_install_enforces_hades_requires(self, profile_env, monkeypatch):
        # Pin current Hades version to something well below the requirement
        import hades_cli
        monkeypatch.setattr(hades_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            hades_requires=">=99.0.0",
        )
