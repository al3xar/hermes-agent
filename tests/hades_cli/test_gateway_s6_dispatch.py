...
`hades gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`hades -p typo gateway start` should print a clear message and
    ...
    assert "hades profile create typo" in out
...
def test_redirect_noop_on_host(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("HADES_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.delenv("HADES_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_fires_inside_s6_container(
    ...
    assert "HADES_GATEWAY_NO_SUPERVISE" in err
...
def test_redirect_short_circuits_supervised_child(
    ...
    running execs `hades gateway run --replace`, the
    HADES_S6_SUPERVISED_CHILD sentinel must short-circuit the redirect
    ...
    monkeypatch.setenv("HADES_S6_SUPERVISED_CHILD", "1")
    monkeypatch.delenv("HADES_GATEWAY_NO_SUPERVISE", raising=False)
...
def test_redirect_respects_no_supervise_env(
    ...
    """`HADES_GATEWAY_NO_SUPERVISE=1` (env var) must skip the redirect.
    ...
    monkeypatch.delenv("HADES_S6_SUPERVISED_CHILD", raising=False)
    monkeypatch.setenv("HADES_GATEWAY_NO_SUPERVISE", value)
...
def test_redirect_no_supervise_env_falsy_values_dont_opt_out(
    ...
    """Falsy / unrecognized values of HADES_GATEWAY_NO_SUPERVISE must
    NOT opt out. We're strict about what counts as "yes" so a typo
    like `HADES_GATEWAY_NO_SUPERVISE=0` doesn't silently enable the
    ...
    for falsy in ("", "0", "false", "no", "off", "garbage"):
        execvp_calls.clear()
        monkeypatch.setenv("HADES_GATEWAY_NO_SUPERVISE", falsy)
