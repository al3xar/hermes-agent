...
def _setup_hades_auth(
    ...
    *,
    ...
    auth_file = hades_home / "auth.json"
...
def test_xai_oauth_authorize_url_includes_referrer_hades_agent():
    """Attribution: xAI's OAuth server can identify Hades-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "hades-agent"
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "hades auth add xai-oauth" in body
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    hades_home.mkdir(parents=True, exist_ok=True)
    (hades_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    hades_home = tmp_path / "hades"
    _setup_hades_auth(hades_home, access_token="")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    hades_home = tmp_path / "hades"
    _setup_hades_auth(hades_home, refresh_token="")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    hades_home = tmp_path / "hades"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_hades_auth(hades_home, access_token=fresh)
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    monkeypatch.delenv("HADES_XAI_BASE_URL", raising=False)
    ...
    assert creds["source"] == "hades-auth-store"
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_hades_auth(hades_home, access_token=fresh)
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    monkeypatch.setenv("HADES_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``HADES_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    hades_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    hades_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (hades_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    hades_home = tmp_path / "hades"
    _seed_xai_oauth_state(hades_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    ...
    raw = json.loads((hades_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    hades_home = tmp_path / "hades"
    _seed_xai_oauth_state(hades_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``hades model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``hades model``
    ...
    assert "hades model" not in rendered.lower()
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Hades that didn't validate) must not be silently honored on the refresh
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `hades model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `hades auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from agent.credential_pool import load_pool

    hades_home = tmp_path / "hades"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_hades_auth(hades_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_credential_pool_does_not_seed_when_singleton_missing_access_token(tmp_path, monkeypatch):
    ...
    (hades_home / "auth.json").write_text(json.dumps(auth_store))
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`hades auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `hades auth remove`.
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``hades auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `hades auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((hades_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Hades invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_hades_auth(hades_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    hades_home = tmp_path / "hades"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_hades_auth(hades_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Hades process refreshed the singleton
    ...
    profiles + Hades processes."""
    from agent.credential_pool import load_pool

    hades_home = tmp_path / "hades"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_hades_auth(hades_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    ...
    (hades_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_hades_auth(hades_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``hades model`` -> xAI Grok OAuth (or another process
    ...
    _setup_hades_auth(hades_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    ...
    # Simulate the user re-running `hades model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``hades auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_hades_auth(hades_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`hades auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``hades auth status``, ``hades setup``, and the
    ``hades`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    hades_home = tmp_path / "hades"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_hades_auth(hades_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("HADES_HOME", str(hades_home))
