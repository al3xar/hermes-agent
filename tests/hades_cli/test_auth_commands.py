...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_hades_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:hades_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:hades_pkce"
...
def test_auth_add_google_gemini_cli_sets_active_provider(tmp_path, monkeypatch):
    """hades auth add google-gemini-cli must set active_provider in auth.json.
    ...
    # pool entry from pool.add_entry() still present for hades auth list
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """hades auth add qwen-oauth must set active_provider in auth.json.
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `hades auth add nous` must also populate providers.nous so the
    ...
    # raises "Hades is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`hades auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``hades auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``hades auth list`` showed two labels sharing
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """hades auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "hades_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`hades logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (hades_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `hades logout` should target configured Codex if auth has no active provider."""
    hades_home = tmp_path / "hades"
    monkeypatch.setenv("HADES_HOME", str(hades_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (hades_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((hades_home / "auth.json").read_text())
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    updated = json.loads((hades_home / "auth.json").read_text())
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `hades auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("HADES_HOME", str(tmp_path / "hades"))
    hades_home = tmp_path / "hades"
    hades_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `hades auth remove`)
    (hades_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((hades_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    after = json.loads((hades_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`hades auth remove xai 1` must stick even when the env var is exported
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    ...
    (hades_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (hades_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `hades auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `hades auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Hades reads from has a
...
def test_seed_from_singletons_respects_hades_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hades_pkce must not re-seed from ~/.hades/.anthropic_oauth.json when suppressed."""
    hades_home = tmp_path / "hades"
    hades_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("HADES_HOME", str(hades_home))

    import yaml
    (hades_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (hades_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["hades_pkce"]},
    }))

    # Stub the readers so only hades_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_hades_oauth_credentials", lambda: {
    ...
    # hades_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "hades_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (hades_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `hades auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `hades auth add openai-codex`)
