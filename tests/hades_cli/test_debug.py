"""Tests for ``hades debug`` CLI command and debug utilities."""
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, hades_home):
    ...
    def test_empty_primary_reports_file_empty(self, hades_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (hades_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, hades_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = hades_home / "logs" / "agent.log"
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, hades_home):
        ...
        (hades_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, hades_home):
    ...
    def test_unknown_log_returns_none(self, hades_home):
        from hades_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, hades_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from hades_cli.debug import _capture_log_snapshot

        logs_dir = hades_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, hades_home):
    ...
    def test_falls_back_when_primary_empty(self, hades_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of HADES_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def hades_home_with_secret(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("HADES_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, hades_home_with_secret):
        from hades_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, hades_home_with_secret):
        from hades_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, hades_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when HADES_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("HADES_REDACT_SECRETS", "false")
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, hades_home_with_secret
    ):
        from hades_cli.debug import _capture_log_snapshot

        log_path = hades_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, hades_home_with_secret):
        from hades_cli.debug import _capture_log_snapshot
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, hades_home):
        ...
        assert "--- hades dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, hades_home):
        from hades_cli.debug import collect_debug_report
    ...
    def test_report_includes_errors_log(self, hades_home):
        from hades_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, hades_home):
        from hades_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, hades_home):
        from hades_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, hades_home, capsys):
    ...
    def test_share_survives_sweep_failure(self, hades_home, capsys):
    ...
    def test_local_flag_prints_full_logs(self, hades_home, capsys):
    ...
    def test_share_uploads_four_pastes(self, hades_home, capsys):
        ...
        with patch("hades_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- hades dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- hades dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- hades dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        desktop_paste = uploaded_content[3]
        assert "--- hades dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, hades_home, capsys):
    ...
    def test_share_continues_on_log_upload_failure(self, hades_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, hades_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, hades_home_with_secret, capsys
    ):
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "hades debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, hades_home):
        from hades_cli.debug import run_debug
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``hades debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, hades_home):
    ...
    def test_records_pending_to_json(self, hades_home):
    ...
    def test_skips_non_paste_rs_urls(self, hades_home):
    ...
    def test_merges_with_existing_pending(self, hades_home):
    ...
    def test_dedupes_same_url(self, hades_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, hades_home):
    ...
    def test_sweep_deletes_expired_entries(self, hades_home):
    ...
    def test_sweep_leaves_future_entries_alone(self, hades_home):
    ...
    def test_sweep_survives_network_failure(self, hades_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, hades_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, hades_home):
        from hades_cli.debug import run_debug
    ...
    def test_run_debug_survives_sweep_failure(self, hades_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: hades debug" in out
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, hades_home, capsys):
        from hades_cli.debug import run_debug_share
    ...
    def test_share_shows_privacy_notice(self, hades_home, capsys):
        from hades_cli.debug import run_debug_share
    ...
    def test_local_no_privacy_notice(self, hades_home, capsys):
        from hades_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, hades_home):
        from hades_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, hades_home):
        from hades_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, hades_home):
        from hades_cli.debug import build_debug_share
