"""Tests for the multi-board kanban layer (``hades kanban boards …``).
...
* Current-board persistence via ``<root>/kanban/current`` and
  ``HADES_KANBAN_BOARD`` env var.
* ``connect(board=)`` isolation — writes on one board don't leak.
* ``create_board`` / ``list_boards`` / ``remove_board`` round trip.
* CLI surface: ``hades kanban boards list/create/switch/rm``.
* ``_default_spawn`` injects ``HADES_KANBAN_BOARD`` into worker env.
"""
...
class TestPathResolution:
    ...
    def test_env_var_db_override_still_wins(self, fresh_home, tmp_path, monkeypatch):
        """``HADES_KANBAN_DB`` pins the file regardless of board= arg."""
        forced = tmp_path / "custom.db"
        monkeypatch.setenv("HADES_KANBAN_DB", str(forced))
        assert kb.kanban_db_path() == forced
        assert kb.kanban_db_path(board="ignored") == forced

    def test_env_var_workspaces_override(self, fresh_home, tmp_path, monkeypatch):
        forced = tmp_path / "ws"
        monkeypatch.setenv("HADES_KANBAN_WORKSPACES_ROOT", str(forced))
...
class TestCurrentBoard:
    ...
    def test_env_var_takes_precedence(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("HADES_KANBAN_BOARD", "envboard")
    ...
    def test_env_beats_file(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("HADES_KANBAN_BOARD", "b")
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("HADES_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
    ...
    def test_connect_stale_env_uses_fallback_board_without_recreating_it(
        ...
        kb.create_board("persist")
        kb.set_current_board("persist")
        monkeypatch.setenv("HADES_KANBAN_BOARD", "ephemeral")
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``HADES_KANBAN_BOARD`` / DB / workspaces on spawn.
    ...
    def test_default_board_spawn_keeps_legacy_paths(self, fresh_home, monkeypatch):
        ...
        env = captured["env"]
        assert env["HADES_KANBAN_BOARD"] == "default"
        assert env["HADES_KANBAN_DB"] == str(fresh_home / "kanban.db")
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``hades kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
