"""Tests for --ignore-user-config and --ignore-rules flags on `hades chat`.
...
files. In Hades the equivalent isolation is:
...
argparse → cmd_chat → cli.main() → HadesCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("HADES_IGNORE_USER_CONFIG", "HADES_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``HADES_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<hades_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._hades_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_hades_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("HADES_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``HadesCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting HADES_IGNORE_RULES=1 flips HadesCLI.ignore_rules True."""
        monkeypatch.setenv("HADES_IGNORE_RULES", "1")

        # Import HadesCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of HadesCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.HadesCLI)
        # Replicate the exact logic from cli.py HadesCLI.__init__:
    ...
    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("HADES_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.HadesCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``hades_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("HADES_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("HADES_IGNORE_RULES", raising=False)
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("HADES_IGNORE_USER_CONFIG") == "1"
        assert "HADES_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "HADES_IGNORE_USER_CONFIG" not in os.environ
        assert "HADES_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real hades parser accepts both flags."""
        ...
        assert "HADES_IGNORE_USER_CONFIG" in src
        assert "HADES_IGNORE_RULES" in src
