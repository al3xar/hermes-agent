...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "hades_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("hades-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # hades-index source advertises availability + featured search.
            idx = _Src("hades-index")
        ...
        assert {"official", "github", "hades-index"} <= ids
...
class TestUpdateCheckEndpoint:
    """``GET /api/hades/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hades/update`` runs ``hades update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/hades/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/hades/update/check").json()
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `hades tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_hades_action", _fake_spawn)
        r = self.client.post(
