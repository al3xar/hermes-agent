"""Tests that `hades model` always shows the model selection menu for custom
...
class TestCustomProviderModelSwitch:
    ...
    def test_bare_custom_current_provider_matches_env_base_url_before_first_fallback(
        ...
        """`hades model` must mark the custom provider matching model.base_url
    ...
    def test_key_env_providers_dict_entry_does_not_add_api_key(
        ...
        config_path.write_text(
            ...
            "    key_env: HADES_CRS_HENKEE_KEY\n"
            "    transport: anthropic_messages\n"
            "    model: claude-opus-4-7\n"
            "    default_model: claude-opus-4-7\n"
            "custom_providers: []\n"
        )
        monkeypatch.setenv("HADES_CRS_HENKEE_KEY", "cr_live_secret_xyz")
        ...
        assert entry["key_env"] == "HADES_CRS_HENKEE_KEY"
        ...
        assert "${HADES_CRS_HENKEE_KEY}" not in saved_text
    ...
    def test_key_env_providers_dict_preserves_existing_api_key(
        ...
        provider_info = {
            ...
            "api_mode": "anthropic_messages",
            "provider_key": "crs-henkee",
            "api_key_ref": "${HADES_CRS_HENKEE_KEY}",  # raw template preserved
        }
        ...
        assert entry["api_key"] == "${HADES_CRS_HENKEE_KEY}"
        assert "cr_live_secret_xyz" not in saved_text


class TestCustomProviderDiscoverModels:
    """#18726: honor ``discover_models: false`` in the terminal ``hades model``
