...
Complements tests/hades_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/hades_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def kanban_home(tmp_path, monkeypatch):
    ...
    # tests in tests/hades_cli/test_kanban_db.py.
    monkeypatch.setenv("HADES_KANBAN_CRASH_GRACE_SECONDS", "0")
    kb.init_db()
    return home
...
def test_per_task_max_retries_overrides_dispatcher_limit(kanban_home, all_assignees_spawnable):
    ...
    Three-tier resolution order:
      1. ``task.max_retries`` (set via ``create_task(max_retries=N)`` /
         ``hades kanban create --max-retries N``)
...
def test_cli_create_max_runtime_via_duration(kanban_home):
    """`hades kanban create --max-runtime 2h` should persist 7200 seconds."""
...
def test_cli_show_json_carries_runs(kanban_home):
    """hades kanban show --json must include runs[] so scripts that
...
def test_build_worker_context_renders_author_with_safe_framing(kanban_home):
    ...
    + "comment from worker" prefix so a misleading HADES_PROFILE name
    (e.g. "hades-system", "operator") can't be misread as a system
    directive above the comment body. Defense-in-depth — see #22452."""
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="t", assignee="worker")
        kb.add_comment(conn, tid, author="hades-system", body="some note")
        ctx = kb.build_worker_context(conn, tid)

        # No bold-author rendering anywhere in the context.
        assert "**hades-system**" not in ctx
        # Explicit provenance prefix is present.
        assert "comment from worker `hades-system` at " in ctx
...
def test_default_spawn_auto_loads_kanban_worker_skill(kanban_home, monkeypatch):
    ...
    hades subprocess (which would hang trying to call an LLM).
    """
    # Pretend the bundled kanban-worker skill resolves for this isolated
    # HADES_HOME — the fixture creates an empty tmpdir without the
    ...
    env = captured["env"]
    assert env.get("HADES_KANBAN_TASK") == tid
    assert env.get("HADES_PROFILE") == "some-profile"
...
def test_cli_create_skill_flag_repeatable(kanban_home):
    """`hades kanban create --skill a --skill b` persists the list."""
    out = run_slash(
...
def test_cli_show_renders_skills(kanban_home):
    """`hades kanban show <id>` prints a skills row when present."""
    out = run_slash(
...
def test_check_dispatcher_presence_warns_when_no_gateway(monkeypatch):
    ...
    assert "hades gateway start" in msg
...
def test_cli_create_warns_when_no_gateway(kanban_home, monkeypatch, capsys):
    ...
    assert "hades gateway start" in captured.err
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    assert kb_cli._cmd_create(ns) == 0
    captured = capsys.readouterr()
    assert "hades gateway start" not in captured.err
...
def test_cli_create_no_warn_on_triage(kanban_home, monkeypatch, capsys):
    ...
    assert "hades gateway start" not in err
...
def test_cli_daemon_without_force_prints_deprecation_exits_2(kanban_home, capsys):
    """`hades kanban daemon` (no --force) is a deprecation stub."""
    from hades_cli import kanban as kb_cli
    ns = argparse.Namespace(
    ...
    assert "hades gateway start" in err
...
def test_cli_daemon_help_marks_deprecated():
    ...
    assert found_deprecation, (
        ...
        "the migration guidance in `hades kanban --help` output"
    )
...
def test_gateway_dispatcher_watcher_respects_env_override(monkeypatch):
    """HADES_KANBAN_DISPATCH_IN_GATEWAY=0 disables without touching config."""
    import asyncio
    from gateway.run import GatewayRunner
    monkeypatch.setenv("HADES_KANBAN_DISPATCH_IN_GATEWAY", "0")
...
def test_gateway_dispatcher_watcher_env_truthy_uses_config(monkeypatch):
    ...
    monkeypatch.setenv("HADES_KANBAN_DISPATCH_IN_GATEWAY", "yes")
    monkeypatch.setattr(
        _cfg_mod, "load_config",
        lambda: {"kanban": {"dispatch_in_gateway": False}},
    )
