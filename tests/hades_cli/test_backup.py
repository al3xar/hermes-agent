"""Tests for hades backup and import commands."""
...
def _make_hades_tree(root: Path) -> None:
    ...
    # hades-agent repo (should be EXCLUDED)
...
class TestShouldExclude:
    def test_excludes_hades_agent(self):
        from hades_cli.backup import _should_exclude
        assert _should_exclude(Path("hades-agent/run_agent.py"))
        assert _should_exclude(Path("hades-agent/.git/HEAD"))
...
class TestBackup:
    ...
    def test_excludes_hades_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include hades-agent/ directory."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        _make_hades_tree(hades_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            ...
            assert agent_files == [], f"hades-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/hades-backup-*.zip."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("hades-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        ...
        _symlink_file_or_skip(hades_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Hades backup."""
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into hades home."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (hades_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (hades_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (hades_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (hades_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_hades_prefix(self, tmp_path, monkeypatch):
        """Import strips .hades/ prefix if all entries share it."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".hades/config.yaml": "model: test\n",
            ".hades/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (hades_home / "config.yaml").read_text() == "model: test\n"
        assert (hades_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_hades_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a hades backup."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (hades_home / "config.yaml").exists()
        # traversal file should NOT exist outside hades home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (hades_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (hades_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (hades_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (hades_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # hades-agent should NOT be present
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without hades markers fails validation."""
        import io
        from hades_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hades(self):
        """Detects .hades/ prefix wrapping all entries."""
        import io
        from hades_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".hades/config.yaml", "test")
            zf.writestr(".hades/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".hades/"
...
class TestBackupEdgeCases:
    def test_nonexistent_hades_home(self, tmp_path, monkeypatch):
        """Backup exits when hades home doesn't exist."""
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("hades-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_hades_home(self, tmp_path, monkeypatch):
        """Backup handles empty hades home (no files to back up)."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        # Only excluded dirs, no actual files
        (hades_home / "__pycache__").mkdir()
        (hades_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = hades_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_hades(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside hades root."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside hades home
        out_zip = hades_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (hades_home / "config.yaml").exists()
        assert (hades_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (hades_home / "profiles" / "coder" / "config.yaml").exists()
        assert (hades_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "hades -p coder" in coder_wrapper
...
class TestQuickSnapshot:
    ...
    def test_creates_snapshot(self, hades_home):
        from hades_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(hades_home=hades_home)
        assert snap_id is not None
        snap_dir = hades_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, hades_home):
        from hades_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", hades_home=hades_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, hades_home):
        from hades_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(hades_home=hades_home)
        db_copy = hades_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, hades_home):
        from hades_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(hades_home=hades_home)
        assert (hades_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, hades_home):
        from hades_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(hades_home=hades_home)
        with open(hades_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(hades_home=empty) is None

    def test_list_snapshots(self, hades_home):
        from hades_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", hades_home=hades_home)
        id2 = create_quick_snapshot(label="second", hades_home=hades_home)

        snaps = list_quick_snapshots(hades_home=hades_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, hades_home):
        from hades_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", hades_home=hades_home)
        snaps = list_quick_snapshots(limit=3, hades_home=hades_home)
        assert len(snaps) == 3

    def test_restore_config(self, hades_home):
        from hades_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hades_home=hades_home)

        (hades_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (hades_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, hades_home=hades_home)
        assert result is True
        assert "openrouter" in (hades_home / "config.yaml").read_text()

    def test_restore_state_db(self, hades_home):
        from hades_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hades_home=hades_home)

        conn = sqlite3.connect(str(hades_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, hades_home=hades_home)
    ...
    def test_restore_nonexistent(self, hades_home):
        from hades_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hades_home=hades_home) is False

    def test_auto_prune(self, hades_home):
        ...
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", hades_home=hades_home)
        snaps = list_quick_snapshots(limit=100, hades_home=hades_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, hades_home):
        from hades_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hades_home=hades_home)
        deleted = prune_quick_snapshots(keep=3, hades_home=hades_home)
        assert deleted == 7
        assert len(list_quick_snapshots(hades_home=hades_home)) == 3

    def test_snapshot_includes_pairing_directories(self, hades_home):
        ...
        (hades_home / "platforms" / "pairing").mkdir(parents=True)
        (hades_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (hades_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (hades_home / "pairing").mkdir()
        (hades_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (hades_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, hades_home):
        ...
        assert restore_quick_snapshot(snap_id, hades_home=hades_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, hades_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (hades update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``hades update``
    ...
    def hades_home(self, tmp_path):
        root = tmp_path / ".hades"
        root.mkdir()
        _make_hades_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, hades_home):
        from hades_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hades_home=hades_home)
        assert out is not None
        assert out.exists()
        assert out.parent == hades_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, hades_home):
        """Pre-update backup should include the same user data that
        ``hades backup`` would, and should exclude the same directories."""
        from hades_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hades_home=hades_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # hades-agent repo excluded
        assert not any(n.startswith("hades-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, hades_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from hades_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(hades_home=hades_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(hades_home=hades_home)
    ...
    def test_rotation_keeps_only_n(self, hades_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(hades_home=hades_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (hades_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, hades_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from hades_cli.backup import create_pre_update_backup

        (hades_home / "backups").mkdir(exist_ok=True)
        manual = hades_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(hades_home=hades_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from hades_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(hades_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, hades_home):
        ...
        """
        from hades_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hades_home=hades_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, hades_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, hades_home):
        ...
        first = create_pre_update_backup(hades_home=hades_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(hades_home=hades_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(hades_home=hades_home, keep=0)
    ...
    def test_skips_symlinked_files(self, hades_home, tmp_path):
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def hades_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".hades"
        root.mkdir()
        _make_hades_tree(root)
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for hades_cli.config + hades_constants so they pick up HADES_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("hades_cli.config") or mod == "hades_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, hades_home, capsys):
        ...
        assert "hades import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((hades_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, hades_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (hades_home / "backups").exists() or not list(
            (hades_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, hades_home, capsys):
        from hades_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, hades_home, capsys):
        ...
        import yaml
        (hades_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
    ...
    def test_config_disabled_is_silent(self, hades_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((hades_home / "backups").glob("pre-update-*.zip")) \
            if (hades_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, hades_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (hades claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``hades claw migrate`` runs before mutating ~/.hades/."""
    ...
    def test_creates_backup_under_backups_dir(self, hades_home):
        ...
        out = create_pre_migration_backup(hades_home=hades_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `hades
        # import` and the update-backup listing both pick them up.
        assert out.parent == hades_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, hades_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``hades backup`` / ``create_pre_update_backup`` — no drift."""
        from hades_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hades_home=hades_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_hades_import(self, hades_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Hades
        backup — `hades import` should accept it."""
    ...
    def test_does_not_recurse_into_prior_backups(self, hades_home):
        from hades_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(hades_home=hades_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(hades_home=hades_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, hades_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(hades_home=hades_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((hades_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_hades_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.hades yet — nothing to back up."""
        from hades_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(hades_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, hades_home):
        ...
        update_backup = create_pre_update_backup(hades_home=hades_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(hades_home=hades_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`hades update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, hades_home: Path, label="pre-update"):
        from hades_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, hades_home=hades_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from hades_cli.backup import restore_cron_jobs_if_emptied
        hades_home = tmp_path / ".hades"
        jobs_path = hades_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(hades_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, hades_home=hades_home)
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = hades_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, hades_home=hades_home) is None
        assert restore_cron_jobs_if_emptied("", hades_home=hades_home) is None
