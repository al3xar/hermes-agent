...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "HADES_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "HADES_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("HADES_COPILOT_ACP_ARGS", "--acp --stdio --debug")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("HADES_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("hades_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        ...
        hades_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: hades_home / ".env")
        monkeypatch.setattr(config_module, "get_hades_home", lambda: hades_home)
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Hades is unconfigured."""
        from hades_cli import config as config_module
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        config_file = hades_home / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_hades_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Hades has been explicitly configured."""
        import yaml
        from hades_cli import config as config_module
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
