...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("hades_")
        assert name.endswith("_bot")
        assert len(name) == len("hades_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("hades_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_defaults(self):
        ...
        assert "hades_" in link

    def test_name_url_encoded(self):
        link = generate_deep_link(
            manager_bot="Bot",
            suggested_username="test_bot",
            suggested_name="Hades & Friends",
        )
        assert "Hades+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "hades_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/HadesSetupBot/hades_abcdefghijklmnop_bot?name=Hades+Agent",
            "qr_payload": "https://t.me/newbot/HadesSetupBot/hades_abcdefghijklmnop_bot?name=Hades+Agent",
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="hades_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/HadesSetupBot/hades_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/HadesSetupBot/hades_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "hades_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
