...
def test_find_install_script_from_checkout(tmp_path):
    ...
    with patch("hades_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "hades_cli", repo_root=tmp_path)
...
def test_find_install_script_from_wheel(tmp_path):
    ...
    from hades_cli.dep_ensure import _find_install_script
    bundled = tmp_path / "hades_cli" / "scripts"
...
def test_find_install_script_prefers_ps1_on_windows(tmp_path):
    ...
    scripts_dir = tmp_path / "hades_cli" / "scripts"
    ...
    with patch("hades_cli.dep_ensure._IS_WINDOWS", True):
        path, shell = _find_install_script(package_dir=tmp_path / "hades_cli")
...
def test_find_install_script_falls_back_to_repo_root(tmp_path):
    ...
    from hades_cli.dep_ensure import _find_install_script
    with patch("hades_cli.dep_ensure._IS_WINDOWS", False):
        path, shell = _find_install_script(package_dir=tmp_path / "hades_cli", repo_root=repo_root)
...
def test_has_hades_agent_browser_windows_path(tmp_path):
    node_dir = tmp_path / "node"
    node_dir.mkdir(parents=True)
    (node_dir / "agent-browser.cmd").write_text("@echo off")
    from hades_cli.dep_ensure import _has_hades_agent_browser
    with patch("hades_cli.dep_ensure._IS_WINDOWS", True), \
         patch("hades_constants.get_hades_home", return_value=tmp_path):
        assert _has_hades_agent_browser() is True


def test_has_hades_agent_browser_posix_path(tmp_path):
...
def test_has_hades_agent_browser_legacy_node_modules_path(tmp_path):
