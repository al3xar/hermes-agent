...
def _make_hades_node(hades_home: Path) -> Path:
    """Create a fake $HADES_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = hades_home / "node" / "bin"
...
def test_removes_symlinks_pointing_into_hades_node(fake_home):
    hades_home = fake_home / ".hades"
    node_bin = _make_hades_node(hades_home)
    local_bin = fake_home / ".local" / "bin"

    for name in ("node", "npm", "npx"):
        (local_bin / name).symlink_to(node_bin / name)

    removed = uninstall.remove_node_symlinks(hades_home)
...
def test_handles_missing_local_bin(fake_home):
    ...
    assert uninstall.remove_node_symlinks(hades_home) == []


def test_removes_dangling_symlink_into_hades_node(fake_home):
