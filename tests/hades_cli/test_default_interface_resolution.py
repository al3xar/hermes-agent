...
`hades` launches the classic prompt_toolkit REPL by default, but users can
flip ``display.interface: tui`` in config.yaml to make the modern Ink TUI the
default for bare ``hades`` / ``hades chat``. Explicit flags always win:

    --cli                forces the classic REPL (highest precedence)
    --tui / HADES_TUI=1 forces the TUI
...
def _reset_early_cache(monkeypatch):
    ...
    # a fresh value, and make sure no stray HADES_TUI leaks in.
