...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<hades_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<hades_home>/plugins/``; we derive
    ``<hades_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        ...
        import os
        hades_home_str = os.environ.get("HADES_HOME")
        if hades_home_str:
            hades_home = Path(hades_home_str)
        else:
            hades_home = base.parent
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "hades_test" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        hades_home = tmp_path / "hades_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under hades_plugins.<name>."""
        plugins_dir = tmp_path / "hades_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("HADES_HOME", str(tmp_path / "hades_test"))

        # Clean up any prior namespace module
        sys.modules.pop("hades_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "hades_plugins.ns_plugin" in sys.modules
...
class TestPluginCommands:
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        hades_home = tmp_path / "hades_test"
        plugins_dir = hades_home / "plugins"
...
class TestPluginDebugLogging:
    """HADES_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("HADES_PLUGINS_DEBUG", raising=False)
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With HADES_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("HADES_PLUGINS_DEBUG", "1")
        from hades_cli import plugins as plugins_mod
