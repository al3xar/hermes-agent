...
Critical invariant: the dashboard Vite build outputs to hades_cli/web_dist/
(vite.config.ts: outDir: "../../hades_cli/web_dist"), NOT web/dist/.
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "hades_cli" / "web_dist"
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        """Regression: sentinel must be in hades_cli/web_dist/, NOT web/dist/."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        _touch(web_dir / "src" / "App.tsx", offset=-10)
