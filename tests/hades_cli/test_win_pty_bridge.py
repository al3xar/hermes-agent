...
Windows-only counterpart to tests/hades_cli/test_pty_bridge.py.  Drives
...
``hades_cli/web_server.py`` ``/api/pty`` handler — so these tests are the
...
class TestWinPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = WinPtyBridge.spawn(["cmd.exe", "/c", "echo hades-ok"])
        try:
            output = _read_until(bridge, b"hades-ok")
            assert b"hades-ok" in output
        finally:
            bridge.close()
    ...
    def test_env_is_forwarded(self):
        bridge = WinPtyBridge.spawn(
            [
                sys.executable,
                "-c",
                "import os; print('HADES_PTY_TEST=' + os.environ.get('HADES_PTY_TEST',''))",
            ],
            env={**os.environ, "HADES_PTY_TEST": "pty-env-works"},
        )
        try:
