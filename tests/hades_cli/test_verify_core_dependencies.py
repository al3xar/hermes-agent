...
base deps) during ``hades update``, leaving the venv in a broken state
...
class TestVerifyCoreDependencies:
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``hades.exe`` on Windows before installing.
        ...
        is left missing and ``hades`` drops off PATH. Previously this path
        ...
        with patch("hades_cli.main._resolve_install_target_python", return_value=py), \
             ...
             patch("hades_cli.main._run_install_with_heartbeat"), \
             patch("hades_cli.main._quarantine_running_hades_exe", return_value=[]) as mock_quar:
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `hades update`.
