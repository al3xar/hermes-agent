"""Tests for the `hades proxy` subcommand and its upstream adapters."""
...
def _write_auth_store(hades_home: Path, nous_state: Dict[str, Any]) -> Path:
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("HADES_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="hades auth add nous"):
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Hades-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "hades auth add nous" in err
