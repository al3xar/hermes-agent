...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "hades_cli" / "banner.py"
    ...
    monkeypatch.setenv("HADES_HOME", str(tmp_path))
    monkeypatch.delenv("HADES_REVISION", raising=False)
    with patch("hades_cli.banner.subprocess.run") as mock_run, \
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    ...
    # Point HADES_HOME at a temp dir with no hades-agent/.git
    monkeypatch.setenv("HADES_HOME", str(tmp_path))
    with patch("hades_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    HADES_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `hades update` correctly refusing to run there. The guard
