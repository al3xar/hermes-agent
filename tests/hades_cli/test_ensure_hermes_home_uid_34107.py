"""Regression tests for #34107 — Docker UID/GID handling in ensure_hades_home.

When Hades runs in Docker with ``HADES_UID=1000`` / ``HADES_GID=911``,
...
The fix is a ``_chown_to_hades_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_hades_uid_gid
# ---------------------------------------------------------------------------


class TestResolveHadesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("HADES_UID", "1000")
        monkeypatch.setenv("HADES_GID", "911")
        from hades_cli.config import _resolve_hades_uid_gid
        uid, gid = _resolve_hades_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("HADES_UID", raising=False)
        monkeypatch.delenv("HADES_GID", raising=False)
        from hades_cli.config import _resolve_hades_uid_gid
        uid, gid = _resolve_hades_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("HADES_UID", "not-a-number")
        monkeypatch.setenv("HADES_GID", "911")
        from hades_cli.config import _resolve_hades_uid_gid
        uid, gid = _resolve_hades_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("HADES_UID", "")
        monkeypatch.setenv("HADES_GID", "")
        from hades_cli.config import _resolve_hades_uid_gid
        uid, gid = _resolve_hades_uid_gid()
        assert uid is None
        assert gid is None
...
class TestChownToHadesUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_hades_uid(d)  # must not raise
