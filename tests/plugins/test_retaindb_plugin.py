...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", hades_home=str(tmp_path / ".hades"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", hades_home=profile_home)
        assert p._client.project == "hades-coder"
        p.shutdown()
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", hades_home=str(hades_home))
        return p
