...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "hades_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_query_param_default_overrides_current_board_pointer(client):
    ...
    through to whichever board ``hades kanban boards switch`` last pinned.
    """
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(hades_cli, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: (False, "No gateway is running — start `hades gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
# tests/hades_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "hades-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "hades-kanban-card--failed" in js
    assert "hades-kanban-card--failed" in css
