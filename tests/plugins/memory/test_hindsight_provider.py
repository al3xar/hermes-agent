...
def provider(tmp_path, monkeypatch):
    ...
    p.initialize(session_id="test-session", hades_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:hades-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:hades-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:hades-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:hades-agent"]
...
class TestConfig:
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["hades"]["bankId"] == "env-bank"
        assert cfg["banks"]["hades"]["budget"] == "high"
...
class TestPostSetup:
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(hades_home))
        provider.post_setup(str(hades_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        hades_env = user_home / ".hindsight" / "profiles" / "hades.env"
        assert coder_env.exists()
        assert not hades_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_hades_home", lambda: hades_home)
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(hades_home))
        ...
        saved = json.loads((hades_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["metadata"]["source"] == "hades"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        p1.initialize(session_id="resumed-session", hades_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", hades_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("hades") == "hades"
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="hades", profile="coder"
        )
        assert result == "hades"

    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "hades-{profile}", fallback="hades",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "hades-coder"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "hades-{user}" becomes "hades-" -> trimmed to "hades"
        result = _resolve_bank_id_template(
            "hades-{user}", fallback="default",
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "hades-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="hades",
        )
        assert p._bank_id == "hades-coder"
        assert p._bank_id_template == "hades-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "hades-" which collapses to "hades"
        p.initialize(session_id="s1", hades_home=str(tmp_path), platform="cli")
        assert p._bank_id == "hades"
