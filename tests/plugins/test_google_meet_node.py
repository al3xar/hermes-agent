...
def test_registry_defaults_to_hades_home(tmp_path, monkeypatch):
    from plugins.google_meet.node.registry import NodeRegistry

    # _isolate_home already set HADES_HOME to tmp_path/.hades; the
...
def test_cli_status_pings_via_node_client(capsys, monkeypatch):
    ...
    class _FakeClient:
        ...
        def ping(self):
            return {"type": "pong", "display_name": "hades-meet-node"}
