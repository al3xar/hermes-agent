"""Tests for the bundled hades-achievements dashboard plugin.
...
``plugins/hades-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the hades-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    ...
    # swap ``sys.modules['hades_state']`` with auto-restoration. Without
...
def _install_fake_session_db(plugin_api, fake_db):
    ...
    ``sys.modules['hades_state']`` swap is auto-restored at test teardown
    and cannot leak into unrelated tests in the same xdist worker.
    """
    fake_module = type(sys)("hades_state")
    fake_module.SessionDB = lambda: fake_db
    plugin_api._test_monkeypatch.setitem(sys.modules, "hades_state", fake_module)
