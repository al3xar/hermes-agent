...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "hades_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "hades_plugins" not in sys.modules:
        ns = types.ModuleType("hades_plugins")
        ns.__path__ = []
        sys.modules["hades_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "hades_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["hades_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_hades_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_hades_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_hades_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/hades-abc/x.log")) is True
...
class TestBundledDiscovery:
    def _write_enabled_config(self, hades_home, names):
