...
def test_start_spawns_subprocess_and_writes_active_pointer(tmp_path):
    ...
    assert captured_env["HADES_MEET_URL"] == "https://meet.google.com/abc-defg-hij"
    assert captured_env["HADES_MEET_GUEST_NAME"] == "Test Bot"
    assert captured_env["HADES_MEET_DURATION"] == "15m"
...
def test_start_realtime_env_vars_threaded_through():
    ...
    assert captured_env["HADES_MEET_MODE"] == "realtime"
    assert captured_env["HADES_MEET_REALTIME_MODEL"] == "gpt-realtime"
    assert captured_env["HADES_MEET_REALTIME_VOICE"] == "alloy"
    assert captured_env["HADES_MEET_REALTIME_INSTRUCTIONS"] == "Be brief."
    assert captured_env["HADES_MEET_REALTIME_KEY"] == "sk-test"
...
def test_cli_register_includes_node_subcommand():
    """`hades meet` argparse tree includes the node subtree."""
    import argparse
