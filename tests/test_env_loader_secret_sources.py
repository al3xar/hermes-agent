...
These cover the small public surface that lets `hades model` / `hades setup`
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_hades_dotenv()`` is called at module-import time from several
    hot modules (cli.py, hades_cli/main.py, run_agent.py, ...).  The
    ...
    # cli.py, hades_cli/main.py, run_agent.py, trajectory_compressor.py,
