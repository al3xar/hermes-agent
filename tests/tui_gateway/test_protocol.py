...
def server():
    with patch.dict("sys.modules", {
        "hades_constants": MagicMock(get_hades_home=MagicMock(return_value="/tmp/hades_test")),
        "hades_cli.env_loader": MagicMock(),
        "hades_cli.banner": MagicMock(),
        "hades_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `HADES_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("HADES_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("HADES_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/hades-agent-dev": {"name": "hades-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "hades-agent-dev", "session_id": sid},
        })
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "hades-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "hades-agent-dev"
