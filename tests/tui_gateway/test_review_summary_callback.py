...
def server():
    with patch.dict(
        "sys.modules",
        {
            "hades_constants": MagicMock(
                get_hades_home=MagicMock(return_value="/tmp/hades_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'hades-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'hades-release' patched"
    }
