...
def _restore_tool_modules():
    original_hades_home = os.environ.get("HADES_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_hades_home is None:
            os.environ.pop("HADES_HOME", None)
        else:
            os.environ["HADES_HOME"] = original_hades_home
        _reset_modules(("tools", "hades_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    hades_cli = types.ModuleType("hades_cli")
    hades_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["hades_cli"] = hades_cli
    hades_home = tmp_path / "hades-home"
    os.environ["HADES_HOME"] = str(hades_home)
    sys.modules["hades_cli.config"] = types.SimpleNamespace(
        get_hades_home=lambda: hades_home,
    )
    ...
    return {
        "snapshot_store": hades_home / "modal_snapshots.json",
