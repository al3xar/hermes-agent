...
and the shared slash-command handlers (hades_cli/write_approval_commands.py).
...
@pytest.fixture
def hades_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="hades_wa_test_")
...
def test_default_gate_is_off(hades_home):
...
def test_invalid_subsystem_is_off(hades_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(hades_home):
...
def test_memory_gate_on_no_interactive_stages(hades_home):
...
def test_memory_gate_on_then_apply(hades_home):
...
def test_skill_gate_off_allows_create(hades_home):
...
def test_skill_gate_on_always_stages(hades_home):
...
def test_skill_gate_on_then_apply_writes_file(hades_home):
...
def test_skill_create_diff_is_full_content(hades_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(hades_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(hades_home):
...
def test_handle_approve_all(hades_home):
    from hades_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(hades_home):
    from hades_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(hades_home):
    from hades_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(hades_home):
    from hades_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(hades_home):
...
def test_handle_approval_invalid(hades_home):
    from hades_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(hades_home):
    from hades_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(hades_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(hades_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(hades_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(hades_home, monkeypatch):
    ...
    from tools import write_approval as wa
    _set_approval("memory", True)
    monkeypatch.setenv("HADES_GATEWAY_SESSION", "1")
...
def test_skills_never_prompt_inline_even_with_callback(hades_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(hades_home):
