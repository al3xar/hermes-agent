...
def fake_hades(tmp_path, monkeypatch):
    """Build a two-profile Hades layout and point HADES_HOME at
    the hades-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setattr(fs, "_hades_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_hades):
        from tools.file_tools import write_file_tool
        target = fake_hades["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_hades):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_hades["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "hades-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_hades):
    ...
    def test_non_hades_path_unaffected(self, fake_hades, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_hades):
    ...
    def test_cross_profile_patch_bypass(self, fake_hades):
        from tools.file_tools import patch_tool
        target = fake_hades["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_hades):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        assert "not found in active profile 'hades-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_hades, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_hades["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_hades["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_hades_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_hades_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hades):
        """When active profile is 'hades-security', the prompt warns
        ...
        assert "Active Hades profile" in src
        assert "cross_profile=True" in src
        assert "~/.hades/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Hades profile: default" in src
        assert "Active Hades profile: {active_profile}" in src
