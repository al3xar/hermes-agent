...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._hades_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(hades_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(hades_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hades_bin(self):
        """After _INSTALL_FAILED, manual install in HADES_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        hades_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(hades_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(hades_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._hades_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == hades_bin
            assert _tirith_mod._resolved_path == hades_bin
            mock_clear.assert_called_once()
...
class TestHadesHomeIsolation:
    def test_hades_bin_dir_respects_hades_home(self):
        """_hades_bin_dir must use HADES_HOME, not hardcoded ~/.hades."""
        from tools.tirith_security import _hades_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"HADES_HOME": tmpdir}):
            result = _hades_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_hades_home(self):
        """_failure_marker_path must use HADES_HOME, not hardcoded ~/.hades."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"HADES_HOME": "/custom/hades"}):
            result = _failure_marker_path()
        assert result == "/custom/hades/.tirith-install-failed"
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        hades_home = os.getenv("HADES_HOME")
        assert hades_home is not None, "HADES_HOME should be set by conftest"
        assert "hades_test" in hades_home, "Should point to test temp dir"
