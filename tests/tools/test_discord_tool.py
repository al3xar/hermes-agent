...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in hades-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_hades_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["hades-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["hades-discord"]["tools"]
    ...
    def test_discord_tools_not_in_other_toolsets(self):
        ...
        for name, ts in TOOLSETS.items():
            if name in {"hades-discord", "hades-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["hades-discord"], quiet_mode=True)
