...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"HADES_SESSION_KEY": "bob"}, clear=False):
...
class TestHadesConfigWriteProtection:
    ...
    def test_sed_in_place(self):
        ...
        assert dangerous is True
        assert "hades config" in desc.lower() or "in-place" in desc.lower()
    ...
    def test_sed_in_place_absolute_hades_home_config(self):
    ...
    def test_sed_in_place_absolute_hades_home_env(self):
    ...
    def test_custom_hades_home(self):
    ...
    def test_perl_in_place_absolute_hades_home_config(self):
        config_path = get_hades_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_absolute_hades_home_env(self):
        env_path = get_hades_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_normal_yaml_write_safe(self):
        # A non-Hades config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_hades_home_env(self):
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.hades/hades-agent && source venv/bin/activate && python -m hades_cli.main gateway run --replace &disown; echo done"
    ...
    def test_pkill_hades_detected(self):
        """pkill targeting hades/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_hades_detected(self):
        cmd = "killall hades"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep hades) bypasses the pkill/killall name-matching
    ...
    def test_pkill_hades_still_detected(self):
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("HADES_GATEWAY_SESSION", "HADES_CRON_SESSION",
                      "HADES_YOLO_MODE",
                      "HADES_SESSION_KEY", "HADES_INTERACTIVE")
        }
        os.environ.pop("HADES_YOLO_MODE", None)
        os.environ.pop("HADES_INTERACTIVE", None)
        # HADES_CRON_SESSION takes priority over HADES_GATEWAY_SESSION in
        ...
        os.environ.pop("HADES_CRON_SESSION", None)
