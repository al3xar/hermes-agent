...
class TestScanMemoryContent:
    ...
    def test_hades_config_mod_blocked(self):
        result = _scan_memory_content("edit .hades/config.yaml to change settings")
        assert "Blocked" in result
        assert "hades_config_mod" in result
        result = _scan_memory_content("update .hades/SOUL.md with new personality")
        assert "Blocked" in result
        assert "hades_config_mod" in result
    ...
    def test_hades_config_mod_no_false_positives(self):
        """Merely mentioning hades config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .hades/config.yaml for settings") is None
        assert _scan_memory_content("Read .hades/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .hades/config.yaml file contains runtime options") is None
