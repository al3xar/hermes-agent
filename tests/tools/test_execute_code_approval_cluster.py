...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad HADES_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: HADES_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("HADES_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("HADES_CRON_SESSION", "1")
    monkeypatch.delenv("HADES_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_hades_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "HADES_HOME": "/h", "HADES_PROFILE": "p",
        "HADES_CONFIG": "/c.yaml", "HADES_ENV": "/e",
        # other HADES_* → dropped (broad prefix removed)
        "HADES_BASE_URL": "https://x", "HADES_INTERACTIVE": "1",
        "HADES_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("HADES_HOME", "HADES_PROFILE", "HADES_CONFIG", "HADES_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "HADES_BASE_URL", "HADES_INTERACTIVE", "HADES_KANBAN_DB",
...
def test_env_scrub_logs_dropped_hades_vars(caplog):
    """Dropping a non-allowlisted, non-secret HADES_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset HADES_* var."""
    import logging
    ...
    env = {
        ...
        "HADES_BASE_URL": "https://x",   # dropped → logged
        "HADES_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "HADES_API_KEY": "sk",       # secret → dropped silently (not logged)
    ...
    assert "HADES_BASE_URL" not in out and "HADES_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "HADES_BASE_URL" in msgs and "HADES_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "HADES_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped HADES_* vars."""
    import logging
