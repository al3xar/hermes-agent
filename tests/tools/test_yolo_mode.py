"""Tests for --yolo (HADES_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When HADES_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("HADES_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With HADES_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """HADES_YOLO_MODE should not be set by default."""
    ...
    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for HADES_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("HADES_YOLO_MODE", "")
        monkeypatch.setenv("HADES_INTERACTIVE", "1")
        monkeypatch.setenv("HADES_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("HADES_YOLO_MODE") returns ""
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("HADES_YOLO_MODE", value)
        monkeypatch.setenv("HADES_INTERACTIVE", "1")
        monkeypatch.setenv("HADES_SESSION_KEY", "test-session")
