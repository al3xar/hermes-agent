...
@pytest.fixture
def isolated_hades_home(tmp_path, monkeypatch):
...
class TestCredentialPoolSeedsFromDotEnv:
    ...
    load_hades_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_hades_home):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_hades_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
    ...
    def test_openrouter_key_from_dotenv_only(self, isolated_hades_home):
        """OpenRouter path has its own branch — verify it also reads .env."""
        _write_env_file(isolated_hades_home, OPENROUTER_API_KEY="sk-or-dotenv-abc")
    ...
    def test_empty_dotenv_no_entries(self, isolated_hades_home):
...
class TestAuthResolvesFromDotEnv:
    ...
    def test_key_from_dotenv_only(self, isolated_hades_home):
        """Key in .env but not os.environ → _resolve returns it with the env var source."""
        _write_env_file(isolated_hades_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_hades_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_hades_home):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_hades_home, monkeypatch):
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_hades_home):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_hades_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
