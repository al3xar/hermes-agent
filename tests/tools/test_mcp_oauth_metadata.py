...
Covers:
- :class:`HadesTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.HadesMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _HADES_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# HadesTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = HadesTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        storage = HadesTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (HadesMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: HadesTokenStorage, **context_attrs):
    ...
    """
    if _HADES_PROVIDER_CLS is None:
        pytest.skip("MCP SDK auth not available")
    provider = _HADES_PROVIDER_CLS.__new__(_HADES_PROVIDER_CLS)
    provider._hades_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        storage = HadesTokenStorage("mgr-srv")
        ...
        with patch.object(
            _HADES_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_initialize_skips_restore_when_in_memory_present(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        storage = HadesTokenStorage("mgr-srv2")
    ...
    def test_persist_metadata_if_changed_writes_on_first_discover(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        storage = HadesTokenStorage("persist-srv")
        assert storage.load_oauth_metadata() is None
    ...
    def test_persist_metadata_noop_when_unchanged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        storage = HadesTokenStorage("noop-srv")
        ...
        with patch.object(
            HadesTokenStorage, "save_oauth_metadata"
        ) as save_spy:
            provider._persist_oauth_metadata_if_changed()
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("HADES_HOME", str(tmp_path))
        storage = HadesTokenStorage("flow-srv")
        ...
        with patch.object(
            _HADES_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
