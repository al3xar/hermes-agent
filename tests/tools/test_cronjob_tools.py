...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `hades-agent-dev` skill contains a postmortem
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because hades uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("HADES_INTERACTIVE", raising=False)
        monkeypatch.delenv("HADES_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("HADES_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("HADES_INTERACTIVE", false_like_value)
        monkeypatch.delenv("HADES_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("HADES_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["HADES_INTERACTIVE", "HADES_GATEWAY_SESSION", "HADES_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("HADES_INTERACTIVE", "HADES_GATEWAY_SESSION", "HADES_EXEC_ASK"):
