...
- Signal forwarding runs through the init so ``docker stop`` triggers
  hades's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, hades accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, hades accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into hades,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, hades still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``hades-ink`` workspace package is
    ...
    assert "ui-tui/packages/hades-ink/" in dockerfile_text
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    # ``hades-ink`` is a bundled workspace package referenced from
    ...
    # ``rm -rf`` + ``npm install --omit=dev --prefix node_modules/@hades/ink``)
    ...
    assert "ui-tui/packages/hades-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled hades-ink workspace package "
