...
When HADES_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the hades user's home directory ($HADES_HOME == /opt/data).
Runtime-writable trees under ``/opt/hades`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/hades/gateway`` is one such
