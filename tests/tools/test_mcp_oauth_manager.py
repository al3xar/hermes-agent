...
def test_hades_provider_subclass_exists():
    """HadesMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _HADES_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _HADES_PROVIDER_CLS is not None
    assert issubclass(_HADES_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_hades_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns HadesMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _HADES_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("HADES_HOME", str(tmp_path))
    ...
    assert _HADES_PROVIDER_CLS is not None
    assert isinstance(provider, _HADES_PROVIDER_CLS)
    assert provider._hades_server_name == "srv"
