...
class TestCronDenyMode:
    """When HADES_CRON_SESSION is set and cron_mode=deny, dangerous commands are blocked."""
...
class TestCronApproveMode:
    """When HADES_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        monkeypatch.setenv("HADES_CRON_SESSION", "1")
        monkeypatch.setenv("HADES_YOLO_MODE", "1")
        monkeypatch.delenv("HADES_INTERACTIVE", raising=False)
        monkeypatch.delenv("HADES_GATEWAY_SESSION", raising=False)

        # _YOLO_MODE_FROZEN is frozen at module import time (security: prevents
        # prompt injection from runtime-setting HADES_YOLO_MODE). When the
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds HADES_SESSION_PLATFORM via contextvars for
