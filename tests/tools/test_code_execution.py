...
UDS socket lifecycle, hades_tools generation, timeout enforcement,
...
class TestHadesToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_hades_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)

    def test_generates_subset(self):
        src = generate_hades_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_hades_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_hades_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_hades_tools_module(["terminal"])
        self.assertIn("HADES_RPC_SOCKET", src)
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_hades_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"hades_rpc\")", src)
        self.assertNotIn('os.environ.get("HADES_RPC_DIR", "/tmp/hades_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_hades_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/hades_exec_", mkdir_cmd)
        self.assertIn("HADES_RPC_DIR=/data/data/com.termux/files/usr/tmp/hades_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/hades_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/hades_exec_", mkdir_cmd)
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_single_tool_call(self):
        ...
        code = """
from hades_tools import terminal
...
"""
        ...
        # terminal won't be in hades_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from hades_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from hades_tools import json_parse
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from hades_tools import shell_quote
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `hades tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `hades tools code_execution,vision`.
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_hades_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("HADES_RPC_SOCKET", child_env)
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("HADES_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestExecuteCodeEdgeCases(unittest.TestCase):
    ...
    def test_none_enabled_tools_uses_all(self):
        ...
        code = (
            "from hades_tools import terminal, web_search, read_file\n"
    ...
    def test_empty_enabled_tools_uses_all(self):
        ...
        code = (
            "from hades_tools import terminal, web_search\n"
            "print('imports ok')\n"
        )
        with patch("model_tools.handle_function_call",
                    return_value=json.dumps({"ok": True})):
