...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``hades tools`` repaint and at
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `hades tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``hades tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``hades auth`` sign-in) rather than an env-var key.
    Patches at the ``hades_cli.runtime_provider.resolve_runtime_provider``
