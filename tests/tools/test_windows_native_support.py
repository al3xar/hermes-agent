...
code paths Hades takes on native Windows.
...
class TestConfigureWindowsStdio:
    ...
    - respect HADES_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("HADES_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("HADES_DISABLE_WINDOWS_UTF8", optout)
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "hades_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, hades_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "hades_cli/main.py", "gateway/run.py"],
    )
...
class TestSubprocessCompatHelpers:
    """hades_cli/_subprocess_compat.py POSIX + Windows behaviour."""
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``hades update``
        (which runs under hades-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# hades_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "hades_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the HADES_RPC_SOCKET format.
    """
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_hades_home(self):
        root = Path(__file__).resolve().parents[2]
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_hades_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "hades_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "hades_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "hades_cli" / "gateway.py").read_text(encoding="utf-8")
