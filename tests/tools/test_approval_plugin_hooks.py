...
def isolated_session(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("HADES_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: HADES_INTERACTIVE is set, the
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: HADES_GATEWAY_SESSION is set and a
