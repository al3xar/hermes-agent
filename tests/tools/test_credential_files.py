...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        hades_home.mkdir()
        (hades_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"HADES_HOME": str(hades_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(hades_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.hades/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.hades/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        hades_home = tmp_path / ".hades"
        (hades_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"HADES_HOME": str(hades_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.hades")

        assert mounts[0]["container_path"] == "/home/user/.hades/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.hades/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.hades/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file one level above hades_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside hades_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        ...
        hades_home.mkdir()
        monkeypatch.setenv("HADES_HOME", str(hades_home))
        (hades_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_hades_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of HADES_HOME must be allowed."""
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        subdir = hades_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside hades_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside hades_home pointing outside
        symlink = hades_home / "evil_link.json"
        try:
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, hades_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(hades_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(hades_home, [str(sensitive)])
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        ...
        (hades_home / "oauth.json").write_text("{}")
        self._write_config(hades_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        (hades_home / "cache" / "documents").mkdir(parents=True)
        (hades_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("HADES_HOME", str(hades_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.hades/cache/documents" in paths
        assert "/root/.hades/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.hades/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        hades_home = tmp_path / ".hades"
        hades_home.mkdir()
        # Use legacy dir name — get_hades_dir prefers old if it exists
        (hades_home / "document_cache").mkdir()
        (hades_home / "image_cache").mkdir()
        monkeypatch.setenv("HADES_HOME", str(hades_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(hades_home / "document_cache") in host_paths
        assert str(hades_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.hades/cache/documents" in container_paths
        assert "/root/.hades/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        hades_home = tmp_path / ".hades"
        img_dir = hades_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.hades/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.hades")
            == "/home/agent/.hades/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        hades_home = tmp_path / ".hades"
        (hades_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("HADES_HOME", str(hades_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(hades_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        hades_home = tmp_path / ".hades"
        doc_dir = hades_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        hades_home = tmp_path / ".hades"
        ss_dir = hades_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.hades/cache/screenshots/session_abc/screen1.png"
