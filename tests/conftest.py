"""Shared fixtures for the hades-agent test suite.
...
4. **No HADES_SESSION_* inheritance** — the agent's current gateway
...
# HADES_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_HADES_BEHAVIORAL_VARS = frozenset({
    "HADES_YOLO_MODE",
    "HADES_INTERACTIVE",
    "HADES_QUIET",
    "HADES_TOOL_PROGRESS",
    ...
    "HADES_SESSION_CHAT_ID",
    "HADES_SESSION_CHAT_NAME",
    "HADES_SESSION_THREAD_ID",
    "HADES_SESSION_SOURCE",
    "HADES_SESSION_KEY",
    "HADES_GATEWAY_SESSION",
    "HADES_CRON_SESSION",
    "_HADES_GATEWAY",
    "HADES_PLATFORM",
    "HADES_MODEL",
    "HADES_INFERENCE_MODEL",
    "HADES_INFERENCE_PROVIDER",
    "HADES_TUI_PROVIDER",
    "HADES_MANAGED",
    "HADES_DEV",
    "HADES_CONTAINER",
    ...
    "HADES_TIMEZONE",
    "HADES_REDACT_SECRETS",
    "HADES_BACKGROUND_NOTIFICATIONS",
    "HADES_EXEC_ASK",
    "HADES_HOME_MODE",
    "HADES_AGENT_USE_LEGACY_SESSION_KEYS",
    ...
    "HADES_KANBAN_BOARD",
    "HADES_KANBAN_HOME",
    "HADES_KANBAN_WORKSPACES_ROOT",
    "HADES_KANBAN_LOGS_ROOT",
    "HADES_KANBAN_TASK",
    "HADES_KANBAN_WORKSPACE",
    "HADES_KANBAN_RUN_ID",
    "HADES_KANBAN_CLAIM_LOCK",
    "HADES_KANBAN_DISPATCH_IN_GATEWAY",
    "HADES_TENANT",
    ...
    "HADES_DASHBOARD_OAUTH_CLIENT_ID",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    # 2. Blank behavioral HADES_* vars that could change test semantics.
    for name in _HADES_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    fake_hades_home = tmp_path / "hades_test"
    fake_hades_home.mkdir()
    (fake_hades_home / "sessions").mkdir()
    (fake_hades_home / "cron").mkdir()
    (fake_hades_home / "memories").mkdir()
    (fake_hades_home / "skills").mkdir()
    monkeypatch.setenv("HADES_HOME", str(fake_hades_home))
...
@pytest.fixture(autouse=True)
def _isolate_hades_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal hades config dict suitable for unit tests."""
    return {
...
# correlated with a ``tests/hades_cli/`` pytest run starting up.
#
...
def _live_system_guard(request, monkeypatch):
    ...
    targeting hades/python patterns are also blocked.
    """
    ...
    def _matches_hades_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _HADES_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_hades_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch hades-themed patterns + a
    ...
    def _check_subprocess_cmd(name, cmd):
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting hades/python could hit the live gateway. "
