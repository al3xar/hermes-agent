...
def test_subprocess_pkill_hades_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["pkill", "-f", "hades"])


def test_subprocess_pkill_hades_gateway_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
...
def test_subprocess_killall_hades_blocked():
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.run(["killall", "hades"])
...
def test_systemctl_unrelated_unit_passes_through():
    """systemctl restart of a non-hades unit is allowed (we only protect hades)."""
...
def test_subprocess_pkill_with_unrelated_pattern_passes_through():
    """``pkill -f some-unrelated-pattern`` (no hades/python) is fine."""
