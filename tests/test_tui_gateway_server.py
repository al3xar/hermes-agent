...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # review on #19835).
    monkeypatch.setenv("HADES_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("HADES_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("HADES_VOICE", "1")
    monkeypatch.delenv("HADES_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "hades_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "hades_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    # whose tools live in hades-cli's universe (see toolsets.py).
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "hades_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("HADES_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "hades_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown HADES_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    ...
    monkeypatch.setenv("HADES_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/hades-test "}}
    )
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    ...
    assert server._resolve_startup_runtime() == ("nous/hades-test", "nous")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("HADES_MODEL", "nous/hades-test")
    monkeypatch.delenv("HADES_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("HADES_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "hades_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/hades-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("HADES_MODEL", "sonnet")
    monkeypatch.delenv("HADES_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("HADES_INFERENCE_PROVIDER", raising=False)
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with HadesCLI / gateway: fallback_providers stays first and keeps
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_hades_home", tmp_path)
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("HADES_YOLO_MODE", "false")
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("HADES_EXEC_ASK", raising=False)
    monkeypatch.delenv("HADES_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("HADES_INTERACTIVE", raising=False)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("hades_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing HADES_INFERENCE_PROVIDER on a switch leaked the new
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "HADES_TUI_PROVIDER" not in os.environ
        assert "HADES_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write HADES_MODEL / HADES_TUI_PROVIDER
    ...
    try:
        ...
        assert "HADES_MODEL" not in os.environ
        assert "HADES_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors HadesCLI._manual_compress's
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_hades_home
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `hades model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `hades model` and the classic CLI /model picker.
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``hades --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "hades_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "hades_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_hades_cli_reload_env(monkeypatch):
...
def test_session_save_writes_under_hades_home_with_system_prompt(monkeypatch, tmp_path):
    ...
    Regression: the gateway handler wrote ``hades_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Hades.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("hades_conversation_*.json"))
    ...
    assert payload["model"] == "hades-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Hades."
