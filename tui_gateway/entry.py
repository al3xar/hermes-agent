...
# hades modules.  hades_cli sets HADES_PYTHON_SRC_ROOT before spawning this
# subprocess; inserting it first ensures the installed packages win.
_src_root = os.environ.get("HADES_PYTHON_SRC_ROOT", "")
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `HADES_TUI_SIDECAR_URL`, set by the dashboard's
...
# tested; override via ``HADES_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``HADES_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``hades --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def main():
    ...
    # server stalled the whole shell on "summoning hades…" for the full
