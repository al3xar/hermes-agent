"""Persistent slash-command worker — one HadesCLI per TUI session.
...
def _env_float(name: str, default: float) -> float:
    ...
    import time on a typo (e.g. ``HADES_SLASH_WATCHDOG_POLL_S=2s``) and kill
...
_WATCHDOG_POLL_S = max(0.05, _env_float("HADES_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, _env_float("HADES_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _run(cli: HadesCLI, command: str) -> str:
...
def main():
    ...
    # Start before the (hundreds-of-ms) HadesCLI build — that window is itself
    ...
    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = HadesCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
