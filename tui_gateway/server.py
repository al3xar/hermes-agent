...
load_hades_dotenv(
    hades_home=_hades_home, project_env=Path(__file__).parent.parent / ".env"
)
...
_CRASH_LOG = os.path.join(_hades_home, "logs", "tui_gateway_crash.log")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("HADES_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent HadesCLI subprocess for slash commands."""
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `hades --tui` keeps real _stdio.
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `hades --tui`, so it must NOT count as dead here (doing
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_hades_home).resolve():
        return None
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `hades` shows the prompt before constructing AIAgent; the TUI used
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _hades_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_hades_home_override()
        home = override if isinstance(override, str) and override else _hades_home
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] HADES_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown HADES_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in HADES_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid HADES_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid HADES_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    from hades_cli.model_switch import parse_model_flags, switch_model
    ...
    #
    # We deliberately do NOT write process-global env vars (HADES_MODEL /
    # HADES_INFERENCE_MODEL / HADES_TUI_PROVIDER / HADES_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # HADES_DEV_CREDITS so the payload stays clean when the flag is off.
...
def _load_fallback_model():
    ...
    stays in parity with ``HadesCLI.__init__`` and ``gateway/run.py``:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `HADES_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    # ``hades --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.hades/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.hades/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Hades Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Hades/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Hades Desktop. If it serves Hades/Desktop UI or another unrelated app, stop that process and report failure.",
        ...
        try:
            ...
            if session:
                ...
                if enable:
                    ...
                    os.environ.pop("HADES_YOLO_MODE", None)
                    nv = "0"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_hades_home), "display": display_hades_home()})
    if key == "project":
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from hades_cli.browser_connect import try_launch_chrome_debug
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_hades_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``hades plugins`` / the dashboard, so the three surfaces
